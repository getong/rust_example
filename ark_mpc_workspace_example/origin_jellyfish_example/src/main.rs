...
use ark_ed_on_bn254::{EdwardsConfig, Fq as EdwardsBaseField, Fr as EdwardsScalarField};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  println!("origin Jellyfish / ark-mpc mini example");
...
fn run_relation_example() -> Result<(), CircuitError> {
  ...
  println!(
    "jf-relation: built and checked 6 * 7 == 42, gates={}, vars={}",
    circuit.num_gates(),
    circuit.num_vars()
  );
...
fn run_elgamal_example() {
  ...
  let plaintext = vec![
    EdwardsBaseField::from(11_u64),
    EdwardsBaseField::from(22_u64),
    EdwardsBaseField::from(33_u64),
  ];
  let randomness = EdwardsScalarField::rand(&mut rng);
  ...
  println!(
    "jf-elgamal: encrypted/decrypted {} Edwards-on-BN254 base field elements",
    plaintext.len()
  );
}

async fn run_mock_mpc_example() -> Result<(), Box<dyn std::error::Error>> {
  let a = SystemScalar::from(19_u64);
  let b = SystemScalar::from(23_u64);
  ...
  let expected = SystemScalar::from(42_u64);
...
fn describe_plonk_type() {
  println!(
    "jf-plonk: proof type wired for BN254 is {}",
