...
use deno_config::glob::{FileCollector, FilePatterns, PathOrPattern, PathOrPatternSet, WalkEntry};
use deno_core::{
  ModuleSpecifier,
...
pub fn canonicalize_path_maybe_not_exists(path: &Path) -> Result<PathBuf, Error> {
  deno_path_util::fs::canonicalize_path_maybe_not_exists(&CliSys::default(), path)
}
...
/// Collects module specifiers that satisfy the given predicate as a file path, by recursively
/// walking `include`. Specifiers that start with http and https are left intact.
    ...
    for path_or_pattern in path_or_patterns {
      match path_or_pattern {
        PathOrPattern::Path(path) => {
          ...
          } else if always_include_specified || !file_patterns.exclude.matches_path(&path) {
    ...
    let result = collect_specifiers(
      CollectSpecifiersOptions {
        file_patterns: FilePatterns {
          ...
          exclude: PathOrPatternSet::new(vec![PathOrPattern::Path(ignore_dir_path.to_path_buf())]),
        },
        vendor_folder: None,
