...
use super::draw_thread::{DrawThread, DrawThreadGuard, DrawThreadRenderer};
use crate::colors;
...
impl ProgressBarInner {
  ...
  pub fn add_entry(&self, kind: ProgressMessagePrompt, message: String) -> Arc<ProgressBarEntry> {
    let mut internal_state = self.state.lock();
...
impl ProgressBar {
  ...
  pub fn new(style: ProgressBarStyle) -> Self {
    Self {
      inner: ProgressBarInner::new(match style {
        ...
        ProgressBarStyle::ProgressBars => Arc::new(renderer::BarProgressBarRenderer {
          display_human_download_size: false,
        }),
        ProgressBarStyle::TextOnly => Arc::new(renderer::TextOnlyProgressBarRenderer::default()),
      }),
    }
  }
  ...
  pub fn deferred_update_with_prompt(&self, kind: ProgressMessagePrompt, msg: &str) -> UpdateGuard {
    // only check if progress bars are supported once we go
    // to update so that we lazily initialize the progress bar
    if ProgressBar::are_supported() {
