...
use deno_core::{ModuleSourceCode, error::AnyError};
    ...
    ModuleSourceCode::Bytes(code) => {
      if let Some(range) = find_source_map_range(code.as_bytes()) {
        ...
        ModuleSourceCode::Bytes(match code {
          ModuleCodeBytes::Static(bytes) => ModuleCodeBytes::Static(&bytes[.. source_map_index]),
          ModuleCodeBytes::Boxed(bytes) => {
            // todo(dsherret): should be possible without cloning
            ModuleCodeBytes::Boxed(bytes[.. source_map_index].to_vec().into_boxed_slice())
          }
...
impl Utf16Map {
  ...
  pub fn offset_utf16(&self, line: u32, col: u32) -> Result<TextSize, AnyError> {
  ...
  pub fn utf16_to_utf8_offset(&self, utf16_offset: TextSize) -> Option<TextSize> {
    ...
    let col_utf8 = self.utf16_to_utf8_col(pos.line_index as u32, pos.column_index as u32);
    Some(line_start_utf8 + col_utf8)
  }

  /// Convert a UTF-8 byte offset to UTF-16 byte offset
  pub fn utf8_to_utf16_offset(&self, utf8_offset: TextSize) -> Option<TextSize> {
...
mod tests {
  ...
  use deno_core::{ModuleCodeBytes, ModuleCodeString};

  use super::*;

  #[test]
  fn test_source_map_from_code() {
    let to_string = |bytes: Vec<u8>| -> String { String::from_utf8(bytes.to_vec()).unwrap() };
    assert_eq!(
  ...
  fn test_source_without_source_map() {
    ...
    fn run_test(input: &'static str, output: &'static str) {
      let forms = [
        ...
        ModuleSourceCode::Bytes(ModuleCodeBytes::Arc(input.as_bytes().to_vec().into())),
      ];
      for form in forms {
...
",
  ...
  fn test_offset_emoji() {
    ...
    let utf16_offset_after = map.utf8_to_utf16_offset(utf8_offset_after).unwrap();
