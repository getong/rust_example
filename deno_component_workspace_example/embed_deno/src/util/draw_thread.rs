...
use deno_core::{parking_lot::Mutex, unsync::spawn_blocking};
...
impl DrawThread {
  ...
  pub fn hide() {
    let internal_state = &*INTERNAL_STATE;
    let mut internal_state = internal_state.lock();
    let is_showing = internal_state.has_draw_thread && internal_state.hide_count == 0;
      ...
      loop {
        let mut delay_ms = 120;
        {
          ...
          if let Some(entries) = maybe_entries {
            ...
            // 4. This thread - Calls renderer.render within internal lock, which attempts to
            //    acquire the other thread's Render's internal lock causing a deadlock
