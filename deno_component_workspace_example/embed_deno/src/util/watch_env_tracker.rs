...
impl WatchEnvTracker {
  ...
  fn handle_dotenvy_error(error: dotenvy::Error, file_path: &Path, log_level: Option<log::Level>) {
    ...
    if log_level.map(|l| l >= log::Level::Info).unwrap_or(true) {
      match error {
        ...
        dotenvy::Error::Io(_) => eprintln!(
          "{} The `--env-file` flag was used, but the environment file specified '{}' was not \
           found.",
          colors::yellow("Warning"),
          file_path.display()
        ),
        dotenvy::Error::EnvVar(_) => eprintln!(
          "{} One or more of the environment variables isn't present or not unicode within the \
  ...
  fn _cleanup_removed_variables(
    ...
    for var_name in inner.unused_variables.iter() {
      if !inner.original_env.contains_key(var_name) {
        ...
        if log_level.map(|l| l >= log::Level::Debug).unwrap_or(false) {
          eprintln!(
            "{} Variable '{}' removed from environment as it's no longer present in any loaded \
             file",
            colors::yellow("Debug"),
        ...
        if log_level.map(|l| l >= log::Level::Debug).unwrap_or(false) {
          eprintln!(
            "{} Variable '{}' restored to original value as it's no longer present in any loaded \
             file",
            colors::yellow("Debug"),
            var_name.to_string_lossy()
          );
  ...
  pub fn load_env_variables_from_env_files(
    ...
    for env_file_name in env_file_names.iter().rev() {
      self.load_env_file_inner(env_file_name.to_path_buf(), log_level, &mut inner);
    }
...
pub fn load_env_variables_from_env_files(
  ...
  for env_file_name in env_file_names.iter().rev() {
    match dotenvy::from_filename(env_file_name) {
      Ok(_) => (),
      Err(error) => {
        WatchEnvTracker::handle_dotenvy_error(error, env_file_name, flags_log_level);
      }
    }
  }
}
