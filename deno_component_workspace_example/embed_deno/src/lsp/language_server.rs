...
impl Inner {
      ...
      .flat_map(|c| c.import_source.iter().chain(c.import_source_types.iter()))
    {
      ...
      self.task_queue.queue_task(Box::new(|ls: LanguageServer| {
        spawn(async move {
          ...
          if let Err(err) = ls.cache(vec![specifier], referrer, false, true).await {
            lsp_warn!("{:#}", err);
          }
        });
      }));
  ...
  async fn did_change_watched_files(&mut self, params: DidChangeWatchedFilesParams) {
    ...
    if changes.iter().any(|(specifier, _)| {
      ...
      self.send_diagnostics_update();
      if self.config.diagnostic_capable() && self.config.diagnostic_refresh_capable() {
  ...
  async fn code_action(
    ...
    // Organize imports
    if kinds.is_empty() || kinds.contains(&CodeActionKind::SOURCE_ORGANIZE_IMPORTS) {
