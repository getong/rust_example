...
pub mod tests {
  ...
  impl TestPackageSearchApi {
    pub fn with_package_version(mut self, name: &str, version: &str, exports: &[&str]) -> Self {
      let exports_by_version = self.package_versions.entry(name.to_string()).or_default();
  ...
  impl PackageSearchApi for TestPackageSearchApi {
    ...
    async fn exports(&self, nv: &PackageNv) -> Result<Arc<Vec<String>>, AnyError> {
      let Some(exports_by_version) = self.package_versions.get(nv.name.as_str()) else {
        return Err(anyhow!("Package not found."));
      };
