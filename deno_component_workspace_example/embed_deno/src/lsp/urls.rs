...
/// VSCode URI.
const URL_TO_URI_PATH: &percent_encoding::AsciiSet = &percent_encoding::CONTROLS
  .add(b' ')
  .add(b'!')
  .add(b'$')
  .add(b'&')
...
pub fn url_to_uri(url: &Url) -> Result<Uri, AnyError> {
  ...
  if has_drive_letter {
    ...
    input.push_str(&percent_encoding::utf8_percent_encode(rest, URL_TO_URI_PATH).to_string());
  } else {
    input.push_str(&percent_encoding::utf8_percent_encode(path, URL_TO_URI_PATH).to_string());
  }
  if let Some(query) = url.query() {
    input.push('?');
    input.push_str(&percent_encoding::utf8_percent_encode(query, URL_TO_URI_QUERY).to_string());
  }
  if let Some(fragment) = url.fragment() {
    input.push('#');
    input
      .push_str(&percent_encoding::utf8_percent_encode(fragment, URL_TO_URI_FRAGMENT).to_string());
  }
  Ok(
    Uri::from_str(&input)
      .inspect_err(|err| lsp_warn!("Could not convert URL \"{url}\" to URI: {err}"))?,
  )
}
