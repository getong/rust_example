...
use dashmap::DashMap;
use deno_config::{glob::FilePatterns, workspace::WorkspaceDirLintConfig};
...
use crate::{
  ...
  tools::lint::{CliLinter, CliLinterOptions, LintRuleProvider, PluginHostProxy},
};
    ...
    self
      ...
      .or_insert_with(|| {
        ...
        let workspace_resolver = self
          ...
          .clone();
        let lint_rule_provider = LintRuleProvider::new(Some(workspace_resolver));
        let lint_config = config_data
          .and_then(|d| {
            d.member_dir
              .to_lint_config(FilePatterns::new_with_base(d.member_dir.dir_path()))
              .inspect_err(|err| lsp_warn!("Couldn't read lint configuration: {}", err))
              .ok()
          })
        ...
        let lint_options = LintOptions::resolve(lint_config.clone(), &LintFlags::default())
          ...
          == Some("react")
        {
          ...
          LintConfig {
            ...
            default_jsx_fragment_factory: default_jsx_fragment_factory.map(String::from),
          }
        } else {
          LintConfig {
...
impl LoadPluginsThread {
  fn create() -> Self {
    let (request_tx, mut request_rx) = tokio::sync::mpsc::unbounded_channel::<LoadPluginsRequest>();
...
static LOAD_PLUGINS_THREAD: Lazy<LoadPluginsThread> = Lazy::new(LoadPluginsThread::create);
