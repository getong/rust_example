...
use super::{config::ConfigData, search::PackageSearchApi};
use crate::{
  args::{jsr_api_url, jsr_url},
  file_fetcher::{CliFileFetcher, TextDecodedFile},
  jsr::{JsrFetchResolver, partial_jsr_package_version_info_from_slice},
  sys::CliSys,
};
...
impl JsrCacheResolver {
  ...
  pub fn jsr_to_resource_url(&self, req_ref: &JsrPackageReqReference) -> Option<ModuleSpecifier> {
    ...
    if let Some(workspace_package) = self.workspace_packages_by_name.get(&nv.name) {
  ...
  pub fn lookup_bare_specifier_for_workspace_file(&self, specifier: &Url) -> Option<String> {
    ...
    let export = Self::lookup_export_for_version_info(&workspace_package.version_info, path)?;
    if export == "." {
      Some(name.to_string())
    } else {
  ...
  pub fn lookup_export_for_path(&self, nv: &PackageNv, path: &str) -> Option<String> {
    let info = self.package_version_info(nv)?;
    Self::lookup_export_for_version_info(&info, path)
  }

  fn lookup_export_for_version_info(info: &JsrPackageVersionInfo, path: &str) -> Option<String> {
    ...
    for (export, path_) in info.exports() {
      ...
      if sloppy_fallback.is_none() {
        ...
        let path_ = path_
          ...
          .unwrap_or(path_);
        if path_ == path {
          sloppy_fallback = Some(export.strip_prefix("./").unwrap_or(export).to_string());
        }
      }
    }
    sloppy_fallback
  ...
  pub fn package_info(&self, name: &StackString) -> Option<Arc<JsrPackageInfo>> {
  ...
  pub fn package_version_info(&self, nv: &PackageNv) -> Option<Arc<JsrPackageVersionInfo>> {
...
fn read_cached_url(url: &ModuleSpecifier, cache: &Arc<dyn HttpCache>) -> Option<Vec<u8>> {
  cache
...
impl PackageSearchApi for CliJsrSearchApi {
  ...
  async fn exports(&self, nv: &PackageNv) -> Result<Arc<Vec<String>>, AnyError> {
