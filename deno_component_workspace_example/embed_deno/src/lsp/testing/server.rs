...
use super::{definitions::TestModule, execution::TestRun, lsp_custom};
use crate::lsp::{
...
fn as_delete_notification(url: &ModuleSpecifier) -> Result<TestingNotification, AnyError> {
...
pub type TestServerTests = Arc<tokio::sync::Mutex<HashMap<ModuleSpecifier, (TestModule, String)>>>;
...
impl TestServer {
  pub fn new(
    ...
    let (update_channel, mut update_rx) = mpsc::unbounded_channel::<Arc<StateSnapshot>>();
        ...
        loop {
          match update_rx.recv().await {
            ...
            Some(snapshot) => {
              ...
              let mut keys: HashSet<ModuleSpecifier> = tests.keys().cloned().collect();
              for document in snapshot
                .document_modules
                .documents
                .filtered_docs(|d| d.is_file_like() && d.is_diagnosable())
              {
                let Some(module) = snapshot.document_modules.primary_module(&document) else {
                  continue;
                };
                ...
                let valid = if let Some((_, old_script_version)) = tests.get(&module.specifier) {
                ...
                if !valid {
                  ...
                  let test_module = module
                    ...
                    .unwrap_or_else(|| TestModule::new(module.specifier.as_ref().clone()));
                  if !test_module.is_empty() {
                    if let Ok(params) = test_module.as_replace_notification(mru.as_deref()) {
                      client.send_test_notification(params);
                    }
                  } else if !was_empty && let Ok(params) = as_delete_notification(&module.specifier)
                  {
                    client.send_test_notification(params);
                  }
                  tests.insert(
  ...
  pub async fn run_request(
    ...
    let test_run = { TestRun::init(&params, self.tests.clone(), workspace_settings).await };
  ...
  pub(crate) fn update(&self, snapshot: Arc<StateSnapshot>) -> Result<(), AnyError> {
