...
fn parse_test_context_param(param: Option<&ast::Pat>) -> Option<(Option<String>, Option<String>)> {
    ...
    Some(ast::Pat::Object(object_pattern)) => {
      for prop in &object_pattern.props {
        match prop {
            ...
            match &key_value_pat_prop.key {
              ...
              ast::PropName::Ident(ident) => {
                if ident.sym.eq("step") {
                  if let ast::Pat::Ident(ident) = key_value_pat_prop.value.as_ref() {
          ...
          ast::ObjectPatProp::Assign(assign_pat_prop) if assign_pat_prop.key.sym.eq("step") => {
    ...
    match expr {
      ast::Expr::Object(obj_lit) => {
        ...
        for prop in &obj_lit.props {
          ...
          let ast::PropName::Ident(ast::IdentName { sym, .. }) = &key_value_prop.key else {
            continue;
          };
          if sym == "name" {
        ...
        let (id, _) = test_module.register(name, Some(range), false, parent_id.map(str::to_owned));
        if !should_recurse {
          return;
        }
        for prop in &obj_lit.props {
          ...
          match prop.as_ref() {
            ...
            ast::Prop::Method(method_prop) => {
              let ast::PropName::Ident(ast::IdentName { sym, .. }) = &method_prop.key else {
                continue;
              };
              if sym == "fn" {
                visit_fn(
      ...
      ast::Expr::Fn(fn_expr) => {
        if let Some(ast::Ident { sym, .. }) = fn_expr.ident.as_ref() {
          ...
          let (id, _) =
            test_module.register(name, Some(range), false, parent_id.map(str::to_owned));
          if !should_recurse {
            return;
          }
          visit_fn(&fn_expr.function, &id, text_info, test_module, is_describe);
...
impl Visit for TestCollector {
  fn visit_call_expr(&mut self, node: &ast::CallExpr) {
    ...
    let is_describe = match prop_chain {
      ...
      ["", "", "describe"] | ["", "describe", "ignore" | "only" | "skip"] => true,
  ...
  fn test_test_collector_unknown_test() {
    ...
    assert_eq!(
      ...
      &TestModule {
        specifier: test_module.specifier.clone(),
        defs: vec![(
          "6d05d6dc35548b86a1e70acaf24a5bc2dd35db686b35b685ad5931d201b4a918".to_string(),
          TestDefinition {
            id: "6d05d6dc35548b86a1e70acaf24a5bc2dd35db686b35b685ad5931d201b4a918".to_string(),
