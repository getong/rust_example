...
use deno_core::{ModuleSpecifier, url::Url};
use deno_path_util::url_to_file_path;

use crate::{
  cache::{DenoDir, GlobalHttpCache, HttpCache, LocalLspHttpCache},
  lsp::{
    config::Config,
...
impl LspCache {
  pub fn new(global_cache_url: Option<Url>) -> Self {
    ...
    let deno_dir_root = deno_cache_dir::resolve_deno_dir(&sys, global_cache_path)
      .expect("should be infallible with absolute custom root");
    let deno_dir = DenoDir::new(sys.clone(), deno_dir_root);
    let global = Arc::new(GlobalHttpCache::new(sys, deno_dir.remote_folder_path()));
    Self {
      deno_dir,
      global,
      ...
      .map(|(scope, config_data)| {
        (
          ...
          config_data
            .vendor_dir
            .as_ref()
            .map(|v| Arc::new(LocalLspHttpCache::new(v.clone(), self.global.clone()))),
        )
      })
      .collect();
  }
  ...
  pub fn for_specifier(&self, file_referrer: Option<&ModuleSpecifier>) -> Arc<dyn HttpCache> {
  ...
  pub fn unvendored_specifier(&self, specifier: &ModuleSpecifier) -> Option<ModuleSpecifier> {
