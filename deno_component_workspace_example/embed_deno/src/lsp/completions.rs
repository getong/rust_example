...
use deno_ast::{LineAndColumnIndex, SourceTextInfo};
use deno_core::{
  ModuleSpecifier, resolve_path, resolve_url,
...
use deno_semver::{jsr::JsrPackageReqReference, package::PackageNv};
...
use super::{
  ...
  documents::{DocumentModule, DocumentModules, ServerDocumentKind},
  jsr::CliJsrSearchApi,
  lsp_custom,
  npm::CliNpmSearchApi,
  registries::ModuleRegistry,
  resolver::LspResolver,
  search::PackageSearchApi,
  tsc,
};
use crate::{
  jsr::JsrFetchResolver,
  util::path::{is_importable_ext, relative_specifier},
};
...
fn to_narrow_lsp_range(text_info: &SourceTextInfo, range: deno_graph::PositionRange) -> lsp::Range {
...
pub async fn get_import_completions(
  ...
  .await
  {
    Some(completion_list) => Some(lsp::CompletionResponse::List(completion_list)),
    _ => {
      match get_npm_completions(&module.specifier, text, &range, npm_search_api).await {
        Some(completion_list) => Some(lsp::CompletionResponse::List(completion_list)),
        _ => {
          match get_node_completions(text, &range) {
            Some(completion_list) => Some(lsp::CompletionResponse::List(completion_list)),
            _ => {
              match get_import_map_completions(&module.specifier, text, &range, maybe_import_map) {
...
fn get_node_completions(specifier: &str, range: &lsp::Range) -> Option<CompletionList> {
  ...
  let specifiers = document_modules
    ...
    .filter_map(|d| {
      if let ServerDocumentKind::RemoteUrl { url, .. } = &d.kind {
        ...
        return Some(relative_specifier(&module.specifier, url).unwrap_or_else(|| url.to_string()));
      }
      None
    });
  specifiers
...
mod tests {
  ...
  use crate::{
    cache::HttpCache,
    lsp::{
      cache::LspCache, documents::LanguageId, search::tests::TestPackageSearchApi, urls::url_to_uri,
    },
  };
  ...
  fn setup(
    ...
    for (specifier, source, version, language_id) in open_sources {
      let specifier = resolve_url(specifier).expect("failed to create specifier");
      let uri = url_to_uri(&specifier).unwrap();
      document_modules.open_document(uri, *version, *language_id, (*source).into(), None);
    }
  ...
  fn test_get_local_completions() {
    ...
    let specifier = ModuleSpecifier::from_file_path(file_c).expect("could not create");
    ...
    for item in actual.items {
      match item.text_edit {
        Some(lsp::CompletionTextEdit::Edit(text_edit)) => {
          assert!(["./b", "./f.mjs", "./g.json"].contains(&text_edit.new_text.as_str()));
        }
        _ => unreachable!(),
      }
    }
  ...
  async fn test_get_remote_completions() {
    ...
    let module = document_modules
      ...
      .unwrap();
    let actual = get_remote_completions(&module, "h", &range, &document_modules);
    assert_eq!(
      actual,
      vec![lsp::CompletionItem {
  ...
  fn test_parse_bare_specifier_version_index() {
    ...
    assert_eq!(
      parse_bare_specifier_version_index("@longer_than_right_one/arbitrary_string@"),
      Some(39)
    );
  }
  ...
  async fn test_get_jsr_completions() {
    ...
    let actual = get_jsr_completions(&referrer, "jsr:as", &range, None, &jsr_search_api, None)
      .await
      .unwrap();
    assert_eq!(
      actual,
  ...
  async fn test_get_npm_completions() {
    ...
    let referrer = ModuleSpecifier::parse("file:///referrer.ts").unwrap();
    let actual = get_npm_completions(&referrer, "npm:puppe", &range, &npm_search_api)
      .await
      .unwrap();
    assert_eq!(
      actual,
  ...
  async fn test_get_npm_completions_for_versions() {
    ...
    let referrer = ModuleSpecifier::parse("file:///referrer.ts").unwrap();
    let actual = get_npm_completions(&referrer, "npm:puppeteer@", &range, &npm_search_api)
      .await
      .unwrap();
    assert_eq!(
      actual,
