...
impl Performance {
  ...
  /// structure, which is then passed to `.measure()` to finalize the duration
  /// and add it to the internal buffer.
  pub fn mark_with_args<S: AsRef<str>, V: Serialize>(&self, name: S, args: V) -> PerformanceMark {
  ...
  /// ```
  pub fn measure_scope<S: AsRef<str>>(self: &Arc<Self>, name: S) -> PerformanceScopeMark {
