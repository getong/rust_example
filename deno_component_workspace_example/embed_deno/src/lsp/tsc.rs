...
use deno_core::{
  JsRuntime, ModuleSpecifier, OpState, PollEventLoopOptions, RuntimeOptions,
  anyhow::anyhow,
  convert::{Smi, ToV8},
  error::AnyError,
  futures::FutureExt,
...
use deno_lib::{util::result::InfallibleResultExt, worker::create_isolate_create_params};
...
use super::{
  ...
  config,
  documents::{DocumentModule, DocumentText},
  language_server,
  language_server::StateSnapshot,
  logging::lsp_log,
  performance::{Performance, PerformanceMark},
  refactor::{
    ALL_KNOWN_REFACTOR_ACTION_KINDS, EXTRACT_CONSTANT, EXTRACT_INTERFACE, EXTRACT_TYPE,
    RefactorCodeActionData,
  },
  semantic_tokens,
  semantic_tokens::SemanticTokensBuilder,
  text::LineIndex,
  urls::{uri_to_url, url_to_uri},
};
use crate::{
  args::{FmtOptionsConfig, jsr_url},
  lsp::{documents::Document, logging::lsp_warn, resolver::SingleReferrerGraphResolver},
  tsc::{MISSING_DEPENDENCY_SPECIFIER, ResolveArgs},
  util::{
    path::{relative_specifier, to_percent_decoded_str},
    v8::convert,
  },
};

static BRACKET_ACCESSOR_RE: Lazy<Regex> = lazy_regex!(r#"^\[['"](.+)[\['"]\]$"#);
static CAPTION_RE: Lazy<Regex> = lazy_regex!(r"<caption>(.*?)</caption>\s*\r?\n((?:\s|\S)*)");
...
static JSDOC_LINKS_RE: Lazy<Regex> =
  lazy_regex!(r"(?i)\{@(link|linkplain|linkcode) (https?://[^ |}]+?)(?:[| ]([^{}\n]+?))?\}");
...
const FILE_EXTENSION_KIND_MODIFIERS: &[&str] = &[".d.ts", ".ts", ".tsx", ".js", ".jsx", ".json"];
...
struct FormatCodeSettings {
  ...
  insert_space_after_opening_and_before_closing_nonempty_parenthesis: Option<bool>,
  ...
  insert_space_after_opening_and_before_closing_template_string_braces: Option<bool>,
  insert_space_after_opening_and_before_closing_jsx_expression_braces: Option<bool>,
...
impl From<&FmtOptionsConfig> for FormatCodeSettings {
  fn from(config: &FmtOptionsConfig) -> Self {
    FormatCodeSettings {
      ...
      insert_space_after_opening_and_before_closing_nonempty_parenthesis: Some(false),
      insert_space_after_opening_and_before_closing_nonempty_brackets: Some(false),
      insert_space_after_opening_and_before_closing_nonempty_braces: Some(true),
      insert_space_after_opening_and_before_closing_template_string_braces: Some(false),
      insert_space_after_opening_and_before_closing_jsx_expression_braces: Some(false),
...
pub struct PendingChange {
  ...
  pub new_compiler_options_by_key: Option<BTreeMap<CompilerOptionsKey, Arc<CompilerOptions>>>,
...
impl<'a> ToV8<'a> for PendingChange {
  ...
  fn to_v8(
    ...
    let modified_scripts = {
      let mut modified_scripts_v8 = Vec::with_capacity(self.modified_scripts.len());
      for (specifier, kind) in &self.modified_scripts {
        ...
        let pair = v8::Array::new_with_elements(scope, &[specifier, kind]).into();
        modified_scripts_v8.push(pair);
      }
      v8::Array::new_with_elements(scope, &modified_scripts_v8).into()
    };
    let project_version = v8::Integer::new_from_unsigned(scope, self.project_version as u32).into();
    let new_compiler_options_by_key =
      if let Some(new_compiler_options_by_key) = self.new_compiler_options_by_key {
        serde_v8::to_v8(
          scope,
    ...
    let new_notebook_keys = if let Some(new_notebook_keys) = self.new_notebook_keys {
      serde_v8::to_v8(scope, new_notebook_keys.into_iter().collect::<Vec<_>>()).unwrap_or_else(
        |err| {
          lsp_warn!("Couldn't serialize ts configs: {err}");
          v8::null(scope).into()
        },
...
impl PendingChange {
  fn coalesce(
    ...
    new_compiler_options_by_key: Option<BTreeMap<CompilerOptionsKey, Arc<CompilerOptions>>>,
    ...
    for (spec, new) in modified_scripts {
      if let Some((_, current)) = self.modified_scripts.iter_mut().find(|(s, _)| s == &spec) {
  ...
  pub fn project_changed(
    ...
    let modified_scripts = documents
      ...
      .filter_map(|(document, change_kind)| {
        let (specifier, media_type) = snapshot.document_modules.primary_specifier(document)?;
  ...
  pub async fn get_code_fixes(
    ...
    let req = TscRequest::GetCodeFixesAtPosition(Box::new((
      ...
      (&snapshot
        ...
        .options)
        .into(),
      UserPreferences::from_config_for_specifier(&snapshot.config, &module.specifier),
    )));
    self
      ...
      .and_then(|mut changes| {
        for changes in &mut changes {
          ...
          for text_changes in &mut changes.text_changes {
            ...
            text_changes.new_text = to_percent_decoded_str(&text_changes.new_text);
          }
        }
        Ok(changes)
      })
...
/// (extensions/typescript-language-features) https://github.com/microsoft/vscode/blob/bd2df940d74b51105aefb11304e028d2fb56a9dc/extensions/typescript-language-features/src/languageFeatures/completions.ts#L440
impl From<ScriptElementKind> for lsp::CompletionItemKind {
  fn from(kind: ScriptElementKind) -> Self {
    match kind {
      ...
      | ScriptElementKind::ParameterElement => lsp::CompletionItemKind::VARIABLE,
      ScriptElementKind::MemberVariableElement
      | ScriptElementKind::MemberGetAccessorElement
      | ScriptElementKind::MemberSetAccessorElement => lsp::CompletionItemKind::FIELD,
      ScriptElementKind::FunctionElement | ScriptElementKind::LocalFunctionElement => {
        lsp::CompletionItemKind::FUNCTION
      }
      ScriptElementKind::MemberFunctionElement
      | ScriptElementKind::ConstructSignatureElement
      | ScriptElementKind::CallSignatureElement
      | ScriptElementKind::IndexSignatureElement => lsp::CompletionItemKind::METHOD,
      ScriptElementKind::EnumElement => lsp::CompletionItemKind::ENUM,
      ScriptElementKind::EnumMemberElement => lsp::CompletionItemKind::ENUM_MEMBER,
      ScriptElementKind::ModuleElement | ScriptElementKind::ExternalModuleName => {
...
impl TextSpan {
  pub fn from_range(range: &lsp::Range, line_index: Arc<LineIndex>) -> Result<Self, AnyError> {
...
fn display_parts_to_string(
  ...
  for part in parts {
    match part.kind.as_str() {
      ...
      _ => out.push(
        // should decode percent-encoding string when hovering over the right edge of module
        // specifier like below module "file:///path/to/🦕"
        to_percent_decoded_str(&part.text),
        // NOTE: The reason why an example above that lacks `.ts` extension is caused by the
        // implementation of tsc itself. The request `tsc.request.getQuickInfoAtPosition`
        // receives the payload from tsc host as follows. {
        //   text_span: {
        //     start: 19,
        //     length: 9,
        //   },
  ...
  pub fn to_hover(
    ...
    if let Some(tags) = &self.tags {
      let tags_preview = tags
        .iter()
        .map(|tag_info| get_tag_documentation(tag_info, module, language_server))
        .collect::<Vec<String>>()
        .join("  \n\n");
...
impl DocumentSpan {
  fn normalize(&mut self, specifier_map: &TscSpecifierMap) -> Result<(), AnyError> {
...
impl DocumentSpan {
  pub fn to_link(
    ...
    let (target_range, target_selection_range) = if let Some(context_span) = &self.context_span {
      (
    ...
    let origin_selection_range = if let Some(original_context_span) = &self.original_context_span {
      Some(original_context_span.to_range(module.line_index.clone()))
    } else {
      self
        .original_text_span
        .as_ref()
        .map(|original_text_span| original_text_span.to_range(module.line_index.clone()))
    };
  ...
  pub fn to_lsp(
    ...
    let location = self.file.as_ref().and_then(|f| {
      ...
      let range = self
        ...
        .unwrap_or_else(|| lsp::Range::new(lsp::Position::new(0, 0), lsp::Position::new(0, 0)));
      Some(lsp::Location {
        uri: target_module.uri.as_ref().clone(),
        range,
      })
...
impl NavigationTree {
    ...
    for span in self.spans.iter() {
      ...
      for child in children.iter() {
        let should_traverse_child = child
          .spans
          .iter()
          .map(|child_span| TextRange::at(child_span.start.into(), child_span.length.into()))
          .any(|child_range| range.intersect(child_range).is_some());
        if should_traverse_child {
          let included_child =
            child.collect_document_symbols(line_index.clone(), &mut symbol_children);
      ...
      if should_include {
        ...
        if let Some(name_span) = self.name_span.as_ref() {
          let name_range = TextRange::at(name_span.start.into(), name_span.length.into());
  ...
  pub fn walk<F>(&self, token: &CancellationToken, callback: &F) -> Result<(), AnyError>
  where
...
impl RenameLocation {
  pub fn collect_into_workspace_edit(
    locations_with_modules: impl IntoIterator<Item = (RenameLocation, Arc<DocumentModule>)>,
    new_name: &str,
    language_server: &language_server::Inner,
    token: &CancellationToken,
    ...
    for (location, module) in locations_with_modules {
      ...
      let Some(target_module) = language_server.document_modules.module_for_specifier(
        &target_specifier,
        module.scope.as_deref(),
        Some(&module.compiler_options_key),
      ) else {
    ...
    if includes_non_files {
      language_server.client.show_message(
        lsp::MessageType::WARNING,
        "The renamed symbol had references in non-file schemed modules. These have not been \
         modified.",
      );
    }
  ...
  pub fn to_highlight(
    ...
    for hs in &self.highlight_spans {
      ...
      highlights.push(lsp::DocumentHighlight {
        ...
        kind: match hs.kind {
          HighlightSpanKind::WrittenReference => Some(lsp::DocumentHighlightKind::WRITE),
...
impl Classifications {
  pub fn to_semantic_tokens(
    ...
    for i in 0 .. token_count {
      ...
      let token_type = Classifications::get_token_type_from_classification(ts_classification);
      let token_modifiers =
        Classifications::get_token_modifier_from_classification(ts_classification);
  ...
  pub fn is_preferred(&self, all_actions: &[RefactorActionInfo]) -> bool {
    if EXTRACT_CONSTANT.matches(&self.name) {
      ...
      return if let Some(scope) = get_scope(&self.name) {
        all_actions
          .iter()
          .filter(|other| !std::ptr::eq(&self, other) && EXTRACT_CONSTANT.matches(&other.name))
          .all(|other| {
...
impl ApplicableRefactorInfo {
  pub fn to_code_actions(
    ...
    for action in self.actions.iter() {
      ...
      code_actions.push(self.as_inline_code_action(action, uri, range, &self.name));
    }
    Ok(code_actions)
  }
...
pub fn file_text_changes_to_workspace_edit<'a>(
  changes_with_modules: impl IntoIterator<Item = (&'a FileTextChanges, &'a Arc<DocumentModule>)>,
  language_server: &language_server::Inner,
  token: &CancellationToken,
  ...
  for (change, module) in changes_with_modules {
    ...
    let Some(ops) = change.to_text_document_change_ops(module, language_server) else {
      continue;
    };
    all_ops.extend(ops);
  }
...
impl CallHierarchyItem {
  ...
  fn is_source_file_item(&self) -> bool {
    self.kind == ScriptElementKind::ScriptElement
      || self.kind == ScriptElementKind::ModuleElement && self.selection_span.start == 0
  }
}
...
impl CallHierarchyIncomingCall {
  ...
  pub fn try_resolve_call_hierarchy_incoming_call(
    ...
    let (from, target_module) =
      self
        .from
        .to_call_hierarchy_item(module, language_server, maybe_root_path)?;
  ...
  if let Some(code_actions) = maybe_code_actions {
        ...
        if module.specifier.as_str() == change.file_name {
          additional_text_edits.extend(change.text_changes.iter().map(|tc| {
            ...
            if let Some(specifier_rewrite) = &data.specifier_rewrite {
              ...
              if let Some(deno_types_specifier) = &specifier_rewrite.new_deno_types_specifier {
    ...
    if has_remaining_commands_or_edits {
      ...
      command = Some(lsp::Command {
        ...
        arguments: Some(vec![json!(module.specifier.to_string()), json!(actions)]),
      });
    }
...
fn get_parameters_from_parts(parts: &[SymbolDisplayPart]) -> Vec<String> {
  ...
  for (idx, part) in parts.iter().enumerate() {
    if ["methodName", "functionName", "text", "propertyName"].contains(&part.kind.as_str()) {
      if paren_count == 0 && brace_count == 0 {
        is_in_fn = true;
      }
    } else if part.kind == "parameterName" {
      if paren_count == 1 && brace_count == 0 && is_in_fn {
        let is_optional = matches!(parts.get(idx + 1), Some(next) if next.text == "?");
        // Skip `this` and optional parameters.
        if !is_optional && part.text != "this" {
          parameters.push(format!("${{{}:{}}}", parameters.len() + 1, &part.text));
        }
      }
...
impl CompletionEntry {
  fn normalize(&mut self, specifier_map: &TscSpecifierMap) {
    ...
    let Ok(raw) = serde_json::from_value::<CompletionEntryDataAutoImport>(data.clone()) else {
      return;
    };
    if let Some(file_name) = &raw.file_name {
      if let Ok(normalized) = specifier_map.normalize(file_name) {
        self.auto_import_data = Some(CompletionNormalizedAutoImportData { raw, normalized });
      }
    } else if SUPPORTED_BUILTIN_NODE_MODULES.contains(&raw.module_specifier.as_str())
      && let Ok(normalized) = resolve_url(&format!("node:{}", &raw.module_specifier))
    {
      self.auto_import_data = Some(CompletionNormalizedAutoImportData { raw, normalized });
    }
  }
  ...
  fn get_filter_text(&self, context: Option<(&DocumentModule, u32)>) -> Option<String> {
  ...
  fn as_completion_item(
    ...
    let mut kind: Option<lsp::CompletionItemKind> = Some(self.kind.clone().into());
      ...
      && let Some(import_data) = &self.auto_import_data
    {
      ...
      let resolution_lookup = resolution_lookup_cache
        ...
        .or_insert_with(|| {
          if let Some(specifier) =
            import_mapper.check_specifier(&import_data.normalized, &module.specifier)
          {
          ...
          if let Some(specifier) = relative_specifier(&module.specifier, &import_data.normalized) {
            return ResolutionLookup::PrettySpecifier(specifier);
          }
          if Url::parse(&import_data.raw.module_specifier).is_ok() {
            return ResolutionLookup::PrettySpecifier(import_data.normalized.to_string());
          }
      ...
      if let ResolutionLookup::PrettySpecifier(new_specifier) = resolution_lookup {
        ...
        {
          new_deno_types_specifier = Some(std::mem::replace(&mut new_specifier, code_specifier));
        }
        display_source.clone_from(&new_specifier);
        if new_specifier != import_data.raw.module_specifier || new_deno_types_specifier.is_some() {
    ...
    let text_edit = if let (Some(text_span), Some(new_text)) = (range, &insert_text) {
...
impl OutliningSpan {
  pub fn to_folding_range(
    ...
    lsp::FoldingRange {
      ...
      end_line: self.adjust_folding_end_line(&range, line_index, content, line_folding_only),
  ...
  fn get_folding_range_kind(&self, span_kind: &OutliningSpanKind) -> Option<lsp::FoldingRangeKind> {
    match span_kind {
...
impl SignatureHelpItem {
  pub fn into_signature_information(
    ...
    let prefix_text = display_parts_to_string(&self.prefix_display_parts, module, language_server);
    let params_text = self
      .parameters
      .iter()
      .map(|param| display_parts_to_string(&param.display_parts, module, language_server))
      .collect::<Vec<String>>()
      .join(", ");
    let suffix_text = display_parts_to_string(&self.suffix_display_parts, module, language_server);
    let documentation = display_parts_to_string(&self.documentation, module, language_server);
    lsp::SignatureInformation {
      ...
      parameters: Some(
        self
          .parameters
          .into_iter()
          .map(|param| param.into_parameter_information(module, language_server))
          .collect(),
      ),
...
impl SelectionRange {
  pub fn to_selection_range(&self, line_index: Arc<LineIndex>) -> lsp::SelectionRange {
    lsp::SelectionRange {
      range: self.text_span.to_range(line_index.clone()),
      parent: self
        .parent
        .as_ref()
        .map(|parent_selection| Box::new(parent_selection.to_selection_range(line_index))),
    }
  }
}
...
impl TscSpecifierMap {
  ...
  pub fn denormalize(&self, specifier: &ModuleSpecifier, media_type: MediaType) -> String {
...
impl State {
  ...
  fn get_module(&self, specifier: &ModuleSpecifier) -> Option<Arc<DocumentModule>> {
...
fn op_load<'s>(
  ...
  let maybe_load_response = if let Some(source) = crate::tsc::load_raw_import_source(&specifier) {
    Some(LoadResponse {
...
impl<'a> ToV8<'a> for TscRequestArray {
  ...
  fn to_v8(
    ...
    let compiler_options_key = serde_v8::to_v8(scope, self.compiler_options_key)?;
...
#[op2(fast)]
fn op_respond(state: &mut OpState, #[string] response: String, #[string] error: String) {
...
fn span_with_context(_state: &State, span: super::trace::Span) -> super::trace::EnteredSpan {
...
#[op2(fast)]
fn op_make_span(op_state: &mut OpState, #[string] _s: &str, needs_context: bool) -> *const c_void {
  let state = op_state.borrow_mut::<State>();
  if !state.tracing_enabled() {
    return deno_core::ExternalPointer::new(TracingSpan(None)).into_raw();
  }
  let sp = super::logging::lsp_tracing_info_span!("js", otel.name = format!("js::{_s}").as_str());
...
fn op_script_names(state: &mut OpState) -> ScriptNames {
  let _span = super::logging::lsp_tracing_info_span!("op_script_names").entered();
  let state = state.borrow_mut::<State>();
    ...
    state.state_snapshot.compiler_options_resolver.entries()
  {
    ...
    for (referrer, relative_specifiers) in compiler_options_data
      ...
      .map(|(r, f)| {
        let relative_specifiers =
          Box::new(f.iter().map(|f| &f.relative_specifier)) as Box<dyn Iterator<Item = &String>>;
      ...
      )
    {
      let resolver = SingleReferrerGraphResolver {
        ...
        jsx_import_source_config: compiler_options_data.jsx_import_source_config.as_deref(),
      };
      for relative_specifier in relative_specifiers {
        ...
        if let Ok(req_ref) = deno_semver::npm::NpmPackageReqReference::from_specifier(&specifier) {
        ...
        let Some(module) = state.state_snapshot.document_modules.module_for_specifier(
          &specifier,
          scope.as_deref(),
          Some(compiler_options_key),
        ) else {
    ...
    .cells_by_notebook_uri()
  {
    ...
    if let Some(global_script_names) = result.by_compiler_options_key.get(compiler_options_key) {
  ...
  fn new(mut js_runtime: JsRuntime) -> Self {
    let server_main_loop_fn_global = {
      ...
      let server_main_loop_fn_str =
        v8::String::new_external_onebyte_static(scope, b"serverMainLoop").unwrap();
...
fn run_tsc_thread(
  ...
  let mut extensions = deno_runtime::snapshot_info::get_extensions_in_snapshot();
...
impl From<&config::InlayHintsParamNamesEnabled> for IncludeInlayParameterNameHints {
...
struct UserPreferences {
  ...
  #[serde(skip_serializing_if = "Option::is_none")]
  pub import_module_specifier_preference: Option<ImportModuleSpecifierPreference>,
  #[serde(skip_serializing_if = "Option::is_none")]
  ...
  #[serde(skip_serializing_if = "Option::is_none")]
  pub include_inlay_parameter_name_hints: Option<IncludeInlayParameterNameHints>,
  #[serde(skip_serializing_if = "Option::is_none")]
...
impl UserPreferences {
  pub fn from_config_for_specifier(config: &config::Config, specifier: &ModuleSpecifier) -> Self {
    ...
    let base_preferences = Self {
      ...
      include_completions_with_snippet_text: Some(config.snippet_support_capable()),
    ...
    let Some(language_settings) = config.language_settings_for_specifier(specifier) else {
    ...
    Self {
      ...
      include_completions_for_module_exports: Some(
        language_settings.suggest.enabled && language_settings.suggest.auto_imports,
      ),
      ...
      include_completions_with_insert_text: Some(language_settings.suggest.enabled),
      ...
      include_inlay_variable_type_hints: Some(language_settings.inlay_hints.variable_types.enabled),
...
enum TscRequest {
  ...
  GetEditsForFileRename(Box<(String, String, FormatCodeSettings, UserPreferences)>),
...
impl TscRequest {
  ...
  fn to_server_request<'s>(
    ...
    let args = match self {
      TscRequest::GetDiagnostics(args) => ("$getDiagnostics", Some(serde_v8::to_v8(scope, args)?)),
  ...
  fn method(&self) -> &'static str {
    match self {
      ...
      TscRequest::GetTypeDefinitionAtPosition(_) => "getTypeDefinitionAtPosition",
      ...
      TscRequest::GetImplementationAtPosition(_) => "getImplementationAtPosition",
      TscRequest::GetOutliningSpans(_) => "getOutliningSpans",
      TscRequest::ProvideCallHierarchyIncomingCalls(_) => "provideCallHierarchyIncomingCalls",
      ...
      TscRequest::GetEncodedSemanticClassifications(_) => "getEncodedSemanticClassifications",
...
mod tests {
  ...
  use crate::{
    ...
    lsp::{
      cache::LspCache,
      compiler_options::LspCompilerOptionsResolver,
      config::{Config, WorkspaceSettings},
      documents::{DocumentModules, LanguageId},
      lint::LspLinterResolver,
  ...
  async fn setup(
    ...
    config
      ...
      .await;
    let resolver = Arc::new(LspResolver::from_config(&config, &cache, None).await);
    let compiler_options_resolver = Arc::new(LspCompilerOptionsResolver::new(&config, &resolver));
  ...
  fn test_replace_links() {
    ...
    let actual = replace_links(r"test {@link http://deno.land/x/mod.ts a link} test");
    assert_eq!(actual, r"test [a link](http://deno.land/x/mod.ts) test");
    let actual = replace_links(r"test {@linkcode http://deno.land/x/mod.ts a link} test");
    assert_eq!(actual, r"test [`a link`](http://deno.land/x/mod.ts) test");
  }
  ...
  async fn test_modify_sources() {
    ...
    let scope = snapshot
      ...
      .scope_for_specifier(&specifier)
      .map(|s| s.as_ref());
    let dep_specifier = resolve_url("https://deno.land/x/example/a.ts").unwrap();
    snapshot
      .cache
      .global()
      .set(
  ...
  fn include_suppress_inlay_hint_settings() {
    ...
    assert_eq!(
      user_preferences.include_inlay_parameter_name_hints_when_argument_matches_name,
      Some(false)
    );
  }
