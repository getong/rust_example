...
use deno_lib::standalone::virtual_fs::{
  BuiltVfs, DENO_COMPILE_GLOBAL_NODE_MODULES_DIR_NAME, OffsetWithLength, VfsEntry,
  VirtualDirectory, VirtualDirectoryEntries, VirtualFile, VirtualSymlinkParts,
  WindowsSystemRootablePath,
};
...
fn vfs_as_display_tree(vfs: &BuiltVfs, executable_name: &str) -> DisplayTreeNode {
  ...
  impl EntryOutput<'_> {
    pub fn size(&self) -> Size {
      match self {
        ...
        EntryOutput::Subset(children) => children.iter().map(|c| c.output.size()).sum(),
      ...
      fn format_size(size: Size) -> String {
        if size.unique == size.total {
          ...
          format!(
            ...
            deno_terminal::colors::gray(format!(" - {} unique", human_size(size.unique as f64)))
          )
        }
      }
      ...
      vfs_dir
        ...
        .map(|entry| DirEntryOutput {
          ...
          output: match entry {
            ...
            VfsEntry::Symlink(virtual_symlink) => EntryOutput::Symlink(&virtual_symlink.dest_parts),
          },
        })
        .collect()
    }
  ...
  fn analyze_entry<'a>(
    ...
    match entry {
      VfsEntry::Dir(virtual_directory) => analyze_dir(path, virtual_directory, seen_offsets),
      VfsEntry::File(file) => EntryOutput::File(file_size(file, seen_offsets)),
      VfsEntry::Symlink(virtual_symlink) => EntryOutput::Symlink(&virtual_symlink.dest_parts),
    }
  }
  ...
  fn analyze_dir<'a>(
    ...
    if vfs_dir.name == DENO_COMPILE_GLOBAL_NODE_MODULES_DIR_NAME {
      return EntryOutput::Subset(show_global_node_modules_dir(vfs_dir, seen_offsets));
    }
  ...
  let mut child_entries = include_all_entries(&vfs.root_path, &vfs.entries, &mut seen_offsets);
