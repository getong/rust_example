...
use deno_ast::{MediaType, ModuleKind, ModuleSpecifier};
...
use deno_lib::{
  ...
  shared::ReleaseChannel,
  standalone::{
    binary::{
      CjsExportAnalysisEntry, MAGIC_BYTES, Metadata, NodeModules, RemoteModuleEntry,
      SerializedResolverWorkspaceJsrPackage, SerializedWorkspaceResolver,
      SerializedWorkspaceResolverImportMap, SpecifierDataStore, SpecifierId,
    },
    virtual_fs::{
      BuiltVfs, DENO_COMPILE_GLOBAL_NODE_MODULES_DIR_NAME, VfsBuilder, VfsEntry, VirtualDirectory,
      VirtualDirectoryEntries, WindowsSystemRootablePath,
    },
  },
  util::{hash::FastInsecureHasher, text_encoding::is_valid_utf8, v8::construct_v8_flags},
  version::DENO_VERSION_INFO,
};
use deno_npm::{
  NpmSystemInfo,
  resolution::{SerializedNpmResolutionSnapshot, ValidSerializedNpmResolutionSnapshot},
};
use deno_path_util::{
  fs::atomic_write_file_with_retries, url_from_directory_path, url_to_file_path,
};
use deno_resolver::{
  file_fetcher::{FetchLocalOptions, FetchOptions, FetchPermissionsOptionRef},
...
use crate::{
  args::{CliOptions, CompileFlags, get_default_v8_flags},
  cache::DenoDir,
  file_fetcher::CliFileFetcher,
  http_util::HttpClientProvider,
  module_loader::CliEmitter,
  node::CliCjsModuleExportAnalyzer,
  npm::CliNpmResolver,
  resolver::CliCjsTracker,
  sys::CliSys,
  util::{
    archive,
...
impl<'a> DenoCompileBinaryWriter<'a> {
  ...
  pub async fn write_bin(&self, options: WriteBinOptions<'_>) -> Result<(), AnyError> {
    // Select base binary based on target
    let mut original_binary = self.get_base_binary(options.compile_flags).await?;
    ...
    if options.compile_flags.no_terminal {
      ...
      set_windows_binary_to_gui(&mut original_binary).context("Setting windows binary to GUI.")?;
    }
  ...
  async fn get_base_binary(&self, compile_flags: &CompileFlags) -> Result<Vec<u8>, AnyError> {
    ...
    if let Some(path) = get_dev_binary_path() {
      ...
      return std::fs::read(&path)
        .with_context(|| format!("Could not find denort at '{}'", path.to_string_lossy()));
    }
  ...
  async fn download_base_binary(
    ...
    let response = {
      ...
      self
        .http_client_provider
        .get_or_create()?
        .download_with_progress_and_retries(download_url.parse()?, &Default::default(), &progress)
        .await?
    };
    let bytes = response
      .into_bytes()
    ...
    atomic_write_file_with_retries(&CliSys::default(), output_path, &bytes, CACHE_PERM)
  ...
  async fn write_standalone_binary(
    ...
    let ca_data = match self.cli_options.ca_data() {
      Some(CaData::File(ca_file)) => {
        Some(std::fs::read(ca_file).with_context(|| format!("Reading {ca_file}"))?)
      }
    ...
    let mut remote_modules_store = SpecifierDataStore::with_capacity(specifiers_count);
      ...
      let (maybe_original_source, media_type) = match module {
        deno_graph::Module::Js(m) => {
          ...
          if m.media_type.is_emittable() {
            ...
            let (source, source_map) = self.emitter.emit_source_for_deno_compile(
              &m.specifier,
              m.media_type,
              module_kind,
              &m.source.text,
        ...
        deno_graph::Module::Wasm(m) => (Some(m.source.clone()), MediaType::Wasm),
        ...
        if module.specifier().scheme() == "file" {
          ...
          vfs
            .add_file_with_data(
              ...
              deno_lib::standalone::virtual_fs::AddFileDataOptions {
                ...
                mtime: file_path.metadata().ok().and_then(|m| m.modified().ok()),
              },
            )
            .with_context(|| format!("Failed adding '{}'", file_path.display()))?;
        } else {
          ...
          remote_modules_store.add(
            ...
            RemoteModuleEntry {
              ...
              maybe_cjs_export_analysis: maybe_cjs_export_analysis.map(Cow::Owned),
            },
          );
        }
      }
    ...
    let mut redirects_store = SpecifierDataStore::with_capacity(graph.redirects.len());
    ...
    for (file_path, file) in vfs.iter_files() {
      ...
      if self.cjs_tracker.is_maybe_cjs(&specifier, media_type)? {
        ...
        let analysis = match cjs_analysis_result {
          ...
          Err(err) => {
            log::debug!("Had cjs export analysis error for '{}': {}", specifier, err);
    ...
    let root_dir_url = match &vfs.root_path {
      WindowsSystemRootablePath::Path(dir) => Some(url_from_directory_path(dir)?),
    ...
    let code_cache_key = if self.cli_options.code_cache_enabled() {
      ...
      for module in graph.modules() {
        if let Some(source) = module.source() {
          hasher.write(root_dir_url.specifier_key(module.specifier()).as_bytes());
    ...
    let node_modules = match &self.npm_resolver {
      CliNpmResolver::Managed(_) => npm_snapshot.as_ref().map(|_| NodeModules::Managed {
        node_modules_dir: self.npm_resolver.root_node_modules_path().map(|path| {
          root_dir_url
            .specifier_key(&ModuleSpecifier::from_directory_path(path).unwrap())
            .into_owned()
        }),
      }),
      CliNpmResolver::Byonm(resolver) => Some(NodeModules::Byonm {
        root_node_modules_dir: resolver.root_node_modules_path().map(|node_modules_dir| {
          root_dir_url
            .specifier_key(&ModuleSpecifier::from_directory_path(node_modules_dir).unwrap())
            .into_owned()
        }),
      }),
    };
    ...
    let env_vars_from_env_file = match self.cli_options.env_file_name() {
      Some(env_filenames) => {
        ...
        for env_filename in env_filenames.iter().rev() {
          log::info!(
            "{} Environment variables from the file \"{}\" were embedded in the generated \
             executable file",
    ...
    let metadata = Metadata {
      ...
      v8_flags: construct_v8_flags(&get_default_v8_flags(), self.cli_options.v8_flags(), vec![]),
  ...
  ) -> Result<deno_cache_dir::file_fetcher::File, deno_resolver::file_fetcher::FetchError> {
    self
      .file_fetcher
      .fetch_with_options(
        specifier,
        ...
        FetchOptions {
          ...
          maybe_auth: None,
          maybe_accept: None,
          maybe_cache_setting: Some(&deno_cache_dir::file_fetcher::CacheSetting::Use),
        },
      )
      .await
  }
    ...
    match &self.npm_resolver {
      CliNpmResolver::Managed(npm_resolver) => {
        if let Some(node_modules_path) = npm_resolver.root_node_modules_path() {
          ...
          let mut packages = snapshot.as_serialized().packages.iter().collect::<Vec<_>>();
          packages.sort_by(|a, b| a.id.cmp(&b.id)); // determinism
          let current_system = NpmSystemInfo::default();
          for package in packages {
            let folder = npm_resolver.resolve_pkg_folder_from_pkg_id(&package.id)?;
            if !package.system.matches_system(&current_system) && !folder.exists() {
              log::warn!(
  ...
  fn build_vfs_consolidating_global_npm_cache(&self, mut vfs: VfsBuilder) -> BuiltVfs {
    match &self.npm_resolver {
      CliNpmResolver::Managed(npm_resolver) => {
        ...
        // Flatten all the registries folders into a single ".deno_compile_node_modules/localhost"
        // folder that will be used by denort when loading the npm cache. This avoids us
        // exposing the user's private registry information and means we don't have to
        // bother serializing all the different registry config into the binary.
          ...
          match entry {
            VfsEntry::Dir(mut dir) => {
              for entry in dir.entries.take_inner() {
                ...
                if let Some(existing) = localhost_entries.insert(entry.name().to_string(), entry) {
                  panic!(
        ...
        let index = parent_dir
          .entries
          .binary_search(DENO_COMPILE_GLOBAL_NODE_MODULES_DIR_NAME, case_sensitivity)
          .unwrap();
        ...
        let mut last_name = Cow::Borrowed(DENO_COMPILE_GLOBAL_NODE_MODULES_DIR_NAME);
        for ancestor in parent.ancestors().map(Some).chain(std::iter::once(None)) {
          ...
          if let Ok(index) = dir.entries.binary_search(&last_name, case_sensitivity) {
...
fn write_binary_bytes(
  ...
  if target.contains("linux") {
    libsui::Elf::new(&original_bin).append("d3n0l4nd", &data_section_bytes, &mut file_writer)?;
...
fn serialize_binary_data_section(
  ...
  let npm_snapshot = npm_snapshot.map(serialize_npm_snapshot).unwrap_or_default();
...
fn serialize_npm_snapshot(mut snapshot: SerializedNpmResolutionSnapshot) -> Vec<u8> {
...
fn get_file_env_vars(filename: String) -> Result<IndexMap<String, String>, dotenvy::Error> {
