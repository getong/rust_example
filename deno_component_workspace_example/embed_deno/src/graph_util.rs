...
use deno_config::{
  deno_json,
  deno_json::{CompilerOptionTypesDeserializeError, NodeModulesDirMode},
  workspace::JsrPackageConfig,
};
use deno_core::{ModuleSpecifier, error::AnyError, parking_lot::Mutex, serde_json};
...
use deno_resolver::{
  ...
  deno_json::{
    CompilerOptionsResolver, JsxImportSourceConfigResolver, ToMaybeJsxImportSourceConfigError,
  },
  file_fetcher::GraphLoaderReporterRc,
  graph::{
    EnhanceGraphErrorMode, enhance_graph_error, enhanced_integrity_error_message,
    format_deno_graph_error,
  },
  npm::DenoInNpmPackageChecker,
};
use deno_runtime::deno_permissions::PermissionsContainer;
use deno_semver::{SmallStackString, jsr::JsrDepPackageReq};
use import_map::ImportMapErrorKind;
use node_resolver::errors::NodeJsErrorCode;
use sys_traits::FsMetadata;

use crate::{
  args::{CliLockfile, CliOptions, config_to_deno_graph_workspace_member, jsr_url},
  cache,
  cache::{GlobalHttpCache, ModuleInfoCache},
  colors,
  file_fetcher::{CliDenoGraphLoader, CliFileFetcher},
  npm::{CliNpmGraphResolver, CliNpmInstaller, CliNpmResolver},
  resolver::{CliCjsTracker, CliResolver},
  sys::CliSys,
  type_checker::{CheckError, CheckOptions, TypeChecker},
  util::{file_watcher::WatcherCommunicator, fs::canonicalize_path, progress_bar::ProgressBar},
};
...
fn has_module_graph_error_for_tsc_diagnostic(sys: &CliSys, error: &ModuleGraphError) -> bool {
  match error {
    ModuleGraphError::ModuleError(error) => module_error_for_tsc_diagnostic(sys, error).is_some(),
...
pub fn resolution_error_for_tsc_diagnostic(
  ...
  match error {
    ...
    ResolutionError::InvalidSpecifier { error, range } => match error {
      ...
      SpecifierError::ImportPrefixMissing { specifier, .. } => Some(ResolutionErrorRef {
        specifier,
        range,
    ...
    ResolutionError::ResolverError {
      ...
      ResolveError::Other(error) => {
        if let Some(error) = error.get_ref().downcast_ref::<import_map::ImportMapError>() {
          match error.as_kind() {
            ...
            ImportMapErrorKind::UnmappedBareSpecifier(specifier, _) => Some(ResolutionErrorRef {
              specifier,
              range,
              is_module_not_found: false,
            }),
...
impl ModuleGraphCreator {
  ...
  pub async fn create_publish_graph(
    ...
    if self.options.type_check_mode().is_true() && !graph_has_external_remote(&graph) {
    ...
    if options.build_fast_check_graph {
      ...
      self.module_graph_builder.build_fast_check_graph(
        ...
        BuildFastCheckGraphOptions {
          workspace_fast_check: WorkspaceFastCheckOption::Enabled(&fast_check_workspace_members),
        },
      )?;
    }
  ...
  fn type_check_graph(&self, graph: ModuleGraph) -> Result<Arc<ModuleGraph>, CheckError> {
...
pub enum BuildGraphWithNpmResolutionError {
  ...
  #[class(generic)]
  #[error(
    "Resolving npm specifier entrypoints this way is currently not supported with \
     \"nodeModules\": \"manual\". In the meantime, try with --node-modules-dir=auto instead"
  )]
...
impl ModuleGraphBuilder {
  ...
  pub async fn build_graph_with_npm_resolution(
    ...
    let mut loader = match options.loader {
      ...
      None => MutLoaderRef::Owned(self.create_graph_loader_with_root_permissions()),
    };
  ...
  pub fn create_graph_loader_with_root_permissions(&self) -> CliDenoGraphLoader {
    self.create_graph_loader_with_permissions(self.root_permissions_container.clone())
  }
  ...
  pub fn graph_roots_valid(
    ...
    graph_valid(
      ...
      GraphValidOptions {
        ...
        will_type_check,
        check_js: CheckJsOption::Custom(self.compiler_options_resolver.as_ref()),
...
impl deno_graph::source::Reporter for FileWatcherReporter {
  fn on_load(&self, specifier: &ModuleSpecifier, modules_done: usize, modules_total: usize) {
