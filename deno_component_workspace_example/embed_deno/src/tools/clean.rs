...
use deno_graph::{ModuleGraph, packages::PackageSpecifiers};
use deno_npm_installer::graph::NpmCachingStrategy;
use sys_traits::{FsCanonicalize, FsCreateDirAll};
use walkdir::WalkDir;
...
use crate::{
  ...
  util::{
    fs::FsCleaner,
    progress_bar::{ProgressBar, ProgressBarStyle, ProgressMessagePrompt},
  },
};

pub async fn clean(flags: Arc<Flags>, clean_flags: CleanFlags) -> Result<(), AnyError> {
  if !clean_flags.except_paths.is_empty() {
    return clean_except(flags, &clean_flags.except_paths, clean_flags.dry_run).await;
  }
  ...
  if deno_dir.root.exists() {
    ...
    let progress_guard = progress_bar.update_with_prompt(ProgressMessagePrompt::Cleaning, "");
...
async fn clean_except(
  ...
  let deno_dir_root_canonical =
    try_get_canonicalized_root_dir(&sys, &deno_dir.root).unwrap_or(deno_dir.root.clone());
  ...
  if deno_dir_root_canonical != deno_dir.root {
    keep_paths_trie.add_rewrite(deno_dir.root.clone(), deno_dir_root_canonical.clone());
  }
  ...
  for package in snap.all_system_packages(&options.npm_system_info()) {
    ...
    keep_paths_trie.insert(
      npm_cache.package_folder_for_id(&deno_npm::NpmPackageCacheFolderId {
  ...
  if let Some(dir) = node_modules_path {
    clean_node_modules(&mut node_modules_cleaned, &node_modules_keep, dir, dry_run)?;
  }
    ...
    && let GlobalOrLocalHttpCache::Local(cache) = local_or_global_http_cache
  {
    ...
    add_jsr_meta_paths(
      ...
      &mut trie,
      jsr_url,
      &|url| match cache.local_path_for_url(url) {
        Ok(path) => Ok(path),
        Err(err) => {
          log::warn!("failed to get local path for jsr meta url {}: {}", url, err);
          Ok(None)
        }
      },
    )?;
...
fn log_stats(cleaner: &FsCleaner, dir: &Path) {
  if cleaner.bytes_removed == 0 && cleaner.dirs_removed == 0 && cleaner.files_removed == 0 {
    return;
  }
  log::info!(
    "{} {}",
...
fn clean_node_modules(
  ...
  let mut setup_cache =
    deno_npm_installer::LocalSetupCache::load(CliSys::default(), base.join(".setup-cache.bin"));
  ...
  clean_node_modules_symlinks(cleaner, &keep_names, dir, dry_run, &mut |name| {
...
mod tests {
  ...
  fn path_trie() {
    ...
    #[cfg(unix)]
    {
      trie.add_rewrite(Path::new("/RewriteMe").into(), Path::new("/Actual").into());
    }
    #[cfg(windows)]
    {
      trie.add_rewrite(
