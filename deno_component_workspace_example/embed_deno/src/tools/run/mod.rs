...
use deno_core::{anyhow::Context, error::AnyError, futures::FutureExt};
use deno_lib::{
  standalone::binary::SerializedWorkspaceResolverImportMap, worker::LibWorkerFactoryRoots,
};
...
use crate::{
  args::{EvalFlags, Flags, RunFlags, WatchFlagsWithPaths},
  factory::CliFactory,
  util,
  util::{file_watcher::WatcherRestartMode, watch_env_tracker::WatchEnvTracker},
};
...
pub async fn run_script_with_extension(
  ...
  if watch.is_some() {
    // Avoid implementing the watch runner path twice; it will be close enough
    // to `deno run` for now.
...
pub async fn eval_command(flags: Arc<Flags>, eval_flags: EvalFlags) -> Result<i32, AnyError> {
  let factory = CliFactory::from_flags(flags);
  let cli_options = factory.cli_options()?;
  let file_fetcher = factory.file_fetcher()?;
  let main_module = cli_options.resolve_main_module()?;
...
async fn load_import_map(
  ...
  let json_value = jsonc_parser::parse_to_serde_value(&text, &ParseOptions::default())
    .map_err(|err| AnyError::msg(format!("import map failed to parse: {err}")))?
