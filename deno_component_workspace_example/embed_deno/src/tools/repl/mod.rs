...
use deno_core::{error::AnyError, futures::StreamExt, serde_json, unsync::spawn_blocking};
...
use crate::{
  args::{CliOptions, Flags, ReplFlags},
  cdp, colors,
  factory::CliFactory,
...
use channel::{
  RustylineSyncMessage, RustylineSyncMessageHandler, RustylineSyncResponse, rustyline_channel,
};
use editor::{EditorHelper, ReplEditor};
pub use session::{EvaluationOutput, ReplSession, TsEvaluateResponse};
...
async fn read_eval_file(
  ...
  let specifier = deno_path_util::resolve_url_or_path(eval_file, cli_options.initial_cwd())?;
...
#[allow(clippy::print_stdout)]
pub async fn run(flags: Arc<Flags>, repl_flags: ReplFlags) -> Result<i32, AnyError> {
    ...
    if let ReplMessage::Run { code, output } = command {
      ...
      match repl_session.closing().await {
        ...
        Err(err) => {
          ...
          sender
            .write_all_buf(
              &mut Bytes::from_owner((buf.len() as u32).to_le_bytes()).chain(Bytes::from(buf)),
            )
            .await?;
        }
      };
