...
use deno_config::glob::{FileCollector, FilePatterns, PathOrPattern, PathOrPatternSet};
use deno_core::{
...
use self::ignore_directives::{
  has_file_ignore_directive, lex_comments, parse_next_ignore_directives,
  parse_range_ignore_directives,
};
use crate::{
  args::{CliOptions, FileFlags, Flags},
  cdp,
  factory::CliFactory,
  file_fetcher::TextDecodedFile,
  sys::CliSys,
  tools::test::is_supported_test_path,
  util::text_encoding::source_map_from_code,
};
...
fn generate_coverage_report(
  ...
  let original_comments = lex_comments(&options.script_original_source, options.script_media_type);
  ...
  let coverage_ignore_next_directives =
    parse_next_ignore_directives(&original_comments.comments, &original_text_lines);
  ...
  let runtime_comments = lex_comments(&options.script_runtime_source, MediaType::JavaScript);
  ...
  for function in &options.script_coverage.functions {
    ...
    let line_index =
      range_to_src_line_index(&function.ranges[0], &runtime_text_lines, &maybe_source_map);

    if line_index > 0 && coverage_ignore_next_directives.contains(&(line_index - 1_usize)) {
      continue;
    }

    if coverage_ignore_range_directives
      .iter()
      .any(|range| range.start_line_index <= line_index && range.stop_line_index >= line_index)
    {
      continue;
    }
  ...
  for (block_number, function) in options.script_coverage.functions.iter().enumerate() {
    let block_hits = function.ranges[0].count;
    for (branch_number, range) in function.ranges[1 ..].iter().enumerate() {
      let line_index = range_to_src_line_index(range, &runtime_text_lines, &maybe_source_map);
  ...
  for line_index in 0 .. runtime_text_lines.lines_count() {
    let (line_start_byte_offset, line_end_byte_offset) = runtime_text_lines.line_range(line_index);
    let line_start_char_offset = runtime_text_lines.char_index(line_start_byte_offset);
    let line_end_char_offset = runtime_text_lines.char_index(line_end_byte_offset);
    let ignore = runtime_comments.comments.iter().any(|comment| {
      comment.range.start <= line_start_byte_offset && comment.range.end >= line_end_byte_offset
  ...
  let found_lines_coverage_filter = |(line, _): &(usize, i64)| -> bool {
    if coverage_ignore_range_directives
      .iter()
      .any(|range| range.start_line_index <= *line && range.stop_line_index >= *line)
    {
      return false;
    }
  ...
  coverage_report.found_lines = if let Some(source_map) = maybe_source_map.as_ref() {
    let script_runtime_source_lines = options.script_runtime_source.lines().collect::<Vec<_>>();
          ...
          .filter(|token| {
            ...
            dst_line == index && {
              ...
              let content = script_runtime_source_lines
                .get(dst_line)
                .and_then(|line| line.get(dst_col .. std::cmp::min(dst_col + 2, line.len())))
                .unwrap_or("");
...
fn range_to_src_line_index(
  ...
  let source_lc = text_lines
    .line_and_column_index(text_lines.byte_index_from_char_index(range.start_char_offset));
...
fn collect_coverages(
  ...
  let file_patterns = FilePatterns {
    ...
    include: Some({
      if files.include.is_empty() {
        PathOrPatternSet::new(vec![PathOrPattern::Path(initial_cwd.to_path_buf())])
      } else {
        PathOrPatternSet::from_include_relative_path_or_patterns(initial_cwd, &files.include)?
      }
    }),
    exclude: PathOrPatternSet::new(vec![]),
  };
  let file_paths =
    FileCollector::new(|e| e.path.extension().map(|ext| ext == "json").unwrap_or(false))
      .ignore_git_folder()
      .ignore_node_modules()
  ...
  let coverage_patterns = FilePatterns {
    base: initial_cwd.to_path_buf(),
    include: None,
    exclude: PathOrPatternSet::from_exclude_relative_path_or_patterns(initial_cwd, &files.ignore)
  ...
  for file_path in file_paths {
    let new_coverage = fs::read_to_string(file_path.as_path())
      ...
      .and_then(|json| serde_json::from_str::<cdp::ScriptCoverage>(&json).map_err(AnyError::from))
...
fn filter_coverages(
  ...
  let doc_test_re = Regex::new(r"\$\d+-\d+\.(js|mjs|cjs|jsx|ts|mts|cts|tsx)$").unwrap();
...
pub fn cover_files(
  ...
  let get_message = |specifier: &ModuleSpecifier| -> String {
    format!(
      "Failed to fetch \"{}\" from cache. Before generating coverage report, run `deno test \
       --coverage` to ensure consistent state.",
      specifier,
    )
  };
  ...
  for script_coverage in script_coverages {
    let module_specifier =
      deno_path_util::resolve_url_or_path(&script_coverage.url, cli_options.initial_cwd())?;

    let maybe_file_result = file_fetcher.get_cached_source_or_local(&module_specifier);
    ...
    let transpiled_code = match file.media_type {
      ...
      MediaType::TypeScript | MediaType::Jsx | MediaType::Mts | MediaType::Cts | MediaType::Tsx => {
        let module_kind =
          ModuleKind::from_is_cjs(cjs_tracker.is_maybe_cjs(&file.specifier, file.media_type)?);
        Some(
          match emitter.maybe_cached_emit(&file.specifier, module_kind, &file.source)? {
            Some(code) => code,
            None => {
              return Err(anyhow!(
                "Missing transpiled source code for: \"{}\".
              Before generating coverage report, run `deno test --coverage` to ensure consistent \
                 state.",
                file.specifier,
              ));
            }
    ...
    let coverage_report = generate_coverage_report(GenerateCoverageReportOptions {
    ...
    .with_context(|| format!("Failed to generate coverage report for file ({module_specifier})"))?;
