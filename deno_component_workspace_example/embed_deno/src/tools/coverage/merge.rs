...
use super::range_tree::{RangeTree, RangeTreeArena};
use crate::cdp;
...
pub fn merge_processes(mut processes: Vec<ProcessCoverage>) -> Option<ProcessCoverage> {
  if processes.len() <= 1 {
    return processes.pop();
  }
  let mut url_to_scripts: BTreeMap<String, Vec<cdp::ScriptCoverage>> = BTreeMap::new();
  ...
  let result: Vec<cdp::ScriptCoverage> = url_to_scripts
    ...
    .map(|(script_id, scripts)| {
      let mut merged: cdp::ScriptCoverage = merge_scripts(scripts.to_vec()).unwrap();
...
pub fn merge_scripts(mut scripts: Vec<cdp::ScriptCoverage>) -> Option<cdp::ScriptCoverage> {
  if scripts.len() <= 1 {
    return scripts.pop();
  }
  let (script_id, url) = {
  ...
  let mut range_to_funcs: BTreeMap<CharRange, Vec<cdp::FunctionCoverage>> = BTreeMap::new();
...
pub fn merge_functions(mut funcs: Vec<cdp::FunctionCoverage>) -> Option<cdp::FunctionCoverage> {
  if funcs.len() <= 1 {
    return funcs.pop();
  }
...
fn into_start_events<'a>(trees: Vec<&'a mut RangeTree<'a>>) -> Vec<StartEvent<'a>> {
...
impl<'a> Iterator for StartEventQueue<'a> {
  ...
  fn next(&mut self) -> Option<<Self as Iterator>::Item> {
    let pending_offset: Option<usize> = match &self.pending {
      Some(start_event) if !start_event.trees.is_empty() => Some(start_event.offset),
      _ => None,
    };
