...
use super::{CoverageReport, util};
use crate::{args::CoverageType, colors};
...
pub trait CoverageReporter {
  fn done(&self, coverage_root: &Path, file_reports: &[(CoverageReport, String)]);
  ...
  fn collect_summary<'a>(
    ...
    let root = match util::find_root(urls).and_then(|root_path| root_path.to_file_path().ok()) {
      Some(path) => path,
    ...
    for (report, file_text) in file_reports {
      ...
      while let Some(path) = summary_path {
        ...
        stats.branch_miss += report.branches.iter().filter(|b| !b.is_hit).count();
...
impl SummaryCoverageReporter {
  ...
  fn print_coverage_line(&self, node: &str, node_max: usize, stats: &CoverageStats) {
    let CoverageStats {
      line_hit,
      line_miss,
      branch_hit,
    ...
    let (_, line_percent, line_class) = util::calc_coverage_display_info(*line_hit, *line_miss);
...
impl CoverageReporter for SummaryCoverageReporter {
  fn done(&self, _coverage_root: &Path, file_reports: &[(CoverageReport, String)]) {
    ...
    let header = format!("| {node:node_max$} | Branch % | Line % |", node = "File");
...
impl LcovCoverageReporter {
  ...
  fn report(&self, coverage_report: &CoverageReport, _file_text: &str) -> Result<(), AnyError> {
...
impl DetailedCoverageReporter {
  ...
  fn report(&self, coverage_report: &CoverageReport, file_text: &str) -> Result<(), AnyError> {
...
impl CoverageReporter for HtmlCoverageReporter {
  fn done(&self, coverage_root: &Path, file_reports: &[(CoverageReport, String)]) {
    ...
    for (node, stats) in &summary {
      let report_path = self.get_report_path(coverage_root, node, stats.file_text.is_none());
...
impl HtmlCoverageReporter {
  ...
  pub fn get_report_path(&self, coverage_root: &Path, node: &str, is_dir: bool) -> PathBuf {
    if is_dir {
  ...
  pub fn create_html(
    ...
    let breadcrumb_navigation = self.create_breadcrumbs_navigation(&breadcrumbs_parts, is_dir);
  ...
  pub fn create_html_header(&self, breadcrumb_navigation: &str, stats: &CoverageStats) -> String {
    let CoverageStats {
      line_hit,
      line_miss,
      branch_hit,
  ...
  pub fn create_html_summary_table(&self, node: &String, summary: &CoverageSummary) -> String {
    ...
    let table_rows: Vec<String> = children
      .iter()
      .map(|(is_file, c)| {
        let CoverageStats {
          line_hit,
        ...
        let path = Path::new(
          c.strip_prefix(&format!("{node}{}", std::path::MAIN_SEPARATOR))
            .unwrap_or(c),
        )
        .to_str()
        .unwrap();
        let path = path.replace(std::path::MAIN_SEPARATOR, "/");
        let path_label = if *is_file {
          path.to_string()
        } else {
          format!("{}/", path)
        };
        let path_link = if *is_file {
          format!("{}.html", path)
        } else {
          format!("{}index.html", path_label)
        };
      ...
      <tr>
        ...
        <td class='pic {line_class}'>
          <div class='chart'>
            <div class='cover-fill' style='width: {line_percent:.1}%'></div><div \
           class='cover-empty' style='width: calc(100% - {line_percent:.1}%)'></div>
          </div>
        </td>
  ...
  pub fn create_html_code_table(&self, file_text: &str, report: &CoverageReport) -> String {
    ...
    let line_coverage = (0 .. line_num)
      .map(|i| {
        if let Some((_, count)) = report.found_lines.iter().find(|(line, _)| i == *line) {
          if *count == 0 {
            "<span class='cline-any cline-no'>&nbsp;</span>".to_string()
          } else {
            format!(
              "<span class='cline-any cline-yes' title='This line is covered {count} \
               time{}'>x{count}</span>",
              if *count > 1 { "s" } else { "" }
            )
          }
    ...
    let branch_coverage = (0 .. line_num)
      .map(|i| {
        let branch_is_missed = report
          .branches
          .iter()
          .any(|b| b.line_index == i && !b.is_hit);
        if branch_is_missed {
  ...
  pub fn create_breadcrumbs_navigation(&self, breadcrumbs_parts: &[&str], is_dir: bool) -> String {
