...
use deno_ast::swc::common::{DUMMY_SP, Span};
...
impl SerializeCtx {
  ...
  pub fn append_inner<K>(&mut self, kind: K, span_lo: u32, span_hi: u32) -> PendingRef
  where
    K: Into<u8> + Display + Clone,
  {
  ...
  pub fn write_maybe_ref<P>(&mut self, prop: P, parent: &PendingRef, value: Option<NodeRef>)
  where
    P: Into<u8> + Display + Clone,
  {
    if let Some(v) = value {
  ...
  pub fn write_maybe_undef_ref<P>(&mut self, prop: P, parent: &PendingRef, value: Option<NodeRef>)
  where
    P: Into<u8> + Display + Clone,
  {
    if let Some(v) = value {
  ...
  pub fn write_ref_vec<P>(&mut self, prop: P, parent_ref: &PendingRef, value: Vec<NodeRef>)
  where
    P: Into<u8> + Display + Clone,
  {
