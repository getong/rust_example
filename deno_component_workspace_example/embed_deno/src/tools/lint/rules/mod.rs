...
use deno_config::{deno_json::LintRulesConfig, workspace::WorkspaceDirectory};
use deno_core::{anyhow::bail, error::AnyError};
use deno_graph::ModuleGraph;
use deno_lint::{diagnostic::LintDiagnostic, rules::LintRule, tags};
...
impl CliLintRule {
  ...
  pub fn help_docs_url(&self) -> Cow<'static, str> {
    use CliLintRuleKind::*;
    match &self.0 {
      DenoLint(rule) => Cow::Owned(format!("https://docs.deno.com/lint/rules/{}", rule.code())),
...
impl LintRuleProvider {
  ...
  pub fn all_rules(&self) -> Vec<CliLintRule> {
    ...
    let cli_lint_rules = vec![CliLintRule(CliLintRuleKind::Extended(Box::new(
      no_sloppy_imports::NoSloppyImportsRule::new(self.workspace_resolver.clone()),
    )))];
    let cli_graph_rules = vec![CliLintRule(CliLintRuleKind::Package(Box::new(
      no_slow_types::NoSlowTypesRule,
    )))];
    deno_lint_rules
      .into_iter()
...
fn get_default_tags(maybe_workspace_dir: Option<&WorkspaceDirectory>) -> Vec<String> {
  ...
  if let Some(member_dir) = maybe_workspace_dir {
    ...
    if member_dir.maybe_deno_json().is_some() || member_dir.maybe_pkg_json().is_some() {
...
fn filtered_rules(
  ...
  let tags_set = maybe_tags.map(|tags| tags.into_iter().collect::<HashSet<_>>());
