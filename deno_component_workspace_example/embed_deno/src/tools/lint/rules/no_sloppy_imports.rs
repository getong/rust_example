...
use deno_graph::{
  Range,
  source::{ResolutionKind, ResolveError},
};
use deno_lint::{
  diagnostic::{LintDiagnosticDetails, LintDiagnosticRange, LintDocsUrl, LintFix, LintFixChange},
  rules::LintRule,
  tags,
};
use deno_resolver::workspace::{SloppyImportsResolutionReason, WorkspaceResolver};
use text_lines::LineAndColumnIndex;

use super::ExtendedLintRule;
use crate::{graph_util::CliJsrUrlProvider, sys::CliSys};
...
impl NoSloppyImportsRule {
  pub fn new(workspace_resolver: Option<Arc<WorkspaceResolver<CliSys>>>) -> Self {
...
const DOCS_URL: &str =
  "https://docs.deno.com/runtime/manual/tools/unstable_flags/#--unstable-sloppy-imports";
...
impl LintRule for NoSloppyImportsRule {
  fn lint_program_with_ast_view<'view>(
    ...
    for (referrer, (specifier, sloppy_reason)) in resolver.captures.borrow_mut().drain() {
      let start_range = context.text_info().loc_to_source_pos(LineAndColumnIndex {
        line_index: referrer.range.start.line,
        column_index: referrer.range.start.character,
      });
      let end_range = context.text_info().loc_to_source_pos(LineAndColumnIndex {
      ...
      context.add_diagnostic_details(
        ...
        LintDiagnosticDetails {
          ...
          fixes: context
            ...
            .map(|relative| {
              vec![LintFix {
                description: Cow::Owned(sloppy_reason.quick_fix_message_for_specifier(&specifier)),
                changes: vec![LintFixChange {
                  new_text: Cow::Owned({
                    ...
                    let current_text = context.text_info().range_text(&source_range);
...
struct SloppyImportCaptureResolver<'a> {
  ...
  captures: RefCell<HashMap<Range, (deno_ast::ModuleSpecifier, SloppyImportsResolutionReason)>>,
}
    ...
    let resolution = self
      ...
      .resolve(
        ...
        match resolution_kind {
          ResolutionKind::Execution => deno_resolver::workspace::ResolutionKind::Execution,
          ResolutionKind::Types => deno_resolver::workspace::ResolutionKind::Types,
        },
      )
