...
use deno_core::{anyhow::bail, error::AnyError, serde_json, url};
use deno_error::JsErrorClass;
use deno_graph::{
  Dependency, GraphKind, Module, ModuleError, ModuleErrorKind, ModuleGraph, Resolution,
};
use deno_lib::{util::checksum, version::DENO_VERSION_INFO};
use deno_npm::{
  NpmPackageId, NpmResolutionPackage, npm_rc::ResolvedNpmRc, resolution::NpmResolutionSnapshot,
};
use deno_npm_installer::graph::NpmCachingStrategy;
use deno_path_util::resolve_url_or_path;
use deno_resolver::{DenoResolveErrorKind, display::DisplayTreeNode};
use deno_semver::{npm::NpmPackageReqReference, package::PackageReq};
use deno_terminal::colors;

use crate::{
  args::{Flags, InfoFlags},
  display,
  factory::CliFactory,
  graph_util::graph_exit_integrity_errors,
  npm::CliManagedNpmResolver,
};

const JSON_SCHEMA_VERSION: u8 = 1;

pub async fn info(flags: Arc<Flags>, info_flags: InfoFlags) -> Result<(), AnyError> {
  ...
  if let Some(specifier) = info_flags.file {
    ...
    let cwd_url = url::Url::from_directory_path(cli_options.initial_cwd()).unwrap();
    ...
    let maybe_import_specifier = match resolver.resolve(
      ...
      Ok(resolved) => match resolved {
        deno_resolver::workspace::MappedResolution::Normal { specifier, .. }
        ...
        deno_resolver::workspace::MappedResolution::PackageJson {
          ...
          deno_package_json::PackageJsonDepValue::Workspace(version_req) => {
            let pkg_folder =
              resolver.resolve_workspace_pkg_json_folder_for_pkg_json_dep(alias, version_req)?;
            Some(
              node_resolver
                .resolve_package_subpath_from_deno_module(
                  pkg_folder,
        ...
        deno_resolver::workspace::MappedResolution::PackageJsonImport { pkg_json } => Some(
          node_resolver
    ...
    let mut loader = module_graph_builder.create_graph_loader_with_root_permissions();
    ...
    if info_flags.json {
      ...
      if let Some(output) = json_graph.as_object_mut() {
        output.shift_insert(0, "version".to_string(), JSON_SCHEMA_VERSION.into());
      }
...
fn print_cache_info(
  ...
  if let Some(location) = &location {
    origin_dir = origin_dir.join(checksum::r#gen(&[location.to_string().as_bytes()]));
  }
      ...
      if matches!(module.get("kind").and_then(|k| k.as_str()), Some("npm")) {
          ...
          && let Some(module) = module.as_object_mut()
        {
            ...
            && let Some(specifier) = value.as_str()
          {
            ...
            redirects_to_add.push((specifier.to_string(), new_specifier.clone()));
      ...
      if let Some(dependencies) = dependencies {
        for dep in dependencies.iter_mut().flat_map(|d| d.as_object_mut()) {
          if let Some(specifier) = dep.get("specifier").and_then(|s| s.as_str())
            ...
            && let Ok(pkg) = npm_snapshot.resolve_pkg_from_pkg_req(npm_ref.req())
          {
            dep.insert(
              "npmPackage".to_string(),
              pkg.id.as_serialized().into_string().into(),
          ...
          if let Some(code) = dep.get_mut("code").and_then(|c| c.as_object_mut()) {
            code.remove("resolutionMode");
          }
          if let Some(types) = dep.get_mut("types").and_then(|c| c.as_object_mut()) {
...
impl NpmInfo {
  pub fn build<'a>(
    ...
    for module in graph.modules() {
      if let Module::Npm(module) = module
        && let Ok(package) = npm_snapshot.resolve_pkg_from_pkg_req(module.pkg_req_ref.req())
      {
        info
          .resolved_ids
  ...
  pub fn resolve_package(&self, req: &PackageReq) -> Option<&NpmResolutionPackage> {
...
impl<'a> GraphDisplayContext<'a> {
  pub fn write<TWrite: Write>(
    ...
    let npm_info = match managed_npm_info {
      Some((npm_resolver, npm_snapshot)) => NpmInfo::build(graph, npm_resolver, npm_snapshot),
  ...
  fn build_module_info(&mut self, module: &Module, type_dep: bool) -> DisplayTreeNode {
    ...
    let header_text = if was_seen {
      ...
      let maybe_size = match &package_or_specifier {
        Package(package) => self.npm_info.package_sizes.get(&package.id).copied(),
        ...
        Specifier(_) => match module {
          Module::Js(module) => {
            if let Some(types_dep) = &module.maybe_types_dependency
              && let Some(child) = self.build_resolved_info(&types_dep.dependency, true)
            {
          ...
          Module::Json(_) | Module::Npm(_) | Module::Node(_) | Module::External(_) => {}
        },
      }
    }
    tree_node
  ...
  fn build_npm_deps(&mut self, package: &NpmResolutionPackage) -> Vec<DisplayTreeNode> {
    ...
    for dep_id in deps.into_iter() {
      ...
      let mut child =
        DisplayTreeNode::from_text(format!("npm:/{} {}", dep_id.as_serialized(), size_str));
      if let Some(package) = self.npm_info.packages.get(dep_id)
        && !package.dependencies.is_empty()
      {
        let was_seen = !self.seen.insert(package.id.as_serialized().into_string());
        if was_seen {
  ...
  fn build_error_info(
    ...
    match err.as_kind() {
      ...
      ModuleErrorKind::UnsupportedModuleTypeForSourcePhaseImport { .. } => self.build_error_msg(
        specifier,
  ...
  fn build_error_msg(&self, specifier: &ModuleSpecifier, error_msg: &str) -> DisplayTreeNode {
