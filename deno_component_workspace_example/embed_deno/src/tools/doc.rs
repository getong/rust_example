...
use deno_config::glob::{FilePatterns, PathOrPatternSet};
use deno_core::{
  anyhow::{Context, bail},
  error::AnyError,
  serde_json,
...
use deno_doc as doc;
use deno_doc::html::{UrlResolveKind, UsageComposer, UsageComposerEntry};
use deno_graph::{
  CheckJsOption, GraphKind, ModuleSpecifier, analysis::ModuleAnalyzer, ast::EsParser,
  source::NullFileSystem,
};
use deno_lib::version::DENO_VERSION_INFO;
use deno_npm_installer::graph::NpmCachingStrategy;
use doc::{DocDiagnostic, html::ShortPath};
use indexmap::IndexMap;

use crate::{
  args::{DocFlags, DocHtmlFlag, DocSourceFileFlag, Flags},
  colors, display,
  factory::CliFactory,
  graph_util::{GraphWalkErrorsOptions, graph_exit_integrity_errors, graph_walk_errors},
  sys::CliSys,
  tsc::get_types_declaration_file_text,
  util::fs::{CollectSpecifiersOptions, collect_specifiers},
};
...
async fn generate_doc_nodes_for_builtin_types(
  ...
  let source_file_specifier = ModuleSpecifier::parse("file:///lib.deno.d.ts").unwrap();
...
pub async fn doc(flags: Arc<Flags>, doc_flags: DocFlags) -> Result<(), AnyError> {
...
impl deno_doc::html::HrefResolver for DocResolver {
  fn resolve_path(&self, current: UrlResolveKind, target: UrlResolveKind) -> String {
  ...
  fn resolve_external_jsdoc_module(
    ...
    if let Ok(url) = deno_core::url::Url::parse(module) {
      match url.scheme() {
        "npm" => {
          let res = deno_semver::npm::NpmPackageReqReference::from_str(module).ok()?;
        ...
        "jsr" => {
          let res = deno_semver::jsr::JsrPackageReqReference::from_str(module).ok()?;
          let name = &res.req().name;
...
fn generate_docs_directory(
  ...
  let category_docs = if let Some(category_docs_path) = &html_options.category_docs_path {
  ...
  let symbol_redirect_map =
    if let Some(symbol_redirect_map_path) = &html_options.symbol_redirect_map_path {
  ...
  let default_symbol_map =
    if let Some(default_symbol_map_path) = &html_options.default_symbol_map_path {
  ...
  let mut options = deno_doc::html::GenerateOptions {
    ...
    symbol_redirect_map,
    default_symbol_map,
    markdown_renderer: deno_doc::html::comrak::create_renderer(None, None, None),
  ...
  let mut files = deno_doc::html::generate(ctx).context("Failed to generate HTML documentation")?;
...
fn print_docs_to_stdout(
  ...
  doc_nodes.retain(|doc_node| !matches!(doc_node.def, doc::node::DocNodeDef::Import { .. }));
  ...
  display::write_to_stdout_ignore_sigpipe(details.as_bytes()).map_err(AnyError::from)
}
