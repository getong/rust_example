...
use crate::{
  args::{DenoXShimName, Flags, XFlags, XFlagsKind},
  factory::CliFactory,
  node::CliNodeResolver,
  npm::{CliManagedNpmResolver, CliNpmResolver},
  tools::pm::CacheTopLevelDepsOptions,
  util::{
    console::{ConfirmOptions, confirm},
  ...
  match &npm_resolver {
    ...
    deno_resolver::npm::NpmResolver::Managed(npm_resolver) => {
      ...
      for id in npm_resolver.resolution().top_level_packages() {
        ...
        let bins = match node_resolver.resolve_npm_binary_commands_for_package(&package_folder) {
          Ok(bins) => bins,
          Err(_) => {
...
fn run_js_file(
  ...
  let _temp_file = if let Some(state) = &npm_process_state {
    let fd = deno_runtime::deno_process::npm_process_state_tempfile(state.as_bytes())
  ...
  match bin_value {
    ...
    BinValue::Executable(mut path_buf) => {
      ...
      permissions.check_run(
        &deno_runtime::deno_permissions::RunQueryDescriptor::Path(PathQueryDescriptor::new(
          &factory.sys(),
...
fn find_bin_value(bins: &BTreeMap<String, BinValue>, bin_name: &str) -> Option<BinValue> {
  bins
    .get(bin_name)
    .or_else(|| {
...
async fn maybe_run_local_npm_bin(
  ...
  let mut bins = resolve_local_bins(node_resolver, npm_resolver, factory).await?;
  ...
  run_bin_value(factory, flags, bin_value, npm_process_state, unstable_args).map(Some)
}
...
fn write_shim(out_dir: &Path, shim_name: DenoXShimName) -> Result<(), AnyError> {
  #[cfg(unix)]
  {
...
pub async fn run(flags: Arc<Flags>, x_flags: XFlags) -> Result<i32, AnyError> {
  let command_flags = match x_flags.kind {
    ...
    XFlagsKind::Print => {
      ...
      let bins = resolve_local_bins(node_resolver, npm_resolver, &factory).await?;
      if bins.is_empty() {
  ...
  if is_file_like && Path::new(&command_flags.command).is_file() {
    return Err(anyhow::anyhow!(
      "Use 'deno run' to run a local file directly, 'deno x' is intended for running commands \
       from packages."
    ));
  }
  ...
  match thing_to_run {
    ReqRefOrUrl::Npm(npm_package_req_reference) => {
      ...
      if let Ok(package_folder) = npm_resolver.resolve_pkg_folder_from_deno_module_req(
        npm_package_req_reference.req(),
        &deno_path_util::url_from_directory_path(cli_options.initial_cwd()).unwrap(),
      ) {
        let bin_name = if let Some(sub_path) = npm_package_req_reference.sub_path() {
          sub_path
        } else {
          npm_package_req_reference.req().name.as_str()
        };

        let bins = node_resolver.resolve_npm_binary_commands_for_package(&package_folder)?;
      ...
      runner_flags.node_modules_dir = Some(deno_config::deno_json::NodeModulesDirMode::Manual);
      ...
      if let Some(exit_code) = res {
        ...
        let managed_npm_resolver = managed_factory.npm_resolver().await?.as_managed().unwrap();
    ...
    ReqRefOrUrl::Jsr(jsr_package_req_reference) => {
      ...
      let url = deno_core::url::Url::parse(&jsr_package_req_reference.to_string())?;
      run_js_file(
        &url,
        &permission_args,
        &flags.argv,
...
fn bin_commands_for_package(
  ...
  let pkg_id = managed_npm_resolver.resolve_pkg_id_from_deno_module_req(package_req)?;
  let package_folder = managed_npm_resolver.resolve_pkg_folder_from_pkg_id(&pkg_id)?;
  node_resolver
...
async fn autoinstall_package(
  ...
  fn make_new_flags(old_flags: &Flags, temp_dir: &Path) -> Arc<Flags> {
    ...
    new_flags.node_modules_dir = Some(deno_config::deno_json::NodeModulesDirMode::Auto);
    ...
    new_flags.config_flag =
      crate::args::ConfigFlag::Path(temp_dir.join("deno.json").to_string_lossy().into_owned());
  ...
  let temp_dir = create_package_temp_dir(Some(req_ref.prefix()), req_ref.req(), reload, deno_dir)?;
