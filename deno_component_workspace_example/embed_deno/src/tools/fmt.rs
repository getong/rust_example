...
use deno_config::glob::{FileCollector, FilePatterns};
use deno_core::{
  anyhow::{Context, anyhow, bail},
  error::AnyError,
  futures,
  parking_lot::Mutex,
  unsync::spawn_blocking,
  url::Url,
};
...
use crate::{
  args::{
    CliOptions, Flags, FmtFlags, FmtOptions, FmtOptionsConfig, ProseWrap, UnstableFmtOptions,
  },
  cache::{CacheDBHash, Caches, IncrementalCache},
  colors,
  factory::CliFactory,
  sys::CliSys,
  util,
  util::{file_watcher, fs::canonicalize_path, path::get_extension},
};

/// Format JavaScript/TypeScript files.
pub async fn format(flags: Arc<Flags>, fmt_flags: FmtFlags) -> Result<(), AnyError> {
  if fmt_flags.is_stdin() {
    let factory = CliFactory::from_flags(flags);
    let cli_options = factory.cli_options()?;
    let start_dir = &cli_options.start_dir;
    let fmt_config = start_dir.to_fmt_config(FilePatterns::new_with_base(start_dir.dir_path()))?;
  ...
  if let Some(watch_flags) = &fmt_flags.watch {
      ...
      move |flags, watcher_communicator, changed_paths| {
        ...
        Ok(async move {
          ...
          for paths_with_options in &mut paths_with_options_batches {
            let _ = watcher_communicator.watch_paths(paths_with_options.paths.clone());
    ...
    let cli_options = factory.cli_options()?;
    let caches = factory.caches()?;
    let paths_with_options_batches = resolve_paths_with_options_batches(cli_options, &fmt_flags)?;
    return format_files(caches, cli_options, &fmt_flags, paths_with_options_batches).await;
  }
...
fn resolve_paths_with_options_batches(
  ...
  let members_fmt_options = cli_options.resolve_fmt_options_for_members(fmt_flags)?;
  let mut paths_with_options_batches = Vec::with_capacity(members_fmt_options.len());
  for (_ctx, member_fmt_options) in members_fmt_options {
    let files = collect_fmt_files(cli_options, member_fmt_options.files.clone());
    if !files.is_empty() {
...
fn maybe_show_format_confirmation(
  ...
  let confirm_result = util::console::confirm(util::console::ConfirmOptions {
    default: true,
    message: format!(
      "{} It looks like you're not in a workspace. Are you sure you want to format the entire \
       '{}' directory?",
  ...
  if confirm_result {
    ...
    bail!(
      "Did not format non-workspace directory. Run again specifying the current directory (ex. \
       `deno fmt .`)"
    )
  }
}
...
fn collect_fmt_files(cli_options: &CliOptions, files: FilePatterns) -> Vec<PathBuf> {
  FileCollector::new(|e| {
  ...
  dprint_plugin_markdown::format_text(file_text, &markdown_config, move |tag, text, line_width| {
    ...
    if matches!(
      ...
      match extension {
        ...
        "css" | "scss" | "sass" | "less" => format_css(&fake_filename, text, fmt_options),
        "html" | "svg" | "xml" => format_html(&fake_filename, text, fmt_options, unstable_options),
        ...
        _ => {
          ...
          dprint_plugin_typescript::format_text(dprint_plugin_typescript::FormatTextOptions {
            ...
            external_formatter: Some(&create_external_formatter_for_typescript(unstable_options)),
          })
        }
      }
    } else {
...
fn format_yaml(
  ...
  let formatted_str = pretty_yaml::format_text(file_text, &get_resolved_yaml_config(fmt_options))
    .map_err(AnyError::from)?;
...
pub fn format_html(
  ...
  let format_result = markup_fmt::format_text(
    file_text,
    markup_fmt::detect_language(file_path).unwrap_or(markup_fmt::Language::Html),
    ...
    |text, hints| {
      ...
      match hints.ext {
        "css" | "scss" | "sass" | "less" => {
          ...
          if hints.attr {
            malva_config.language.quotes = if let Some(true) = fmt_options.single_quote {
        ...
        "json" | "jsonc" => {
          ...
          dprint_plugin_json::format_text(&path, text, &json_config).map(|formatted| {
        ...
        _ => {
          let mut typescript_config_builder = get_typescript_config_builder(fmt_options);
  ...
  .map_err(|error| match error {
    markup_fmt::FormatError::Syntax(error) => {
      fn inner(error: &markup_fmt::SyntaxError, file_path: &Path) -> Option<String> {
  ...
  let options = config::FormatOptions {
    layout: config::LayoutOptions {
      ...
      line_break: match config.new_line_kind {
        dprint_core::configuration::NewLineKind::LineFeed => config::LineBreak::Lf,
        dprint_core::configuration::NewLineKind::CarriageReturnLineFeed => config::LineBreak::Crlf,
    ...
    language: config::LanguageOptions {
      ...
      declaration_order_group_by: config::DeclarationOrderGroupBy::NonDeclaration,
    },
  };
...
fn format_embedded_html(
  ...
  let options = config::FormatOptions {
    ...
    language: config::LanguageOptions {
      ...
      closing_tag_line_break_for_empty: config::ClosingTagLineBreakForEmpty::Fit,
...
pub fn format_file(
  ...
  let maybe_result = match ext.as_str() {
    ...
    "css" | "scss" | "sass" | "less" => format_css(file_path, &file.text, fmt_options)?,
    "html" | "xml" | "svg" => format_html(file_path, &file.text, fmt_options, unstable_options)?,
    ...
    "ipynb" => {
      dprint_plugin_jupyter::format_text(&file.text, |file_path: &Path, file_text: String| {
...
impl Formatter for CheckFormatter {
  async fn handle_files(
    ...
    run_parallelized(paths, {
      ...
      move |file_path| {
        ...
        if !file.had_bom && incremental_cache.is_file_same(&file_path, &file.text) {
          return Ok(());
        }
        ...
        match format_file(
          ...
          Ok(Some(formatted_text)) => {
            ...
            let diff = deno_resolver::display::diff(&file.text, &formatted_text);
            info!("");
  ...
  fn finish(&self) -> Result<(), AnyError> {
    let not_formatted_files_count = self.not_formatted_files_count.load(Ordering::Relaxed);
    ...
    if not_formatted_files_count == 0 {
      ...
      Err(anyhow!(
        "Found {not_formatted_files_count} not formatted {not_formatted_files_str} in \
         {checked_files_str}",
      ))
    }
  }
...
impl Formatter for RealFormatter {
  ...
  fn finish(&self) -> Result<(), AnyError> {
    let formatted_files_count = self.formatted_files_count.load(Ordering::Relaxed);
    debug!(
      "Formatted {} {}",
...
fn format_stdin(fmt_flags: &FmtFlags, fmt_options: FmtOptions, ext: &str) -> Result<(), AnyError> {
...
fn get_typescript_config_builder(
  ...
  if let Some(use_braces) = options.use_braces {
    builder.use_braces(match use_braces {
      ...
      UseBraces::WhenNotSingleLine => dprint_config::UseBraces::WhenNotSingleLine,
    });
  }
  ...
  if let Some(brace_position) = options.brace_position {
    builder.brace_position(match brace_position {
      ...
      BracePosition::SameLineUnlessHanging => dprint_config::BracePosition::SameLineUnlessHanging,
    });
  }

  if let Some(single_body_position) = options.single_body_position {
    builder.single_body_position(match single_body_position {
      SingleBodyPosition::Maintain => dprint_config::SameOrNextLinePosition::Maintain,
      SingleBodyPosition::NextLine => dprint_config::SameOrNextLinePosition::NextLine,
      SingleBodyPosition::SameLine => dprint_config::SameOrNextLinePosition::SameLine,
    });
  }
  ...
  if let Some(next_control_flow_position) = options.next_control_flow_position {
    builder.next_control_flow_position(match next_control_flow_position {
      NextControlFlowPosition::Maintain => dprint_config::NextControlFlowPosition::Maintain,
      NextControlFlowPosition::NextLine => dprint_config::NextControlFlowPosition::NextLine,
      NextControlFlowPosition::SameLine => dprint_config::NextControlFlowPosition::SameLine,
    });
  }
  ...
  if let Some(trailing_commas) = options.trailing_commas {
    builder.trailing_commas(match trailing_commas {
      ...
      TrailingCommas::OnlyMultiLine => dprint_config::TrailingCommas::OnlyMultiLine,
    });
  }
  ...
  if let Some(jsx_bracket_position) = options.jsx_bracket_position {
    builder.jsx_bracket_position(match jsx_bracket_position {
      BracketPosition::Maintain => dprint_config::SameOrNextLinePosition::Maintain,
      BracketPosition::NextLine => dprint_config::SameOrNextLinePosition::NextLine,
      BracketPosition::SameLine => dprint_config::SameOrNextLinePosition::SameLine,
    });
  }
  ...
  {
    builder.jsx_force_new_lines_surrounding_content(jsx_force_new_lines_surrounding_content);
  }
  ...
  if let Some(space_surrounding_properties) = options.space_surrounding_properties {
    builder.space_surrounding_properties(space_surrounding_properties);
    builder.import_declaration_space_surrounding_named_imports(space_surrounding_properties);
    builder.export_declaration_space_surrounding_named_exports(space_surrounding_properties);
  }
...
fn get_resolved_malva_config(options: &FmtOptionsConfig) -> malva::config::FormatOptions {
  use malva::config::*;
...
fn get_resolved_markup_fmt_config(options: &FmtOptionsConfig) -> markup_fmt::config::FormatOptions {
  use markup_fmt::config::*;
...
fn get_resolved_yaml_config(options: &FmtOptionsConfig) -> pretty_yaml::config::FormatOptions {
...
fn read_file_contents(file_path: &Path) -> Result<FileContents<'_>, AnyError> {
  ...
  let charset = deno_media_type::encoding::detect_charset_local_file(&file_bytes);
  let text = deno_media_type::encoding::decode_owned_source(charset, file_bytes.to_vec())
...
fn write_file_contents(file_path: &Path, file_contents: &str) -> Result<(), AnyError> {
  Ok(fs::write(file_path, file_contents)?)
}

pub async fn run_parallelized<F>(file_paths: Vec<PathBuf>, f: F) -> Result<(), AnyError>
where
