...
use deno_ast::{
  ParseDiagnostic, SourcePos, SourceRange, SourceRanged, SourceTextInfo,
  diagnostics::{
  ...
  swc::common::util::take::Take,
};
use deno_core::{anyhow::anyhow, error::AnyError, parking_lot::Mutex, url::Url};
  ...
  pub fn print_and_error(&self) -> Result<(), AnyError> {
    ...
    if errors > 0 {
      if has_slow_types_errors {
        log::error!("This package contains errors for slow types. Fixing these errors will:\n");
        log::error!("  1. Significantly improve your package users' type checking performance.");
    ...
    let (specifier, source_pos) = match loc {
      ...
      DiagnosticLocation::ModulePosition {
        ...
        Some(match source_pos {
          ...
          DiagnosticSourcePos::ByteIndex(index) => text_info.range().start() + index,
          DiagnosticSourcePos::LineAndCol { line, column } => text_info.line_start(line) + column,
        }),
      ),
    };
...
impl Diagnostic for PublishDiagnostic {
  ...
  fn code(&self) -> Cow<'_, str> {
    ...
    match &self {
      ...
      BannedTripleSlashDirectives { .. } => Cow::Borrowed("banned-triple-slash-directives"),
  ...
  fn message(&self) -> Cow<'_, str> {
    ...
    match &self {
      ...
      UnsupportedFileType { kind, .. } => Cow::Owned(format!("unsupported file type '{kind}'")),
      InvalidExternalImport { kind, .. } => {
        Cow::Owned(format!("invalid import to a {kind} specifier"))
      }
      ExcludedModule { .. } => {
        Cow::Borrowed("module in package's module graph was excluded from publishing")
      }
      ...
      BannedTripleSlashDirectives { .. } => {
        Cow::Borrowed("triple slash directives that modify globals are not allowed")
      }
      RelativePackageImport {
        from_package_name,
        to_package_name,
  ...
  fn location(&self) -> DiagnosticLocation<'_> {
    ...
    match &self {
      ...
      InvalidPath { path, .. } => DiagnosticLocation::Path { path: path.clone() },
      DuplicatePath { path, .. } => DiagnosticLocation::Path { path: path.clone() },
      ...
      RelativePackageImport {
        ...
        Some(referrer) => from_referrer_range(&referrer.referrer, &referrer.text_info),
  ...
  fn snippet(&self) -> Option<DiagnosticSnippet<'_>> {
    ...
    match &self {
      ...
      InvalidPath { .. } | DuplicatePath { .. } | UnsupportedFileType { .. } => None,
      InvalidExternalImport {
        referrer,
        text_info,
        ..
      ...
      RelativePackageImport { maybe_referrer, .. } => maybe_referrer
        .as_ref()
        .and_then(|referrer| from_range(&referrer.text_info, &referrer.referrer)),
  ...
  fn hint(&self) -> Option<Cow<'_, str>> {
    ...
    match &self {
      ...
      ExcludedModule { .. } => Some(Cow::Borrowed(
        "remove the module from 'exclude' and/or 'publish.exclude' in the config file or use \
         'publish.exclude' with a negative glob to unexclude from gitignore",
      )),
      ...
      RelativePackageImport {
        ...
        "replace the relative import with a bare specifier that imports from the other package by \
         name and optionally an export (ex. \"{}\")",
        to_package_name
      ))),
      SyntaxError(diagnostic) => diagnostic.hint(),
      MissingLicense { .. } => Some(Cow::Borrowed(
        "add a \"license\" field. Alternatively, add a LICENSE file to the package and ensure it \
         is not ignored from being published",
      )),
      UnstableRawImport { .. } => Some(Cow::Borrowed(
        "for the time being, embed the data directly into a JavaScript file (ex. as encoded \
         base64 text)",
      )),
    }
  }
  ...
  fn info(&self) -> Cow<'_, [Cow<'_, str>]> {
    ...
    match &self {
      ...
      InvalidPath { .. } => Cow::Borrowed(&[Cow::Borrowed(
        "to portably support all platforms, including windows, the allowed characters in package \
         paths are limited",
      )]),
      ...
      ExcludedModule { .. } => Cow::Owned(vec![Cow::Borrowed(
        "excluded modules referenced via a package export will error at runtime due to not \
      ...
      RelativePackageImport { .. } => Cow::Borrowed(&[Cow::Borrowed(
        "importing modules in another package using a relative import won't work once the \
  ...
  fn docs_url(&self) -> Option<Cow<'_, str>> {
    ...
    match &self {
      ...
      InvalidPath { .. } => Some(Cow::Borrowed("https://jsr.io/go/invalid-path")),
      DuplicatePath { .. } => Some(Cow::Borrowed(
        "https://jsr.io/go/case-insensitive-duplicate-path",
      )),
      UnsupportedFileType { .. } => Some(Cow::Borrowed("https://jsr.io/go/unsupported-file-type")),
      InvalidExternalImport { .. } => {
        Some(Cow::Borrowed("https://jsr.io/go/invalid-external-import"))
      }
      ExcludedModule { .. } => Some(Cow::Borrowed("https://jsr.io/go/excluded-module")),
      MissingConstraint { .. } => Some(Cow::Borrowed("https://jsr.io/go/missing-constraint")),
      ...
      MissingLicense { .. } => Some(Cow::Borrowed("https://jsr.io/go/missing-license")),
      UnstableRawImport { .. } => Some(Cow::Borrowed(
