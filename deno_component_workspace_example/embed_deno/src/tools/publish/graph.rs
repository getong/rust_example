...
use deno_ast::{
  ParsedSource, SourceRangedForSpanned, SourceTextInfo, swc::common::comments::CommentKind,
};
use deno_core::{error::AnyError, url::Url};
use deno_graph::{ModuleEntryRef, ModuleGraph, ResolutionResolved, WalkOptions};
  ...
  pub fn collect_diagnostics_for_graph(
        ...
        if visited.insert(resolution.specifier.clone()) {
          match resolution.specifier.scheme() {
            ...
            "jsr" => {
              ...
              if let Ok(jsr_req_ref) = JsrPackageReqReference::from_specifier(&resolution.specifier)
    ...
    while let Some((specifier, entry)) = iter.next() {
      ...
      for diagnostic in parsed_source.diagnostics() {
        diagnostics_collector.push(PublishDiagnostic::SyntaxError(diagnostic.clone()));
      }

      check_for_banned_triple_slash_directives(&parsed_source, diagnostics_collector);

      for (specifier_text, dep) in &module.dependencies {
        for asset_import in dep.imports.iter().filter(|i| i.attributes.has_asset()) {
...
fn check_for_banned_triple_slash_directives(
  ...
  for comment in comments {
    ...
    if triple_slash_re.is_match(&comment.text) {
      diagnostics_collector.push(PublishDiagnostic::BannedTripleSlashDirectives {
