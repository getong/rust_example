...
use deno_package_json::{PackageJsonDepValue, PackageJsonDepWorkspaceReq};
use deno_resolver::workspace::{MappedResolution, PackageJsonDepResolution, WorkspaceResolver};
use deno_runtime::deno_node::is_builtin_node_module;
use deno_semver::{Version, VersionReq};
use sys_traits::{FsMetadata, FsRead};

use crate::{node::CliNodeResolver, sys::CliSys};
...
impl Diagnostic for SpecifierUnfurlerDiagnostic {
  fn level(&self) -> DiagnosticLevel {
    match self {
      ...
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonFileSpecifier { .. } => DiagnosticLevel::Error,
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonJsrSpecifier { .. } => DiagnosticLevel::Error,
    }
  }
  ...
  fn code(&self) -> Cow<'_, str> {
    match self {
      ...
      Self::UnsupportedPkgJsonFileSpecifier { .. } => "unsupported-file-specifier",
      Self::UnsupportedPkgJsonJsrSpecifier { .. } => "unsupported-jsr-specifier",
    }
    .into()
  }
  ...
  fn hint(&self) -> Option<Cow<'_, str>> {
    match self {
      ...
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonFileSpecifier { .. } => {
        Some("change the package dependency to point to something on npm instead".into())
      }
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonJsrSpecifier { .. } => {
        Some("move the JSR package dependency to deno.json instead".into())
      }
    }
  }
  ...
  fn info(&self) -> Cow<'_, [Cow<'_, str>]> {
    match self {
      ...
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonFileSpecifier { .. } => Cow::Borrowed(&[]),
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonJsrSpecifier { .. } => Cow::Borrowed(&[]),
    }
  }
  ...
  pub fn unfurl_specifier_reporting_diagnostic(
    ...
    match self.unfurl_specifier(referrer, specifier, resolution_kind) {
      ...
      Err(diagnostic) => {
        ...
        match diagnostic {
          UnfurlSpecifierError::UnsupportedPkgJsonFileSpecifier { package_name } => {
            diagnostic_reporter(
              SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonFileSpecifier {
          ...
          UnfurlSpecifierError::UnsupportedPkgJsonJsrSpecifier { package_name } => {
            diagnostic_reporter(
              SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonJsrSpecifier {
                specifier: referrer.clone(),
                package_name,
          ...
          MappedResolution::PackageJson {
            ...
            Ok(dep) => match dep {
              PackageJsonDepValue::File(_) => {
                return Err(UnfurlSpecifierError::UnsupportedPkgJsonFileSpecifier {
                  package_name: alias.to_string(),
                });
              }
              PackageJsonDepValue::JsrReq(_) => {
                return Err(UnfurlSpecifierError::UnsupportedPkgJsonJsrSpecifier {
                  package_name: alias.to_string(),
                });
              }
...
mod tests {
  ...
  fn test_unfurling() {
    ...
    let deno_json_url = ModuleSpecifier::from_file_path(cwd.join("deno.json")).unwrap();
    let value = json!({
      "imports": {
    ...
    let unfurler = SpecifierUnfurler::new(None, Arc::new(workspace_resolver), true);
...
"#;
      let specifier = ModuleSpecifier::from_file_path(cwd.join("mod.ts")).unwrap();
...
export type * from "./c";
"#;
      let specifier = ModuleSpecifier::from_file_path(cwd.join("mod.d.ts")).unwrap();
      let source = parse_ast(&specifier, source_code);
      let mut d = Vec::new();
      let mut reporter = |diagnostic| d.push(diagnostic);
      let unfurled_source = unfurl(&unfurler, &specifier, &source, &mut reporter);
...
"#;
  ...
  fn test_unfurling_npm_dep_workspace_specifier() {
    ...
    let workspace_resolver = WorkspaceResolver::new_raw(
      ...
      vec![ResolverWorkspaceJsrPackage {
        is_link: false,
        base: ModuleSpecifier::from_directory_path(cwd.join("publish/jsr.json")).unwrap(),
...
"#;
      let specifier = ModuleSpecifier::from_file_path(cwd.join("publish").join("mod.ts")).unwrap();
      let source = parse_ast(&specifier, source_code);
      let mut d = Vec::new();
      let mut reporter = |diagnostic| d.push(diagnostic);
      let unfurled_source = unfurl(&unfurler, &specifier, &source, &mut reporter);
...
"#;
      let specifier =
        ModuleSpecifier::from_file_path(cwd.join("publish").join("other.ts")).unwrap();
      let source = parse_ast(&specifier, source_code);
      let mut d = Vec::new();
      let mut reporter = |diagnostic| d.push(diagnostic);
      let unfurled_source = unfurl(&unfurler, &specifier, &source, &mut reporter);
