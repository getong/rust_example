...
use base64::{Engine as _, engine::general_purpose::STANDARD_NO_PAD, prelude::BASE64_STANDARD};
use deno_core::{anyhow, anyhow::bail, error::AnyError, serde_json, url::Url};
use http_body_util::BodyExt;
use once_cell::sync::Lazy;
use p256::{elliptic_curve, pkcs8::AssociatedOid};
use serde::{Deserialize, Serialize};
use sha2::Digest;
use spki::der::{EncodePem, asn1, pem::LineEnding};

use super::auth::{gha_oidc_token, is_gha};
use crate::{http_util, http_util::HttpClient, registry::OidcTokenResponse};
...
pub async fn generate_provenance(
  ...
  if gha_oidc_token().is_none() {
    bail!("Provenance generation in Github Actions requires 'id-token' permission");
  };
...
pub async fn attest(
  ...
  let transparency_logs = testify(http_client, &content, &key_material.certificate).await?;
...
static DEFAULT_FULCIO_URL: Lazy<String> = Lazy::new(|| {
  env::var("FULCIO_URL").unwrap_or_else(|_| "https://fulcio.sigstore.dev".to_string())
});
...
impl<'a> FulcioSigner<'a> {
  pub fn new(http_client: &'a HttpClient) -> Result<Self, AnyError> {
    ...
    let ephemeral_signer = EcdsaKeyPair::from_pkcs8(ALGORITHM, document.as_ref())?;
  ...
  pub async fn sign(
    ...
    let challenge = self.ephemeral_signer.sign(&self.rng, subject.as_bytes())?;
  ...
  async fn create_signing_certificate(
    ...
    let body: SigningCertificateResponse = http_util::body_to_json(response).await?;
...
static DEFAULT_REKOR_URL: Lazy<String> =
  Lazy::new(|| env::var("REKOR_URL").unwrap_or_else(|_| "https://rekor.sigstore.dev".to_string()));
...
async fn testify(
  ...
  let dsse = DsseEnvelope {
    ...
    signatures: [RekorSignature {
      sig: BASE64_STANDARD.encode(content.dsse_envelope.signatures[0].sig.clone()),
...
mod tests {
  ...
  use super::{ProvenanceAttestation, Subject, SubjectDigest};
