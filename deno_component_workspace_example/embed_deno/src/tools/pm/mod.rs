...
use deno_semver::{
  ...
  package::{PackageName, PackageNv, PackageReq},
};
use deps::KeyPath;
use jsonc_parser::{
...
use crate::{
  args::{AddFlags, CliOptions, Flags, RemoveFlags},
  factory::CliFactory,
...
impl ConfigUpdater {
  fn new(kind: ConfigKind, config_file_path: PathBuf) -> Result<Self, AnyError> {
    let config_file_contents = std::fs::read_to_string(&config_file_path)
      .with_context(|| format!("Reading config file '{}'", config_file_path.display()))?;
    let cst = CstRootNode::parse(&config_file_contents, &Default::default())
      .with_context(|| format!("Parsing config file '{}'", config_file_path.display()))?;
  ...
  fn get_property_for_mutation(&mut self, key_path: &KeyPath) -> Option<CstObjectProp> {
  ...
  fn add(&mut self, selected: SelectedPackage, dev: bool) {
    fn insert_index(object: &CstObject, searching_name: &str) -> usize {
      object
        ...
        .into_iter()
        .take_while(|prop| {
          let prop_name = prop.name().and_then(|name| name.decoded_value().ok());
          match prop_name {
            Some(current_name) => searching_name.cmp(&current_name) == std::cmp::Ordering::Greater,
            None => true,
          }
        })
        .count()
    ...
    match self.kind {
      ConfigKind::DenoJson => {
        ...
        let value = format!("{}@{}", selected.package_name, selected.version_req);
  ...
  fn set_allow_scripts_value(&mut self, value: jsonc_parser::cst::CstInputValue) {
...
fn create_deno_json(flags: &Arc<Flags>, options: &CliOptions) -> Result<CliFactory, AnyError> {
...
fn package_json_dependency_entry(selected: SelectedPackage) -> (String, String) {
  if let Some(npm_package) = selected.package_name.strip_prefix("npm:") {
    ...
    let version_req = format!("npm:@jsr/{scope_replaced}@{}", selected.version_req);
...
fn load_configs(
  ...
  let (cli_factory, deno_config) = match deno_config {
    ...
    None if npm_config.is_some() && !has_jsr_specifiers() => (cli_factory, None),
    _ => {
...
pub async fn add(
  ...
  let (cli_factory, mut npm_config, mut deno_config) = load_configs(&flags, || {
  ...
  for entry_text in add_flags.packages.iter() {
    let req = AddRmPackageReq::parse(entry_text, add_flags.default_registry.map(|r| r.into()))
...
fn best_version<'a>(versions: impl Iterator<Item = &'a Version>) -> Option<&'a Version> {
...
trait PackageInfoProvider {
  ...
  async fn req_to_nv(&self, req: &PackageReq) -> Result<Option<PackageNv>, AnyError>;
...
impl PackageInfoProvider for Arc<NpmFetchResolver> {
  ...
  async fn latest_version(&self, name: &PackageName) -> Option<Version> {
    let info = self.package_info(name).await?;
    best_version(self.applicable_version_infos(&info).map(|vi| &vi.version)).cloned()
  }
}
...
async fn find_package_and_select_version_for_req(
    ...
    let nv = match main_resolver.req_to_nv(req).await {
      ...
      Err(err) => {
        if req.version_req.version_text() == "*"
          && let Some(pre_release_version) = main_resolver.latest_version(&req.name).await
        {
          return Ok(PackageAndVersion::NotFound {
            package: prefixed_name,
            package_req: req.clone(),
            help: Some(NotFoundHelp::PreReleaseVersion(pre_release_version.clone())),
          });
        }
        return Err(err);
      }
  ...
  match &add_package_req.value {
    AddRmPackageReqValue::Jsr(_) => select(jsr_resolver, npm_resolver, add_package_req).await,
...
impl AddRmPackageReq {
  pub fn parse(
    ...
    match prefix {
      Prefix::Jsr => {
        let req_ref = JsrPackageReqReference::from_str(&format!("jsr:{}", entry_text))?;
      ...
      Prefix::Npm => {
        let req_ref = NpmPackageReqReference::from_str(&format!("npm:{}", entry_text))?;
        let package_req = req_ref.into_inner().req;
        Ok(Ok(AddRmPackageReq {
          alias: maybe_alias.unwrap_or_else(|| package_req.name.clone()),
...
pub async fn remove(flags: Arc<Flags>, remove_flags: RemoveFlags) -> Result<(), AnyError> {
...
async fn npm_install_after_modification(
  ...
  // npm install
  cache_deps::cache_top_level_deps(&cli_factory, jsr_resolver, cache_options).await?;
