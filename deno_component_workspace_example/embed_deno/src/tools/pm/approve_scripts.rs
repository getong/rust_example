...
use deno_config::deno_json::{AllowScriptsConfig, AllowScriptsValueConfig};
use deno_core::{
  anyhow,
  anyhow::{Context, bail},
  error::AnyError,
...
use deno_semver::{
  jsr::{JsrDepPackageReq, JsrDepPackageReqParseError},
  package::{PackageKind, PackageNv, PackageReq},
};
...
use crate::{
  args::{ApproveScriptsFlags, Flags},
  factory::CliFactory,
  npm::CliNpmResolver,
  tools::pm::{ConfigKind, ConfigUpdater, create_deno_json, interactive_picker},
};
...
pub async fn approve_scripts(
  ...
  let deno_json = options
    .workspace()
    .root_deno_json()
    .ok_or_else(|| anyhow::anyhow!("A deno.json file could not be found or created"))?;

  let deno_json_path = url_to_file_path(&deno_json.specifier)?;

  let mut config_updater = ConfigUpdater::new(ConfigKind::DenoJson, deno_json_path.clone())?;
  ...
  let (mut allow_list, mut deny_list) = match allow_scripts_config.allow {
    AllowScriptsValueConfig::All => {
      log::info!("Lifecycle scripts are already allowed for all npm packages in the workspace.",);
      return Ok(());
    }
  ...
  let deny_reqs: Vec<PackageReq> = deny_list.iter().map(|req| req.req.clone()).collect();
  ...
  if !approvals.is_empty() {
    deny_list.retain(|entry| !(entry.kind == PackageKind::Npm && approvals.contains(&entry.req)));
  }
...
fn parse_user_packages(
  ...
  for raw in packages {
    let req =
      parse_npm_package_req(raw).with_context(|| format!("Failed to parse package: {}", raw))?;
...
fn find_script_candidates(
  ...
  for package in snapshot.all_system_packages(system_info) {
    ...
    let specifier = format!("npm:{}@{}", package.id.nv.name, package.id.nv.version);
    let req = PackageReq::from_str(&format!("{}@{}", package.id.nv.name, package.id.nv.version))?;
...
fn pick_candidates(
  ...
  let selected = interactive_picker::select_items(
    ...
    candidates,
    HashSet::new(),
    |_idx, is_selected, is_checked, candidate| render_candidate(candidate, is_selected, is_checked),
  )?;
...
fn allow_scripts_to_value(config: &AllowScriptsConfig) -> jsonc_parser::cst::CstInputValue {
...
fn parse_npm_package_req(text: &str) -> Result<PackageReq, AnyError> {
  let req = match JsrDepPackageReq::from_str_loose(text) {
    ...
    Err(JsrDepPackageReqParseError::NotExpectedScheme(_)) if !text.contains(':') => {
