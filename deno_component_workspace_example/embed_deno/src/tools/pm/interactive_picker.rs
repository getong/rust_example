...
use crate::util::console::{HideCursorGuard, RawMode, new_console_static_text};
  ...
  TRender: FnMut(usize, bool, bool, &T) -> anyhow::Result<TextItem<'static>>,
{
  ...
  loop {
      ...
      && rendered_items.len() + first_line_rows >= rows as usize
    {
      ...
      } else if currently_selected + 1 >= scroll_offset + (rows as usize).saturating_sub(adj) {
        scroll_offset = (currently_selected + 1).saturating_sub(rows as usize) + 1;
      }
      let adj = if scroll_offset == 0 {
        first_line_rows.saturating_sub(1)
      } else {
      ...
      new_items.extend(rendered_items.drain(
        scroll_offset
          .. (scroll_offset + (rows as usize).saturating_sub(adj)).min(rendered_items.len()),
      ));
