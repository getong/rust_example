...
use deno_core::{anyhow, error::AnyError};
use lol_html::{element, html_content::ContentType as LolContentType};
...
impl<'a> Attr<'a> {
  fn new(name: impl Into<Cow<'static, str>>, value: Option<Cow<'a, str>>) -> Self {
    Self {
      name: name.into(),
      value,
    }
  ...
  let mut rewriter = lol_html::HtmlRewriter::new(
    lol_html::Settings {
      element_content_handlers: vec![element!("script[src]", |el| {
        let is_ignored = el.has_attribute("deno-ignore") || el.has_attribute("vite-ignore");
        if is_ignored {
          return Ok(());
        }
...
pub fn load_html_entrypoint(cwd: &Path, path: &Path) -> anyhow::Result<HtmlEntrypoint> {
...
impl<'a, 'f> HtmlOutputFiles<'a, 'f> {
  pub fn new(output_files: &'f mut Vec<OutputFile<'a>>) -> Self {
    let re = lazy_regex::regex!(r"(^.+\.deno-bundle-html.entry)-([^.]+)(\..+)$");
...
impl HtmlEntrypoint {
  ...
  pub fn patch_html_with_response<'a>(
    ...
    let css_out = html_output_files.get_and_update_path(&css_entry_name, |p, _| {
      p.to_string_lossy()
    ...
    let patched = inject_scripts_and_css(&self.contents, to_inject, &self.scripts, css_href)?;
