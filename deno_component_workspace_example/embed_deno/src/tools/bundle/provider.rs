...
use deno_bundle_runtime::{BundleOptions as RtBundleOptions, BundleProvider};
use deno_core::error::AnyError;

use crate::args::{DenoSubcommand, Flags};
...
fn convert_location(location: esbuild_client::protocol::Location) -> rt_bundle::Location {
  rt_bundle::Location {
    file: location.file,
...
fn convert_message(message: esbuild_client::protocol::Message) -> rt_bundle::Message {
  rt_bundle::Message {
    text: message.text,
  ...
  if let Some(files) = std::mem::take(&mut response.output_files) {
    ...
    for output_file in output_files {
      let processed_contents = crate::tools::bundle::maybe_process_contents(
        &output_file,
        crate::tools::bundle::should_replace_require_shim(bundle_flags.platform),
        bundle_flags.minify,
      )?;
...
impl BundleProvider for CliBundleProvider {
  async fn bundle(&self, options: RtBundleOptions) -> Result<rt_bundle::BuildResponse, AnyError> {
    let mut flags_clone = (*self.flags).clone();
    flags_clone.type_check_mode = crate::args::TypeCheckMode::None;
    let write_output =
      options.write && (options.output_dir.is_some() || options.output_path.is_some());
    ...
    std::thread::spawn(move || {
      deno_runtime::tokio_util::create_and_run_current_thread(async move {
        ...
        if write_output {
          ...
          process_output_files(&bundle_flags, &mut result, &bundler.cwd, bundler.input)?;
        }
