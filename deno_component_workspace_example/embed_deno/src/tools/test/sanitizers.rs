...
use deno_core::{
  error::CoreError,
  stats::{
    RuntimeActivity, RuntimeActivityDiff, RuntimeActivityStats, RuntimeActivityStatsFactory,
    RuntimeActivityStatsFilter, RuntimeActivityType,
  },
};
...
fn get_sanitizer_item(activity: RuntimeActivity) -> (RuntimeActivityType, Cow<'static, str>) {
...
fn get_sanitizer_item_ref(activity: &RuntimeActivity) -> (RuntimeActivityType, Cow<'_, str>) {
  let activity_type = activity.activity();
  match activity {
    RuntimeActivity::AsyncOp(_, _, name) => (activity_type, (*name).into()),
    RuntimeActivity::Resource(_, _, name) => (activity_type, name.into()),
...
pub fn create_test_sanitizer_helper(worker: &mut MainWorker) -> TestSanitizerHelper {
...
fn is_empty(top_level: &TopLevelSanitizerStats, diff: &RuntimeActivityDiff) -> bool {
...
pub async fn wait_for_activity_to_stabilize(
  ...
  let mut diff = RuntimeActivityStats::diff(&before_test_stats, &after_test_stats);
