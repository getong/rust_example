...
use deno_core::{
  parking_lot,
  parking_lot::lock_api::{RawMutex, RawMutexTimed},
};
use deno_runtime::deno_io::{AsyncPipeRead, PipeRead, PipeWrite, pipe};
use tokio::{
  io::{AsyncRead, AsyncReadExt, ReadBuf},
  sync::mpsc::{UnboundedReceiver, UnboundedSender, WeakUnboundedSender, error::SendError},
};
...
/// Create a [`TestEventSenderFactory`] and [`TestEventReceiver`] pair. The
/// [`TestEventSenderFactory`] may be used to create [`TestEventSender`]s and stdio streams for
/// multiple workers in the system. The [`TestEventReceiver`] will be kept alive until the final
/// [`TestEventSender`] is dropped.
    ...
    loop {
      ...
      match file.read(&mut buffer).await {
        ...
        Ok(read) => {
          ...
          // If we don't end with the marker, then we need to search the bytes we read plus four
          // bytes from before. There's still a possibility that the marker could be split
          // because of a pipe buffer that fills up, forcing the flush to be written
          // across two writes and interleaving data between, but that's a risk we take
          // with this sync marker approach.
          let start = (flush.len() - read).saturating_sub(HALF_SYNC_MARKER.len());
          if let Some(offset) = memchr::memmem::find(&flush[start ..], HALF_SYNC_MARKER) {
  ...
  pub fn worker(&self) -> TestEventWorkerSender {
    ...
    std::thread::spawn(move || {
      ...
      runtime.block_on(tokio::task::unconstrained(async move {
        let mut test_stdout = TestStream::new(id, stdout_reader, sender.clone())?;
        let mut test_stderr = TestStream::new(id, stderr_reader, sender)?;

        // This ensures that the stdout and stderr streams in the select! loop below cannot starve
        // each other.
...
impl TestEventSender {
  ...
  pub fn flush(&mut self) -> Result<(), ChannelClosedError> {
    // Two phase lock: mutex1 indicates that we are done our general read phase and are ready for
    // the sync phase. mutex2 indicates that we have completed the sync phase. This prevents
    // deadlock when the pipe is too full to accept the sync marker.
...
#[cfg(test)]
mod tests {
  use deno_core::unsync::{spawn, spawn_blocking};
  ...
  async fn test_flush_large() {
    ...
    let send_handle = spawn_blocking(move || {
      for _ in 0 .. 25000 {
        // Write one pipe buffer's worth of message here. We try a few different sizes of
        // potentially blocking writes.
  ...
  async fn test_interleave() {
    ...
    let send_handle: deno_core::unsync::JoinHandle<()> = spawn_blocking(move || {
