...
use deno_config::glob::{FilePatterns, WalkEntry};
use deno_core::{
  ModuleSpecifier, OpState, PollEventLoopOptions, anyhow,
  anyhow::anyhow,
  error::{AnyError, CoreError, CoreErrorKind, JsError},
  futures::{StreamExt, future, stream},
  located_script_name, serde_v8,
  unsync::{spawn, spawn_blocking},
  url::Url,
  v8,
};
...
use crate::{
  args::{CliOptions, Flags, TestFlags, TestReporterConfig},
  colors, display,
  factory::CliFactory,
  file_fetcher::CliFileFetcher,
  graph_container::CheckSpecifiersOptions,
  graph_util::has_graph_root_local_dependent_changed,
  ops,
  sys::CliSys,
  util::{
    extract::extract_doc_tests,
    file_watcher,
    fs::{CollectSpecifiersOptions, collect_specifiers},
    path::{get_extension, is_script_ext, matches_pattern_or_exact_path},
  },
  worker::{CliMainWorkerFactory, CreateCustomWorkerError},
};
...
pub use channel::{
  TestEventReceiver, TestEventSender, TestEventWorkerSender, create_single_test_event_channel,
...
use crate::tools::{
  ...
  test::channel::ChannelClosedError,
};
...
impl TestContainer {
  pub fn register(&mut self, description: TestDescription, function: v8::Global<v8::Function>) {
    self.descriptions.tests.insert(description.id, description);
    self.test_functions.push(function)
  }

  pub fn register_hook(&mut self, hook_type: String, function: v8::Global<v8::Function>) {
...
impl TestFailure {
  pub fn format(&self, options: &TestFailureFormatOptions) -> Cow<'static, str> {
    match self {
      TestFailure::JsError(js_error) => Cow::Owned(format_test_error(js_error, options)),
      TestFailure::FailedSteps(1) => Cow::Borrowed("1 test step failed."),
      TestFailure::FailedSteps(n) => Cow::Owned(format!("{} test steps failed.", n)),
      TestFailure::IncompleteSteps => Cow::Borrowed(
        "Completed while steps were still running. Ensure all steps are awaited with `await \
         t.step(...)`.",
      ),
  ...
  pub fn overview(&self) -> String {
    match self {
      ...
      TestFailure::IncompleteSteps => "Completed while steps were still running".to_string(),
      ...
      TestFailure::OverlapsWithSanitizers(_) => {
        "Started test step while another test step with sanitizers was running".to_string()
      }
      TestFailure::HasSanitizersAndOverlaps(_) => {
        "Started test step with sanitizers while another test step was running".to_string()
      }
    }
  }
  ...
  pub fn error_location(&self) -> Option<TestLocation> {
    ...
    // - This check guarantees that the next frame is inside of the Deno.test(), and not somewhere
    //   else.
    ...
    let frame = js_error
      ...
      .rfind(|f| {
        f.file_name
          .as_ref()
          .is_some_and(|f| f.starts_with("file:") && !f.contains("node_modules"))
      })?;
...
async fn configure_main_worker(
  ...
  let check_res = |res: Result<(), CoreError>| match res.map_err(|err| err.into_kind()) {
    Ok(()) => Ok(()),
...
async fn slow_test_watchdog(event_tracker: TestEventTracker, test_id: usize) {
  ...
  let mut elapsed = 0;
  loop {
    tokio::time::sleep(Duration::from_secs(base_timeout * (multiplier - elapsed))).await;
    if event_tracker
...
pub async fn run_tests_for_worker(
  ...
  let container = std::mem::take(&mut *state_rc.borrow_mut().borrow_mut::<TestContainer>());
...
async fn run_tests_for_worker_inner(
  ...
  for (desc, function) in tests_to_run.into_iter() {
    ...
    let result = if !before_each_hook_errored {
      ...
      let slow_test_warning = spawn(slow_test_watchdog(event_tracker.clone(), desc.id));
    ...
    .await?
    {
      ...
      if !formatted.is_empty() {
        ...
        fail_fast_tracker.add_failure();
        event_tracker.result(desc, TestResult::Failed(failure), earlier.elapsed())?;
        continue;
      }
    }
...
async fn test_specifiers(
  ...
  let join_handles = specifiers.into_iter().map(move |specifier| {
    ...
    spawn_blocking(move || {
      ...
      let permissions = cli_options.permissions_options_for_dir(&specifier_dir)?;
    ...
    match event {
      TestEvent::Register(description) => {
        for (_, description) in description.into_iter() {
          ...
          // TODO(mmastrac): We shouldn't need to clone here -- we can reuse the descriptions
          // everywhere
...
fn has_supported_test_path_name(path: &Path) -> bool {
  if let Some(name) = path.file_stem() {
    ...
    if basename.ends_with("_test") || basename.ends_with(".test") || basename == "test" {
      return true;
    }
...
async fn fetch_specifiers_with_test_mode(
  ...
  let mut specifiers_with_mode = member_patterns
    .map(|files| collect_specifiers_with_test_mode(cli_options, files.clone(), doc))
...
pub async fn run_tests(flags: Arc<Flags>, test_flags: TestFlags) -> Result<(), AnyError> {
  let factory = CliFactory::from_flags(flags.clone());
  let cli_options = factory.cli_options()?;
  let workspace_test_options = cli_options.resolve_workspace_test_options(&test_flags);
  let file_fetcher = factory.file_fetcher()?;
  let log_level = cli_options.log_level();

  let members_with_test_options = cli_options.resolve_test_options_for_members(&test_flags)?;
...
impl TestEventTracker {
  ...
  fn slow(&self, test_id: usize, duration: Duration) -> Result<(), ChannelClosedError> {
  ...
  fn register(&self, descriptions: Arc<TestDescriptions>) -> Result<(), ChannelClosedError> {
