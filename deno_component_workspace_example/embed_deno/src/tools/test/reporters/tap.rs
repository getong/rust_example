...
use super::{common, fmt::to_relative_path_or_remote_url, *};
...
impl TapTestReporter {
  ...
  fn print_diagnostic(&self, indent: usize, failure: &TestFailure, location: DiagnosticLocation) {
  ...
  fn print_line(indent: usize, status: &str, step: usize, description: &str, directive: &str) {
    println!(
      "{:indent$}{} {} - {}{}",
      "",
      status,
  ...
  fn print_step_result(&mut self, desc: &TestStepDescription, result: &TestStepResult) {
    if self.step_n == 0 {
...
impl TestReporter for TapTestReporter {
  ...
  fn report_plan(&mut self, plan: &TestPlan) {
    ...
    if !self.is_concurrent {
      ...
      // During concurrent execution these would not correspond to the actual test file, so skip
      // them.
      println!(
        "# {}",
