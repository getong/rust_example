...
use super::{
  fmt::{format_test_error, to_relative_path_or_remote_url},
  *,
};
...
pub(super) fn report_sigint(
  ...
  for id in tests_pending {
    ...
    if let Some(desc) = test_steps.get(id) {
      formatted_pending.insert(format_test_step_for_summary(cwd, desc, tests, test_steps));
    }
  }
  writeln!(
    writer,
  ...
  if !summary.failures.is_empty() || !summary.uncaught_errors.is_empty() {
    ...
    for (origin, (failures, uncaught_error)) in failures_by_origin {
      ...
      if let Some(js_error) = uncaught_error {
        ...
        writeln!(
          writer,
          "This error was not caught from a test and caused the test runner to fail on the \
           referenced module."
        )
        .ok();
        writeln!(
          writer,
          "It most likely originated from a dangling promise, event/timeout handler or top-level \
           code."
        )
        .ok();
        writeln!(writer).ok();
