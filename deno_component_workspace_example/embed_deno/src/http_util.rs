...
use deno_core::{
  error::AnyError, futures::StreamExt, parking_lot::Mutex, serde, serde_json, url::Url,
};
use deno_error::{JsError, JsErrorBox};
use deno_lib::version::DENO_VERSION_INFO;
use deno_runtime::{
  deno_fetch,
  deno_fetch::{CreateHttpClientOptions, ResBody, create_http_client},
...
impl HttpClientResponse {
  pub fn into_bytes(self) -> Result<Vec<u8>, DownloadError> {
    match self {
      ...
      Self::NotModified => Err(DownloadErrorKind::UnhandledNotModified.into_box()),
    }
  }
...
fn resolve_redirect_from_response<B>(
  ...
  deno_cache_dir::file_fetcher::resolve_redirect_from_headers(request_url, response.headers())
...
mod test {
  ...
  async fn test_http_client_download_redirect() {
    ...
    let text = client
      .download_text(Url::parse("http://localhost:4546/subdir/redirects/redirect1.js").unwrap())
      .await
      .unwrap();
    ...
    let err = client
      .download_text(Url::parse("http://localhost:4549/subdir/redirects/redirect1.js").unwrap())
      .await
      .err()
      .unwrap();
  ...
  async fn test_fetch_with_cafile_string() {
    ...
    let client = HttpClient::new(
      create_http_client(
        DENO_VERSION_INFO.user_agent,
        CreateHttpClientOptions {
          ca_certs: vec![std::fs::read(test_util::testdata_path().join("tls/RootCA.pem")).unwrap()],
          ..Default::default()
        },
      )
      .unwrap(),
  ...
  /// This test depends on external servers, so we need to be careful to avoid mistaking an offline
  /// machine with a test failure.
  #[tokio::test]
  async fn test_fetch_with_default_certificate_store() {
    let urls: HashSet<_, RandomState> = HashSet::from_iter(PUBLIC_HTTPS_URLS.iter());
  ...
  async fn test_fetch_with_cafile_gzip() {
    let _http_server_guard = test_util::http_server();
    let url = Url::parse("https://localhost:5545/run/import_compression/gziped").unwrap();
    let client = HttpClient::new(
      create_http_client(
        DENO_VERSION_INFO.user_agent,
        CreateHttpClientOptions {
  ...
  async fn test_fetch_with_cafile_brotli() {
    let _http_server_guard = test_util::http_server();
    let url = Url::parse("https://localhost:5545/run/import_compression/brotli").unwrap();
    let client = HttpClient::new(
      create_http_client(
        DENO_VERSION_INFO.user_agent,
        CreateHttpClientOptions {
