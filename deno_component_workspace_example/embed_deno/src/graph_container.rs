...
use deno_core::{error::AnyError, parking_lot::RwLock};
use deno_graph::ModuleGraph;
use deno_runtime::{colors, deno_permissions::PermissionsContainer};

use crate::{
  args::CliOptions,
  module_loader::{ModuleLoadPreparer, PrepareModuleLoadOptions},
  util::{fs::collect_specifiers, path::is_script_ext},
};
...
impl MainModuleGraphContainer {
  ...
  pub fn collect_specifiers(
    ...
    let include_patterns = PathOrPatternSet::from_include_relative_path_or_patterns(
    ...
    collect_specifiers(
      crate::util::fs::CollectSpecifiersOptions {
        file_patterns,
        vendor_folder: self.cli_options.vendor_dir_path().map(ToOwned::to_owned),
