...
use deno_cache_dir::{GlobalOrLocalHttpCache, npm::NpmCacheDir};
use deno_config::workspace::WorkspaceDirectory;
use deno_core::{anyhow::Context, error::AnyError, futures::FutureExt, serde_json, url::Url};
use deno_error::JsErrorBox;
use deno_graph::packages::JsrVersionResolver;
use deno_lib::{
  args::{CaData, get_root_cert_store, npm_process_state},
  npm::{
    NpmRegistryReadPermissionChecker, NpmRegistryReadPermissionCheckerMode,
    create_npm_process_state_provider,
  },
  worker::{LibMainWorkerFactory, LibMainWorkerOptions, LibWorkerFactoryRoots},
};
use deno_npm::{npm_rc::ResolvedNpmRc, resolution::NpmVersionResolver};
use deno_npm_cache::NpmCacheSetting;
use deno_npm_installer::{
  NpmInstallerFactoryOptions,
  lifecycle_scripts::{LifecycleScriptsExecutor, NullLifecycleScriptsExecutor},
  process_state::NpmProcessStateKind,
};
use deno_resolver::{
  cache::ParsedSourceCache,
  cjs::IsCjsResolutionMode,
  deno_json::{CompilerOptionsOverrides, CompilerOptionsResolver},
  factory::{
    ConfigDiscoveryOption, NpmProcessStateOptions, ResolverFactoryOptions,
    SpecifiedImportMapProvider,
  },
  import_map::WorkspaceExternalImportMapLoader,
  loader::MemoryFiles,
  npm::DenoInNpmPackageChecker,
  workspace::WorkspaceResolver,
};
use deno_runtime::{
  FeatureChecker, deno_fs,
  deno_fs::RealFs,
...
use node_resolver::{
  NodeConditionOptions, NodeResolverOptions, cache::NodeResolutionThreadLocalCache,
};
use once_cell::sync::OnceCell;
use sys_traits::EnvCurrentDir;

use crate::{
  args::{BundleFlags, CliLockfile, CliOptions, ConfigFlag, DenoSubcommand, Flags, InstallFlags},
  cache::{Caches, CodeCache, DenoDir, GlobalHttpCache, ModuleInfoCache, SqliteNodeAnalysisCache},
  file_fetcher::{
    CliFileFetcher, CreateCliFileFetcherOptions, TextDecodedFile, create_cli_file_fetcher,
  },
  graph_container::MainModuleGraphContainer,
  graph_util::{FileWatcherReporter, ModuleGraphBuilder, ModuleGraphCreator},
  http_util::HttpClientProvider,
  module_loader::{CliEmitter, CliModuleLoaderFactory, EszipModuleLoader, ModuleLoadPreparer},
  node::{CliNodeResolver, CliPackageJsonResolver},
  npm::{
  ...
  util::{
    file_watcher::WatcherCommunicator,
    progress_bar::{ProgressBar, ProgressBarStyle},
  },
  worker::{CliMainWorkerFactory, CliMainWorkerOptions},
};
  ...
  pub async fn get(&self) -> Result<Option<&Arc<EszipModuleLoader>>, AnyError> {
      ...
      && let DenoSubcommand::Run(run_flags) = self.cli_options.sub_command()
    {
      if self.deferred.get().is_none() {
        let eszip_loader =
          EszipModuleLoader::create(&run_flags.script, self.cli_options.initial_cwd()).await?;
...
struct CliSpecifiedImportMapProvider {
  ...
  workspace_external_import_map_loader: Arc<WorkspaceExternalImportMapLoader<CliSys>>,
}

#[async_trait::async_trait(?Send)]
impl SpecifiedImportMapProvider for CliSpecifiedImportMapProvider {
  async fn get(&self) -> Result<Option<deno_resolver::workspace::SpecifiedImportMap>, AnyError> {
    async fn resolve_import_map_value_from_specifier(
      specifier: &Url,
      file_fetcher: &CliFileFetcher,
    ) -> Result<serde_json::Value, AnyError> {
      if specifier.scheme() == "data" {
        let data_url_text = deno_media_type::data_url::RawDataUrl::parse(specifier)?.decode()?;
        Ok(serde_json::from_str(&data_url_text)?)
      } else {
        let file =
          TextDecodedFile::decode(file_fetcher.fetch_bypass_permissions(specifier).await?)?;
        Ok(serde_json::from_str(&file.source)?)
      }
    }

    let maybe_import_map_specifier = self.cli_options.resolve_specified_import_map_specifier()?;
    match maybe_import_map_specifier {
      Some(specifier) => {
        let value = match self.eszip_module_loader_provider.get().await? {
          ...
          None => resolve_import_map_value_from_specifier(&specifier, &self.file_fetcher)
            .await
      ...
      None => {
        if let Some(import_map) = self.workspace_external_import_map_loader.get_or_load()? {
...
struct CliFactoryServices {
  ...
  permission_desc_parser: Deferred<Arc<RuntimePermissionDescriptorParser<CliSys>>>,
...
impl CliFactory {
  ...
  pub async fn maybe_lockfile(&self) -> Result<Option<&Arc<CliLockfile>>, AnyError> {
  ...
  pub fn http_cache(&self) -> Result<&GlobalOrLocalHttpCache<CliSys>, AnyError> {
  ...
  fn eszip_module_loader_provider(&self) -> Result<&Arc<EszipModuleLoaderProvider>, AnyError> {
    self
      .services
  ...
  pub fn in_npm_pkg_checker(&self) -> Result<&DenoInNpmPackageChecker, AnyError> {
  ...
  pub async fn npm_graph_resolver(&self) -> Result<&Arc<CliNpmGraphResolver>, AnyError> {
    self
  ...
  pub async fn npm_installer_if_managed(&self) -> Result<Option<&Arc<CliNpmInstaller>>, AnyError> {
    self
      .npm_installer_factory()?
  ...
  pub fn npm_installer_factory(&self) -> Result<&CliNpmInstallerFactory, AnyError> {
    self.services.npm_installer_factory.get_or_try_init(|| {
      ...
      Ok(CliNpmInstallerFactory::new(
        ...
        match resolver_factory.npm_resolver()?.as_managed() {
          Some(managed_npm_resolver) => Arc::new(DenoTaskLifeCycleScriptsExecutor::new(
  ...
  pub async fn workspace_resolver(&self) -> Result<&Arc<WorkspaceResolver<CliSys>>, AnyError> {
    self.initialize_npm_resolution_if_managed().await?;
  ...
  pub fn code_cache(&self) -> Result<&Arc<CodeCache>, AnyError> {
    self
      .services
      .code_cache
      .get_or_try_init(|| Ok(Arc::new(CodeCache::new(self.caches()?.code_cache_db()))))
  }
  ...
  pub fn pkg_json_resolver(&self) -> Result<&Arc<CliPackageJsonResolver>, AnyError> {
    Ok(self.resolver_factory()?.pkg_json_resolver())
  }

  pub fn compiler_options_resolver(&self) -> Result<&Arc<CompilerOptionsResolver>, AnyError> {
  ...
  pub async fn module_graph_creator(&self) -> Result<&Arc<ModuleGraphCreator>, AnyError> {
    self
      .services
  ...
  pub fn permission_desc_parser(
    ...
    self
      ...
      .get_or_try_init(|| Ok(Arc::new(RuntimePermissionDescriptorParser::new(self.sys()))))
  }
  ...
  pub fn root_permissions_container(&self) -> Result<&PermissionsContainer, AnyError> {
    self
      .services
  ...
  pub async fn create_module_loader_factory(&self) -> Result<CliModuleLoaderFactory, AnyError> {
    let cli_options = self.cli_options()?;
    ...
    let npm_registry_permission_checker = {
      ...
      } else if let Some(node_modules_dir) = workspace_factory.node_modules_dir_path()? {
        NpmRegistryReadPermissionCheckerMode::Local(node_modules_dir.to_path_buf())
      } else {
        NpmRegistryReadPermissionCheckerMode::Global(self.npm_cache_dir()?.root_dir().to_path_buf())
      };
      Arc::new(NpmRegistryReadPermissionChecker::new(self.sys(), mode))
    };

    let maybe_eszip_loader = self.eszip_module_loader_provider()?.get().await?.cloned();
  ...
  fn create_cli_main_worker_options(&self) -> Result<CliMainWorkerOptions, AnyError> {
    ...
    let create_hmr_runner = if cli_options.has_hmr() {
      ...
      let fn_: crate::worker::CreateHmrRunnerCb =
        Box::new(move || HmrRunnerState::new(emitter.clone(), watcher_communicator.clone()));
      Some(fn_)
    } else {
      None
    };
  ...
  pub fn resolver_factory(&self) -> Result<&Arc<CliResolverFactory>, AnyError> {
    self.services.resolver_factory.get_or_try_init(|| {
      ...
      let node_analysis_cache = Arc::new(SqliteNodeAnalysisCache::new(caches.node_analysis_db()));
      Ok(Arc::new(CliResolverFactory::new(
        ...
        ResolverFactoryOptions {
          ...
          is_cjs_resolution_mode: if options.is_node_main() || options.unstable_detect_cjs() {
          ...
          node_resolver_options: NodeResolverOptions {
            ...
            typescript_version: Some(
              deno_semver::Version::parse_standard(deno_lib::version::DENO_VERSION_INFO.typescript)
                .unwrap(),
            ),
            bundle_mode: matches!(self.flags.subcommand, DenoSubcommand::Bundle(_)),
          ...
          node_code_translator_mode: match options.sub_command() {
            DenoSubcommand::Bundle(_) => node_resolver::analyze::NodeCodeTranslatorMode::Disabled,
          ...
          specified_import_map: Some(Box::new(CliSpecifiedImportMapProvider {
            ...
            eszip_module_loader_provider: self.eszip_module_loader_provider()?.clone(),
          ...
          allow_json_imports: if matches!(self.flags.subcommand, DenoSubcommand::Bundle(_)) {
...
fn new_workspace_factory_options(
  ...
  deno_resolver::factory::WorkspaceFactoryOptions {
    additional_config_file_names: if matches!(flags.subcommand, DenoSubcommand::Publish(..)) {
      &["jsr.json", "jsr.jsonc"]
    } else {
      &[]
    },
    config_discovery: match &flags.config_flag {
      ...
      ConfigFlag::Path(path) => ConfigDiscoveryOption::Path(initial_cwd.join(path)),
    ...
    no_lock: flags.no_lock
      || matches!(
        flags.subcommand,
        DenoSubcommand::Install(InstallFlags::Global(..)) | DenoSubcommand::Uninstall(_)
      ),
    ...
    npm_process_state: npm_process_state(&CliSys::default()).as_ref().map(|s| {
    ...
    root_node_modules_dir_override: flags.internal.root_node_modules_dir_override.clone(),
    vendor: flags.vendor,
  }
}
