...
#[op2(fast)]
fn embed_deno_set_result(state: &mut OpState, #[string] value: String) {
...
#[op2(fast)]
fn embed_deno_set_exit_data(state: &mut OpState, #[string] value: String) {
  let holder = state.borrow::<Arc<Mutex<EmbedResult>>>();
  if let Ok(mut slot) = holder.lock() {
...
deno_core::extension!(
  embed_deno_ext,
  ops = [embed_deno_set_result, embed_deno_set_exit_data],
  esm_entry_point = "ext:embed_deno_ext/embed/result.js",
...
pub fn extension(result_holder: Arc<Mutex<EmbedResult>>) -> Extension {
  embed_deno_ext::init(result_holder)
}
