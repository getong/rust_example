...
use deno_core::{
  FastString, ModuleLoadOptions, ModuleLoadReferrer, ModuleLoader, ModuleResolutionError,
...
use deno_lib::{
  loader::{
    as_deno_resolver_requested_module_type, loaded_module_source_to_module_source_code,
    module_type_from_media_and_requested_type,
  },
  npm::NpmRegistryReadPermissionChecker,
...
use deno_runtime::{
  code_cache,
  deno_node::{NodeRequireLoader, create_host_defined_options, ops::require::UnableToGetCwdError},
  deno_permissions::{CheckSpecifierKind, PermissionsContainer},
};
use deno_semver::npm::NpmPackageReqReference;
use eszip::EszipV2;
use node_resolver::{
  InNpmPackageChecker, NodeResolutionKind, ResolutionMode, errors::PackageJsonLoadError,
};
use sys_traits::{FsMetadata, FsMetadataValue, FsRead};
use tokio_util::compat::TokioAsyncReadCompatExt;

use crate::{
  args::{CliLockfile, CliOptions, DenoSubcommand, TsTypeLib, jsr_url},
  cache::CodeCache,
  file_fetcher::CliFileFetcher,
  graph_container::{MainModuleGraphContainer, ModuleGraphContainer, ModuleGraphUpdatePermit},
  graph_util::{BuildGraphRequest, BuildGraphWithNpmOptions, ModuleGraphBuilder},
  npm::CliNpmResolver,
  resolver::{CliCjsTracker, CliResolver},
  sys::CliSys,
  type_checker::{CheckError, CheckOptions, TypeChecker},
  util::{
    progress_bar::ProgressBar,
    text_encoding::{code_without_source_map, source_map_from_code},
  },
};
...
struct SharedCliModuleLoaderState {
  ...
  npm_registry_permission_checker: Arc<NpmRegistryReadPermissionChecker<CliSys>>,
...
impl InFlightModuleLoadsTracker {
  ...
  pub fn decrease(&self, parsed_source_cache: &Arc<ParsedSourceCache>) {
    ...
    if prev == 1 {
      ...
      let maybe_prev_task = self.cleanup_task_handle.lock().replace(task_handle);
...
impl CliModuleLoaderFactory {
  ...
  fn create_with_lib<TGraphContainer: ModuleGraphContainer>(
    ...
    let module_loader = Rc::new(CliModuleLoader(Rc::new(CliModuleLoaderInner {
      lib,
      is_worker,
      parent_permissions,
      permissions,
    ...
    let node_require_loader = Rc::new(CliNodeRequireLoader {
      ...
      npm_registry_permission_checker: self.shared.npm_registry_permission_checker.clone(),
    });
    CreateModuleLoaderResult {
      module_loader,
      node_require_loader,
...
impl ModuleLoaderFactory for CliModuleLoaderFactory {
  ...
  fn create_for_worker(
    ...
    self.create_with_lib(
      ...
      WorkerModuleGraphContainer::new(Arc::new(ModuleGraph::new(self.shared.graph_kind))),
...
impl<TGraphContainer: ModuleGraphContainer> CliModuleLoaderInner<TGraphContainer> {
  async fn load_inner(
    &self,
      ...
      self.shared.code_cache.as_ref().map(|cache| {
        ...
        let data = cache
          ...
          .inspect(|_| {
            ...
            log::debug!("V8 code cache hit for ES module: {specifier}, [{code_hash:?}]");
          });
        SourceCodeCacheInfo {
          hash: code_hash,
          data,
  ...
  async fn load_code_source(
    ...
    let specifier = if let Ok(reference) = NpmPackageReqReference::from_specifier(specifier) {
  ...
  async fn load_asset(
    ...
    let module_type = match requested_module_type {
      ...
      RequestedModuleType::None => match file.resolve_media_type_and_charset().0 {
  ...
  async fn maybe_reload_dynamic(
    ...
    let specifiers_to_reload = self.check_specifiers_to_reload_for_dynamic_import(graph, specifier);
  ...
  fn check_specifiers_to_reload_for_dynamic_import(
    ...
    while let Some((specifier, module_entry)) = module_iter.next() {
      if specifier.scheme() != "file" || self.loaded_files.borrow().contains(specifier) {
      ...
      let should_reload = match module_entry {
        ...
        deno_graph::ModuleEntryRef::Err(err) => {
          if matches!(err.as_kind(), deno_graph::ModuleErrorKind::Missing { .. }) {
  ...
  fn mtime_of_specifier(&self, specifier: &ModuleSpecifier) -> Option<SystemTime> {
  ...
  fn resolve_referrer(&self, referrer: &str) -> Result<ModuleSpecifier, ResolveReferrerError> {
  ...
  fn inner_resolve(
    ...
    fn ensure_not_jsr_non_jsr_remote_import(
        ...
        && matches!(specifier.scheme(), "http" | "https")
      {
        return Err(JsErrorBox::generic(format!(
          "Importing {} blocked. JSR packages cannot import non-JSR remote modules for security \
           reasons.",
          specifier
        )));
      }
    ...
    let result = self.shared.resolver.resolve_with_graph(
      ...
      ResolveWithGraphOptions {
        ...
        maintain_npm_specifiers: matches!(kind, deno_core::ResolutionKind::DynamicImport)
          && !is_import_meta,
      },
    );
...
impl<TGraphContainer: ModuleGraphContainer> ModuleLoader for CliModuleLoader<TGraphContainer> {
  fn resolve(
    &self,
    specifier: &str,
    referrer: &str,
  ...
  fn prepare_load(
    ...
    async move {
      ...
      if options.is_dynamic_import {
        ...
        if !graph.roots.is_empty() && graph.get(&specifier).is_some() {
          ...
          if did_reload || !graph.roots.contains(&specifier) {
            module_load_preparer.graph_roots_valid(&graph, &[specifier], false, false)?;
          }
          return Ok(());
        }
      }
      ...
      if is_dynamic {
        inner
          .maybe_reload_dynamic(&graph_container.graph(), &specifiers[0], permissions)
          .await
          .map_err(JsErrorBox::from_err)?;
  ...
  fn code_cache_ready(
    ...
    if let Some(cache) = self.0.shared.code_cache.as_ref() {
      // This log line is also used by tests.
      log::debug!("Updating V8 code cache for ES module: {specifier}, [{source_hash:?}]");
      cache.set_sync(
        &specifier,
    ...
    let code = match graph.get(&specifier) {
      ...
      None => {
        ...
        if let Ok(Some(file)) = self
          ...
          .get_cached_source_or_local(&specifier)
        {
          return extract_source_line(&String::from_utf8_lossy(&file.source), line_number);
        } else {
          return None;
        }
      }
...
fn extract_source_line(text: &str, line_number: usize) -> Option<String> {
  ...
  match text.split('\n').nth(line_number) {
    ...
    None => Some(format!(
      "{} Couldn't format source line: Line {} is out of bounds (source may have changed at \
       runtime)",
...
impl EszipModuleLoader {
  pub async fn create(script: &str, cwd: &Path) -> Result<Self, AnyError> {
    ...
    let loaded_eszips: Vec<Result<EszipV2, AnyError>> = loaded_eszips.collect::<Vec<_>>().await;
    ...
    match self.files.get(specifier) {
      Some(source) => {
        let module_source = ModuleSource::new(
          ...
          ModuleSourceCode::Bytes(deno_core::ModuleCodeBytes::Arc(source.clone())),
          specifier,
          None,
        );
        deno_core::ModuleLoadResponse::Sync(Ok(module_source))
      }
      None => deno_core::ModuleLoadResponse::Sync(Err(JsErrorBox::generic("Module not found"))),
    }
  }
}
