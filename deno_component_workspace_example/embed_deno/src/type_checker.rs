...
use crate::{
  args::{CliOptions, CompilerOptions, DenoSubcommand, TsTypeLib, TypeCheckMode},
  cache::{CacheDBHash, Caches, TypeCheckCache},
  graph_util::{BuildFastCheckGraphOptions, ModuleGraphBuilder, module_error_for_tsc_diagnostic},
...
impl TypeChecker {
    ...
    if failed {
      Err(
        FailedTypeCheckingError {
          can_skip: !matches!(self.cli_options.sub_command(), DenoSubcommand::Check(_)),
        }
        .into(),
      )
    } else {
  ...
  pub fn check_diagnostics(
    ...
    let grouped_roots = self.group_roots_by_compiler_options(&graph, options.lib)?;
    ...
    Ok(DiagnosticsByFolderIterator(
      DiagnosticsByFolderIteratorInner::Real(DiagnosticsByFolderRealIterator {
        ...
        type_check_cache: TypeCheckCache::new(self.caches.type_checking_cache_db()),
  ...
  fn group_roots_by_compiler_options<'a>(
    ...
    for root in &graph.roots {
      let compiler_options_data = self.compiler_options_resolver.for_specifier(root);
      let compiler_options = compiler_options_data.compiler_options_for_lib(lib)?;
...
pub struct DiagnosticsByFolderIterator<'a>(DiagnosticsByFolderIteratorInner<'a>);
...
impl DiagnosticsByFolderRealIterator<'_> {
  ...
  fn check_diagnostics_in_folder(
    ...
    let mut missing_diagnostics = missing_diagnostics
      .filter(|d| self.should_include_diagnostic(self.options.type_check_mode, d));
    ...
    let mut response_diagnostics = response
      .diagnostics
      .filter(|d| self.should_include_diagnostic(self.options.type_check_mode, d));
  ...
  fn should_include_diagnostic(&self, type_check_mode: TypeCheckMode, d: &tsc::Diagnostic) -> bool {
...
impl<'a> GraphWalker<'a> {
  ...
  pub fn new(
    ...
    Self {
      ...
      compiler_options_resolver,
      maybe_hasher,
      seen: HashSet::with_capacity(graph.imports.len() + graph.specifiers_count()),
  ...
  fn resolve_pending(&mut self) {
    while let Some((specifier, is_dynamic)) = self.pending.pop_front() {
      let module = match self.graph.try_get(specifier) {
        ...
        Ok(None) => continue,
        Err(err) => {
          if !is_dynamic && let Some(err) = module_error_for_tsc_diagnostic(self.sys, err) {
            self
              .missing_diagnostics
              .push(tsc::Diagnostic::from_missing_error(
                err.specifier.as_str(),
                err.maybe_range,
                maybe_additional_sloppy_imports_message(self.sys, err.specifier),
              ));
          }
          continue;
        }
      ...
      if let Some(deps) = maybe_module_dependencies {
        for dep in deps.values() {
          ...
          if let deno_graph::Resolution::Err(resolution_error) = dep_to_check_error
            && let Some(diagnostic) = tsc::Diagnostic::maybe_from_resolution_error(resolution_error)
          {
    ...
    match module {
      Module::Js(module) => {
        let result = match module.media_type {
          ...
          | MediaType::Dts
          | MediaType::Dmts
          | MediaType::Dcts => Some((module.specifier.clone(), module.media_type)),
  ...
  fn handle_specifier(&mut self, specifier: &'a ModuleSpecifier, is_dynamic: bool) {
    let specifier = self.graph.resolve(specifier);
    if is_dynamic {
...
static TS_CHECK_RE: Lazy<Regex> = lazy_regex::lazy_regex!(r#"(?i)^\s*@ts-check(?:\s+|$)"#);

fn has_ts_check(media_type: MediaType, file_text: &str) -> bool {
  match &media_type {
    MediaType::JavaScript | MediaType::Mjs | MediaType::Cjs | MediaType::Jsx => {
      get_leading_comments(file_text)
        .iter()
...
mod test {
  ...
  use super::{ambient_modules_to_regex_string, get_leading_comments, has_ts_check};

  #[test]
  fn get_leading_comments_test() {
    assert_eq!(
      get_leading_comments("#!/usr/bin/env deno\r\n// test\n/* 1 *//*2*///3\n//\n /**/  /*4 */"),
      vec![
        " test".to_string(),
        " 1 ".to_string(),
        "2".to_string(),
