...
/// A cache database that eagerly initializes itself off-thread, preventing initialization
...
impl CacheDB {
  ...
  fn initialize_connection(
    ...
    if existing_version != version {
      ...
      let mut stmt = conn.prepare("INSERT OR REPLACE INTO info (key, value) VALUES (?1, ?2)")?;
  ...
  fn open_connection_and_init(&self, path: Option<&Path>) -> Result<Connection, rusqlite::Error> {
    let conn = self.actually_open_connection(path)?;
    Self::initialize_connection(self.config, &conn, self.version)?;
    Ok(conn)
  }

  /// This function represents the policy for dealing with corrupted cache files. We try fairly
  /// aggressively to repair the situation, and if we can't, we prefer to log noisily and continue
...
fn open_connection(
  config: &CacheDBConfiguration,
  path: Option<&Path>,
  open_connection_and_init: impl Fn(Option<&Path>) -> Result<Connection, rusqlite::Error>,
  ...
  // There are rare times in the tests when we can't initialize a cache DB the first time, but it
  // succeeds the second time, so we don't log these at a debug level.
  log::trace!(
...
fn handle_failure_mode(
  ...
  match config.on_failure {
    CacheFailure::InMemory => Ok(ConnectionState::Connected(open_connection_and_init(None)?)),
