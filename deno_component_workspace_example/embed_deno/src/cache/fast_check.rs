...
use deno_graph::fast_check::{FastCheckCacheItem, FastCheckCacheKey};
...
impl FastCheckCache {
  ...
  fn ensure_ok<T: Default>(res: Result<T, AnyError>) -> T {
    match res {
      Ok(x) => x,
      Err(err) => {
        // TODO(mmastrac): This behavior was inherited from before the refactoring but it probably
        // makes sense to move it into the cache at some point.
...
impl FastCheckCacheInner {
  ...
  pub fn get(&self, key: FastCheckCacheKey) -> Result<Option<FastCheckCacheItem>, AnyError> {
    let query = "
      SELECT
        data
      FROM
    ...
    let res = self
      .conn
      .query_row(query, params![CacheDBHash::new(key.as_u64())], |row| {
  ...
  pub fn set(&self, key: FastCheckCacheKey, data: &FastCheckCacheItem) -> Result<(), AnyError> {
    let sql = "
...
mod test {
  ...
  use deno_graph::fast_check::{
    FastCheckCache as _, FastCheckCacheModuleItem, FastCheckCacheModuleItemDiagnostic,
  };
  ...
  pub fn cache_general_use() {
    ...
    let value = FastCheckCacheItem {
      dependencies: BTreeSet::from([PackageNv::from_str("@scope/b@1.0.0").unwrap()]),
      modules: vec![(
        ModuleSpecifier::parse("https://jsr.io/test.ts").unwrap(),
        FastCheckCacheModuleItem::Diagnostic(FastCheckCacheModuleItemDiagnostic {
          source_hash: 123,
        }),
      )],
    };
