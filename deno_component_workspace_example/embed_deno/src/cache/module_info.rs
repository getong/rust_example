...
use deno_graph::{analysis::ModuleInfo, ast::ParserModuleAnalyzer};
use deno_resolver::cache::ParsedSourceCache;
use deno_runtime::deno_webstorage::rusqlite::params;

use super::cache_db::{CacheDB, CacheDBConfiguration, CacheDBHash, CacheFailure};
...
impl ModuleInfoCache {
  #[cfg(test)]
  pub fn new_in_memory(version: &'static str, parsed_source_cache: Arc<ParsedSourceCache>) -> Self {
    Self::new(
  ...
  pub fn new(conn: CacheDB, parsed_source_cache: Arc<ParsedSourceCache>) -> Self {
    Self {
      conn,
      parsed_source_cache,
    }
...
impl deno_graph::source::ModuleInfoCacher for ModuleInfoCache {
  fn cache_module_info(
    ...
    let result = self.set_module_info(specifier, media_type, source_hash, module_info);
...
impl ModuleInfoCacheModuleAnalyzer<'_> {
  ...
  pub fn analyze_sync(
    ...
    let source_hash = CacheDBHash::from_hashable(source);
    if let Some(info) = self.load_cached_module_info(specifier, media_type, source_hash) {
      return Ok(info);
    }
    ...
    let module_info = analyzer.analyze_sync(specifier, source.clone(), media_type)?;

    // then attempt to cache it
    self.save_module_info_to_cache(specifier, media_type, source_hash, &module_info);
...
impl deno_graph::analysis::ModuleAnalyzer for ModuleInfoCacheModuleAnalyzer<'_> {
  async fn analyze(
    &self,
    specifier: &ModuleSpecifier,
    source: Arc<str>,
...
mod test {
  use deno_graph::{
    PositionRange,
    analysis::{JsDocImportInfo, SpecifierWithRange},
  };
  ...
  pub fn module_info_cache_general_use() {
    ...
    assert_eq!(
      cache
        .get_module_info(&specifier1, MediaType::JavaScript, CacheDBHash::new(1))
        .unwrap(),
      None
    );
    ...
    assert_eq!(
      cache
        .get_module_info(&specifier2, MediaType::JavaScript, CacheDBHash::new(1))
        .unwrap(),
      None,
    );
    ...
    assert_eq!(
      cache
        .get_module_info(&specifier1, MediaType::TypeScript, CacheDBHash::new(1))
        .unwrap(),
      None,
    );
    ...
    assert_eq!(
      cache
        .get_module_info(&specifier1, MediaType::JavaScript, CacheDBHash::new(2))
        .unwrap(),
      None,
    );
