...
use deno_typescript_go_client_rust::{
  CallbackHandler, SyncRpcChannel,
  types::{
    GetImpliedNodeFormatForFilePayload, Project, ResolveModuleNamePayload,
    ResolveTypeReferenceDirectivePayload,
  },
};
pub use setup::{DownloadError, ensure_tsgo};
...
fn deser<T: serde::de::DeserializeOwned>(payload: impl AsRef<str>) -> Result<T, serde_json::Error> {
...
// work around both by making a fake tsconfig.json file with the `"files"` field set to the root
// file names. it's "synthetic" because it's not actually on disk, we pass a fake path to load it
// from memory.
fn synthetic_config(
...
fn exec_request_inner(
  ...
  let diagnostics = deser::<Vec<deno_typescript_go_client_rust::types::Diagnostic>>(diagnostics)?;
...
fn maybe_rewrite_message(message: String, code: u64) -> String {
  if code == 2304 && message.starts_with("Cannot find name 'Deno'") {
    ...
    "Export assignment cannot be used when targeting ECMAScript modules. Consider using 'export \
     default' or another module format instead. This will start erroring in a future version of \
     Deno 2 in order to align with TypeScript."
      .to_string()
  } else if code == 2339 && message.contains("on type 'typeof Deno'") {
    let regex = lazy_regex::regex!(r#"Property '([^']+)' does not exist on type 'typeof Deno'"#);
    let captures = regex.captures(&message).unwrap();
    let name = captures.get(1).unwrap().as_str();
    format!(
      "Property '{name}' does not exist on type 'typeof Deno'. 'Deno.{name}' is an unstable API. \
       If not, try changing the 'lib' compiler option to include 'deno.unstable' or add a \
       triple-slash directive to the top of your entrypoint (main file): /// <reference \
       lib=\"deno.unstable\" />",
    )
  } else {
    message
...
struct HandlerState {
  ...
  module_kind_map: HashMap<String, deno_typescript_go_client_rust::types::ResolutionMode>,
  ...
  fn handle_callback(
    ...
    match name {
      ...
      "resolveModuleName" => {
        ...
        {
          append_raw_import_fragment(&mut out_name, import_attribute_type.as_str());
          extension = Some("ts");
        }
      ...
      "getPackageJsonScopeIfApplicable" => {
        ...
        get_package_json_scope_if_applicable(&mut state, payload)
          .inspect(|res| log::debug!("getPackageJsonScopeIfApplicable -> {}", res))
      }
      "getPackageScopeForPath" => {
      ...
      "getImpliedNodeFormatForFile" => {
        ...
        if let Some(module_kind) = state.module_kind_map.get(&payload.file_name) {
          ...
          let load_result = load_inner(&mut state, &payload.file_name).map_err(adhoc)?;
          if let Some(load_result) = load_result {
            ...
            let module_kind = state
              ...
              .copied()
              .unwrap_or(deno_typescript_go_client_rust::types::ResolutionMode::ESM);
            Ok(jsons!(&module_kind)?)
          } else {
            Ok(jsons!(
      ...
      "isNodeSourceFile" => {
        ...
        let result = ModuleSpecifier::parse(&path)
          .ok()
          .or_else(|| deno_path_util::resolve_url_or_path(&path, &state.current_dir).ok())
          .and_then(|specifier| {
            state
              .maybe_npm
              .as_ref()
...
fn resolve_name(
  ...
  let referrer =
    if let Some(remapped_specifier) = handler.maybe_remapped_specifier(&payload.containing_file) {
      remapped_specifier.clone()
    } else {
      deno_path_util::resolve_url_or_path(&payload.containing_file, &handler.current_dir)
        .map_err(adhoc)?
    };
  ...
  let result = super::resolve_specifier_for_tsc(
    ...
    match payload.resolution_mode {
      deno_typescript_go_client_rust::types::ResolutionMode::None => super::ResolutionMode::Import,
      deno_typescript_go_client_rust::types::ResolutionMode::CommonJS => {
        super::ResolutionMode::Require
      }
      deno_typescript_go_client_rust::types::ResolutionMode::ESM => super::ResolutionMode::Import,
    },
    maybe_npm,
    referrer_module,
...
fn get_resolution_mode(
  ...
  if is_cjs {
    ...
    match media_type {
      ...
      | MediaType::Wasm
      | MediaType::SourceMap
      | MediaType::Unknown => deno_typescript_go_client_rust::types::ResolutionMode::None,
