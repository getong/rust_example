...
use deno_lib::util::{checksum, hash::FastInsecureHasher};
use deno_npm::resolution::PackageReqNotFoundError;
use deno_resolver::npm::{
  ResolvePkgFolderFromDenoReqError,
  managed::{ResolvePkgFolderFromDenoModuleError, ResolvePkgFolderFromPkgIdError},
};
    ...
    Some(Module::Npm(_)) => {
        ...
        && let Ok(req_ref) = NpmPackageReqReference::from_specifier(specifier)
      {
        let managed_resolver = npm
          ...
          .unwrap();
        let pkg_id = managed_resolver.resolve_pkg_id_from_deno_module_req(req_ref.req())?;
        let package_folder = managed_resolver.resolve_pkg_folder_from_pkg_id(&pkg_id)?;
          ...
          Err(err) => match err.code() {
            NodeJsErrorCode::ERR_TYPES_NOT_FOUND => {
              ...
              if let Some((_, types_nv)) = deno_resolver::npm::find_definitely_typed_package(
                ...
                let package_folder =
                  managed_resolver.resolve_pkg_folder_from_deno_module(types_nv)?;
                let res_result = npm.node_resolver.resolve_package_subpath_from_deno_module(
                  &package_folder,
                  req_ref.sub_path(),
                  Some(referrer),
...
pub fn resolve_specifier_for_tsc(
  ...
  let maybe_result = match resolved_dep {
    ...
    _ => {
      match resolve_non_graph_specifier_types(&specifier, referrer, resolution_mode, maybe_npm) {
...
pub trait Mapper {
  fn maybe_remapped_specifier(&self, specifier: &str) -> Option<&ModuleSpecifier>;
}
...
pub fn load_for_tsc<T: LoadContent, M: Mapper>(
  ...
  let specifier = deno_path_util::resolve_url_or_path(load_specifier, current_dir)?;
  ...
  // handle the request for that module here.
    ...
    let specifier =
      if let Some(remapped_specifier) = remapper.maybe_remapped_specifier(load_specifier) {
        remapped_specifier
      } else {
        &specifier
      };
      ...
      match module {
        ...
        Module::External(module) => {
          if module.specifier.scheme() != "file" {
            ...
            load_from_node_modules(&specifier, maybe_npm, &mut media_type, &mut is_cjs)?
          }
        }
      }
...
pub static IGNORED_DIAGNOSTIC_CODES: LazyLock<HashSet<u64>> = LazyLock::new(|| {
  [
    // TS1452: 'resolution-mode' assertions are only supported when `moduleResolution` is `node16`
    // or `nodenext`. We specify the resolution mode to be CommonJS for some npm files and this
    ...
    2834, 2835, // TS5009: Cannot find the common subdirectory path for the input files.
    5009,
