...
use deno_resolver::{
  deno_json::JsxImportSourceConfigResolver,
  npm::{ResolvePkgFolderFromDenoReqError, managed::ResolvePkgFolderFromDenoModuleError},
};
    ...
    Some(Module::Npm(module)) => {
      if let Some(npm) = maybe_npm {
        let package_folder = npm
          .npm_resolver
          .resolve_pkg_folder_from_deno_module_req(module.pkg_req_ref.req(), referrer)?;
        ...
        let maybe_url = match res_result {
          ...
          Err(err) => match err.code() {
            NodeJsErrorCode::ERR_MODULE_NOT_FOUND | NodeJsErrorCode::ERR_TYPES_NOT_FOUND => None,
...
pub fn resolve_specifier_for_tsc(
  ...
  if referrer.scheme() == "asset" && deno_path_util::is_relative_specifier(&specifier) {
  ...
  let maybe_result = match resolved_dep {
    ...
    _ => {
      let result =
        resolve_non_graph_specifier_types(&specifier, referrer, resolution_mode, maybe_npm);
