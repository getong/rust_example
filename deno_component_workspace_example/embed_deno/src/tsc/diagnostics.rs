...
use deno_graph::{ModuleGraph, ResolutionError};
...
impl Diagnostic {
  ...
  pub fn include_when_remote(&self) -> bool {
    /// TS6133: value is declared but its value is never read (noUnusedParameters and
    /// noUnusedLocals)
    const TS6133: u64 = 6133;
    /// TS4114: This member must have an 'override' modifier because it overrides a member in the
    /// base class 'X'.
  ...
  pub fn apply_fast_check_source_maps(&mut self, graph: &ModuleGraph) {
    fn visit_diagnostic(d: &mut Diagnostic, graph: &ModuleGraph) {
      if let Some(specifier) = d
        ...
        && let Some(fast_check_module) = module.js().and_then(|m| m.fast_check_module())
      {
        // todo(dsherret): use a short lived cache to prevent parsing
        // source maps so often
        if let Ok(source_map) = SourceMap::from_slice(fast_check_module.source_map.as_bytes())
          && let Some(start) = d.start.as_mut()
        {
          let maybe_token = source_map.lookup_token(start.line as u32, start.character as u32);
...
fn display_diagnostics(f: &mut fmt::Formatter, diagnostics: &Diagnostics) -> fmt::Result {
...
mod tests {
  ...
  fn test_diagnostics_related_info() {
    ...
    assert_eq!(
      strip_ansi_codes(&actual),
      "TS2552 [ERROR]: Cannot find name \'foo_Bar\'. Did you mean \
       \'foo_bar\'?\nfoo_Bar();\n~~~~~~~\n    at test.ts:8:1\n\n    \'foo_bar\' is declared \
       here.\n    function foo_bar() {\n             ~~~~~~~\n        at test.ts:4:10"
    );
  }
