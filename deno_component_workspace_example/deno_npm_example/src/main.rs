...
use deno_npm::{
  registry::{
    NpmPackageInfo, NpmPackageVersionInfo, NpmRegistryApi, NpmRegistryPackageInfoLoadError,
  },
  resolution::{AddPkgReqsOptions, NpmResolutionSnapshot},
};
...
async fn main() {
  let mut solver = MinimalReproductionSolver::new(&["@aws-cdk/aws-ecs"], |snapshot| {
    snapshot
      .all_packages_for_every_system()
      .any(|s| s.id.as_serialized().chars().filter(|c| *c == '_').count() > 200)
  })
  .await;
  ...
  pub async fn attempt_reduce_dependendencies(&mut self) -> bool {
    ...
    for package_nv in package_nvs {
      ...
      for dep_name in dep_names {
        ...
        let mut new_version_info = self.api.get_version_info(&package_nv).clone();
...
impl SubsetRegistryApi {
  ...
  pub fn set_package_version_info(&self, nv: &PackageNv, version_info: NpmPackageVersionInfo) {
...
// copy from https://github.com/denoland/deno_npm/blob/main/examples/min_repro_solver.rs
