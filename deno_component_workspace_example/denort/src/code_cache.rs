...
use deno_core::{anyhow::bail, error::AnyError, parking_lot::Mutex, unsync::sync::AtomicFlag};
...
impl DenoCompileCodeCache {
  pub fn new(file_path: PathBuf, cache_key: u64) -> Self {
    ...
    match deserialize(&file_path, cache_key) {
      Ok(data) => {
        ...
        Self {
          strategy: CodeCacheStrategy::SubsequentRun(SubsequentRunCodeCacheStrategy {
  ...
  pub fn enabled(&self) -> bool {
    match &self.strategy {
      CodeCacheStrategy::FirstRun(strategy) => !strategy.is_finished.is_raised(),
      CodeCacheStrategy::SubsequentRun(strategy) => !strategy.is_finished.is_raised(),
    }
  }
}
...
impl FirstRunCodeCacheStrategy {
  fn write_cache_data(&self, cache_data: &HashMap<CodeCacheKey, DenoCompileCodeCacheEntry>) {
    let count = cache_data.len();
    let temp_file = get_atomic_path(&sys_traits::impls::RealSys, &self.file_path);
...
fn serialize_with_writer<T: Write>(
  ...
  for ((specifier, _), entry) in cache {
    let len: u64 = entry.data.len() as u64 + specifier.len() as u64 + 1 + 4 + 8 + 8;
...
fn deserialize_with_reader<T: Read>(
  ...
  fn new_vec_sized<T: Clone>(capacity: usize, default_value: T) -> Result<Vec<T>, AnyError> {
    let mut vec = Vec::new();
  ...
  for i in 0 .. len {
    let pos = i * 8;
    lengths.push(u64::from_le_bytes(entry_len_bytes[pos .. pos + 8].try_into()?) as usize);
  }
  ...
  for len in lengths {
    ...
    let source_hash =
      u64::from_le_bytes(buffer[source_hash_start_pos .. entry_data_hash_start_pos].try_into()?);
    let specifier_end_pos = try_subtract(source_hash_start_pos, 4)?;
    let specifier_len =
      u32::from_le_bytes(buffer[specifier_end_pos .. source_hash_start_pos].try_into()?) as usize;
    let specifier_start_pos = try_subtract(specifier_end_pos, specifier_len)?;
    let specifier = String::from_utf8(buffer[specifier_start_pos .. specifier_end_pos].to_vec())?;
    ...
    let actual_entry_data_hash: u64 = FastInsecureHasher::new_without_deno_version()
      .write(&buffer)
      .finish();
