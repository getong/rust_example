...
use deno_core::{
  FastString, ModuleLoadOptions, ModuleLoadReferrer, ModuleLoader, ModuleSourceCode, ModuleType,
...
use deno_lib::{
  args::{CaData, RootCertStoreLoadError, get_root_cert_store, npm_pkg_req_ref_to_binary_command},
  loader::{
  ...
  standalone::binary::NodeModules,
  util::{
    hash::FastInsecureHasher,
    text_encoding::{from_utf8_lossy_cow, from_utf8_lossy_owned},
    v8::construct_v8_flags,
  },
  worker::{
...
use deno_resolver::{
  ...
  cjs::{CjsTracker, IsCjsResolutionMode},
  loader::NpmModuleLoader,
  npm::{
    ByonmNpmResolverCreateOptions, CreateInNpmPkgCheckerOptions, DenoInNpmPackageChecker,
    NpmReqResolver, NpmReqResolverOptions, NpmResolver, NpmResolverCreateOptions,
    managed::{
      ManagedInNpmPkgCheckerCreateOptions, ManagedNpmResolverCreateOptions, NpmResolutionCell,
    },
  },
  workspace::{MappedResolution, SloppyImportsOptions, WorkspaceResolver},
};
use deno_runtime::{
...
use node_resolver::{
  DenoIsBuiltInNodeModuleChecker, NodeResolutionKind, NodeResolver, PackageJsonResolver,
  PackageJsonThreadLocalCache, ResolutionMode,
  analyze::{CjsModuleExportAnalyzer, NodeCodeTranslator},
  cache::NodeResolutionSys,
  errors::PackageJsonLoadError,
};
use sys_traits::EnvCurrentDir;

use crate::{
  binary::{DenoCompileModuleSource, StandaloneData, StandaloneModules},
  code_cache::DenoCompileCodeCache,
  file_system::{DenoRtSys, FileBackedVfs},
  node::{
    CjsCodeAnalyzer, DenoRtCjsTracker, DenoRtNodeCodeTranslator, DenoRtNodeResolver,
    DenoRtNpmModuleLoader, DenoRtNpmReqResolver,
  },
};
...
impl SharedModuleLoaderState {
  fn get_code_cache(&self, specifier: &Url, source: &[u8]) -> Option<SourceCodeCacheInfo> {
...
impl ModuleLoader for EmbeddedModuleLoader {
  fn resolve(
    ...
    let referrer = if referrer == "." {
      ...
      let current_dir = self.sys.env_current_dir().map_err(JsErrorBox::from_err)?;
    ...
    match mapped_resolution {
      Ok(MappedResolution::WorkspaceJsrPackage { specifier, .. }) => Ok(specifier),
  ...
  fn load(
    ...
    if original_specifier.scheme() == "data" {
      let data_url_text = match deno_media_type::data_url::RawDataUrl::parse(original_specifier)
        ...
        async move {
          let code_source = shared
            ...
            .load(
              ...
              &as_deno_resolver_requested_module_type(&options.requested_module_type),
            )
            .await
            .map_err(JsErrorBox::from_err)?;
          let code_cache_entry = match options.requested_module_type {
            RequestedModuleType::None => {
              shared.get_code_cache(&code_source.specifier, code_source.source.as_bytes())
            }
    ...
    match self.shared.modules.read(original_specifier) {
      Ok(Some(module)) => {
        match options.requested_module_type {
          ...
          RequestedModuleType::Other(_) | RequestedModuleType::None | RequestedModuleType::Json => {
            // ignore
          }
        }

        let media_type = module.media_type;
        let (module_specifier, module_type, module_source) = module.into_parts();
          ...
          deno_core::ModuleLoadResponse::Async(
            async move {
              let source = match module_source {
                DenoCompileModuleSource::String(string) => Cow::Borrowed(string),
                DenoCompileModuleSource::Bytes(module_code_bytes) => match module_code_bytes {
              ...
              let module_source = match source {
                ...
                Cow::Borrowed(source) => ModuleSourceCode::String(FastString::from_static(source)),
              };
              let code_cache_entry =
                shared.get_code_cache(&module_specifier, module_source.as_bytes());
              Ok(deno_core::ModuleSource::new_with_redirect(
                module_type,
                module_source,
                &original_specifier,
      ...
      Ok(None) => deno_core::ModuleLoadResponse::Sync(Err(JsErrorBox::type_error(format!(
        "Module not found: {}",
        original_specifier
      )))),
      Err(err) => {
        deno_core::ModuleLoadResponse::Sync(Err(JsErrorBox::type_error(format!("{:?}", err))))
      }
    }
  }
...
impl NodeRequireLoader for EmbeddedModuleLoader {
  ...
  fn load_text_file_lossy(&self, path: &std::path::Path) -> Result<FastString, JsErrorBox> {
    ...
    let file_bytes = self
      .shared
      .vfs
      .read_file_offset_with_len(file_entry.transpiled_offset.unwrap_or(file_entry.offset))
      .map_err(JsErrorBox::from_err)?;
...
pub async fn run(
    ...
    let pkg_jsons = metadata
      ...
      .map(|(relative_path, json)| {
        let path = root_dir_url
          ...
          .to_file_path()
          .unwrap();
        let pkg_json = deno_package_json::PackageJson::load_from_value(path, json)?;
        Ok(Arc::new(pkg_json))
      })
    ...
    WorkspaceResolver::new_raw(
      ...
      metadata
        ...
        .map(|pkg| ResolverWorkspaceJsrPackage {
          is_link: false, /* only used for enhancing the diagnostic, which isn't shown in deno
                           * compile */
  ...
  let permissions = {
    ...
    match &mut permissions.allow_read {
      ...
      None => {
        permissions.allow_read = Some(vec![root_path.to_string_lossy().into_owned()]);
      }
    }

    let desc_parser = Arc::new(RuntimePermissionDescriptorParser::new(sys.clone()));
    let permissions = Permissions::from_options(desc_parser.as_ref(), &permissions)?;
  ...
  let main_module = match NpmPackageReqReference::from_specifier(&main_module) {
    Ok(package_ref) => {
      ...
      worker_factory.resolve_npm_binary_entrypoint(&pkg_folder, package_ref.sub_path())?
    }
    Err(_) => main_module,
  };
