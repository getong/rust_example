...
use deno_lib::standalone::virtual_fs::{
...
use sys_traits::{
  FsCopy,
  boxed::{BoxedFsDirEntry, BoxedFsMetadataValue, FsMetadataBoxed, FsReadDirBoxed},
};
use url::Url;
...
impl FileSystem for DenoRtSys {
  ...
  async fn copy_file_async(
    ...
    if self.is_vfs_path(&oldpath) {
      ...
      tokio::task::spawn_blocking(move || {
        fs.copy_to_real_path(&oldpath.as_checked_path(), &newpath.as_checked_path())
          .map(|_| ())
          .map_err(FsError::Io)
      })
      .await?
  ...
  async fn cp_async(&self, from: CheckedPathBuf, to: CheckedPathBuf) -> FsResult<()> {
    self.error_if_in_vfs(&to)?;
...
impl sys_traits::BaseFsCloneFile for DenoRtSys {
  fn base_fs_clone_file(&self, _from: &Path, _to: &Path) -> std::io::Result<()> {
...
impl sys_traits::FsFileSetTimes for FsFileAdapter {
  fn fs_file_set_times(&mut self, times: sys_traits::FsFileTimes) -> std::io::Result<()> {
    match self {
...
impl sys_traits::FsFileLock for FsFileAdapter {
  fn fs_file_lock(&mut self, mode: sys_traits::FsFileLockMode) -> std::io::Result<()> {
    match self {
  ...
  fn fs_file_try_lock(&mut self, mode: sys_traits::FsFileLockMode) -> std::io::Result<()> {
    match self {
    ...
    for component in relative_path.components() {
      ...
      let current_dir = match current_entry {
        ...
        VfsEntryRef::Symlink(symlink) => {
          ...
          let (resolved_path, entry) = self.find_entry_inner(&dest, seen, case_sensitivity)?;
...
impl FileBackedVfs {
  ...
  pub fn open_file(self: &Arc<Self>, path: &Path) -> std::io::Result<FileBackedVfsFile> {
  ...
  pub fn read_link(&self, path: &Path) -> std::io::Result<PathBuf> {
    ...
    match entry {
      VfsEntryRef::Symlink(symlink) => Ok(symlink.resolve_dest_from_root(&self.fs_root.root_path)),
      VfsEntryRef::Dir(_) | VfsEntryRef::File(_) => Err(std::io::Error::other("not a symlink")),
    }
  }
  ...
  pub fn read_file_all(&self, file: &VirtualFile) -> std::io::Result<Cow<'static, [u8]>> {
  ...
  pub fn read_file_offset_with_len(
    ...
    let read_range = self.get_read_range(offset_with_len, 0, offset_with_len.len)?;
    match &self.vfs_data {
  ...
  pub fn read_file(&self, file: &VirtualFile, pos: u64, buf: &mut [u8]) -> std::io::Result<usize> {
  ...
  fn get_read_range(
    ...
    let file_offset = self.fs_root.start_file_offset + file_offset_and_len.offset;
...
mod test {
  ...
  use test_util::{TempDir, assert_contains};
  ...
  fn builds_and_uses_virtual_fs() {
    ...
    builder
      .add_file_with_data_raw_for_testing(&src_path.join("a.txt"), "data".into(), None)
      .unwrap();
    builder
    ...
    builder
      .add_file_with_data_raw_for_testing(&src_path.join("c.txt"), "c".into(), None)
      .unwrap();
    builder
      .add_file_with_data_raw_for_testing(&src_path.join("sub_dir").join("d.txt"), "d".into(), None)
      .unwrap();
  ...
  fn into_virtual_fs(builder: VfsBuilder, temp_dir: &TempDir) -> (PathBuf, FileBackedVfs) {
