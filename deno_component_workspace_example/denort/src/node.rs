...
use deno_resolver::{
  ...
  npm::{DenoInNpmPackageChecker, NpmReqResolver},
};
use deno_runtime::{deno_fs::FileSystem, deno_permissions::CheckedPath};
use node_resolver::{
  DenoIsBuiltInNodeModuleChecker,
  analyze::{CjsAnalysis, CjsAnalysisExports, EsmAnalysisMode, NodeCodeTranslator},
};

use crate::{binary::StandaloneModules, file_system::DenoRtSys};

pub type DenoRtCjsTracker = deno_resolver::cjs::CjsTracker<DenoInNpmPackageChecker, DenoRtSys>;
...
pub type DenoRtNodeCodeTranslator = NodeCodeTranslator<
  ...
  DenoRtSys,
>;
pub type DenoRtNodeResolver =
  deno_runtime::deno_node::NodeResolver<DenoInNpmPackageChecker, DenoRtNpmResolver, DenoRtSys>;
...
impl CjsCodeAnalyzer {
  ...
  fn inner_cjs_analysis<'a>(
    ...
    let analysis = if is_maybe_cjs {
      ...
      match data {
        Some(data) => {
          let data: CjsExportAnalysisEntry =
            bincode::deserialize(&data).map_err(|err| JsErrorBox::generic(err.to_string()))?;
          match data {
        ...
        None => {
          if log::log_enabled!(log::Level::Debug) {
            if self.sys.is_specifier_in_vfs(specifier) {
              log::debug!(
                "No CJS export analysis was stored for '{}'. Assuming ESM. This might indicate a \
                 bug in Deno.",
                specifier
              );
            } else {
              log::debug!(
                "Analyzing potentially CommonJS files is not supported at runtime in a compiled \
                 executable ({}). Assuming ESM.",
                specifier
              );
            }
          }
    ...
    let source = match source {
      ...
      None => {
        if let Ok(path) = deno_path_util::url_to_file_path(specifier) {
          ...
          if let Ok(source_from_file) = self.sys.read_text_file_lossy_async(path.into_owned()).await
          {
            source_from_file
          } else {
            return Ok(CjsAnalysis::Cjs(CjsAnalysisExports {
