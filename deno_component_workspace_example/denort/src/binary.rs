...
use deno_lib::standalone::{
  ...
  virtual_fs::{VirtualDirectory, VirtualDirectoryEntries},
};
...
pub fn deserialize_binary_data_section(
  ...
  // 1. Metadata
  let (input, data) = read_bytes_with_u64_len(input).context("reading metadata")?;
  let metadata: Metadata = serde_json::from_slice(data).context("deserializing metadata")?;
  // 2. Npm snapshot
