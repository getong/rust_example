...
use deno_core::{
  FastString, ModuleCodeBytes, ModuleSourceCode, ModuleType,
  anyhow::{Context, bail},
  error::AnyError,
  serde_json,
  url::Url,
...
use deno_lib::standalone::{
  binary::{
    DenoRtDeserializable, MAGIC_BYTES, Metadata, RemoteModuleEntry, SpecifierDataStore, SpecifierId,
  },
  virtual_fs::{VfsEntry, VirtualDirectory, VirtualDirectoryEntries},
};
...
use deno_semver::{StackString, package::PackageReq};
use indexmap::IndexMap;
use sys_traits::{FsCanonicalize, FsRead};
use thiserror::Error;

use crate::file_system::{FileBackedVfs, VfsRoot};
...
pub fn extract_standalone(cli_args: Cow<[OsString]>) -> Result<StandaloneData, AnyError> {
...
pub fn extract_vfs_to_disk(vfs: &FileBackedVfs, extraction_dir: &Path) -> Result<(), AnyError> {
  ...
  std::fs::File::create(&done_marker).context("Failed to write extraction done marker")?;
...
fn choose_and_create_extraction_dir(hash_str: &str) -> Result<PathBuf, AnyError> {
  let current_exe =
  ...
  std::fs::create_dir_all(&dir)
    .with_context(|| format!("Failed to create extraction directory: {}", dir.display()))?;
  Ok(dir)
}
    ...
    match entry {
      VfsEntry::Dir(sub_dir) => {
        ...
        std::fs::create_dir(&*disk_path)
          ...
          .with_context(|| format!("Failed to create directory: {}", disk_path.display()))?;
      ...
      VfsEntry::File(file) => {
        ...
        #[cfg(unix)]
        {
          ...
          std::fs::OpenOptions::new()
            ...
            .with_context(|| format!("Failed to write file: {}", disk_path.display()))?;
        }
        #[cfg(not(unix))]
        {
...
fn create_symlink(target: &Path, link_path: &Path, dest_is_dir: bool) -> Result<(), AnyError> {
  #[cfg(unix)]
  {
  ...
  #[cfg(windows)]
  {
    ...
    create_link(target, link_path)
      .or_else(|_| {
        // may be left over from an interrupted extraction
        let _ = std::fs::remove_file(link_path).or_else(|_| std::fs::remove_dir(link_path));
        create_link(target, link_path)
      })
      .or_else(|_| {
...
fn find_section() -> Result<&'static [u8], AnyError> {
  ...
  match libsui::find_section("d3n0l4nd").context("Failed reading standalone binary section.") {
...
fn read_from_file_fallback() -> Result<&'static [u8], AnyError> {
  ...
  const RESOURCE_SECTION_HEADER_NAME: &[u8] = &[
    0x44, 0x00, 0x33, 0x00, 0x4E, 0x00, 0x30, 0x00, 0x4C, 0x00, 0x34, 0x00, 0x4E, 0x00, 0x44, 0x00,
  ];
  ...
  let Some(marker_pos) = find_in_bytes(mmap, RESOURCE_SECTION_HEADER_NAME) else {
    bail!("Failed to find first section name.");
  };
  let next_bytes = &mmap[marker_pos + RESOURCE_SECTION_HEADER_NAME.len() ..];
  let Some(marker_pos) = find_in_bytes(next_bytes, RESOURCE_SECTION_HEADER_NAME) else {
    bail!("Failed to find second section name.");
  };
  let next_bytes = &next_bytes[marker_pos + RESOURCE_SECTION_HEADER_NAME.len() ..];
...
fn read_section_metadata(data: &'static [u8]) -> Result<(Metadata, &'static [u8]), AnyError> {
  let (input, found) = read_magic_bytes(data)?;
  if !found {
    bail!("Did not find magic bytes.");
  }
  let (input, metadata_bytes) = read_bytes_with_u64_len(input).context("reading metadata")?;
...
fn deserialize_binary_data_section(
  ...
  // 2. Npm snapshot
  let (input, data) = read_bytes_with_u64_len(input).context("reading npm snapshot")?;
  ...
  let (input, specifiers_store) =
    SpecifierStore::deserialize(root_dir_url, input).context("deserializing specifiers")?;
  // 4. Redirects
  let (input, redirects_store) =
    SpecifierDataStore::<SpecifierId>::deserialize(input).context("deserializing redirects")?;
  // 5. Remote modules
  ...
  let (input, vfs_files_data) = read_bytes_with_u64_len(input).context("reading vfs files data")?;
  ...
  let modules_store =
    RemoteModulesStore::new(specifiers_store, redirects_store, remote_modules_store);
...
impl SpecifierStore {
  pub fn deserialize<'a>(root_dir_url: &Url, input: &'a [u8]) -> std::io::Result<(&'a [u8], Self)> {
...
impl StandaloneModules {
  ...
  pub fn path_in_root(&self, path: &Path) -> bool {
    deno_path_util::normalize_path(Cow::Borrowed(path)).starts_with(self.vfs.root())
  }
  ...
  pub fn read<'a>(
    ...
    if specifier.scheme() == "file" {
      let path = deno_path_util::url_to_file_path(specifier).map_err(JsErrorBox::from_err)?;
...
impl<'a> DenoCompileModuleData<'a> {
  pub fn into_parts(self) -> (&'a Url, ModuleType, DenoCompileModuleSource) {
    ...
    let (media_type, source) = match self.media_type {
      ...
      MediaType::Wasm => (ModuleType::Wasm, DenoCompileModuleSource::Bytes(data)),
      ...
      | MediaType::SourceMap
      | MediaType::Sql
      | MediaType::Unknown => (ModuleType::JavaScript, DenoCompileModuleSource::Bytes(data)),
    };
      ...
      match self.redirects.get(specifier) {
        ...
        None => {
          ...
          return Ok(Some(DenoCompileModuleData {
            ...
            cjs_export_analysis: entry.maybe_cjs_export_analysis.as_ref().map(handle_cow_ref),
          }));
        }
      }
    }
...
fn deserialize_npm_snapshot(
  ...
  let (input, data_ids_to_npm_ids) =
    parse_vec_n_times(input, packages_len, parse_id).context("deserializing id")?;
  ...
  let (input, packages) = parse_vec_n_times_with_index(input, packages_len, |input, index| {
