...
use deno_core::{ModuleSpecifier, OpState, op2, v8};
use deno_error::JsErrorBox;
use deno_runtime::deno_permissions::{ChildPermissionsArg, PermissionsContainer};
use uuid::Uuid;

use crate::tools::test::{
  TestContainer, TestDescription, TestEvent, TestEventSender, TestFailure, TestLocation,
  TestStepDescription, TestStepResult,
};
...
#[op2(fast)]
fn op_test_event_step_result_ok(state: &mut OpState, #[smi] id: usize, #[smi] duration: u64) {
  let sender = state.borrow_mut::<TestEventSender>();
  sender
...
#[op2(fast)]
fn op_test_event_step_result_ignored(state: &mut OpState, #[smi] id: usize, #[smi] duration: u64) {
  let sender = state.borrow_mut::<TestEventSender>();
  sender
