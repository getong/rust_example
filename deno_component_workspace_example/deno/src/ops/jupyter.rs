...
use deno_core::{OpState, error::AnyError, op2, parking_lot::Mutex, serde_json};
use deno_error::JsErrorBox;
use jupyter_runtime::{
  InputRequest, JupyterMessage, JupyterMessageContent, KernelIoPubConnection, StreamContent,
};
use tokio::sync::mpsc;
...
pub fn op_jupyter_input(
  ...
  if let Some(last_request) = maybe_last_request {
    let JupyterMessageContent::ExecuteRequest(msg) = &last_request.content else {
      return None;
    };
    ...
    let join_handle = std::thread::spawn(move || stdin_connection_proxy.lock().rx.blocking_recv());
...
pub async fn op_jupyter_broadcast(
  ...
  let maybe_last_request = last_execution_request.lock().clone();
  if let Some(last_request) = maybe_last_request {
    let content = JupyterMessageContent::from_type_and_content(&message_type, content.clone())
      .map_err(|err| {
        log::error!(
...
pub fn op_jupyter_create_png_from_texture(
  ...
  let (command_encoder, maybe_err) = texture.instance.device_create_command_encoder(
    texture.device_id,
  ...
  let data = {
    ...
    let slice = unsafe { std::slice::from_raw_parts(slice_pointer.as_ptr(), range_size as usize) };

    let mut unpadded = Vec::with_capacity((unpadded_bytes_per_row * texture.size.height) as _);

    for i in 0 .. texture.size.height {
      unpadded.extend_from_slice(
        &slice
          [((i * padded_bytes_per_row) as usize) .. (((i + 1) * padded_bytes_per_row) as usize)]
