...
use deno_core::{anyhow::Context, error::AnyError, futures::future::LocalBoxFuture};
use deno_semver::package::PackageNv;
use deno_task_shell::{
  ExecutableCommand, ExecuteResult, KillSignal, ShellCommand, ShellCommandContext, ShellPipeReader,
  ShellPipeWriter,
};
use lazy_regex::Lazy;
use regex::Regex;
...
pub async fn run_task(mut opts: RunTaskOptions<'_>) -> Result<TaskResult, AnyError> {
  let script = get_script_with_args(opts.script, opts.argv);
  let seq_list = deno_task_shell::parser::parse(&script)
    .with_context(|| format!("Error parsing script '{}'.", opts.task_name))?;
  let env_vars = prepare_env_vars(opts.env_vars, opts.init_cwd, opts.root_node_modules_dir);
  ...
  let state =
    deno_task_shell::ShellState::new(env_vars, opts.cwd, opts.custom_commands, opts.kill_signal);
  let stdio = opts.stdio.unwrap_or_default();
  let (TaskStdio(stdout_read, stdout_write), TaskStdio(stderr_read, stderr_write)) =
...
fn prepare_env_vars(
  ...
  if !env_vars.contains_key(OsStr::new(crate::npm::NPM_CONFIG_USER_AGENT_ENV_VAR)) {
    env_vars.insert(
...
pub(crate) static USE_PKG_JSON_HIDDEN_ENV_VAR_NAME: &str = "DENO_INTERNAL_TASK_USE_PKG_JSON";
...
impl ShellCommand for NpmCommand {
  fn execute(&self, mut context: ShellCommandContext) -> LocalBoxFuture<'static, ExecuteResult> {
      ...
      && !context.args.iter().any(|s| s.to_string_lossy().starts_with('-'))
    {
      ...
      return ExecutableCommand::new("deno".to_string(), std::env::current_exe().unwrap()).execute(
        ShellCommandContext {
          args,
          state,
          ..context
...
impl ShellCommand for NodeCommand {
  fn execute(&self, context: ShellCommandContext) -> LocalBoxFuture<'static, ExecuteResult> {
    ...
    {
      return ExecutableCommand::new("node".to_string(), PathBuf::from("node")).execute(context);
    }
    ...
    ExecutableCommand::new("deno".to_string(), std::env::current_exe().unwrap()).execute(
      ShellCommandContext {
        args,
        state,
        ..context
...
impl ShellCommand for NodeGypCommand {
  fn execute(&self, context: ShellCommandContext) -> LocalBoxFuture<'static, ExecuteResult> {
    ...
    {
      log::warn!(
        "{} node-gyp was used in a script, but was not listed as a dependency. Either add it as a \
         dependency or install it globally (e.g. `npm install -g node-gyp`)",
        crate::colors::yellow("Warning")
      );
    }
    ExecutableCommand::new("node-gyp".to_string(), "node-gyp".to_string().into()).execute(context)
  }
}
    ...
    if let Some(first_arg) = context.args.first().cloned() {
      match context.state.resolve_custom_command(&first_arg) {
        ...
        _ => {
          ...
          let npx_path = match context.state.resolve_command_path(OsStr::new("npx")) {
            Ok(npx) => npx,
            Err(err) => {
              let _ = context.stderr.write_line(&format!("{}", err));
              return Box::pin(std::future::ready(ExecuteResult::from_exit_code(
...
impl ShellCommand for NpmPackageBinCommand {
  fn execute(&self, context: ShellCommandContext) -> LocalBoxFuture<'static, ExecuteResult> {
    ...
    let executable_command =
      deno_task_shell::ExecutableCommand::new("deno".to_string(), std::env::current_exe().unwrap());
...
fn resolve_bin_dir_entry_command(entry: std::fs::DirEntry) -> Option<NodeModulesFileRunCommand> {
...
fn resolve_execution_path_from_npx_shim(file_path: PathBuf, text: &str) -> Option<PathBuf> {
  static SCRIPT_PATH_RE: Lazy<Regex> = lazy_regex::lazy_regex!(r#""\$basedir\/([^"]+)" "\$@""#);
  ...
  if let Some(first_line) = maybe_first_line {
    ...
    // struct.
    if first_line == "#!/usr/bin/env node" || first_line == "#!/usr/bin/env -S node" {
  ...
  SCRIPT_PATH_RE
    .captures(text)
    .and_then(|c| c.get(1))
    .map(|relative_path| file_path.parent().unwrap().join(relative_path.as_str()))
}
...
fn resolve_managed_npm_commands(
  ...
  for id in npm_resolver.resolution().top_level_packages() {
    ...
    let bin_commands = node_resolver.resolve_binary_commands(&package_folder)?;
