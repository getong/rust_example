...
use deno_graph::packages::{JsrPackageInfo, JsrPackageInfoVersion, JsrPackageVersionInfo};
use deno_semver::{
  Version,
  ...
  I: IntoIterator<Item = (&'a Version, &'a JsrPackageInfoVersion)>,
{
  ...
  versions
    .into_iter()
    .rev()
    .find(|(v, i)| !i.yanked && req.version_req.tag().is_none() && req.version_req.matches(v))
    .map(|(v, _)| v.clone())
}
...
impl JsrFetchResolver {
  ...
  pub async fn force_refresh_package_info(&self, name: &str) -> Option<Arc<JsrPackageInfo>> {
    ...
    let file = file_fetcher
      .fetch_with_options(
        ...
        deno_resolver::file_fetcher::FetchOptions {
          maybe_cache_setting: Some(&deno_cache_dir::file_fetcher::CacheSetting::ReloadAll),
          ..Default::default()
        },
      )
      .await
