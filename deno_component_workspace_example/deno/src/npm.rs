...
use dashmap::DashMap;
use deno_core::{error::AnyError, serde_json, url::Url};
use deno_error::JsErrorBox;
use deno_lib::version::DENO_VERSION_INFO;
use deno_npm::{
  NpmResolutionPackage,
  npm_rc::ResolvedNpmRc,
  registry::{NpmPackageInfo, NpmPackageVersionInfosIterator},
  resolution::{NpmResolutionSnapshot, NpmVersionResolver},
};
use deno_npm_cache::{NpmCacheHttpClientBytesResponse, NpmCacheHttpClientResponse};
use deno_npm_installer::{
  BinEntries, CachedNpmPackageExtraInfoProvider, ExpectedExtraInfo,
  lifecycle_scripts::{
    LIFECYCLE_SCRIPTS_RUNNING_ENV_VAR, LifecycleScriptsExecutor, LifecycleScriptsExecutorOptions,
    PackageWithScript, is_broken_default_install_script,
  },
};
...
use crate::{
  ...
  sys::CliSys,
  task_runner::TaskStdio,
  util::progress_bar::{ProgressBar, ProgressMessagePrompt},
};

pub type CliNpmInstallerFactory =
  deno_npm_installer::NpmInstallerFactory<CliNpmCacheHttpClient, ProgressBar, CliSys>;
pub type CliNpmInstaller = deno_npm_installer::NpmInstaller<CliNpmCacheHttpClient, CliSys>;
...
pub type CliNpmGraphResolver =
  deno_npm_installer::graph::NpmDenoGraphResolver<CliNpmCacheHttpClient, CliSys>;
...
impl CliNpmCacheHttpClient {
  pub fn new(http_client_provider: Arc<HttpClientProvider>, progress_bar: ProgressBar) -> Self {
    Self {
  ...
  async fn download_with_retries_on_any_tokio_runtime(
    ...
    client
      ...
      .map(|response| match response {
        ...
        crate::http_util::HttpClientResponse::NotFound => NpmCacheHttpClientResponse::NotFound,
      ...
      .map_err(|err| {
        ...
        let status_code = match err.as_kind() {
          ...
          | NotFound
          | Other(_) => None,
          BadResponse(bad_response_error) => Some(bad_response_error.status_code.as_u16()),
        };
...
impl NpmFetchResolver {
  ...
  pub async fn req_to_nv(&self, req: &PackageReq) -> Result<Option<PackageNv>, AnyError> {
    ...
    let maybe_get_nv = || async {
      ...
      let version_info = self.version_resolver.resolve_best_package_version_info(
        &req.version_req,
        &package_info,
        Vec::new().into_iter(),
      )?;
...
pub enum DenoTaskLifecycleScriptsError {
  ...
  #[class(inherit)]
  #[error("failed to create npm process state tempfile for running lifecycle scripts")]
...
impl LifecycleScriptsExecutor for DenoTaskLifeCycleScriptsExecutor {
  async fn execute(&self, options: LifecycleScriptsExecutorOptions<'_>) -> Result<(), AnyError> {
    ...
    let temp_file_fd =
      deno_runtime::deno_process::npm_process_state_tempfile(options.process_state.as_bytes())
    ...
    {
      // add custom commands for binaries from the package's dependencies. this will take precedence
      // over the baseline commands, so if the package relies on a bin that conflicts with one
      // higher in the dependency tree, the correct bin will be used.
...
impl DenoTaskLifeCycleScriptsExecutor {
  pub fn new(npm_resolver: ManagedNpmResolverRc<CliSys>, progress_bar: ProgressBar) -> Self {
    Self {
      npm_resolver,
      progress_bar,
    }
  ...
  async fn resolve_baseline_custom_commands<'a>(
    ...
    custom_commands.insert("npx".to_string(), Rc::new(crate::task_runner::NpxCommand));

    custom_commands.insert("npm".to_string(), Rc::new(crate::task_runner::NpmCommand));

    custom_commands.insert("node".to_string(), Rc::new(crate::task_runner::NodeCommand));
