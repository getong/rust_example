...
use deno_npm::{
  NpmResolutionPackage, npm_rc::ResolvedNpmRc, registry::NpmPackageInfo,
  resolution::NpmResolutionSnapshot,
};
...
impl NpmFetchResolver {
  pub fn new(file_fetcher: Arc<CliFileFetcher>, npmrc: Arc<ResolvedNpmRc>) -> Self {
    Self {
  ...
  pub async fn req_to_nv(&self, req: &PackageReq) -> Option<PackageNv> {
    ...
    let maybe_get_nv = || async {
      ...
      let version = version_from_package_info(&package_info, &req.version_req)?.clone();
