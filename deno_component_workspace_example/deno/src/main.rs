...
use crate::{
  args::{
    CompletionsFlags, DenoSubcommand, Flags, flags_from_vec_with_initial_cwd, get_default_v8_flags,
  },
  util::{
    display,
...
fn exit_for_error(error: AnyError, initial_cwd: Option<&std::path::Path>) -> ! {
  let error_string = match js_error_downcast_ref(&error) {
    Some(e) => {
      let initial_cwd =
        initial_cwd.and_then(|cwd| deno_path_util::url_from_directory_path(cwd).ok());
...
pub fn main() {
  ...
  let (result, initial_cwd) = create_and_run_current_thread_with_maybe_metrics(future);
...
async fn resolve_flags_and_init(
  ...
  let mut flags = match flags_from_vec_with_initial_cwd(args, initial_cwd.clone()) {
    Ok(flags) => flags,
...
fn wait_for_start(
  ...
  impl Future<
    Output = Result<Option<(UnconfiguredRuntime, Vec<std::ffi::OsString>, PathBuf)>, AnyError>,
  > + use<>,
> {
...
async fn initialize_tunnel(flags: &Flags) -> Result<(), deno_core::anyhow::Error> {
  ...
  let (token, org, app) =
    if let (Some(token), Some(org), Some(app)) = (&env_token, env_org, env_app) {
      (token.clone(), org, app)
