...
pub use deno_config::{
  deno_json::{
    BenchConfig, CompilerOptions, ConfigFile, FmtOptionsConfig, LintRulesConfig, ProseWrap,
  },
  glob::FilePatterns,
  workspace::TsTypeLib,
};
use deno_config::{
  deno_json::{
    FmtConfig, NodeModulesDirMode, PermissionConfigValue, PermissionsObjectWithBase, TestConfig,
  },
  workspace::{
    Workspace, WorkspaceDirLintConfig, WorkspaceDirectory, WorkspaceDirectoryRc,
    WorkspaceLintConfig,
  },
};
use deno_core::{anyhow::bail, error::AnyError, url::Url};
use deno_graph::GraphKind;
use deno_lib::{
  args::{CaData, has_flag_env_var, npm_pkg_req_ref_to_binary_command, npm_process_state},
  version::DENO_VERSION_INFO,
  worker::StorageKeyResolver,
};
use deno_npm::NpmSystemInfo;
use deno_npm_installer::{LifecycleScriptsConfig, graph::NpmCachingStrategy};
use deno_path_util::resolve_url_or_path;
use deno_resolver::factory::resolve_jsr_url;
use deno_runtime::{
  deno_node::ops::ipc::ChildIpcSerialization,
  deno_permissions::{AllowRunDescriptor, PathDescriptor, PermissionsOptions},
};
use deno_semver::{StackString, npm::NpmPackageReqReference};
...
fn resolve_fmt_options(fmt_flags: &FmtFlags, mut options: FmtOptionsConfig) -> FmtOptionsConfig {
...
impl WorkspaceMainModuleResolver {
  fn resolve_main_module(&self, specifier: &str, cwd: &Url) -> Result<Url, AnyError> {
    ...
    let url = match resolution {
      deno_resolver::workspace::MappedResolution::Normal { specifier, .. } => specifier,
      deno_resolver::workspace::MappedResolution::WorkspaceJsrPackage { specifier, .. } => {
        specifier
      }
      ...
      deno_resolver::workspace::MappedResolution::PackageJsonImport { pkg_json } => self
        .node_resolver
...
impl CliOptions {
  ...
  pub fn node_ipc_init(&self) -> Result<Option<(i64, ChildIpcSerialization)>, AnyError> {
    let maybe_node_channel_fd = std::env::var("NODE_CHANNEL_FD").ok();
    let maybe_node_channel_serialization =
      if let Ok(serialization) = std::env::var("NODE_CHANNEL_SERIALIZATION_MODE") {
  ...
  fn resolve_main_module_with_resolver_if_bare(
    ...
    match resolver {
      Some(resolver)
        if !raw_specifier.starts_with('.') && !Path::new(raw_specifier).is_absolute() =>
      {
      ...
      .get_or_init(|| {
        Ok(match &self.flags.subcommand {
          ...
          DenoSubcommand::Repl(_) => {
            deno_path_util::resolve_path("./$deno$repl.mts", self.initial_cwd())?
          }
          DenoSubcommand::Run(run_flags) => {
            if run_flags.is_stdin() {
              ...
              let default_resolve = || {
                let url = resolve_url_or_path(&run_flags.script, self.initial_cwd())?;
                if self.is_node_main()
                  && url.scheme() == "file"
                  && MediaType::from_specifier(&url) == MediaType::Unknown
                {
                  Ok::<_, AnyError>(
                    try_resolve_node_binary_main_entrypoint(&run_flags.script, self.initial_cwd())?
                      .unwrap_or(url),
                  )
                } else {
                  Ok(url)
          ...
          DenoSubcommand::Serve(run_flags) => {
            self.resolve_main_module_with_resolver_if_bare(&run_flags.script, resolver, || {
              resolve_url_or_path(&run_flags.script, self.initial_cwd()).map_err(|e| e.into())
            })?
          }
          _ => {
            bail!("No main module.")
  ...
  pub fn resolve_fmt_options_for_members(
    ...
    let cli_arg_patterns = fmt_flags.files.as_file_patterns(self.initial_cwd())?;
  ...
  pub fn resolve_lint_options_for_members(
    ...
    let cli_arg_patterns = lint_flags.files.as_file_patterns(self.initial_cwd())?;
    let member_configs = self
      .workspace()
  ...
  pub fn resolve_workspace_test_options(&self, test_flags: &TestFlags) -> WorkspaceTestOptions {
  ...
  pub fn resolve_test_options_for_members(
    ...
    let cli_arg_patterns = test_flags.files.as_file_patterns(self.initial_cwd())?;
  ...
  pub fn resolve_workspace_bench_options(&self, bench_flags: &BenchFlags) -> WorkspaceBenchOptions {
  ...
  pub fn resolve_bench_options_for_members(
    ...
    let cli_arg_patterns = bench_flags.files.as_file_patterns(self.initial_cwd())?;
    let workspace_dir_configs = self
      .workspace()
  ...
  pub fn permissions_options_for_dir(
    ...
    if let DenoSubcommand::Serve(serve_flags) = &self.flags.subcommand {
      augment_permissions_with_serve_flags(&mut permissions_options, serve_flags)?;
    }
    ...
    let config_permissions = if let Some(name) = &self.flags.permission_set {
      ...
      if !self.flags.has_permission() {
        ...
        if let Some((name, config_file_url)) = set_config_permission_name {
          ...
          bail!(
            "{} permissions were found in the config file. Did you mean to run with `-P` or a \
             permission flag?\n    at {}",
            name,
            config_file_url
          );
  ...
  fn implicit_allow_import(&self) -> Vec<String> {
    ...
    if self.cache_setting() == CacheSetting::Only {
      ...
      let mut imports =
        Vec::with_capacity(builtin_allowed_import_hosts.len() + cli_arg_urls.len() + 1);
  ...
  fn get_cli_arg_urls(&self) -> Vec<Cow<'_, Url>> {
    ...
    self
      ...
      .or_else(|| match &self.flags.subcommand {
        DenoSubcommand::Cache(cache_flags) => Some(files_to_urls(&cache_flags.files)),
        DenoSubcommand::Check(check_flags) => Some(files_to_urls(&check_flags.files)),
  ...
  pub fn unstable_detect_cjs(&self) -> bool {
    self.flags.unstable_config.detect_cjs || self.workspace().has_unstable("detect-cjs")
  }
  ...
  pub fn unstable_raw_imports(&self) -> bool {
    self.flags.unstable_config.raw_imports || self.workspace().has_unstable("raw-imports")
  }
  ...
  pub fn unstable_sloppy_imports(&self) -> bool {
    self.flags.unstable_config.sloppy_imports || self.workspace().has_unstable("sloppy-imports")
  }
  ...
  pub fn lifecycle_scripts_config(&self) -> LifecycleScriptsConfig {
    LifecycleScriptsConfig {
      ...
      explicit_install: matches!(
        self.sub_command(),
        DenoSubcommand::Install(_) | DenoSubcommand::Cache(_) | DenoSubcommand::Add(_)
      ),
    }
  }

  pub fn unstable_npm_lazy_caching(&self) -> bool {
    self.flags.unstable_config.npm_lazy_caching || self.workspace().has_unstable("npm-lazy-caching")
  }
...
fn resolve_import_map_specifier(
  ...
  if let Some(import_map_path) = maybe_import_map_path {
    ...
    {
      log::warn!(
        "{} the configuration file \"{}\" contains an entry for \"importMap\" that is being \
         ignored.",
        colors::yellow("Warning"),
        config_file.specifier,
      );
    }
    let specifier = deno_path_util::resolve_url_or_path(import_map_path, current_dir)
...
// DO NOT make this public. People should use
// `cli_options.permissions_options/permissions_options_for_dir`
fn flags_to_permissions_options(
  ...
  let make_fs_config_value_absolute = |value: &str| match &config_dir {
    Some(dir_path) => PathDescriptor::new_known_cwd(Cow::Borrowed(Path::new(value)), dir_path)
      .into_path_buf()
      .into_os_string()
      .into_string()
      .unwrap(),
...
mod test {
  use deno_config::deno_json::{
    AllowDenyIgnorePermissionConfig, AllowDenyPermissionConfig, PermissionsObject,
  };
  ...
  fn resolve_import_map_flags_take_precedence() {
    ...
    let expected_specifier = deno_path_util::url_from_file_path(&import_map_path).unwrap();
    assert!(actual.is_ok());
  ...
  fn resolve_import_map_none() {
    ...
    let config_specifier = Url::parse("file:///deno/deno.jsonc").unwrap();
    let config_file = ConfigFile::new(config_text, config_specifier).unwrap();
    let actual = resolve_import_map_specifier(None, Some(&config_file), Path::new("/"));
    assert!(actual.is_ok());
    let actual = actual.unwrap();
  ...
  fn test_flags_to_permission_options() {
    let base_dir = std::env::current_dir().unwrap().join("sub");
    {
      ...
      let config = PermissionsObjectWithBase {
        base: deno_path_util::url_from_file_path(&base_dir.join("deno.json")).unwrap(),
        permissions: PermissionsObject {
          ...
          read: AllowDenyIgnorePermissionConfig {
            ...
            deny: Some(PermissionConfigValue::Some(vec!["./read-deny".to_string()])),
          ...
          import: AllowDenyPermissionConfig {
            ...
            deny: Some(PermissionConfigValue::Some(vec!["example.com".to_string()])),
          },
          env: AllowDenyIgnorePermissionConfig {
            allow: Some(PermissionConfigValue::Some(vec!["env-allow".to_string()])),
            deny: Some(PermissionConfigValue::Some(vec!["env-deny".to_string()])),
            ignore: Some(PermissionConfigValue::Some(vec!["env-ignore".to_string()])),
          },
          net: AllowDenyPermissionConfig {
            allow: Some(PermissionConfigValue::Some(vec!["net-allow".to_string()])),
          ...
          ffi: AllowDenyPermissionConfig {
            allow: Some(PermissionConfigValue::Some(vec!["./ffi-allow".to_string()])),
            deny: Some(PermissionConfigValue::Some(vec!["./ffi-deny".to_string()])),
          },
          sys: AllowDenyPermissionConfig {
            allow: Some(PermissionConfigValue::Some(vec!["sys-allow".to_string()])),
            deny: Some(PermissionConfigValue::Some(vec!["sys-deny".to_string()])),
          },
        },
      };
      let permissions_options = flags_to_permissions_options(&flags, Some(&config)).unwrap();
      assert_eq!(
        permissions_options,
        PermissionsOptions {
