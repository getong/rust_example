...
use clap::{
  Arg, ArgAction, ArgMatches, ColorChoice, Command, ValueHint,
  builder::{FalseyValueParser, styling::AnsiColor},
  error::ErrorKind,
  value_parser,
};
use clap_complete::{
...
static ENV_VARS: &[EnvVar] = &[
  EnvVar {
    ...
    description: "A semi-colon separated list of bearer tokens and hostnames\nto use when \
                  fetching remote modules from private repositories",
  ...
  EnvVar {
    ...
    description: "Enable Node.js compatibility mode - extensionless imports, built-in\nNode.js \
                  modules, CommonJS detection and more.",
    example: None,
  },
  EnvVar {
    name: "DENO_DIR",
  ...
  EnvVar {
    ...
    description: cstr!(
      "Comma-separated list of order dependent certificate stores.\nPossible values: \"system\", \
       \"mozilla\" <p(245)>(defaults to \"mozilla\")</>"
    ),
    example: None,
  },
  EnvVar {
...
<y>Commands:</><y>Execution:</>
  ...
  <y>Dependency management:</>
    ...
    <g>uninstall</>    Uninstalls a \
     dependency or an executable script in the installation root's bin directory
    <g>outdated</>     \
  ...
  <y>Tooling:</>
    ...
    <g>coverage</>     Print coverage reports<g>deploy</>       \
    ...
    <g>test</>         Run tests
                  <p(245)>deno test  |  \
     deno test test.ts</>
    <g>publish</>      Publish the current working directory's package or \
     workspace
          ...
          && args.iter().any(|arg| arg == "install")
        {
          e.insert(
            ...
            clap::error::ContextValue::StyledStr(
              "Note: Permission flags can only be used in a global setting".into(),
            ),
          );
        }
  ...
  match matches.remove_subcommand() {
    Some((subcommand, mut m)) => {
      ...
      if let Some(arg) = pre_subcommand_arg {
        ...
        let mut err = clap::error::Error::new(ErrorKind::UnknownArgument).with_cmd(&app);
        err.insert(
...
fn enable_unstable(command: Command) -> Command {
  command
    ...
    .mut_args(|arg| {
      ...
      if arg.get_help_heading() == Some(UNSTABLE_HEADING) && arg.get_long_help().is_some() {
        arg.hide(false)
      } else {
        arg
      }
...
fn add_dev_arg() -> Arg {
  Arg::new("dev")
    ...
    .short('D')
    .help(
      "Add the package as a dev dependency. Note: This only applies when adding to a \
       `package.json` file.",
    )
    .action(ArgAction::SetTrue)
}
  ...
  .defer(|cmd| {
    cmd
      .arg(
        Arg::new("packages")
          .help(
            "Packages to approve (npm specifiers). When omitted, you will be prompted to select \
             from installed packages with lifecycle scripts.",
          )
          .num_args(0 ..)
          .action(ArgAction::Append),
      )
...
fn approve_scripts_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
...
Evaluate the given files, run all benches declared \
       with 'Deno.bench()' and report results to standard output:
  <p(245)>deno bench src/fetch_bench.ts \
       src/signal_bench.ts</>

If you specify a directory instead of a file, the path is expanded to \
       all contained files matching the glob <c>{*_,*.,}bench.{js,mjs,ts,mts,jsx,tsx}</>:
  <p(245)>deno \
       bench src/</>
    ...
    compile_args(cmd)
      ...
      .arg(
        Arg::new("inline-imports")
          .long("inline-imports")
          .help(cstr!(
            "Whether to inline imported modules into the importing file <p(245)>[default: true]</>"
          ))
          .require_equals(true)
          .default_value("true")
...
Unless \
       --reload is specified, this command will not re-download already cached dependencies

<y>Read \
       more:</> <c>https://docs.deno.com/go/check</>"
    ),
    UnstableArgsConfig::ResolutionAndRuntime,
  )
  .defer(|cmd| {
...
Cross-compiling to different target architectures is supported using the <c>--target</> \
       flag.
On the first invocation of `deno compile`, Deno will download the relevant binary and cache \
       it in <c>$DENO_DIR</>.
  ...
  This flag can be passed multiple times, to include multiple additional modules.</>",
      ...
      .arg(
        Arg::new("exclude")
          .long("exclude")
          .help(cstr!(
            "Excludes a file/directory in the compiled executable.
  ...
  For example, to exclude a certain folder in the bundled node_modules directory.</>",
      ...
      .arg(
        Arg::new("output")
          ...
          .value_parser(value_parser!(String))
          .help(cstr!(
            "Output file <p(245)>(defaults to $PWD/<<inferred-name>>)</>"
          ))
          .value_hint(ValueHint::FilePath)
          .help_heading(COMPILE_HEADING),
      )
...
Output documentation in HTML format:
    <p(245)>deno doc --html --name=\"My \
       library\" ./path/to/module.ts</>

Lint a module for documentation diagnostics:
    <p(245)>deno \
       doc --lint ./path/to/module.ts</>
    ...
    cmd
      ...
      .arg(
        Arg::new("strip-trailing-html")
          ...
          .help(
            "Remove trailing .html from various links. Will still generate files with a .html \
             extension",
          )
          .requires("html")
          .action(ArgAction::SetTrue)
...
Supported file \
       types which are behind corresponding unstable flags (see formatting options):
  <p(245)>HTML, \
    ...
    cmd
      ...
      .arg(
        Arg::new("ext")
          ...
          .value_parser([
            "ts", "tsx", "js", "jsx", "mts", "mjs", "cts", "cjs", "md", "json", "jsonc", "css",
            "scss", "sass", "less", "html", "svelte", "vue", "astro", "yml", "yaml", "ipynb",
            "sql", "vto", "njk",
          ])
          .help_heading(FMT_HEADING)
          .requires("files"),
      ...
      .arg(
        Arg::new("use-tabs")
          ...
          .require_equals(true)
          .help(cstr!(
            "Use tabs instead of spaces for indentation <p(245)>[default: false]</>"
          ))
          .help_heading(FMT_HEADING),
      )
      .arg(
      ...
      .arg(
        Arg::new("prose-wrap")
          ...
          .help(cstr!(
            "Define how prose should be wrapped <p(245)>[default: always]</>"
          ))
          .help_heading(FMT_HEADING),
      )
      .arg(
        Arg::new("no-semicolons")
          ...
          .require_equals(true)
          .help(cstr!(
            "Don't use semicolons except where necessary <p(245)>[default: false]</>"
          ))
          .help_heading(FMT_HEADING),
      )
      .arg(
...
Get information about a module:
  <p(245)>deno \
       info jsr:@std/http/file-server</>

The following information is shown:
  local: Local path of the \
       file
  type: JavaScript, TypeScript, or JSON
  emit: Local path of compiled source code (TypeScript \
       only)
...
<y>Read more:</> <c>https://docs.deno.com/go/info</>\
  ...
  .defer(|cmd| {
    cmd
      ...
      .arg(location_arg().conflicts_with("file").help(cstr!(
        "Show files used for origin bound APIs like the Web Storage API when running a script \
         with <c>--location=<<HREF>></>"
      )))
...
fn jupyter_subcommand() -> Command {
  ...
  .arg(
    Arg::new("name")
      ...
      .short('n')
      .help(cstr!(
        "Set a name for the kernel (defaults to 'deno'). <p(245)>Useful when maintaing multiple \
         Deno kernels.</>"
      ))
      .value_parser(value_parser!(String))
      .conflicts_with("kernel"),
  ...
  .arg(
    Arg::new("display")
      ...
      .short('d')
      .help(cstr!(
        "Set a display name for the kernel (defaults to 'Deno'). <p(245)>Useful when maintaing \
         multiple Deno kernels.</>"
      ))
      .value_parser(value_parser!(String))
      .requires("install"),
  )
...
fn update_and_outdated_args() -> [Arg; 6] {
  [
    Arg::new("filters")
      ...
      .action(ArgAction::Append)
      .help(concat!(
        "Filters selecting which packages to act on. Can include wildcards (*) to match multiple \
         packages. ",
        "If a version requirement is specified, the matching packages will be updated to the \
         given requirement."
      )),
    Arg::new("latest")
      .long("latest")
...
Filters can be used to select which packages to act on. Filters can include wildcards (*) to match \
       multiple packages.
  <p(245)>deno update --latest \"@std/*\"</>
  <p(245)>deno update --latest \"react*\"</>
Note that filters act on their aliases configured in deno.json / package.json, not the actual \
       package names:
  Given \"foobar\": \"npm:react@17.0.0\" in deno.json or package.json, the filter \"foobar\" would \
       update npm:react to
  the latest version.
...
using \
     the Language Server Protocol. Usually humans do not use this subcommand directly.
For example, \
     'deno lsp' can provide IDEs with go-to-definition support and automatic code formatting.

How to \
     connect various editors and IDEs to 'deno lsp': https://docs.deno.com/go/lsp",
  )
}
    ...
    cmd
      ...
      .arg(
        Arg::new("ext")
          ...
          .value_name("EXT")
          .help(
            "Specify the file extension to lint when reading from stdin.For example, use `jsx` to \
             lint JSX files or `tsx` for TSX files.This argument is necessary because stdin input \
             does not automatically infer the file type.Example usage: `cat file.jsx | deno lint \
             - --ext=jsx`.",
          ),
      )
      .arg(
...
fn repl_subcommand() -> Command {
  command(
    "repl",
    cstr!(
      "Starts a read-eval-print-loop, which lets you interactively build up program state in the \
       global context.
...
TypeScript is supported, however it is not type-checked, only transpiled."
    ...
    cmd
      .arg(
        Arg::new("eval-file")
          ...
          .use_value_delimiter(true)
          .help(
            "Evaluates the provided file(s) as scripts when the REPL starts. Accepts file paths \
             and URLs",
          )
          .value_hint(ValueHint::AnyPath),
      )
  ...
  <g>DENO_REPL_HISTORY</>  Set REPL history file path. History file is disabled when the value is \
         empty.
...
fn apply_node_options(flags: &mut Flags) {
  ...
  let cmd = Command::new("node-options-parser")
    ...
    .ignore_errors(true);
  let mut matches = cmd.get_matches_from(std::iter::once("node-options-parser").chain(args));
  ...
  if flags.inspect_publish_uid.is_none() {
    flags.inspect_publish_uid = matches.remove_one::<InspectPublishUid>("inspect-publish-uid");
  }
}
...
By default all programs are run \
         in sandbox without access to disk, network or ability to spawn subprocesses.
  <p(245)>deno \
         run https://examples.deno.land/hello-world.ts</>

Grant permission to read from disk and listen \
         to network:
...
Specifying the filename \
         '-' to read the file from stdin.
  <p(245)>curl https://examples.deno.land/hello-world.ts | \
         deno run -</>

<y>Read more:</> <c>https://docs.deno.com/go/run</>"
      ),
      UnstableArgsConfig::ResolutionAndRuntime,
    ),
    false,
...
The serve command uses the default exports of the main \
         module to determine which servers to start.

Start a server defined in server.ts:
  <p(245)>deno \
         serve server.ts</>

Start a server defined in server.ts, watching for changes and running on \
         port 5050:
  <p(245)>deno serve --watch --port 5050 server.ts</>

<y>Read more:</> <c>https://docs.deno.com/go/serve</>\
         "
      ),
      UnstableArgsConfig::ResolutionAndRuntime,
    ),
  ...
  .arg(
    Arg::new("port")
      .long("port")
      .help(cstr!(
        "The TCP port to serve on. Pass 0 to pick a random free port <p(245)>[default: 8000]</>"
      ))
...
Evaluate the given modules, run all tests declared \
       with <bold>Deno.</><y>test()</> and report results to standard output:
  <p(245)>deno test src/fetch_test.ts \
       src/signal_test.ts</>
...
or <c>**/__tests__/**</>:
 <p(245)>deno test src/</><y>Read \
    ...
    runtime_args(cmd, true, true, true)
      ...
      .arg(
        Arg::new("trace-leaks")
          .long("trace-leaks")
          .help(
            "Enable tracing of leaks. Useful when debugging leaking ops in test, but impacts test \
             execution time",
          )
          .action(ArgAction::SetTrue)
          .help_heading(TEST_HEADING),
  ...
  <p(245)>Note: running multiple `deno test --clean` calls in series or parallel for the same \
             coverage directory may cause race conditions.</>"
          ))
          .action(ArgAction::SetTrue)
          .help_heading(TEST_HEADING),
      )
...
fn parallel_arg(descr: &str) -> Arg {
  Arg::new("parallel")
    .long("parallel")
    .help(format!(
      "Run {descr} in parallel. Parallelism defaults to the number of available CPUs or the value \
       of the DENO_JOBS environment variable"
    ))
    .action(ArgAction::SetTrue)
}
...
The version is resolved via <p(245)>https://dl.deno.land</> \
       and then downloaded
from either there or GitHub releases, replacing the current executable.

If \
       you want to not replace the current Deno executable but instead download an
update to a different \
       location, use the <c>--output</> flag:
  <p(245)>deno upgrade --output $HOME/my_deno</>

<y>Read \
       more:</> <c>https://docs.deno.com/go/upgrade</>",
      UPGRADE_USAGE
    ),
    UnstableArgsConfig::None,
  )
    ...
    cmd
      ...
      .arg(
        Arg::new("version-or-hash-or-channel")
          .help(cstr!(
            "Version <p(245)>(v1.46.0)</>, channel <p(245)>(rc, canary)</> or commit hash \
             <p(245)>(9bc2dd29ad6ba334fd57a20114e367d3c04763d4)</>"
          ))
          .value_name("VERSION")
          .action(ArgAction::Append)
          .trailing_var_arg(true),
...
See the Deno 1.x to 2.x Migration Guide for migration instructions: \
...
fn publish_subcommand() -> Command {
    ...
    cmd
      ...
      .arg(
        Arg::new("dry-run")
          .long("dry-run")
          .help(
            "Prepare the package for publishing performing all checks and validations without \
             uploading",
          )
          .action(ArgAction::SetTrue)
          .help_heading(PUBLISH_HEADING),
  ...
  <p(245)>Enabled by default on Github actions, publicly links the package to where it was built \
             and published from.</>"
          ))
          .action(ArgAction::SetTrue)
          .help_heading(PUBLISH_HEADING),
      ...
      .arg(
        Arg::new("set-version")
          .long("set-version")
          .help(cstr!(
            "Set version for a package to be published.
  <p(245)>This flag can be used while publishing individual packages and cannot be used in a \
             workspace.</>"
          ))
          .value_name("VERSION")
          .help_heading(PUBLISH_HEADING),
...
fn deny_import_arg() -> Arg {
  Arg::new("deny-import")
    ...
    .value_name("IP_OR_HOSTNAME")
    .help(cstr!(
      "Deny importing from remote hosts. Optionally specify denied IP addresses and host names, \
       with ports as necessary."
    ))
    .value_parser(flags_net::validator)
}
...
pub fn inspect_value_parser(host_and_port: &str) -> Result<SocketAddr, String> {
  ...
  let host_ip = host_and_port
    .parse::<IpAddr>()
    .map_err(|e| format!("Invalid inspector host '{host_and_port}': {:?}", e))?;
  ...
  app
    .arg(
      Arg::new("inspect")
        ...
        .help(cstr!(
          "Activate inspector on host:port <p(245)>[default: 127.0.0.1:9229]</>. Host and port \
           are optional. Using port 0 will assign a random free port."
        ))
        .num_args(0 ..= 1)
        .require_equals(true)
    ...
    .arg(
      Arg::new("inspect-brk")
        ...
        .default_missing_value("127.0.0.1:9229")
        .help(
          "Activate inspector on host:port, wait for debugger to connect and break at the start \
           of user script",
        )
        .num_args(0 ..= 1)
        .require_equals(true)
    ...
    .arg(
      Arg::new("inspect-wait")
        ...
        .default_missing_value("127.0.0.1:9229")
        .help(
          "Activate inspector on host:port and wait for debugger to connect before running user \
           code",
        )
        .num_args(0 ..= 1)
        .require_equals(true)
...
fn inspect_publish_uid_value_parser(value: &str) -> Result<InspectPublishUid, String> {
...
fn env_file_arg() -> Arg {
  ...
  Where multiple declarations for the same environment variable exist in your .env file, the first \
       one encountered is applied. This is determined by the order of the files you pass as \
       arguments.</>"
    ))
    .value_hint(ValueHint::FilePath)
...
fn no_check_arg() -> Arg {
  Arg::new("no-check")
    ...
    .long("no-check")
    .help(
      "Skip type-checking. If the value of \"remote\" is supplied, diagnostic errors from remote \
...
fn check_arg(checks_local_by_default: bool) -> Arg {
  ...
  if checks_local_by_default {
    arg.help(cstr!(
      "Set type-checking behavior. This subcommand type-checks local modules by default, so \
       adding --check is redundant
...
fn config_arg() -> Arg {
  ...
  automatically detected; \
       in that case this flag is not necessary.
...
fn node_modules_dir_arg() -> Arg {
  fn parse_node_modules_dir_mode(s: &str) -> Result<NodeModulesDirMode, String> {
    match s {
...
fn allow_scripts_arg() -> Arg {
  ...
  <p(245)>Note: Scripts will only be executed when using a node_modules directory \
       (`--node-modules-dir`)</>"
    ))
}
  ...
  fn with_unstable_args(self, cfg: UnstableArgsConfig) -> Self {
    ...
    let mut cmd = self.arg(
      Arg::new("unstable")
        .long("unstable")
        .help(cstr!(
          "The `--unstable` flag has been deprecated. Use granular `--unstable-*` flags instead
  <p(245)>To view the list of individual unstable feature flags, run this command again with \
           --help=unstable</>"
        ))
        .action(ArgAction::SetTrue)
...
fn allow_scripts_arg_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
  ...
  if parts.len() == 0 {
    ...
    flags.allow_scripts = PackagesAllowedScripts::Some(
      parts
        ...
        .map(|result| {
          let value = result?;
          let dep = JsrDepPackageReq::from_str_loose(&value)
            .map_err(|e| clap::Error::raw(clap::error::ErrorKind::InvalidValue, e))?;
...
fn doc_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
  ...
  let html = if matches.get_flag("html") {
    ...
    let symbol_redirect_map_path = matches.remove_one::<String>("symbol-redirect-map");
    let strip_trailing_html = matches.get_flag("strip-trailing-html");
    let default_symbol_map_path = matches.remove_one::<String>("default-symbol-map");
    let output = matches
...
fn fmt_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
...
fn init_parse(flags: &mut Flags, matches: &mut ArgMatches) -> Result<(), clap::Error> {
...
fn install_parse(
  ...
  if global {
    ...
    flags.subcommand = DenoSubcommand::Install(InstallFlags::Global(InstallFlagsGlobal {
      name,
      module_urls,
      args,
      root,
    ...
    .map(|packages| add_parse_inner(matches, Some(packages)))
  {
    if matches!(flags.config_flag, ConfigFlag::Disabled) {
      return Err(
        app
          .override_usage("deno install [OPTIONS] [PACKAGE]...")
          .error(
            clap::error::ErrorKind::ArgumentConflict,
            format!(
              "deno install can't be used to add packages if `--no-config` is passed.\n{} to \
               cache the packages without adding to a config, pass the `--entrypoint` flag",
    ...
    flags.subcommand =
      DenoSubcommand::Install(InstallFlags::Local(InstallFlagsLocal::Add(add_files)))
  } else {
    flags.subcommand = DenoSubcommand::Install(InstallFlags::Local(InstallFlagsLocal::TopLevel(
      InstallTopLevelFlags { lockfile_only },
    )));
  }
  Ok(())
}

fn json_reference_parse(flags: &mut Flags, _matches: &mut ArgMatches, mut app: Command) {
  use deno_core::serde_json::json;

  app.build();

  fn serialize_command(mut command: Command, top_level: bool) -> deno_core::serde_json::Value {
    let args = command
      .get_arguments()
      .filter(|arg| {
        !arg.is_hide_set()
...
fn lint_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
  unstable_args_parse(flags, matches, UnstableArgsConfig::ResolutionOnly);
  ext_arg_parse(flags, matches);
  config_args_parse(flags, matches);
  allow_and_deny_import_parse(flags, matches)?;
  ...
  match matches.remove_many::<String>("script_arg") {
    ...
    _ => {
      if bare {
        return Err(
          app
            .override_usage("deno [OPTIONS] [COMMAND] [SCRIPT_ARG]...")
            .error(
              clap::error::ErrorKind::MissingRequiredArgument,
              "[SCRIPT_ARG] may only be omitted with --v8-flags=--help, else to use the repl with \
               arguments, please use the `deno repl` subcommand",
            ),
        );
      } else {
...
impl EnvCompleter for ZshCompleterUnsorted {
  ...
  fn write_registration(
    ...
    let completer = shlex::try_quote(completer).unwrap_or(std::borrow::Cow::Borrowed(completer));
...
fn upgrade_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let output = matches.remove_one::<String>("output");
  let version_or_hash_or_channel = matches.remove_one::<String>("version-or-hash-or-channel");
...
fn publish_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
...
fn x_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
  let kind = if let Some(shim_name) = matches.remove_one::<DenoXShimName>("install-alias") {
    XFlagsKind::InstallAlias(shim_name)
  } else if let Some(mut script_arg) = matches.remove_many::<String>("script_arg") {
    ...
    if c == ',' {
      if let Some(next) = chars.next() {
        if next == ',' {
          ...
          if current.is_empty() {
            return Err(std::io::Error::other(String::from("Empty values are not allowed")).into());
          }
...
fn permission_args_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
  ...
  if matches.get_flag("allow-hrtime") || matches.get_flag("deny-hrtime") {
    ...
    {
      eprintln!(
        "{} `allow-hrtime` and `deny-hrtime` have been removed in Deno 2, as high resolution time \
         is now always allowed",
...
fn node_modules_and_vendor_dir_arg_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  node_modules_arg_parse(flags, matches);
...
fn reload_arg_validate(urlstr: String) -> Result<String, clap::Error> {
  if urlstr.is_empty() {
    return Err(std::io::Error::other(String::from("Missing url. Check for extra commas.")).into());
  }
...
fn watch_arg_parse(matches: &mut ArgMatches) -> clap::error::Result<Option<WatchFlags>> {
...
fn unstable_args_parse(flags: &mut Flags, matches: &mut ArgMatches, cfg: UnstableArgsConfig) {
  ...
  flags.unstable_config.lazy_dynamic_imports = matches.get_flag("unstable-lazy-dynamic-imports");
...
mod tests {
  ...
  fn global_flags() {
    ...
    assert_eq!(
      flags,
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string())),
  ...
  fn run_watch() {
    ...
    let r = flags_from_vec(svec!["deno", "--watch", "--no-clear-screen", "script.ts"]);
    let flags = r.unwrap();
    assert_eq!(
      flags,
      Flags {
  ...
  fn run_watch_with_excluded_paths() {
    let r = flags_from_vec(svec!("deno", "--watch", "--watch-exclude=foo", "script.ts"));
  ...
  fn script_args() {
    ...
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("gist.ts".to_string())),
        argv: svec!["--title", "X"],
        permissions: PermissionFlags {
          allow_net: Some(vec![]),
          ..Default::default()
  ...
  fn lint() {
    ...
    let r = flags_from_vec(svec!["deno", "lint", "--rules", "--rules-tags=recommended"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Lint(LintFlags {
  ...
  fn check() {
    ...
    let r = flags_from_vec(svec!["deno", "check", "--doc", "--doc-only", "script.ts"]);
    assert_eq!(
      r.unwrap_err().kind(),
      clap::error::ErrorKind::ArgumentConflict
    );
    ...
    for all_flag in ["--remote", "--all"] {
      ...
      let r = flags_from_vec(svec!["deno", "check", all_flag, "--no-remote", "script.ts"]);
      assert_eq!(
        r.unwrap_err().kind(),
        clap::error::ErrorKind::ArgumentConflict
      );
  ...
  fn allow_env_allowlist_multiple() {
    let r = flags_from_vec(svec!["deno", "run", "--allow-env=HOME,PATH", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn deny_env_denylist_multiple() {
    let r = flags_from_vec(svec!["deno", "run", "--deny-env=HOME,PATH", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn deny_env_ignorelist_multiple() {
    let r = flags_from_vec(svec!["deno", "run", "--ignore-env=HOME,PATH", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn allow_env_allowlist_validator() {
    ...
    assert!(r.is_err());
    let r = flags_from_vec(svec!["deno", "run", "--allow-env=H\0ME", "script.ts"]);
    assert!(r.is_err());
  }
  ...
  fn allow_sys_allowlist() {
    let r = flags_from_vec(svec!["deno", "run", "--allow-sys=hostname", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn deny_sys_denylist_validator() {
    let r = flags_from_vec(svec!["deno", "run", "--deny-sys=hostname", "script.ts"]);
    assert!(r.is_ok());
    let r = flags_from_vec(svec![
      "deno",
      "run",
    ...
    assert!(r.is_err());
    let r = flags_from_vec(svec!["deno", "run", "--deny-sys=hostname,foo", "script.ts"]);
    assert!(r.is_err());
  }
  ...
  fn reload_validator() {
    ...
    let r = flags_from_vec(svec!["deno", "run", "--reload=./relativeurl/", "script.ts"]);
    assert!(r.is_err(), "Should reject relative urls that start with ./");

    let r = flags_from_vec(svec!["deno", "run", "--reload=relativeurl/", "script.ts"]);
    assert!(r.is_err(), "Should reject relative urls");

    let r = flags_from_vec(svec!["deno", "run", "--reload=/absolute", "script.ts"]);
  ...
  #[test]
  fn run_env_defined() {
    let r = flags_from_vec(svec!["deno", "run", "--env=.another_env", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn run_env_file_defined() {
    let r = flags_from_vec(svec!["deno", "run", "--env-file=.another_env", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn install() {
    ...
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Install(InstallFlags::Global(InstallFlagsGlobal {
          name: None,
    ...
    let r = flags_from_vec(svec!["deno", "install", "-g", "jsr:@std/http/file-server"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Install(InstallFlags::Global(InstallFlagsGlobal {
  ...
  fn uninstall() {
    ...
    let r = flags_from_vec(svec!["deno", "uninstall", "file_server", "@std/load"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Uninstall(UninstallFlags {
  ...
  #[test]
  fn no_check_remote() {
    let r = flags_from_vec(svec!["deno", "run", "--no-check=remote", "script.ts"]);
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("script.ts".to_string(),)),
  ...
  fn test_watch_with_excluded_paths() {
    let r = flags_from_vec(svec!("deno", "test", "--watch", "--watch-exclude=foo",));
    ...
    let r = flags_from_vec(svec!("deno", "test", "--watch=foo", "--watch-exclude=bar",));
    let flags = r.unwrap();
    assert_eq!(
      flags,
      Flags {
    ...
    let r = flags_from_vec(svec!["deno", "test", "--watch", "--watch-exclude=foo,bar",]);
    ...
    assert_eq!(
      ...
      Flags {
        subcommand: DenoSubcommand::Doc(DocFlags {
          ...
          html: None,
          lint: false,
          source_files: DocSourceFileFlag::Paths(vec!["path/to/module.ts".to_string()]),
  ...
  fn inspect_default_host() {
    ...
    assert_eq!(
      r.unwrap(),
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags::new_default("foo.js".to_string(),)),
        inspect: Some("127.0.0.1:9229".parse().unwrap()),
        code_cache_enabled: true,
        ..Flags::default()
      }
  ...
  fn test_config_path_args() {
    ...
    let flags = flags_from_vec(svec!["deno", "https://example.com/foo.js"]).unwrap();
    assert_eq!(flags.config_path_args(&cwd), None);

    let flags = flags_from_vec(svec!["deno", "lint", "dir/a/a.js", "dir/b/b.js"]).unwrap();
    assert_eq!(
      flags.config_path_args(&cwd),
    ...
    let flags = flags_from_vec(svec!["deno", "install", "-e", "sub/test.js"]).unwrap();
    assert_eq!(flags.config_path_args(&cwd), Some(vec![cwd.join("sub")]));
  ...
  fn test_no_clear_watch_flag_without_watch_flag() {
    ...
    assert!(&error_message.contains("error: the following required arguments were not provided:"));
  ...
  fn run_with_check() {
    ...
    let r = flags_from_vec(svec!["deno", "run", "--no-check", "--check", "script.ts",]);
    assert!(r.is_err());
  }
    ...
    for cmd in ["add", "install"] {
      let mk_flags = |flags: AddFlags| -> Flags {
        match cmd {
          ...
          "install" => Flags {
            subcommand: DenoSubcommand::Install(InstallFlags::Local(InstallFlagsLocal::Add(flags))),
            ..Flags::default()
          },
          _ => unreachable!(),
        }
  ...
  fn bare_with_flag_no_file() {
    ...
    assert!(err.to_string().contains(
      "error: [SCRIPT_ARG] may only be omitted with --v8-flags=--help, else to use the repl with \
       arguments, please use the `deno repl` subcommand"
    ));
    assert!(
      err
  ...
  fn node_modules_dir_default() {
    let r = flags_from_vec(svec!["deno", "run", "--node-modules-dir", "./foo.ts"]);
    let flags = r.unwrap();
    assert_eq!(
      flags,
      Flags {
...
Usage: deno repl [OPTIONS] [-- [ARGS]...]\n"
  ...
  fn approve_scripts_subcommand() {
    let cases = [
      ...
      (
        ...
        ApproveScriptsFlags {
          packages: vec!["npm:pkg1@1.0.0".to_string(), "npm:pkg2@2.0.0".to_string()],
          lockfile_only: false,
        },
      ),
      (
  ...
  fn preload_flag_test() {
    let flags = flags_from_vec(svec!["deno", "run", "--preload", "preload.js", "main.ts"]).unwrap();
    assert_eq!(
      flags,
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags {
    ...
    let flags = flags_from_vec(svec!["deno", "run", "--preload", "data:,()", "main.ts"]).unwrap();
    assert_eq!(
      flags,
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags {
  ...
  fn require_flag_test() {
    let flags = flags_from_vec(svec!["deno", "run", "--require", "require.js", "main.ts"]).unwrap();
    assert_eq!(
      flags,
      Flags {
        subcommand: DenoSubcommand::Run(RunFlags {
  ...
  fn node_options_require_prepend_to_cli() {
    ...
    let flags = flags_from_vec(svec!["deno", "run", "--require", "cli.js", "script.ts",]).unwrap();
    set_test_node_options(None);
  ...
  fn node_options_combined() {
    ...
    set_test_node_options(Some("--require foo.js --inspect-publish-uid=stderr,http"));
    let flags = flags_from_vec(svec!["deno", "run", "script.ts",]).unwrap();
    set_test_node_options(None);
  ...
  fn node_options_ignores_unknown_flags() {
    set_test_node_options(Some("--require known.js --unknown-flag --another-unknown"));
    let flags = flags_from_vec(svec!["deno", "run", "script.ts",]).unwrap();
    set_test_node_options(None);
