...
use crate::{args::Flags, colors, util::fs::canonicalize_path};
...
impl PrintConfig {
  ...
  pub fn new_with_banner(banner: &'static str, job_name: &'static str, clear_screen: bool) -> Self {
    Self {
      banner,
      job_name,
      clear_screen,
...
impl WatcherCommunicator {
  ...
  pub fn watch_paths(&self, paths: Vec<PathBuf>) -> Result<(), SendError<Vec<PathBuf>>> {
    if paths.is_empty() {
      return Ok(());
    }
  ...
  pub async fn watch_for_changed_paths(&self) -> Result<Option<Vec<PathBuf>>, RecvError> {
  ...
  pub fn send(&self, paths: Option<Vec<PathBuf>>) -> Result<(), SendError<Option<Vec<PathBuf>>>> {
    match *self.restart_mode.lock() {
      WatcherRestartMode::Automatic => self.restart_tx.send(()).map_err(|_| SendError(None)),
...
///   changes. For example, in the case where we would like to bundle, then `operation` would have
///   the logic for it like bundling the code.
  ...
  F: Future<Output = Result<(), AnyError>>,
{
  ...
  let watcher_communicator = Arc::new(WatcherCommunicator::new(WatcherCommunicatorOptions {
