...
use deno_ast::{
  MediaType, SourceRangedForSpanned as _,
  swc::{
    ast,
    atoms::Atom,
    common::{DUMMY_SP, comments::CommentKind},
    ecma_visit::{Visit, VisitMut, VisitWith as _, visit_mut_pass},
    utils as swc_utils,
  },
};
...
use crate::{file_fetcher::TextDecodedFile, util::path::mapped_specifier_for_tsc};
...
fn extract_inner(file: File, wrap_kind: WrapKind) -> Result<Vec<File>, AnyError> {
  ...
  let extracted_files = if file.media_type == MediaType::Unknown {
    ...
    extract_files_from_source_comments(&file.specifier, file.source.clone(), file.media_type)?
  };
...
impl ExportCollector {
  fn to_import_specifiers(
    ...
    if let Some(default_export) = &self.default_export {
      ...
      {
        import_specifiers.push(ast::ImportSpecifier::Default(ast::ImportDefaultSpecifier {
          span: DUMMY_SP,
          local: ast::Ident {
            span: DUMMY_SP,
    ...
    for named_export in &self.named_exports {
      ...
      import_specifiers.push(ast::ImportSpecifier::Named(ast::ImportNamedSpecifier {
        span: DUMMY_SP,
        local: ast::Ident {
          span: DUMMY_SP,
          ctxt: Default::default(),
...
impl Visit for ExportCollector {
  ...
  fn visit_export_default_decl(&mut self, export_default_decl: &ast::ExportDefaultDecl) {
  ...
  fn visit_export_default_expr(&mut self, export_default_expr: &ast::ExportDefaultExpr) {
    if let ast::Expr::Ident(ident) = &*export_default_expr.expr {
      self.default_export = Some(ident.sym.clone());
    }
  }

  fn visit_export_named_specifier(&mut self, export_named_specifier: &ast::ExportNamedSpecifier) {
...
/// 2. If `wrap_kind` is [`WrapKind::DenoTest`], wraps the content of the file in a `Deno.test`
///    call.
///
  ...
  fn visit_mut_program(&mut self, node: &mut ast::Program) {
    let new_module_items = match node {
      ast::Program::Module(module) => {
                ...
                ast::ModuleDecl::ExportDefaultDecl(export_default_decl) => {
                  let stmt = match &export_default_decl.decl {
                    ...
                    ast::DefaultDecl::TsInterfaceDecl(ts_interface_decl) => {
                      ast::Stmt::Decl(ast::Decl::TsInterface(ts_interface_decl.clone()))
                    }
                  };
                  stmts.push(stmt);
                }
        ...
        transformed_items.extend(module_decls.into_iter().map(ast::ModuleItem::ModuleDecl));
        let import_specifiers = self
          .exports_from_base
          .to_import_specifiers(&self.atoms_to_be_excluded_from_import);
        if !import_specifiers.is_empty() {
          transformed_items.push(ast::ModuleItem::ModuleDecl(ast::ModuleDecl::Import(
            ast::ImportDecl {
              span: DUMMY_SP,
        ...
        match self.wrap_kind {
          ...
          WrapKind::NoWrap => {
            transformed_items.extend(stmts.into_iter().map(ast::ModuleItem::Stmt));
          }
        }
      ...
      ast::Program::Script(script) => {
        ...
        match self.wrap_kind {
          ...
          WrapKind::NoWrap => {
            transformed_items.extend(script.body.clone().into_iter().map(ast::ModuleItem::Stmt));
          }
        }
