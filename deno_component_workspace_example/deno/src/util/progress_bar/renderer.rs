...
impl ProgressBarRenderer for BarProgressBarRenderer {
  fn render(&self, data: ProgressData) -> String {
    ...
    let same_line_text_width = elapsed_text.len() + total_text_max_width + bytes_text_max_width + 3; // space, open and close brace
...
impl ProgressBarRenderer for TextOnlyProgressBarRenderer {
  fn render(&self, data: ProgressData) -> String {
    ...
    let mut display_str = format!("{} {} ", prompt.as_text(), SPINNER_CHARS[last_tick]);
    ...
    if let Some(display_entry) = non_empty_entry {
      ...
      display_str.push_str(&colors::gray(format!("  {}{}\n", message, bytes_text)).to_string());
    } else {
