...
use deno_core::{
  InspectorPostMessageError, InspectorPostMessageErrorKind, LocalInspectorSession,
  error::CoreError,
  parking_lot::Mutex,
  serde_json::{
    json, {self},
...
use crate::{
  ...
  util::file_watcher::{WatcherCommunicator, WatcherRestartMode},
};
...
impl HmrRunnerState {
  pub fn new(emitter: Arc<CliEmitter>, watcher_communicator: Arc<WatcherCommunicator>) -> Self {
  ...
  pub fn callback(&self, msg: deno_core::InspectorMsg) {
    ...
    let message: serde_json::Value = serde_json::from_str(&msg.content).unwrap();
  ...
  fn handle_notification(&self, notification: cdp::Notification) {
    if notification.method == "Runtime.exceptionThrown" {
      let exception_thrown =
        serde_json::from_value::<cdp::ExceptionThrown>(notification.params).unwrap();
      ...
      let params = serde_json::from_value::<cdp::ScriptParsed>(notification.params).unwrap();
      // .map_err(JsErrorBox::from_err)?;
