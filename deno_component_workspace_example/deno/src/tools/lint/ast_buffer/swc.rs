...
use deno_ast::{
  ...
  swc::{
    ast::{
      AssignTarget, AssignTargetPat, BindingIdent, BlockStmtOrExpr, Callee, ClassMember, Decl,
      Decorator, DefaultDecl, ExportSpecifier, Expr, ExprOrSpread, FnExpr, ForHead, Function,
      Ident, IdentName, ImportSpecifier, JSXAttrName, JSXAttrOrSpread, JSXAttrValue, JSXElement,
  ...
  view::{
    Accessibility, AssignOp, BinaryOp, MetaPropKind, MethodKind, TsKeywordTypeKind,
    TsTypeOperatorOp, UnaryOp, UpdateOp, VarDeclKind,
  },
};

use super::{
  buffer::{AstBufSerializer, CommentKind, NodeRef},
  ts_estree::{
    AstNode, MethodKind as TsEstreeMethodKind, PropertyKind, SourceKind, TsEsTreeBuilder,
    TsKeywordKind, TsModuleKind,
  },
};
use crate::util::text_encoding::Utf16Map;

pub fn serialize_swc_to_buffer(parsed_source: &ParsedSource, utf16_map: &Utf16Map) -> Vec<u8> {
  ...
  match program.as_ref() {
    Program::Module(module) => {
      let children = module
        ...
        .iter()
        .map(|item| match item {
          ModuleItem::ModuleDecl(module_decl) => serialize_module_decl(&mut ctx, module_decl),
...
fn serialize_module_decl(ctx: &mut TsEsTreeBuilder, module_decl: &ModuleDecl) -> NodeRef {
  match module_decl {
      ...
      let specifiers = node
        ...
        .map(|spec| match spec {
          ImportSpecifier::Named(spec) => {
            ...
            ctx.write_import_spec(&spec.span, spec.is_type_only, local, imported)
          }
    ...
    ModuleDecl::ExportDecl(node) => {
      ...
      ctx.write_export_named_decl(&node.span, is_type_only, vec![], None, vec![], Some(decl))
    }
    ModuleDecl::ExportNamed(node) => {
      ...
      if let Some(ExportSpecifier::Namespace(ns)) = node.specifiers.first() {
            ...
            match spec {
              ExportSpecifier::Named(spec) => {
                ...
                let exported = spec
                  .exported
                  .as_ref()
                  .map_or(serialize_module_export_name(ctx, &spec.orig), |exported| {
                    serialize_module_export_name(ctx, exported)
                  });

                ctx.write_export_spec(&spec.span, spec.is_type_only, local, exported)
              }
        ...
        ctx.write_export_named_decl(&node.span, node.type_only, specifiers, source, attrs, None)
      }
    }
    ModuleDecl::ExportDefaultDecl(node) => {
      let (is_type_only, decl) = match &node.decl {
        ...
        DefaultDecl::Fn(node) => {
          ...
          let type_params = maybe_serialize_ts_type_param_decl(ctx, &fn_obj.type_params);
          ...
          let return_type = maybe_serialize_ts_type_ann(ctx, &fn_obj.return_type);
          let body = fn_obj
            .body
            .as_ref()
        ...
        DefaultDecl::TsInterfaceDecl(node) => {
          ...
          let type_param = maybe_serialize_ts_type_param_decl(ctx, &node.type_params);
          ...
          let body_pos = ctx.write_ts_interface_body(&node.body.span, body_elem_ids);
    ...
    ModuleDecl::TsImportEquals(node) => {
      ...
      let module_ref = match &node.module_ref {
        TsModuleRef::TsEntityName(entity) => serialize_ts_entity_name(ctx, entity),
      ...
      ctx.write_export_ts_import_equals(&node.span, node.is_type_only, ident, module_ref)
    }
...
fn serialize_expr(ctx: &mut TsEsTreeBuilder, expr: &Expr) -> NodeRef {
  match expr {
    ...
    Expr::Bin(node) => {
      ...
      match node_type {
        AstNode::LogicalExpression => ctx.write_logical_expr(&node.span, flag_str, left, right),
        AstNode::BinaryExpression => ctx.write_bin_expr(&node.span, flag_str, left, right),
        _ => unreachable!(),
      }
    }
    Expr::Assign(node) => {
      let left = match &node.left {
        AssignTarget::Simple(simple_assign_target) => {
          match simple_assign_target {
            SimpleAssignTarget::Ident(target) => serialize_binding_ident(ctx, target, None),
            ...
            SimpleAssignTarget::Paren(target) => serialize_expr(ctx, &target.expr),
            SimpleAssignTarget::OptChain(target) => {
              serialize_expr(ctx, &Expr::OptChain(target.clone()))
            }
            SimpleAssignTarget::TsAs(target) => serialize_expr(ctx, &Expr::TsAs(target.clone())),
    ...
    Expr::Class(node) => {
      ...
      let type_params = maybe_serialize_ts_type_param_decl(ctx, &node.class.type_params);
    ...
    Expr::TsConstAssertion(node) => {
      ...
      let type_name = ctx.write_identifier(&node.span, "const", false, None, None);
    ...
    Expr::OptChain(node) => {
      let expr = match node.base.as_ref() {
        OptChainBase::Member(member_expr) => serialize_member_expr(ctx, member_expr, true),
...
fn serialize_prop_or_spread(ctx: &mut TsEsTreeBuilder, prop: &PropOrSpread) -> NodeRef {
  match prop {
    ...
    PropOrSpread::Prop(prop) => {
      ...
      let (key, value) = match prop.as_ref() {
        ...
        Prop::Assign(assign_prop) => {
          ...
          let child_pos = ctx.write_assign_pat(&assign_prop.span, left, right, None);
      ...
      ctx.write_property(&prop.span(), shorthand, computed, method, kind, key, value)
    }
  }
}

fn serialize_member_expr(ctx: &mut TsEsTreeBuilder, node: &MemberExpr, optional: bool) -> NodeRef {
  let mut computed = false;
  let obj = serialize_expr(ctx, node.obj.as_ref());

  let prop = match &node.prop {
    MemberProp::Ident(ident_name) => serialize_ident_name(ctx, ident_name),
    MemberProp::PrivateName(private_name) => serialize_private_name(ctx, private_name),
...
fn serialize_expr_or_spread(ctx: &mut TsEsTreeBuilder, arg: &ExprOrSpread) -> NodeRef {
...
fn serialize_ident(ctx: &mut TsEsTreeBuilder, ident: &Ident, type_ann: Option<NodeRef>) -> NodeRef {
...
fn serialize_module_export_name(ctx: &mut TsEsTreeBuilder, name: &ModuleExportName) -> NodeRef {
  match &name {
...
fn serialize_decl(ctx: &mut TsEsTreeBuilder, decl: &Decl) -> NodeRef {
  match decl {
    ...
    Decl::Fn(node) => {
      ...
      let type_param_id = maybe_serialize_ts_type_param_decl(ctx, &node.function.type_params);
      let return_type = maybe_serialize_ts_type_ann(ctx, &node.function.return_type);
    ...
    Decl::TsTypeAlias(node) => {
      ...
      ctx.write_ts_type_alias(&node.span, node.declare, ident, type_param, type_ann)
    }
    Decl::TsEnum(node) => {
      ...
      let members = node
        ...
        .map(|member| {
          let ident = match &member.id {
            ...
            TsEnumMemberId::Str(lit_str) => serialize_lit(ctx, &Lit::Str(lit_str.clone())),
          };
    ...
    Decl::TsModule(node) => {
      let ident = match &node.id {
        ...
        TsModuleName::Str(str_lit) => serialize_lit(ctx, &Lit::Str(str_lit.clone())),
      };
...
fn serialize_ts_namespace_body(ctx: &mut TsEsTreeBuilder, node: &TsNamespaceBody) -> NodeRef {
  match node {
...
fn serialize_ts_type_elem(ctx: &mut TsEsTreeBuilder, node: &TsTypeElement) -> NodeRef {
  match node {
    TsTypeElement::TsCallSignatureDecl(ts_call) => {
      let type_ann = maybe_serialize_ts_type_param_decl(ctx, &ts_call.type_params);
    ...
    TsTypeElement::TsConstructSignatureDecl(sig) => {
      let type_params = maybe_serialize_ts_type_param_decl(ctx, &sig.type_params);
      ...
      // Must be present
      let return_type = maybe_serialize_ts_type_ann(ctx, &sig.type_ann).unwrap();
    ...
    TsTypeElement::TsMethodSignature(sig) => {
      let key = serialize_expr(ctx, &sig.key);
      let type_parms = maybe_serialize_ts_type_param_decl(ctx, &sig.type_params);
      let params = sig
        .params
        .iter()
        .map(|param| serialize_ts_fn_param(ctx, param))
...
fn serialize_ts_index_sig(ctx: &mut TsEsTreeBuilder, node: &TsIndexSignature) -> NodeRef {
  let params = node
    .params
    .iter()
    .map(|param| serialize_ts_fn_param(ctx, param))
  ...
  ctx.write_ts_index_sig(&node.span, node.is_static, node.readonly, params, type_ann)
}
...
fn serialize_private_name(ctx: &mut TsEsTreeBuilder, node: &PrivateName) -> NodeRef {
  ctx.write_private_identifier(&node.span, node.name.as_str())
}

fn serialize_jsx_element(ctx: &mut TsEsTreeBuilder, node: &JSXElement) -> NodeRef {
...
fn serialize_jsx_fragment(ctx: &mut TsEsTreeBuilder, node: &JSXFragment) -> NodeRef {
...
fn serialize_jsx_children(ctx: &mut TsEsTreeBuilder, children: &[JSXElementChild]) -> Vec<NodeRef> {
  children
    .iter()
    .map(|child| {
      match child {
        JSXElementChild::JSXText(text) => ctx.write_jsx_text(&text.span, &text.raw, &text.value),
...
fn serialize_jsx_member_expr(ctx: &mut TsEsTreeBuilder, node: &JSXMemberExpr) -> NodeRef {
...
fn serialize_jsx_element_name(ctx: &mut TsEsTreeBuilder, node: &JSXElementName) -> NodeRef {
  match &node {
    JSXElementName::Ident(ident) => serialize_jsx_identifier(ctx, ident),
    JSXElementName::JSXMemberExpr(member) => serialize_jsx_member_expr(ctx, member),
    JSXElementName::JSXNamespacedName(ns) => serialize_jsx_namespaced_name(ctx, ns),
  }
}

fn serialize_jsx_opening_element(ctx: &mut TsEsTreeBuilder, node: &JSXOpeningElement) -> NodeRef {
    ...
    .map(|attr| match attr {
      JSXAttrOrSpread::JSXAttr(attr) => {
        let name = match &attr.name {
          JSXAttrName::Ident(name) => serialize_ident_name_as_jsx_identifier(ctx, name),
          JSXAttrName::JSXNamespacedName(node) => serialize_jsx_namespaced_name(ctx, node),
        };
  ...
  ctx.write_jsx_opening_elem(&node.span, node.self_closing, name, attrs, type_args)
}

fn serialize_jsx_container_expr(ctx: &mut TsEsTreeBuilder, node: &JSXExprContainer) -> NodeRef {
...
fn serialize_jsx_namespaced_name(ctx: &mut TsEsTreeBuilder, node: &JSXNamespacedName) -> NodeRef {
...
fn serialize_ident_name_as_jsx_identifier(ctx: &mut TsEsTreeBuilder, node: &IdentName) -> NodeRef {
  ctx.write_jsx_identifier(&node.span, &node.sym)
}

fn serialize_jsx_identifier(ctx: &mut TsEsTreeBuilder, node: &Ident) -> NodeRef {
  ctx.write_jsx_identifier(&node.span, &node.sym)
}
...
fn serialize_pat(
  ...
  match pat {
    ...
    Pat::Array(node) => {
      ...
      ctx.write_arr_pat(&node.span, node.optional, type_ann, children, decorators)
    }
    Pat::Rest(node) => {
      let type_ann = maybe_serialize_ts_type_ann(ctx, &node.type_ann);
      ...
      let children = node
        ...
        .map(|prop| match prop {
          ...
          ObjectPatProp::Assign(assign_pat_prop) => {
            ...
            let mut value = serialize_binding_ident(ctx, &assign_pat_prop.key, None);

            let shorthand = assign_pat_prop.value.is_none();

            if let Some(assign) = &assign_pat_prop.value {
              let expr = serialize_expr(ctx, assign);
              value = ctx.write_assign_pat(&assign_pat_prop.span, value, expr, None);
            }
          ...
          ObjectPatProp::Rest(rest_pat) => serialize_pat(ctx, &Pat::Rest(rest_pat.clone()), None),
        })
        .collect::<Vec<_>>();

      ctx.write_obj_pat(&node.span, node.optional, type_ann, children, decorators)
    }
    Pat::Assign(node) => {
...
fn serialize_for_head(ctx: &mut TsEsTreeBuilder, for_head: &ForHead) -> NodeRef {
  match for_head {
    ForHead::VarDecl(var_decl) => serialize_decl(ctx, &Decl::Var(var_decl.clone())),
    ForHead::UsingDecl(using_decl) => serialize_decl(ctx, &Decl::Using(using_decl.clone())),
    ForHead::Pat(pat) => serialize_pat(ctx, pat, None),
  }
}

fn serialize_spread(ctx: &mut TsEsTreeBuilder, expr: &Expr, span: &Span) -> NodeRef {
  let expr = serialize_expr(ctx, expr);
  ctx.write_spread(span, expr)
}

fn serialize_ident_name(ctx: &mut TsEsTreeBuilder, ident_name: &IdentName) -> NodeRef {
  ctx.write_identifier(&ident_name.span, ident_name.sym.as_str(), false, None, None)
}

fn serialize_prop_name(ctx: &mut TsEsTreeBuilder, prop_name: &PropName) -> NodeRef {
  match prop_name {
    ...
    PropName::BigInt(big_int) => serialize_lit(ctx, &Lit::BigInt(big_int.clone())),
  }
}
...
fn serialize_class_member(ctx: &mut TsEsTreeBuilder, member: &ClassMember) -> Option<NodeRef> {
  match member {
    ClassMember::Constructor(node) => {
      ...
      let params = node
        ...
        .map(|param| match param {
          ...
          ParamOrTsParamProp::Param(param) => serialize_pat(ctx, &param.pat, None),
        })
        .collect::<Vec<_>>();
      ...
      let value = ctx.write_fn_expr(&node.span, false, false, None, None, params, None, body);
    ...
    ClassMember::TsIndexSignature(node) => Some(serialize_ts_index_sig(ctx, node)),
...
fn serialize_class_method(
  ...
  let type_params = maybe_serialize_ts_type_param_decl(ctx, &function.type_params);
  let params = function
    .params
    .iter()
    .map(|param| {
...
fn serialize_binding_ident(
  ...
  let type_ann = maybe_serialize_ts_type_ann(ctx, &node.type_ann);
  ctx.write_identifier(&node.span, &node.sym, node.optional, type_ann, decorators)
}

fn serialize_ts_param_inst(ctx: &mut TsEsTreeBuilder, node: &TsTypeParamInstantiation) -> NodeRef {
  let params = node
    .params
    .iter()
  ...
  match node {
    ...
    TsType::TsFnOrConstructorType(node) => match node {
      TsFnOrConstructorType::TsFnType(node) => {
        ...
        ctx.write_ts_fn_type(&node.span, type_params, param_ids, Some(return_type))
      }
    ...
    TsType::TsTypeQuery(node) => {
      let expr_name = match &node.expr_name {
        TsTypeQueryExpr::TsEntityName(entity) => serialize_ts_entity_name(ctx, entity),
      ...
      let children = node
        ...
        .map(|elem| {
          if let Some(label) = &elem.label {
            ...
            ctx.write_ts_named_tuple_member(&elem.span, label, type_id, optional)
          } else {
    ...
    TsType::TsTypePredicate(node) => {
      let param_name = match &node.param_name {
        TsThisTypeOrIdent::TsThisType(node) => ctx.write_ts_this_type(&node.span),
        TsThisTypeOrIdent::Ident(ident) => serialize_ident(ctx, ident, None),
      };

      let type_ann = maybe_serialize_ts_type_ann(ctx, &node.type_ann);

      ctx.write_ts_type_predicate(&node.span, node.asserts, param_name, type_ann)
    }
...
fn serialize_ts_lit_type(ctx: &mut TsEsTreeBuilder, node: &TsLitType) -> NodeRef {
  match &node.lit {
    TsLit::Number(lit) => {
...
fn serialize_ts_entity_name(ctx: &mut TsEsTreeBuilder, node: &TsEntityName) -> NodeRef {
  match &node {
...
fn serialize_ts_type_ann(ctx: &mut TsEsTreeBuilder, node: &TsTypeAnn) -> NodeRef {
...
fn serialize_ts_type_param(ctx: &mut TsEsTreeBuilder, node: &TsTypeParam) -> NodeRef {
...
fn serialize_ts_type_param_decl(ctx: &mut TsEsTreeBuilder, node: &TsTypeParamDecl) -> NodeRef {
  let params = node
    .params
    .iter()
...
fn serialize_ts_fn_param(ctx: &mut TsEsTreeBuilder, node: &TsFnParam) -> NodeRef {
  match node {
    ...
    TsFnParam::Object(pat) => serialize_pat(ctx, &Pat::Object(pat.clone()), None),
  }
}
