...
use deno_ast::{ModuleSpecifier, ParsedSource, SourceTextInfo};
use deno_core::{
...
use deno_path_util::{resolve_url_or_path, url_from_file_path};
use deno_runtime::{WorkerExecutionMode, tokio_util, worker::MainWorker};
use tokio::sync::{mpsc, oneshot};

use crate::{
  args::{DenoSubcommand, Flags, LintFlags, PermissionFlags},
  factory::CliFactory,
  ops::lint::LintPluginContainer,
  tools::lint::serialize_ast_to_buffer,
  util::text_encoding::Utf16Map,
};
...
async fn create_plugin_runner_inner(
  ...
  let main_module = resolve_url_or_path("./$deno$lint.mts", cli_options.initial_cwd()).unwrap();
  ...
  let (install_plugins_fn, run_plugins_for_file_fn) = {
    ...
    let module_exports: v8::Local<v8::Object> = v8::Local::new(scope, obj).try_into().unwrap();
    ...
    let install_plugins_fn_val = module_exports
      ...
      .unwrap();
    let install_plugins_fn: v8::Local<v8::Function> = install_plugins_fn_val.try_into().unwrap();

    let run_plugins_for_file_fn_name = RUN_PLUGINS_FOR_FILE.v8_string(scope).unwrap();
...
impl PluginHost {
  ...
  fn run_plugins(
    ...
    let ast_buf = v8::ArrayBuffer::with_backing_store(scope, &store.make_shared());
    let ast_bin_v8: v8::Local<v8::Value> =
      v8::Uint8Array::new(scope, ast_buf, 0, ast_buf.byte_length())
        .unwrap()
        .into();
    let run_plugins_for_file = v8::Local::new(scope, &*self.run_plugins_for_file_fn);
    let undefined = v8::undefined(scope);

    let _run_plugins_result = {
      v8::tc_scope!(tc_scope, scope);
      let _run_plugins_result =
        run_plugins_for_file.call(tc_scope, undefined.into(), &[file_name_v8, ast_bin_v8]);
  ...
  async fn load_plugins(
    ...
    deno_core::scope!(scope, &mut self.worker.js_runtime);
    let install_plugins_local = v8::Local::new(scope, &*self.install_plugins_fn.clone());
    let exclude_v8: v8::Local<v8::Value> = exclude.map_or(v8::null(scope).into(), |v| {
      let elems = v
        .iter()
    ...
    let plugins_info_result = {
      v8::tc_scope!(tc_scope, scope);
      let plugins_info_result = install_plugins_local.call(tc_scope, undefined.into(), args);
      if let Some(exception) = tc_scope.exception() {
        let error = JsError::from_v8_exception(tc_scope, exception);
        return Err(error.into());
      }
    ...
    let infos: Vec<PluginInfo> = deno_core::serde_v8::from_v8(scope, plugins_info)?;
