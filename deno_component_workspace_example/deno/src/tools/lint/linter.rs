...
use deno_ast::{MediaType, ModuleSpecifier, ParsedSource, SourceTextInfo};
use deno_core::{anyhow::Context, error::AnyError, futures::FutureExt as _, parking_lot::Mutex};
use deno_graph::ModuleGraph;
use deno_lint::{
  diagnostic::LintDiagnostic,
  linter::{
    ExternalLinterCb, ExternalLinterResult, LintConfig as DenoLintConfig, LintFileOptions,
    Linter as DenoLintLinter, LinterOptions,
  },
};
...
use super::{
  ConfiguredRules, plugins,
  plugins::PluginHostProxy,
  rules::{FileOrPackageLintRule, PackageLintRule},
};
use crate::{
  sys::CliSys,
  util::{fs::specifier_from_file_path, text_encoding::Utf16Map},
};
...
impl CliLinter {
  ...
  pub fn lint_with_ast(
    ...
    let external_linter_container =
      ExternalLinterContainer::new(self.maybe_plugin_runner.clone(), Some(token));
...
fn apply_lint_fixes_and_relint(
  ...
  let (source, diagnostics) = match lint_with_text(new_text) {
    ...
    Err(err) => {
      ...
      while let Some(last_diagnostic) = diagnostics.pop() {
        ...
        if success {
          ...
          for change in &lint_fix.changes {
            let utf8_start = (change.range.start - text_info.range().start) as u32;
      ...
      return Err(err).context("A lint fix caused a syntax error. This is a bug in a lint rule.");
    }
  };
...
fn apply_lint_fixes(text_info: &SourceTextInfo, diagnostics: &[LintDiagnostic]) -> Option<String> {
...
fn filter_overlapping_text_changes(
  ...
  let mut filtered: Vec<deno_ast::TextChange> = Vec::with_capacity(text_changes.len());
  for change in text_changes.into_iter() {
    let overlaps_last = filtered
      .last()
      .map(|prev| change.range.start <= prev.range.end)
      .unwrap_or(false);
    let is_duplicate_import = change.new_text.trim_start().starts_with("import ")
...
fn run_plugins(
  ...
  let fut = async move {
    ...
    let serialized_ast = plugin_runner.serialize_ast(&parsed_source, &utf16_map)?;
  ...
  pub fn new(
    ...
    if let Some(plugin_runner) = maybe_plugin_runner {
      ...
      let cb = Arc::new(move |parsed_source: ParsedSource| {
        ...
        let file_path = match deno_path_util::url_to_file_path(parsed_source.specifier()) {
          Ok(path) => path,
          Err(err) => {
