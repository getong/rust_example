...
use deno_ast::{ModuleSpecifier, ParsedSource};
use deno_config::{
  deno_json::LintRulesConfig,
  glob::{FileCollector, FilePatterns},
  workspace::{WorkspaceDirectory, WorkspaceDirectoryRc},
};
use deno_core::{
  anyhow::{anyhow, bail},
  error::AnyError,
  ...
  parking_lot::Mutex,
  serde_json,
  unsync::future::{LocalFutureExt, SharedLocal},
};
...
use log::debug;
use reporters::{LintReporter, create_reporter};

use crate::{
  args::{CliOptions, Flags, LintFlags, LintOptions, WorkspaceLintOptions},
  cache::{CacheDBHash, Caches, IncrementalCache},
  colors,
  factory::CliFactory,
  graph_util::{CreatePublishGraphOptions, ModuleGraphCreator},
  sys::CliSys,
  tools::fmt::run_parallelized,
  util::{
    display, file_watcher, file_watcher::WatcherCommunicator, fs::canonicalize_path,
    path::is_script_ext, sync::AtomicFlag,
  },
};
...
pub use linter::{CliLinter, CliLinterOptions};
pub use plugins::{PluginHostProxy, PluginLogger, create_runner_and_load_plugins};
pub use rules::{ConfiguredRules, LintRuleProvider, collect_no_slow_type_diagnostics};

const JSON_SCHEMA_VERSION: u8 = 1;

static STDIN_FILE_NAME: &str = "$deno$stdin.mts";

pub async fn lint(flags: Arc<Flags>, lint_flags: LintFlags) -> Result<(), AnyError> {
  ...
  let workspace_lint_options = cli_options.resolve_workspace_lint_options(&lint_flags)?;
  let success = if is_stdin {
    ...
    let paths_with_options_batches = resolve_paths_with_options_batches(cli_options, &lint_flags)?;
...
async fn lint_with_watch(flags: Arc<Flags>, lint_flags: LintFlags) -> Result<(), AnyError> {
...
fn resolve_paths_with_options_batches(
  ...
  let members_lint_options = cli_options.resolve_lint_options_for_members(lint_flags)?;
  let mut paths_with_options_batches = Vec::with_capacity(members_lint_options.len());
...
impl WorkspaceLinter {
  pub fn new(
    ...
    let reporter_lock = Arc::new(Mutex::new(create_reporter(workspace_options.reporter_kind)));
    Self {
      caches,
      lint_rule_provider,
  ...
  pub async fn lint_files(
    ...
    if !plugin_specifiers.is_empty() {
      ...
      let runner =
        plugins::create_runner_and_load_plugins(plugin_specifiers, logger, exclude).await?;
    ...
    let linter = Arc::new(CliLinter::new(CliLinterOptions {
      ...
      deno_lint_config: resolve_lint_config(&self.compiler_options_resolver, member_dir.dir_url())?,
  ...
  fn run_package_rules(
    ...
    let workspace_module_graph_future = self.workspace_module_graph.as_ref().unwrap().clone();
    ...
    let fut = async move {
      ...
      let export_urls = publish_config.config_file.resolve_export_value_urls()?;
...
fn collect_lint_files(cli_options: &CliOptions, files: FilePatterns) -> Vec<PathBuf> {
  FileCollector::new(|e| {
    is_script_ext(e.path) || (e.path.extension().is_none() && cli_options.ext_flag().is_some())
  })
  .ignore_git_folder()
  .ignore_node_modules()
  .use_gitignore()
...
fn lint_stdin(
  ...
  let lint_config = start_dir.to_lint_config(FilePatterns::new_with_base(start_dir.dir_path()))?;
  let deno_lint_config = resolve_lint_config(compiler_options_resolver, start_dir.dir_url())?;
  let lint_options = LintOptions::resolve(lint_config, &lint_flags)?;
  let configured_rules =
    lint_rule_provider.resolve_lint_rules_err_empty(lint_options.rules, Some(start_dir))?;
...
fn resolve_lint_config(
  ...
  let transpile_options = &compiler_options_resolver
    ...
    .transpile_options()?
    .transpile;
  let jsx_classic_options = transpile_options.jsx.as_ref().and_then(|jsx| match jsx {
  ...
  Ok(deno_lint::linter::LintConfig {
    ...
    default_jsx_fragment_factory: jsx_classic_options.map(|o| o.fragment_factory.clone()),
  })
}
...
mod tests {
  ...
  fn get_all_rules() -> Vec<String> {
    let rule_provider = LintRuleProvider::new(None);
    let configured_rules = rule_provider.resolve_lint_rules(Default::default(), None);
  ...
  fn all_lint_rules_are_listed_in_schema_file() {
    ...
    let rules_schema_path = util::root_path().join("cli/schemas/lint-rules.v1.json");
