...
use deno_ast::{MediaType, TextLines, swc::common::comments::CommentKind};
...
pub fn lex_comments(source: &str, media_type: MediaType) -> CoverageComments {
  ...
  for token in deno_ast::lex(source, media_type) {
    match token.inner {
      ...
      deno_ast::TokenOrComment::Comment { kind, text } => comments.push(CoverageComment {
        kind,
        text,
        range: token.range,
      }),
      ...
      if prefix == COVERAGE_IGNORE_START_DIRECTIVE {
        if log::log_enabled!(log::Level::Warn) && depth > 0 {
          let unterminated_loc =
            text_lines.line_and_column_display(current_range.as_ref().unwrap().start);
          let loc = text_lines.line_and_column_display(comment.range.start);
          log::warn!(
            "WARNING: Nested {} comment at {}:{}:{}. A previous {} comment at {}:{}:{} is \
             unterminated.",
        ...
        depth -= 1;
        if depth == 0 {
          let start_line_index = text_lines.line_index(current_range.take().unwrap().start);
...
pub fn has_file_ignore_directive(comments: &CoverageComments) -> bool {
  ...
  // three cases:
  // 1. No comments. There are no comments in the file, and therefore no coverage directives.
  // 2. No code. There is at least one comment in the file, but no code. We can try to parse this as
  //    a file ignore directive.
  // 3. Comments and code. There are comments and code in the file. We need to check if the first
  //    comment comes before the first line of code. If it does, we can try and parse it as a file
  //    ignore directive. Otherwise, there is no valid file ignore directive.
  ...
  match (first_comment, first_module_item) {
    (None, _) => false,
    (Some(first_comment), None) => is_ignore_comment(COVERAGE_IGNORE_FILE_DIRECTIVE, first_comment),
...
fn is_ignore_comment(ignore_diagnostic_directive: &str, comment: &CoverageComment) -> bool {
  if comment.kind != CommentKind::Line {
    return false;
  }
...
mod tests {
  ...
  use deno_ast::{MediaType, TextLines};
  ...
  mod coverage_ignore_next {
    ...
    fn test_parse_next_ignore_comments() {
      ...
      "#;
      let (comments, text_lines) = parse_with_text_lines(source_code);
      let line_directives = parse_next_ignore_directives(&comments.comments, &text_lines);
  ...
  mod coverage_ignore_file {
    ...
    fn test_parse_global_ignore_directives_with_explanation() {
      let comments = parse("// deno-coverage-ignore-file -- reason for ignoring");
      assert!(has_file_ignore_directive(&comments));
    }

    #[test]
    fn test_parse_global_ignore_directives_argument_and_explanation() {
      let comments = parse("// deno-coverage-ignore-file foo -- reason for ignoring");
      assert!(has_file_ignore_directive(&comments));
    }
