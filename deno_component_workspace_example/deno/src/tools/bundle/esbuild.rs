...
use crate::{
  cache::DenoDir,
  npm::{CliNpmCache, CliNpmCacheHttpClient, CliNpmRegistryInfoProvider},
  sys::CliSys,
};
...
pub async fn ensure_esbuild(
  ...
  let nv = PackageNv::from_str(&format!("{}@{}", pkg_name, ESBUILD_VERSION)).unwrap();
  ...
  if let Some(dist) = &version_info.dist {
    ...
    let package_folder = npm_cache.package_folder_for_nv_and_url(&nv, registry_url);
