...
use deno_ast::{EmitOptions, MediaType, ModuleKind, ModuleSpecifier};
...
use deno_graph::{ModuleErrorKind, Position};
use deno_path_util::resolve_url_or_path;
use deno_resolver::{
  cache::ParsedSourceCache,
  graph::{ResolveWithGraphError, ResolveWithGraphOptions},
  loader::{
    LoadCodeSourceError, LoadCodeSourceErrorKind, LoadPreparedModuleErrorKind, LoadedModuleOrAsset,
    LoadedModuleSource, RequestedModuleType,
  },
  npm::managed::ResolvePkgFolderFromDenoModuleError,
};
use deno_runtime::deno_permissions::PermissionsContainer;
use deno_semver::npm::NpmPackageReqReference;
use esbuild_client::{EsbuildFlagsBuilder, EsbuildService, protocol, protocol::BuildResponse};
use indexmap::{IndexMap, IndexSet};
use node_resolver::{
  NodeResolutionKind, ResolutionMode,
  errors::{PackageNotFoundError, PackageSubpathResolveError},
};
...
use crate::{
  ...
  graph_container::{MainModuleGraphContainer, ModuleGraphContainer, ModuleGraphUpdatePermit},
  module_loader::{
    CliDenoResolverModuleLoader, CliEmitter, ModuleLoadPreparer, PrepareModuleLoadOptions,
  },
  node::CliNodeResolver,
  npm::CliNpmResolver,
  resolver::{CliCjsTracker, CliResolver},
  sys::CliSys,
  tools::bundle::externals::ExternalsMatcher,
  util::file_watcher::WatcherRestartMode,
};
...
pub async fn prepare_inputs(
  ...
  let resolved_entrypoints = resolve_entrypoints(resolver, init_cwd, &bundle_flags.entrypoints)?;
  ...
  if html_paths.is_empty() {
    ...
    let roots = resolve_roots(resolved_entrypoints, sys, npm_resolver, node_resolver);
    ...
    *plugin_handler.resolved_roots.write() = Arc::new(fully_resolved_roots);
    ...
    for html_path in &html_paths {
      ...
      let virtual_module_path = deno_path_util::url_from_file_path(&entry.virtual_module_path)?;
    ...
    let roots = resolve_roots(script_entry_urls, sys, npm_resolver, node_resolver);
...
pub async fn bundle_init(
  ...
  let module_graph_container = factory.main_module_graph_container().await?.clone();
...
pub async fn bundle(mut flags: Arc<Flags>, bundle_flags: BundleFlags) -> Result<(), AnyError> {
  {
    let flags_mut = Arc::make_mut(&mut flags);
    flags_mut.unstable_config.sloppy_imports = true;
  }
...
fn metafile_from_response(response: &BuildResponse) -> Result<esbuild_client::Metafile, AnyError> {
  Ok(serde_json::from_str::<esbuild_client::Metafile>(
    response
      .metafile
      .as_deref()
      .ok_or_else(|| deno_core::anyhow::anyhow!("expected a metafile to be present"))?,
  )?)
}
...
async fn bundle_watch(
  ...
  let mut print_config =
    crate::util::file_watcher::PrintConfig::new_with_banner("Watcher", "Bundle", true);
      ...
      Ok(async move {
        ...
        if response.errors.is_empty() {
          ...
          *current_roots.borrow_mut() = new_watched.clone();
          ...
          let _ = watcher_communicator.watch_paths(current_roots.borrow().clone());
        }
...
impl EsbuildBundler {
  ...
  async fn reload_specifiers(&mut self, changed_paths: &[PathBuf]) -> Result<(), AnyError> {
    self.reload_html_entrypoints(changed_paths)?;
    self.plugin_handler.reload_specifiers(changed_paths).await?;
    Ok(())
  }

  fn reload_html_entrypoints(&mut self, changed_paths: &[PathBuf]) -> Result<(), AnyError> {
    let BundlerInput::EntrypointsWithHtml { html_pages, .. } = &mut self.input else {
      return Ok(());
    };
    ...
    for page in html_pages.iter_mut() {
      ...
      let virtual_module_url =
        deno_path_util::url_from_file_path(&updated.virtual_module_path)?.to_string();
...
fn message_to_error(message: &esbuild_client::protocol::Message, current_dir: &Path) -> AnyError {
...
fn replace_require_shim(contents: &str, minified: bool) -> String {
  if minified {
    ...
    re.replace(contents, |c: &regex::Captures<'_>| {
      ...
      format!(
        "import{{createRequire as __deno_internal_createRequire}} from \"node:module\";var \
         {var_name}=__deno_internal_createRequire(import.meta.url);"
      )
    })
    .into_owned()
  } else {
...
fn format_location(location: &esbuild_client::protocol::Location, current_dir: &Path) -> String {
  let url = deno_path_util::resolve_url_or_path(location.file.as_str(), current_dir)
...
fn format_note(note: &esbuild_client::protocol::Note, current_dir: &Path) -> String {
  format!(
    "{}: {}{}",
...
fn format_message(message: &esbuild_client::protocol::Message, current_dir: &Path) -> String {
  format!(
    "{}{}{}{}",
    message.text,
...
enum BundleError {
  ...
  #[error(transparent)]
  PackageReqReferenceParse(#[from] deno_semver::package::PackageReqReferenceParseError),
  #[allow(dead_code)]
  #[error("Http cache error")]
  HttpCache,
}

fn requested_type_from_map(map: &IndexMap<String, String>) -> RequestedModuleType<'_> {
...
impl From<protocol::ImportKind> for PluginImportKind {
  fn from(kind: protocol::ImportKind) -> Self {
    match kind {
      ...
      protocol::ImportKind::ImportStatement => PluginImportKind::ImportStatement,
...
impl esbuild_client::PluginHandler for DenoPluginHandler {
  async fn on_resolve(
    ...
    Ok(result.map(|r| {
      ...
      // output file this import will end up in. We may have to use the metafile and rewrite at the
      // end
...
fn import_kind_to_resolution_mode(kind: esbuild_client::protocol::ImportKind) -> ResolutionMode {
  match kind {
    ...
    protocol::ImportKind::RequireCall | protocol::ImportKind::RequireResolve => {
...
pub enum BundleLoadError {
  ...
  EmitParsedSourceHelperError(#[from] deno_resolver::emit::EmitParsedSourceHelperError),
}
...
fn maybe_ignorable_resolution_error(error: &ResolveWithGraphError) -> Option<String> {
  if let deno_resolver::graph::ResolveWithGraphErrorKind::Resolve(e) = error.as_kind()
    && let deno_resolver::DenoResolveErrorKind::Node(node_err) = e.as_kind()
    && let node_resolver::errors::NodeResolveErrorKind::PackageResolve(pkg_err) = node_err.as_kind()
    && let node_resolver::errors::PackageResolveErrorKind::PackageFolderResolve(pkg_folder_err) =
      pkg_err.as_kind()
    ...
    ) = pkg_folder_err.as_kind()
  {
    ...
    && let deno_graph::source::ResolveError::Other(other_err) = resolve_error.deref()
    && let Some(import_map_err) = other_err
      .get_ref()
      .downcast_ref::<import_map::ImportMapError>()
    && let import_map::ImportMapErrorKind::UnmappedBareSpecifier(..) = import_map_err.as_kind()
  {
...
impl DenoPluginHandler {
  async fn reload_specifiers(&self, specifiers: &[PathBuf]) -> Result<(), AnyError> {
    let mut graph_permit = self.module_graph_container.acquire_update_permit().await;
  ...
  fn bundle_resolve(
    ...
    let mut referrer = resolve_url_or_path(importer.unwrap_or(""), resolve_dir_path)
      .unwrap_or_else(|_| Url::from_directory_path(std::env::current_dir().unwrap()).unwrap());
    if referrer.scheme() == "file" {
      let pth = referrer.to_file_path().unwrap();
      if (pth.is_dir()) && !pth.ends_with(std::path::MAIN_SEPARATOR_STR) {
        referrer.set_path(&format!("{}{}", referrer.path(), std::path::MAIN_SEPARATOR));
      }
    }
  ...
  async fn bundle_load(
    ...
    let (specifier, media_type) = if let RequestedModuleType::Bytes = requested_type {
      ...
      self.specifier_and_type_from_graph(&specifier)?
    {
      ...
      let (media_type, _) =
        deno_media_type::resolve_media_type_and_charset_from_content_type(&specifier, None);
      ...
      Err(e) => match e.as_kind() {
        LoadCodeSourceErrorKind::LoadUnpreparedModule(_) => {
          let file = self
            ...
            .await?;
          let media_type =
            MediaType::from_specifier_and_headers(&specifier, file.maybe_headers.as_ref());
  ...
  async fn create_module_response(
    ...
    match requested_type {
      ...
      Some(RequestedModuleType::Other(_) | RequestedModuleType::None) | None => {}
    }
    if matches!(
      media_type,
  ...
  async fn maybe_transpile(
    ...
    let parsed_source =
      self
        .parsed_source_cache
        .remove_or_parse_module(specifier, media_type, source.clone())?;
    let is_cjs = if let Some(is_known_script) = is_known_script {
      self
        .cjs_tracker
        .is_cjs_with_known_is_script(specifier, media_type, is_known_script)?
    } else {
      self.cjs_tracker.is_maybe_cjs(specifier, media_type)? && parsed_source.compute_is_script()
    };
  ...
  > {
    ...
    let (specifier, media_type, loader) = match module {
      ...
      deno_graph::Module::Npm(_) => {
        let req_ref = NpmPackageReqReference::from_specifier(specifier).unwrap();
        ...
        let (media_type, _charset) =
          deno_media_type::resolve_media_type_and_charset_from_content_type(&url, None);
...
fn file_path_or_url(url: Url) -> Result<String, deno_path_util::UrlToFilePathError> {
  if url.scheme() == "file" {
    Ok(
...
fn media_type_to_loader(media_type: deno_media_type::MediaType) -> esbuild_client::BuiltinLoader {
...
fn resolve_url_or_path_absolute(specifier: &str, current_dir: &Path) -> Result<Url, AnyError> {
...
fn resolve_roots(
  ...
  for url in entrypoints {
    let root = match NpmPackageReqReference::from_specifier(&url) {
      Ok(v) => {
        let referrer =
          ModuleSpecifier::from_directory_path(sys.env_current_dir().unwrap()).unwrap();
...
async fn ensure_esbuild_downloaded(factory: &CliFactory) -> Result<PathBuf, AnyError> {
...
fn configure_esbuild_flags(bundle_flags: &BundleFlags, is_html: bool) -> Vec<String> {
...
fn esbuild_resolve_error_path(error: &esbuild_client::protocol::Message) -> Option<String> {
...
fn handle_esbuild_errors_and_warnings(
  ...
  for error in &response.errors {
    if let Some(path) = esbuild_resolve_error_path(error)
      && let Some(deferred_resolve_error) = deferred_resolve_errors.iter().find(|e| e.path == path)
    {
...
impl<'a> From<&'a esbuild_client::protocol::BuildOutputFile> for OutputFile<'a> {
...
pub fn collect_output_files<'a>(
  ...
  } = input
  {
    let outdir = outdir.ok_or_else(|| {
      deno_core::anyhow::anyhow!("--outdir is required when bundling HTML entrypoints",)
    })?;
...
pub fn process_result(
  ...
  let output_files = collect_output_files(response.output_files.as_deref(), cwd, input, outdir)?;
  let mut exists_cache = std::collections::HashSet::new();
  let mut output_infos = Vec::new();
  for file in output_files.iter() {
    let processed_contents = maybe_process_contents(file, should_replace_require_shim, minified)?;
...
fn print_finished_message(
  ...
  for info in output_infos {
    output.push_str(&format!(
      "\n  {} {}",
      if info.is_js {
        deno_terminal::colors::cyan(format!("{:<longest$}", info.relative_path.display()))
      } else {
        deno_terminal::colors::magenta(format!("{:<longest$}", info.relative_path.display()))
      },
      deno_terminal::colors::gray(crate::display::human_size(info.size as f64,))
    ));
  }
  output.push('\n');
  log::info!("{}", output);
