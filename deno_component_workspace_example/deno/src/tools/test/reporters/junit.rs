...
impl TestReporter for JunitTestReporter {
  ...
  fn report_step_register(&mut self, description: &TestStepDescription) {
    self.test_name_tree.add_node(description.clone().into());
    let test_case_name = self.test_name_tree.construct_full_test_name(description.id);

    let mut case =
      quick_junit::TestCase::new(test_case_name, quick_junit::TestCaseStatus::skipped());
    case.classname = Some(to_relative_path_or_remote_url(
      &self.cwd,
      &description.location.file_name,
    ));
  ...
  fn report_step_result(
    ...
    if let Some(case) = self.cases.get_mut(&description.id) {
      case.status = Self::convert_step_status(result, &self.failure_format_options);
      case.set_time(Duration::from_millis(elapsed));
    }
  }
  ...
  fn report_completed(&mut self) {
    // TODO(mmastrac): This reporter does not handle stdout/stderr yet, and when we do, we may need
    // to redirect pre-and-post-test output somewhere.
  }
  ...
  fn flush_report(
    ...
    if self.output_path == "-" {
      ...
      report
        .serialize(file)
        .with_context(|| format!("Failed to write JUnit report to {}", self.output_path))?;
    }
...
mod tests {
  ...
  fn escapes_short_failure_message() {
    let jserror = JsError {
      exception_message: "Uncaught Error: \x1b[31mtest error\x1b[0m".to_string(),
    ...
    let step_result = TestStepResult::Failed(TestFailure::JsError(Box::new(jserror)));
