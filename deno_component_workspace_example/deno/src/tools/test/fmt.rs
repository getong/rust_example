...
use deno_core::stats::{
  RuntimeActivity, RuntimeActivityDiff, RuntimeActivityTrace, RuntimeActivityType,
};
...
//   - if stack trace consists of mixed user and internal code, the frames below the first user code
//     frame are filtered out
//   - if stack trace consists only of internal code it is preserved as is
pub fn format_test_error(js_error: &JsError, options: &TestFailureFormatOptions) -> String {
...
pub fn format_sanitizer_diff(diff: RuntimeActivityDiff) -> (Vec<String>, Vec<String>) {
  ...
  for ((item_type, item_name, trace), count) in accum.into_iter() {
    if item_type == RuntimeActivityType::Resource {
      ...
      let value = if appeared {
        ...
        format!(
          "{name} was {action1} before the test started, but was {action2} during the test. Do \
           not close resources in a test that were not created during that test."
        )
      };
      output.push(value);
      ...
      let phrase = if appeared {
        ...
        "started before the test, but completed during the test. Async operations should not \
         complete in a test if they were not started in that test"
      };
      let mut value = if let Some([operation, hint]) = OP_DETAILS.get(&item_name) {
        format!(
          "{count_str} async operation{plural} to {operation} {tense} {phrase}. This is often \
           caused by not {hint}."
        )
      } else {
        format!("{count_str} async call{plural} to {item_name} {tense} {phrase}.")
      };
      ...
      let phrase = if appeared {
        ...
        "started before the test, but completed during the test. Intervals and timers should not \
         complete in a test if they were not started in that test"
      };
      let mut value = format!(
        "{count_str} timer{plural} {tense} {phrase}. This is often caused by not calling \
         `clearTimeout`."
      );
      value += &if let Some(trace) = trace {
        format!(" The operation {tense} started here:\n{trace}")
      ...
      let mut value = format!(
        "{count_str} interval{plural} {tense} {phrase}. This is often caused by not calling \
         `clearInterval`."
      );
      value += &if let Some(trace) = trace {
        format!(" The operation {tense} started here:\n{trace}")
...
fn format_sanitizer_accum_item(
  ...
  match activity {
    RuntimeActivity::AsyncOp(_, trace, name) => (activity_type, name.into(), trace),
...
fn pretty_resource_name(name: &str) -> (Cow<'static, str>, &'static str, &'static str) {
...
fn resource_close_hint(name: &str) -> &'static str {
  match name {
    ...
    "fetchResponse" => {
      "Consume or close the response body `ReadableStream`, e.g `await resp.text()` or `await \
       resp.body.cancel()`."
    }
    ...
    "httpStream" => {
      "Close the inbound HTTP request by responding with `e.respondWith()` or closing the HTTP \
       connection."
    }
    ...
    "textDecoder" => {
      "Close the text decoder by calling `textDecoder.decode('')` or `await \
       textDecoderStream.readable.cancel()`."
    }
    ...
    "childStdout" => {
      "Close the child process stdout by calling `proc.stdout.close()` or `await \
       child.stdout.cancel()`."
    }
    "childStderr" => {
      "Close the child process stderr by calling `proc.stderr.close()` or `await \
       child.stderr.cancel()`."
    }
...
mod tests {
  ...
  leak_format_test!(
    ...
    " - An async call to op_unknown was started in this test, but never completed.\nTo get more \
     details where leaks occurred, run again with the --trace-leaks flag.\n"
  );
}
