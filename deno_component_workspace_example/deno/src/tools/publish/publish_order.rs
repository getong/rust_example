...
impl PublishOrderGraph {
  ...
  pub fn ensure_no_pending(&self) -> Result<(), AnyError> {
    ...
    if self.in_degree.is_empty() {
      ...
      let mut cycle = identify_cycle(pkg_names[0], HashSet::new(), &self.packages).unwrap();
      cycle.reverse();
      bail!(
  ...
  fn compute_depth(&self, package_name: &String, mut visited: HashSet<String>) -> usize {
...
fn build_pkg_deps(
  ...
  for (root_dir_url, pkg_config) in &roots {
    ...
    while let Some(specifier) = pending.pop_front() {
      ...
      let mut dep_specifiers = Vec::with_capacity(module.dependencies.len() + 1);
      ...
      for specifier in dep_specifiers {
        ...
        if specifier.as_str().starts_with(root_dir_url.as_str()) {
          ...
          let found_root = roots
            .iter()
            .find(|(dir_url, _)| specifier.as_str().starts_with(dir_url.as_str()));
