...
use crate::{args::CliOptions, sys::CliSys};
...
fn valid_char(c: char) -> Option<PackagePathValidationError> {
  match c {
    ...
    '\\' | ':' => Some(PackagePathValidationError::InvalidWindowsPathSeparatorChar(
      c,
    )),
    '<' | '>' | '"' | '|' | '?' | '*' => Some(PackagePathValidationError::InvalidWindowsChar(c)),
    ' ' | '\t' | '\n' | '\r' => Some(PackagePathValidationError::InvalidWhitespace(c)),
...
pub enum PackagePathValidationError {
  ...
  #[error("package path must not contain windows reserved names like 'CON' or 'PRN' (found '{0}')")]
  ReservedName(String),

  #[error("path segment must not end in a dot (found '{0}')")]
  TrailingDot(String),

  #[error("package path must not contain windows path separators like '\\' or ':' (found '{0}')")]
  InvalidWindowsPathSeparatorChar(char),

  #[error(
    "package path must not contain windows reserved characters like '<', '>', '\"', '|', '?', or \
     '*' (found '{0}')"
  )]
...
pub fn collect_publish_paths(
  ...
  let publish_paths = collect_paths(opts.cli_options, diagnostics_collector, opts.file_patterns);
  ...
  for path in publish_paths.into_iter().chain(force_include_paths) {
    ...
    let relative_path = relative_path
      ...
      .fold("".to_string(), |mut path, component| {
        path.push('/');
        match component {
          std::path::Component::Normal(normal) => path.push_str(&normal.to_string_lossy()),
    ...
    match PackagePath::new(relative_path.clone()) {
      Ok(package_path) => {
        if !paths.insert(package_path) {
          diagnostics_collector
            .to_owned()
            .push(PublishDiagnostic::DuplicatePath {
              path: path.to_path_buf(),
            });
        }
      }
