...
use deno_ast::{ModuleSpecifier, SourceTextInfo};
use deno_config::{
  deno_json::ConfigFile,
  workspace::{JsrPackageConfig, Workspace},
};
use deno_core::{
  anyhow::{Context, bail},
  error::AnyError,
  futures::{FutureExt, StreamExt, future::LocalBoxFuture, stream::FuturesUnordered},
  serde_json,
...
use self::{
  diagnostics::{
    PublishDiagnostic, PublishDiagnosticsCollector, RelativePackageImportDiagnosticReferrer,
  },
  graph::GraphDiagnosticsCollector,
  module_content::ModuleContentProvider,
  paths::CollectedPublishPath,
  tar::PublishableTarball,
};
use crate::{
  args::{CliOptions, Flags, PublishFlags, jsr_api_url, jsr_url},
  factory::CliFactory,
  graph_util::{CreatePublishGraphOptions, ModuleGraphCreator},
  http_util::HttpClient,
  registry,
  tools::lint::collect_no_slow_type_diagnostics,
  type_checker::{CheckOptions, TypeChecker},
  util::display::human_size,
};
...
use auth::{AuthMethod, get_auth_method};
use publish_order::PublishOrderGraph;
use unfurl::SpecifierUnfurler;

pub async fn publish(flags: Arc<Flags>, publish_flags: PublishFlags) -> Result<(), AnyError> {
  let cli_factory = CliFactory::from_flags(flags);

  let auth_method = get_auth_method(publish_flags.token, publish_flags.dry_run)?;
  ...
  if let Some(version) = &publish_flags.set_version {
    if publish_configs.len() > 1 {
      bail!(
        "Cannot use --set-version when publishing a workspace. Change your cwd to an individual \
         package instead."
      );
    }
  ...
  if std::env::var("DENO_TESTING_DISABLE_GIT_CHECK")
    ...
    && let Some(dirty_text) = check_if_git_repo_dirty(cli_options.initial_cwd()).await
  {
    log::error!("\nUncommitted changes:\n\n{}\n", dirty_text);
...
impl PublishPreparer {
  ...
  pub async fn prepare_packages_for_publishing(
    ...
    let graph = self
      .build_and_check_graph_for_publish(allow_slow_types, diagnostics_collector, &publish_configs)
      .await?;

    let mut package_by_name = HashMap::with_capacity(publish_configs.len());
    let publish_order_graph = publish_order::build_publish_order_graph(&graph, &publish_configs)?;
  ...
  async fn build_and_check_graph_for_publish(
    ...
    if allow_slow_types {
      ...
      Ok(Arc::new(graph))
    } else if std::env::var("DENO_INTERNAL_FAST_CHECK_OVERWRITE").as_deref() == Ok("1") {
      ...
      for module in graph.modules() {
        ...
        if let Some(module) = js.fast_check_module() {
          std::fs::write(js.specifier.to_file_path().unwrap(), module.source.as_ref())?;
        }
      }
      ...
      for package in package_configs {
        ...
        let diagnostics = collect_no_slow_type_diagnostics(&graph, &export_urls);
  ...
  async fn prepare_publish(
    ...
    let tarball = deno_core::unsync::spawn_blocking({
      ...
      move || {
        let root_specifier = ModuleSpecifier::from_directory_path(&root_dir).unwrap();
        let mut publish_paths = paths::collect_publish_paths(paths::CollectPublishPathsOptions {
          root_dir: &root_dir,
        ...
        if !has_license_field && !has_license_file(publish_paths.iter().map(|p| &p.specifier)) {
          if let Some(license_path) = resolve_license_file(&root_dir, cli_options.workspace()) {
            // force including the license file from the package or workspace root
            publish_paths.push(CollectedPublishPath {
              specifier: ModuleSpecifier::from_file_path(&license_path).unwrap(),
              relative_path: "/LICENSE".to_string(),
              maybe_content: Some(
                std::fs::read(&license_path)
                  .with_context(|| format!("failed reading '{}'.", license_path.display()))?,
              ),
              path: license_path,
            });
          } else {
...
async fn get_auth_headers(
  ...
  match auth_method {
    AuthMethod::Interactive => {
      ...
      let auth = registry::parse_response::<registry::CreateAuthorizationResponse>(response)
        .await
        .context("Failed to create interactive authorization")?;
      ...
      loop {
        ...
        let res =
          registry::parse_response::<registry::ExchangeAuthorizationResponse>(response).await;
        match res {
          Ok(res) => {
            log::info!(
              "{} {} {}",
    ...
    AuthMethod::Oidc(oidc_config) => {
      ...
      for permissions in permissions.chunks(16) {
        ...
        let url = format!(
          "{}&audience={}",
          oidc_config.url,
          percent_encoding::percent_encode(audience.as_bytes(), percent_encoding::NON_ALPHANUMERIC)
        );
        ...
        let registry::OidcTokenResponse { value } = serde_json::from_str(&text)
          .with_context(|| format!("Failed to parse OIDC token: '{}' (status {})", text, status))?;
...
async fn check_if_scope_and_package_exist(
  ...
  let response = registry::get_package(client, registry_api_url, scope, package).await?;
...
async fn perform_publish(
  ...
  ensure_scopes_and_packages_exist(http_client, registry_api_url, registry_url, &packages).await?;

  let mut authorizations =
    get_auth_headers(http_client, registry_api_url, &packages, auth_method).await?;

  assert_eq!(prepared_package_by_name.len(), authorizations.len());
  let mut futures: FuturesUnordered<LocalBoxFuture<Result<String, AnyError>>> = Default::default();
  loop {
...
async fn publish_package(
  ...
  let res = registry::parse_response::<registry::PublishingTask>(response).await;
...
fn collect_excluded_module_diagnostics(
  ...
  let graph_specifiers = graph
    ...
    .filter_map(|m| match m {
      deno_graph::Module::Js(_) | deno_graph::Module::Json(_) | deno_graph::Module::Wasm(_) => {
        Some(m.specifier())
      }
...
fn verify_version_manifest(
  ...
  for (specifier, expected) in &manifest.exports {
    let actual = package
      .exports
      .get(specifier)
      .ok_or_else(|| deno_core::anyhow::anyhow!("Export {} not found in the package", specifier))?;
    if actual != expected {
      bail!(
...
fn resolve_license_file(pkg_root_dir: &Path, workspace: &Workspace) -> Option<PathBuf> {
...
fn has_license_file<'a>(mut specifiers: impl Iterator<Item = &'a ModuleSpecifier>) -> bool {
  ...
  specifiers.any(|specifier| {
    specifier
      .path()
      .rsplit_once('/')
      .map(|(_, file)| supported_license_files.contains(file.to_lowercase().as_str()))
      .unwrap_or(false)
  })
}

fn error_missing_exports_field(deno_json: &ConfigFile) -> Result<(), AnyError> {
  static SUGGESTED_ENTRYPOINTS: [&str; 4] = ["mod.ts", "mod.js", "index.ts", "index.js"];
  ...
  bail!(
    "You did not specify an entrypoint in {}. Add `exports` mapping in the configuration file, \
     eg:\n{}",
    deno_json.specifier,
    exports_content
  );
...
mod tests {
  ...
  use super::{
    has_license_file,
    tar::{PublishableTarball, PublishableTarballFile},
