...
use deno_ast::{
  ParsedSource, SourcePos, SourceRange, SourceRangedForSpanned, SourceTextInfo, SourceTextProvider,
  TextChange,
  diagnostics::{
    Diagnostic, DiagnosticLevel, DiagnosticLocation, DiagnosticSnippet, DiagnosticSnippetHighlight,
    DiagnosticSnippetHighlightStyle, DiagnosticSourcePos, DiagnosticSourceRange,
  },
  swc::{
    ast::{Callee, Expr, Lit, MemberProp, MetaPropKind},
    atoms::Atom,
    ecma_visit::{Visit, VisitWith, noop_visit_type},
  },
};
use deno_config::workspace::WorkspaceDirectory;
use deno_core::{ModuleSpecifier, anyhow};
use deno_graph::analysis::{
  DependencyDescriptor, DynamicTemplatePart, StaticDependencyKind, TypeScriptReference,
};
use deno_package_json::{
  PackageJsonDepValue, PackageJsonDepValueParseErrorKind, PackageJsonDepWorkspaceReq,
};
use deno_resolver::{
  npm::NpmResolverSys,
  workspace::{MappedResolution, PackageJsonDepResolution, WorkspaceResolver},
};
use deno_runtime::deno_node::is_builtin_node_module;
use deno_semver::{Version, VersionReq, jsr::JsrDepPackageReq, npm::NpmPackageReqReference};
use node_resolver::NodeResolverSys;

use crate::{
  node::{CliNodeResolver, CliPackageJsonResolver},
  resolver::CliNpmReqResolver,
  sys::CliSys,
};
...
impl Diagnostic for SpecifierUnfurlerDiagnostic {
  fn level(&self) -> DiagnosticLevel {
    match self {
      SpecifierUnfurlerDiagnostic::UnanalyzableDynamicImport { .. } => DiagnosticLevel::Warning,
      SpecifierUnfurlerDiagnostic::UnanalyzableImportMetaResolve { .. } => DiagnosticLevel::Warning,
      SpecifierUnfurlerDiagnostic::ResolvingNpmWorkspacePackage { .. } => DiagnosticLevel::Error,
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonSpecifier { .. } => DiagnosticLevel::Error,
    }
  }
  ...
  fn code(&self) -> Cow<'_, str> {
    match self {
      ...
      Self::UnanalyzableImportMetaResolve { .. } => "unanalyzable-import-meta-resolve",
  ...
  fn message(&self) -> Cow<'_, str> {
    match self {
      Self::UnanalyzableDynamicImport { .. } => "unable to analyze dynamic import".into(),
      Self::UnanalyzableImportMetaResolve { .. } => "unable to analyze import.meta.resolve".into(),
  ...
  fn hint(&self) -> Option<Cow<'_, str>> {
    match self {
      SpecifierUnfurlerDiagnostic::UnanalyzableDynamicImport { .. }
      | SpecifierUnfurlerDiagnostic::UnanalyzableImportMetaResolve { .. } => None,
      SpecifierUnfurlerDiagnostic::ResolvingNpmWorkspacePackage { .. } => Some(
        "make sure the npm workspace package is resolvable and has a version field in its \
         package.json"
          .into(),
      ),
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonSpecifier { .. } => {
        Some("change the package dependency to point to something on npm or JSR instead".into())
      }
    }
  }
  ...
  fn info(&self) -> Cow<'_, [Cow<'_, str>]> {
    match self {
      SpecifierUnfurlerDiagnostic::UnanalyzableDynamicImport { .. } => Cow::Borrowed(&[
        Cow::Borrowed(
          "after publishing this package, imports from the local import map / package.json do not \
           work",
        ),
        Cow::Borrowed(
          "dynamic imports that can not be analyzed at publish time will not be rewritten \
           automatically",
        ),
        Cow::Borrowed(
          "make sure the dynamic import is resolvable at runtime without an import map / \
           package.json",
        ),
      ]),
      SpecifierUnfurlerDiagnostic::UnanalyzableImportMetaResolve { .. } => Cow::Borrowed(&[
        Cow::Borrowed(
          "after publishing this package, import.meta.resolve calls from the local import map / \
           package.json do not work",
        ),
        Cow::Borrowed(
          "import.meta.resolve calls that can not be analyzed at publish time will not be \
           rewritten automatically",
        ),
        Cow::Borrowed(
          "make sure the import.meta.resolve call is resolvable at runtime without an import map \
           / package.json",
        ),
      ]),
      SpecifierUnfurlerDiagnostic::ResolvingNpmWorkspacePackage { .. } => Cow::Borrowed(&[]),
      SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonSpecifier { .. } => Cow::Borrowed(&[]),
    }
  }
...
impl<TSys: SpecifierUnfurlerSys> SpecifierUnfurler<TSys> {
      ...
      Err(diagnostic) => {
        ...
        match diagnostic {
          UnfurlSpecifierError::UnsupportedPkgJsonSpecifier {
            ...
            diagnostic_reporter(SpecifierUnfurlerDiagnostic::UnsupportedPkgJsonSpecifier {
          ...
          UnfurlSpecifierError::Workspace {
            ...
            diagnostic_reporter(SpecifierUnfurlerDiagnostic::ResolvingNpmWorkspacePackage {
              specifier: referrer.clone(),
              package_name,
              text_info: text_info.clone(),
              range,
          ...
          MappedResolution::PackageJson {
            ...
            Ok(dep) => match dep {
              PackageJsonDepValue::File(_) => {
                return Err(UnfurlSpecifierError::UnsupportedPkgJsonSpecifier {
              ...
              PackageJsonDepValue::Workspace(workspace_version_req) => {
                let version_req = match workspace_version_req {
                  PackageJsonDepWorkspaceReq::VersionReq(version_req) => Cow::Borrowed(version_req),
                  PackageJsonDepWorkspaceReq::Caret => {
                    let version = self.find_workspace_npm_dep_version(alias).map_err(|err| {
                      UnfurlSpecifierError::Workspace {
                        package_name: alias.to_string(),
                    ...
                    Cow::Owned(VersionReq::parse_from_npm(&format!("^{}", version)).unwrap())
                  }
  ...
  fn find_workspace_npm_dep_version(&self, pkg_name: &str) -> Result<Version, anyhow::Error> {
    // todo(#24612): warn when this is also a jsr package telling
    // people to map the specifiers in the import map
    let pkg_json = self
      .workspace_resolver
      ...
      .ok_or_else(|| anyhow::anyhow!("unable to find npm package in workspace"))?;
  ...
  fn find_types_package_version_req(
    ...
    let referrer_pkg_jsons = referrer_path
      ...
      .flatten()
      .filter_map(|i| i.ok());
    for pkg_json in referrer_pkg_jsons.chain(self.workspace_dir.workspace.package_jsons().cloned())
    {
      let deps = pkg_json.resolve_local_package_json_deps();
      if let Some(Ok(PackageJsonDepValue::Req(pkg_req))) = deps.get(types_package_name) {
        return Some(pkg_req.version_req.clone());
      }
    }

    let check_dep = |dep: JsrDepPackageReq| {
      if dep.kind == deno_semver::package::PackageKind::Npm && dep.req.name == types_package_name {
  ...
  fn get_types_package_specifier(
    ...
    let npm_req_ref = NpmPackageReqReference::from_str(unfurled_specifier).ok()?;
    ...
    let version_req =
      if let Some(req) = self.find_types_package_version_req(types_pkg_name, referrer) {
        req.to_string()
      } else {
  ...
  fn try_unfurl_dynamic_dep(
    ...
    match &dep.argument {
      deno_graph::analysis::DynamicArgument::String(specifier) => {
        ...
        let maybe_relative_index = text_info.text_str()[range.start .. range.end].find(specifier);
        ...
        let maybe_unfurled = self.unfurl_specifier_reporting_diagnostic(
          module_url,
          specifier,
          deno_resolver::workspace::ResolutionKind::Execution, /* dynamic imports are always
                                                                * execution */
          text_info,
      ...
      deno_graph::analysis::DynamicArgument::Template(parts) => {
        match parts.first() {
          Some(DynamicTemplatePart::String { value: specifier }) => {
            ...
            let range = to_range(text_info, &dep.argument_range);
            let maybe_relative_index = text_info.text_str()[range.start ..].find(specifier);
            let Some(relative_index) = maybe_relative_index else {
              return false;
            };
            let start = range.start + relative_index;
  ...
  pub fn unfurl_to_changes(
      ...
      match dep {
        DependencyDescriptor::Static(dep) => {
          ...
          if dep.types_specifier.is_none()
            ...
            && let Some(types_specifier) = self.get_types_package_specifier(&unfurled, url)
          {
            // insert @ts-types comment before the import declaration
            let specifier_range = dep.specifier_range.as_source_range(text_info);
            let decl_start_byte_index = specifier_collector
              ...
              .unwrap_or_else(|| {
                ...
                let line_start = text_info.line_start(dep.specifier_range.start.line);
        ...
        DependencyDescriptor::Dynamic(dep) => {
          let success =
            self.try_unfurl_dynamic_dep(url, text_info, dep, text_changes, diagnostic_reporter);

          if !success {
            let start_pos = text_info.line_start(dep.argument_range.start.line)
              + dep.argument_range.start.character;
            let end_pos =
              text_info.line_start(dep.argument_range.end.line) + dep.argument_range.end.character;
    ...
    for range in collector.diagnostic_ranges {
      diagnostic_reporter(SpecifierUnfurlerDiagnostic::UnanalyzableImportMetaResolve {
        specifier: url.to_owned(),
        range,
        text_info: text_info.clone(),
      });
...
fn relative_url(resolved: &ModuleSpecifier, referrer: &ModuleSpecifier) -> String {
...
mod tests {
  ...
  use deno_core::{serde_json::json, url::Url};
  use deno_graph::ast::ParserModuleAnalyzer;
  use deno_resolver::factory::{
    ResolverFactory, ResolverFactoryOptions, WorkspaceFactory, WorkspaceFactoryOptions,
  };
  use pretty_assertions::assert_eq;
  use sys_traits::{EnvCurrentDir, impls::InMemorySys};
...
export type * from "./c";
"#;
      let unfurled_source = unfurl_text(&cwd.join("mod.d.ts"), source_code, &unfurler);
      let expected_source = r#"import express from "npm:express@5";"
...
"#;
  ...
  "#;
      let unfurled_source =
        unfurl_text(&cwd.join("publish").join("mod.ts"), source_code, &unfurler);
  ...
  async fn test_unfurl_types_package() {
    async fn run_test(memory_sys: InMemorySys) {
      ...
      memory_sys.fs_insert(cwd.join("node_modules/@types/package/index.d.ts"), "");
      memory_sys.fs_insert(cwd.join("node_modules/@types/package/subpath.d.ts"), "");
      let unfurler = build_unfurler(memory_sys, &cwd).await;
...
export { d, e, f };
"#;
      let unfurled_source = unfurl_text(&cwd.join("mod.ts"), source_code, &unfurler);
...
"#;
  ...
  async fn build_unfurler(sys: InMemorySys, cwd: &Path) -> SpecifierUnfurler<InMemorySys> {
  ...
  fn unfurl_text(
    ...
    let (unfurled_source, d) = unfurl_text_with_diagnostics(path, source_code, unfurler);
    assert_eq!(d.len(), 0);
    unfurled_source
  }
