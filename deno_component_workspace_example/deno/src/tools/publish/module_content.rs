...
use deno_ast::{MediaType, ParsedSource, SourceTextInfo, TextChange};
use deno_core::{anyhow::Context, error::AnyError, url::Url};
use deno_graph::ModuleGraph;
use deno_resolver::{
  cache::{LazyGraphSourceParser, ParsedSourceCache},
  deno_json::CompilerOptionsResolver,
...
use super::{
  diagnostics::{PublishDiagnostic, PublishDiagnosticsCollector},
  unfurl::{PositionOrSourceRangeRef, SpecifierUnfurler, SpecifierUnfurlerDiagnostic},
};
...
impl<TSys: FsMetadata + FsRead> ModuleContentProvider<TSys> {
  ...
  pub fn resolve_content_maybe_unfurling(
    ...
    let source_parser = LazyGraphSourceParser::new(&self.parsed_source_cache, graph);
    ...
    let module_info = deno_graph::ast::ParserModuleAnalyzer::module_info(&parsed_source);
    ...
    let rewritten_text = deno_ast::apply_text_changes(text_info.text_str(), text_changes);
  ...
  fn add_jsx_text_changes(
    ...
    let jsx_options = self.resolve_jsx_options(specifier, text_info, diagnostic_reporter)?;
    ...
    if !leading_comments_has_re(&JSX_RUNTIME_RE) {
      add_text_change(format!("/** @jsxRuntime {} */", jsx_options.jsx_runtime,));
    }
    ...
    if let Some(classic_options) = &jsx_options.jsx_classic {
      if !leading_comments_has_re(&JSX_FACTORY_RE) {
        add_text_change(format!("/** @jsxFactory {} */", classic_options.factory,));
      }
  ...
  fn resolve_jsx_options<'a>(
    ...
    let compiler_options = self.compiler_options_resolver.for_specifier(specifier);
    ...
    let jsx_runtime = match &transpile_options.jsx {
      Some(deno_ast::JsxRuntime::Automatic(_) | deno_ast::JsxRuntime::Precompile(_)) => "automatic",
      ...
      |import_source: &str, referrer: &Url, resolution_kind: ResolutionKind| {
        let maybe_import_source = self
          ...
          .unfurl_specifier_reporting_diagnostic(
            ...
            resolution_kind,
            text_info,
            PositionOrSourceRangeRef::PositionRange(&deno_graph::PositionRange::zeroed()),
            diagnostic_reporter,
          );
    ...
    let classic_options = match &transpile_options.jsx {
      ...
      Some(deno_ast::JsxRuntime::Classic(classic_options)) => Some(Cow::Borrowed(classic_options)),
      Some(deno_ast::JsxRuntime::Precompile(_) | deno_ast::JsxRuntime::Automatic(_)) => None,
    };
    Ok(JsxFolderOptions {
      jsx_runtime,
...
mod test {
  ...
  use deno_resolver::{
    deno_json::CompilerOptionsOverrides,
    factory::ConfigDiscoveryOption,
    npm::{
  ...
  use sys_traits::{FsCreateDirAll, FsWrite, impls::InMemorySys};
  ...
  fn test_module_content_jsx() {
    run_test(&[
      ...
      (
        ...
        Some(
          "/** @jsxRuntime automatic *//** @jsxImportSource npm:react *//** @jsxImportSourceTypes \
           npm:@types/react */export const component = <div></div>;",
        ),
      ),
      (
        "/package-b/main.tsx",
        ...
        Some(
          "/** @jsxRuntime automatic *//** @jsxImportSource npm:react *//** @jsxImportSourceTypes \
           npm:react */export const componentB = <div></div>;",
        ),
      ),
      (
        "/package-a/other.tsx",
      ...
      (
        ...
        "export const component = <div></div>;",
        Some(
          "/** @jsxRuntime classic *//** @jsxFactory React.createElement *//** \
           @jsxFragmentFactory React.Fragment */export const component = <div></div>;",
        ),
      ),
    ]);
  }
  ...
  fn module_content_provider(sys: InMemorySys) -> ModuleContentProvider<InMemorySys> {
    ...
    let resolver = Arc::new(
      WorkspaceResolver::from_workspace(&workspace_dir.workspace, sys.clone(), Default::default())
        .unwrap(),
    );
    let package_json_resolver = Arc::new(PackageJsonResolver::new(sys.clone(), None));
