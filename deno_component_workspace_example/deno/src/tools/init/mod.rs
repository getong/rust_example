...
use deno_core::{anyhow::Context, error::AnyError, futures::FutureExt, serde_json::json};
...
use crate::{
  args::{DenoSubcommand, Flags, InitFlags, InternalFlags, PermissionFlags, RunFlags},
  colors,
  util::{fs::FsCleaner, progress_bar::ProgressBar},
};
...
async fn init_npm(options: InitNpmOptions<'_>) -> Result<i32, AnyError> {
  ...
  fn print_manual_usage(script_name: &str, args: &[String]) -> i32 {
    log::info!(
      "{}",
      cformat!(
        "You can initialize project manually by running <u>deno run {}</> and applying desired \
         permissions.",
  ...
  if !options.yes {
    if std::io::stdin().is_terminal() {
      log::info!(
        cstr!(
          "⚠️  Do you fully trust <y>{}</> package? Deno will invoke code from it with all \
           permissions. Do you want to continue? <p(245)>[y/n]</>"
        ),
        script_name
      );
      loop {
...
fn create_temp_node_modules_parent_dir() -> Result<tempfile::TempDir, AnyError> {
  ...
  let package_json_path = temp_node_modules_parent_dir.path().join("package.json");
  std::fs::write(&package_json_path, "{}")
    .with_context(|| format!("Failed creating '{}'", package_json_path.display()))?;
...
fn attempt_temp_dir_garbage_collection(
  ...
  let progress_bar = ProgressBar::new(crate::util::progress_bar::ProgressBarStyle::TextOnly);
...
fn create_file(dir: &Path, filename: &str, content: &str) -> Result<(), AnyError> {
...
mod test {
  ...
  fn test_attempt_temp_dir_garbage_collection() {
    ...
    attempt_temp_dir_garbage_collection(temp_dir.path().as_path(), reference_date).unwrap();
