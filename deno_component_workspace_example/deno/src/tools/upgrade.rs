...
use deno_lib::{shared::ReleaseChannel, version};
use deno_semver::{SmallStackString, Version};
use once_cell::sync::Lazy;

use crate::{
  args::{Flags, UPGRADE_USAGE, UpgradeFlags},
  colors,
  factory::CliFactory,
  http_util::{HttpClient, HttpClientProvider},
  util::{
    archive,
...
pub static ARCHIVE_NAME: Lazy<String> = Lazy::new(|| format!("deno-{}.zip", env!("TARGET")));
...
impl RealVersionProvider {
  pub fn new(http_client_provider: Arc<HttpClientProvider>, check_kind: UpgradeCheckKind) -> Self {
    Self {
  ...
  UpdateChecker<TEnvironment, TVersionProvider>
{
  ...
  /// Returns the current exe release channel and a version if a new one is available and it should
  /// be prompted about.
...
async fn print_release_notes(current_version: &str, new_version: &str, client: &HttpClient) {
  ...
  let is_switching_from_deno1_to_deno2 = new_semver.major == 2 && current_semver.major == 1;
  ...
  if is_deno_2_rc || is_switching_from_deno1_to_deno2 {
    log::info!(
      ...
      colors::bold("https://docs.deno.com/runtime/manual/advanced/migrate_deprecations")
    );
  }
  ...
  let should_print =
    current_semver.major != new_semver.major || current_semver.minor != new_semver.minor;
...
pub fn check_for_upgrades(http_client_provider: Arc<HttpClientProvider>, cache_file_path: PathBuf) {
  ...
  let version_provider =
    RealVersionProvider::new(http_client_provider.clone(), UpgradeCheckKind::Execution);
  ...
  let should_prompt = log::log_enabled!(log::Level::Info) && std::io::stderr().is_terminal();
  ...
  if let Some((release_channel, upgrade_version)) = update_checker.should_prompt() {
...
pub async fn check_for_upgrades_for_lsp(
  ...
  let version_provider = RealVersionProvider::new(http_client_provider, UpgradeCheckKind::Lsp);
...
async fn check_for_upgrades_for_lsp_with_provider(
  ...
  match release_channel {
    ReleaseChannel::Stable | ReleaseChannel::Rc | ReleaseChannel::Lts => {
      if let Ok(current) = Version::parse_standard(&current_version)
        && let Ok(latest) = Version::parse_standard(&latest_version.version_or_hash)
        && current >= latest
      {
...
>(
  ...
  let release_channel = version_provider.get_current_exe_release_channel();
  let Ok(latest_version) = version_provider.latest_version(release_channel).await else {
    return;
  };
...
pub async fn upgrade(flags: Arc<Flags>, upgrade_flags: UpgradeFlags) -> Result<(), AnyError> {
  ...
  let force_selection_of_new_version = upgrade_flags.force || full_path_output_flag.is_some();

  let requested_version = RequestedVersion::from_upgrade_flags(upgrade_flags.clone())?;
  ...
  let maybe_selected_version_to_upgrade = match &requested_version {
    ...
    RequestedVersion::SpecificVersion(channel, version) => select_specific_version_for_upgrade(
      *channel,
      version.clone(),
      force_selection_of_new_version,
    )?,
  ...
  let Some(selected_version_to_upgrade) = maybe_selected_version_to_upgrade else {
    return Ok(());
  };
  ...
  let Some(archive_data) = download_package(&client, download_url).await? else {
  ...
  fn from_upgrade_flags(upgrade_flags: UpgradeFlags) -> Result<Self, AnyError> {
    ...
    let (channel, passed_version) = if is_canary {
      if !re_hash.is_match(&passed_version) {
        bail!(
          "Invalid commit hash passed ({})\n\nPass a semver, or a full 40 character git commit \
           hash, or a release channel name.\n\nUsage:\n{}",
      ...
      let Ok(semver) = Version::parse_standard(&passed_version) else {
        bail!(
          "Invalid version passed ({})\n\nPass a semver, or a full 40 character git commit hash, \
           or a release channel name.\n\nUsage:\n{}",
          colors::gray(passed_version),
          UPGRADE_USAGE
        );
      };
...
async fn find_latest_version_to_upgrade(
  ...
  let latest_version_found =
    match fetch_latest_version(&client, release_channel, UpgradeCheckKind::Execution).await {
      Ok(v) => v,
      Err(err) => {
...
fn get_latest_version_url(
  ...
  let file_name = match release_channel {
    ...
    ReleaseChannel::Canary => Cow::Owned(format!("canary-{target_tuple}-latest.txt")),
...
fn get_download_url(version: &str, release_channel: ReleaseChannel) -> Result<Url, AnyError> {
...
fn get_banner_url(version: &str, release_channel: ReleaseChannel) -> Option<Url> {
  let download_url = match release_channel {
    ReleaseChannel::Stable => {
...
async fn download_package(
  ...
  let response = client
    ...
    .with_context(|| {
      format!(
        "Failed downloading {download_url}. The version you requested may not have been built for \
...
fn kill_running_deno_lsp_processes() {
  // limit this to `deno lsp` invocations to avoid killing important programs someone might be
  // running
...
fn set_exe_permissions(
  ...
  #[cfg(unix)]
  if std::os::unix::fs::MetadataExt::uid(&metadata) == 0 && !nix::unistd::Uid::effective().is_root()
  {
    bail!(
      concat!(
...
fn check_exe(exe_path: &Path) -> Result<(), AnyError> {
  ...
  if !output.status.success() {
    bail!(
      "Failed to validate Deno executable. This may be because your OS is unsupported or the \
...
impl CheckVersionFile {
  pub fn parse(content: String) -> Option<Self> {
    ...
    let Ok(current_release_channel) = ReleaseChannel::deserialize(&current_release_channel) else {
      return None;
    };
...
mod test {
  ...
  fn test_requested_version() {
    ...
    let req_ver = RequestedVersion::from_upgrade_flags(upgrade_flags.clone()).unwrap();
    ...
    assert_eq!(
      req_ver,
      RequestedVersion::SpecificVersion(ReleaseChannel::Stable, "1.46.0".to_string())
    );
    ...
    upgrade_flags.version = Some("5c69b4861b52ab406e73b9cd85c254f0505cb20f".to_string());
    let req_ver = RequestedVersion::from_upgrade_flags(upgrade_flags.clone()).unwrap();
    assert_eq!(
      req_ver,
    ...
    assert_eq!(
      req_ver,
      RequestedVersion::SpecificVersion(ReleaseChannel::Stable, "1.46.5".to_string())
    );
    ...
    assert_eq!(
      req_ver,
      RequestedVersion::SpecificVersion(ReleaseChannel::Rc, "2.0.0-rc.0".to_string())
    );
    ...
    upgrade_flags.version_or_hash_or_channel = Some("5c69b4861b52a".to_string());
  ...
  fn test_parse_upgrade_check_file() {
    ...
    let maybe_file = CheckVersionFile::parse(
      "2020-01-01T00:00:00+00:00!2020-01-01T00:00:00+00:00!1.2.3!1.2.2".to_string(),
    );
    assert!(maybe_file.is_none());
    // NOTE(bartlomieju): post-1.46 format
    let file = CheckVersionFile::parse(
      "2020-01-01T00:00:00+00:00!2020-01-01T00:00:00+00:00!1.2.3!1.2.2!stable".to_string(),
    )
    .unwrap();
    assert_eq!(
    ...
    let result = CheckVersionFile::parse("2020-01-01T00:00:00+00:00!".to_string());
  ...
  fn test_serialize_upgrade_check_file() {
    let mut file = CheckVersionFile {
      ...
      last_checked: chrono::DateTime::parse_from_rfc3339("2020-01-01T00:00:00Z")
        .unwrap()
        .with_timezone(&chrono::Utc),
  ...
  impl TestUpdateCheckerEnvironment {
    ...
    pub fn set_latest_version(&self, version: &str, release_channel: ReleaseChannel) {
  ...
  fn test_normalize_version_server() {
    ...
    assert_eq!(
      normalize_version_from_server(ReleaseChannel::Stable, "  v1.0.0-test-v\n\n  ").unwrap(),
      AvailableVersion {
    ...
    assert_eq!(
      normalize_version_from_server(ReleaseChannel::Canary, "  v1452345asdf   \n\n   ").unwrap(),
      AvailableVersion {
