...
use deno_core::{
  ...
  anyhow::anyhow,
  error::{AnyError, CoreErrorKind, JsError},
...
use crate::{
  args::{BenchFlags, CliOptions, Flags},
  colors,
  display::write_json_to_stdout,
  factory::CliFactory,
  graph_container::CheckSpecifiersOptions,
  graph_util::has_graph_root_local_dependent_changed,
  ops,
  sys::CliSys,
  tools::test::{TestFilter, format_test_error},
  util::{
    file_watcher,
    fs::{CollectSpecifiersOptions, collect_specifiers},
    path::{is_script_ext, matches_pattern_or_exact_path},
  },
...
use reporters::{BenchReporter, ConsoleReporter, JsonReporter};
...
async fn bench_specifiers(
  ...
  let handler = {
    spawn(async move {
      ...
      if used_only {
        return Err(anyhow!("Bench failed because the \"only\" option was used",));
      }
...
pub async fn run_benchmarks(flags: Arc<Flags>, bench_flags: BenchFlags) -> Result<(), AnyError> {
  let factory = CliFactory::from_flags(flags);
  let cli_options = factory.cli_options()?;
  let workspace_bench_options = cli_options.resolve_workspace_bench_options(&bench_flags);

  let members_with_bench_options = cli_options.resolve_bench_options_for_members(&bench_flags)?;
