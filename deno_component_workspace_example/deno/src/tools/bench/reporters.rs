...
impl BenchReporter for ConsoleReporter {
        ...
        if !stats.high_precision && stats.used_explicit_timers {
          println!(
            ...
            colors::yellow(format!(
              "Warning: start() and end() calls in \"{}\" are ignored because it averages \
        ...
        println!(
          ...
          mitata::reporter::benchmark_error(
            ...
            &mitata::reporter::Error {
              stack: None,
              message: format_test_error(js_error, &TestFailureFormatOptions::default()),
            },
            options
          )
        )
  ...
  fn report_uncaught_error(&mut self, _origin: &str, error: Box<JsError>) {
    ...
    println!(
      "This error was not caught from a benchmark and caused the bench runner to fail on the \
       referenced module."
    );
    println!(
