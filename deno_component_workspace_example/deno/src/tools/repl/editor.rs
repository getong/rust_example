...
use deno_core::{anyhow::Context as _, error::AnyError, parking_lot::Mutex, serde_json};
use rustyline::{
...
impl EditorHelper {
  ...
  fn get_object_expr_properties(&self, object_expr: &str) -> Option<Vec<String>> {
...
fn validate(input: &str) -> ValidationResult {
  ...
  for (token, range) in tokens {
    ...
    match token {
      ...
      Token::LParen | Token::LBracket | Token::LBrace | Token::DollarLBrace => stack.push(token),
      Token::RParen | Token::RBracket | Token::RBrace => {
        match (stack.pop(), token) {
          ...
          (Some(left), _) => {
            ...
            queued_validation_error = Some(ValidationResult::Invalid(Some(format!(
              "Mismatched pairs: {left:?} is not properly closed"
            ))));
          }
          (None, _) => {
            // While technically invalid when unpaired, it should be V8's task to output error
            // instead. Thus marked as valid with no info.
            return ValidationResult::Valid(None);
          }
        }
      }
      ...
      out_line.replace_range(
        ...
        &match item.inner {
          deno_ast::TokenOrComment::Token(token) => match token {
            ...
            Token::Num { .. } | Token::BigInt { .. } => colors::yellow(&line[range]).to_string(),
            Token::Word(word) => match word {
              Word::True | Word::False | Word::Null => colors::yellow(&line[range]).to_string(),
              ...
              Word::Ident(ident) => {
                match ident.as_ref() {
                  ...
                  _ => {
                    ...
                    if matches!(next, Some(deno_ast::TokenOrComment::Token(Token::LParen))) {
          ...
          deno_ast::TokenOrComment::Comment { .. } => colors::gray(&line[range]).to_string(),
        },
      );
    }
...
impl ReplEditor {
  pub fn new(helper: EditorHelper, history_file_path: Option<PathBuf>) -> Result<Self, AnyError> {
...
/// Command to reverse search history , same as rustyline default C-R but that resets repl
...
/// It uses a heuristic to determine if the user is requesting completion or if they want to insert
/// an actual tab The heuristic goes like this:
