...
use deno_ast::{
  ImportsNotUsedAsValues, JsxAutomaticOptions, JsxClassicOptions, ModuleKind, ModuleSpecifier,
  ParseDiagnosticsError, ParsedSource, SourcePos, SourceRangedForSpanned, SourceTextInfo,
...
use deno_graph::{Position, PositionRange, analysis::SpecifierWithRange};
...
use crate::{
  ...
  cdp::RemoteObjectId,
  colors,
  lsp::ReplLanguageServer,
  npm::CliNpmInstaller,
  resolver::CliResolver,
  tools::test::{
    TestEventReceiver, TestEventTracker, TestFailureFormatOptions, report_tests,
    reporters::{PrettyTestReporter, TestReporter},
    run_tests_for_worker, worker_has_tests,
  },
};
...
fn comment_source_to_position_range(
  ...
  PositionRange {
    start: Position::from_source_pos(comment_start + m.start() - padding, text_info),
    end: Position::from_source_pos(comment_start + m.end() + padding, text_info),
  }
}
...
impl ReplSession {
  ...
  pub async fn initialize(
    ...
    let callback = Box::new(move |message| repl_session_state.callback(message));
    ...
    loop {
      ...
      if notification.method == "Runtime.executionContextCreated" {
        let execution_context_created =
          serde_json::from_value::<cdp::ExecutionContextCreated>(notification.params)?;
        assert!(
    ...
    let referrer = deno_core::resolve_path("./$deno$repl.mts", cli_options.initial_cwd()).unwrap();

    let cwd_url = Url::from_directory_path(cli_options.initial_cwd()).map_err(|_| {
      anyhow!(
  ...
  pub fn set_test_reporter_factory(&mut self, f: Box<dyn Fn() -> Box<dyn TestReporter>>) {
    self.test_reporter_factory = f;
  }

  pub async fn closing(&mut self) -> Result<bool, AnyError> {
    let result = self
      .call_function_on_repl_internal_obj(r#"function () { return this.closed; }"#.to_string(), &[])
      .await?
      .result;
    let closed = result
      .value
    ...
    async fn inner(session: &mut ReplSession, line: &str) -> Result<EvaluationOutput, AnyError> {
      match session.evaluate_line_with_object_wrapping(line).await {
        ...
        Err(err) => {
          ...
          match any_and_jserrorbox_downcast_ref::<deno_ast::ParseDiagnostic>(&err) {
            Some(diagnostic) => Ok(EvaluationOutput::Error(format_diagnostic(diagnostic))),
            None => match any_and_jserrorbox_downcast_ref::<ParseDiagnosticsError>(&err) {
  ...
  pub async fn evaluate_line_with_object_wrapping(
    ...
    if worker_has_tests(&mut self.worker) {
      ...
      let event_tracker = TestEventTracker::new(self.worker.js_runtime.op_state());
  ...
  pub async fn set_last_thrown_error(&mut self, error: &cdp::RemoteObject) -> Result<(), AnyError> {
    self
      .post_message_with_event_loop(
        "Runtime.callFunctionOn",
        Some(cdp::CallFunctionOnArgs {
          function_declaration: r#"function (object) { this.lastThrownError = object; }"#
            .to_string(),
  ...
  async fn evaluate_ts_expression(
    ...
    let parsed_source = match parse_source_as(expression.to_string(), deno_ast::MediaType::Tsx) {
      Ok(parsed) => parsed,
      Err(err) => match parse_source_as(expression.to_string(), deno_ast::MediaType::TypeScript) {
        Ok(parsed) => parsed,
        _ => {
          return Err(err);
        }
  ...
  fn analyze_and_handle_jsx(&mut self, parsed_source: &ParsedSource) {
    ...
    if let Some(jsx) = analyzed_pragmas.jsx {
      match &mut self.jsx {
        ...
        deno_ast::JsxRuntime::Automatic(_) | deno_ast::JsxRuntime::Precompile(_) => {
    ...
    if let Some(jsx_import_source) = analyzed_pragmas.jsx_import_source {
      match &mut self.jsx {
        ...
        | deno_ast::JsxRuntime::Precompile(deno_ast::JsxPrecompileOptions { automatic, .. }) => {
...
static JSX_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)^[\s*]*@jsx\s+(\S+)").unwrap());
/// Matches the `@jsxFrag` pragma.
static JSX_FRAG_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)^[\s*]*@jsxFrag\s+(\S+)").unwrap());
...
impl AnalyzedJsxPragmas {
  fn has_any(&self) -> bool {
    self.jsx_import_source.is_some() || self.jsx.is_some() || self.jsx_fragment.is_some()
  }
}

/// Analyze provided source and return information about carious pragmas
/// used to configure the JSX transforms.
fn analyze_jsx_pragmas(parsed_source: &ParsedSource) -> Option<AnalyzedJsxPragmas> {
  if !matches!(
