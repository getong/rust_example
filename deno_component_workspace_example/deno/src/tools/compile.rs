...
use deno_path_util::{resolve_url_or_path, url_from_file_path, url_to_file_path};
...
pub async fn compile(flags: Arc<Flags>, compile_flags: CompileFlags) -> Result<(), AnyError> {
  ...
  let graph = if cli_options.type_check_mode().is_true() {
    ...
    module_graph_creator
      .create_graph(GraphKind::CodeOnly, module_roots, NpmCachingStrategy::Eager)
      .await?
  } else {
    graph
  };
  ...
  temp_filename.push(format!(
    ".tmp-{}",
    faster_hex::hex_encode(&rand::thread_rng().r#gen::<[u8; 8]>(), &mut [0u8; 16]).unwrap()
  ));
  let temp_path = output_path.with_file_name(temp_filename);

  let file = std::fs::File::create(&temp_path)
    .with_context(|| format!("Opening temporary file '{}'", temp_path.display()))?;

  let write_result = binary_writer
    .write_bin(WriteBinOptions {
      writer: file,
      display_output_filename: &output_path.file_name().unwrap().to_string_lossy(),
      graph: &graph,
      entrypoint,
...
pub async fn compile_eszip(flags: Arc<Flags>, compile_flags: CompileFlags) -> Result<(), AnyError> {
  let factory = CliFactory::from_flags(flags);
  let cli_options = factory.cli_options()?;
  let module_graph_creator = factory.module_graph_creator().await?;
  ...
  if let Some(import_map_specifier) = maybe_import_map_specifier {
    ...
    let import_map_content = std::fs::read_to_string(&import_map_path)
      .with_context(|| format!("Failed to read import map: {:?}", import_map_path))?;
  ...
  let mut file = std::fs::File::create(&output_path)
    .with_context(|| format!("Opening ESZip file '{}'", output_path.display()))?;
...
fn get_module_roots_and_include_paths(
  ...
  fn analyze_path(
    ...
    while let Some(path) = pending.pop_front() {
      ...
      {
        let entry = entry
          .with_context(|| format!("Failed reading entry in directory '{}'", path.display()))?;
...
async fn resolve_compile_executable_output_path(
  ...
  output_path
    ...
    .map(|output_path| get_os_specific_filepath(output_path, &compile_flags.target))
}

fn get_os_specific_filepath(output: PathBuf, target: &Option<String>) -> PathBuf {
...
#[cfg(test)]
mod test {
  use deno_npm::{registry::TestNpmRegistryApi, resolution::NpmVersionResolver};
  ...
  async fn resolve_compile_executable_output_path_target_linux() {
    ...
    let bin_name_resolver = BinNameResolver::new(&http_client, &npm_api, &npm_version_resolver);
  ...
  fn test_os_specific_file_path() {
    fn run_test(path: &str, target: Option<&str>, expected: &str) {
      assert_eq!(
        get_os_specific_filepath(PathBuf::from(path), &target.map(|s| s.to_string())),
