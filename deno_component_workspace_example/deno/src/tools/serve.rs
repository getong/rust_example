...
use super::run::{check_permission_before_script, maybe_npm_install};
use crate::{
  args::{Flags, ServeFlags, WatchFlagsWithPaths, WorkspaceMainModuleResolver, parallelism_count},
  factory::CliFactory,
  util::file_watcher::WatcherRestartMode,
  worker::CliMainWorkerFactory,
};
...
pub async fn serve(
  ...
  if let Some(watch_flags) = serve_flags.watch {
    return serve_with_watch(flags, watch_flags, parallelism_count(serve_flags.parallel)).await;
  }
