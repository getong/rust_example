...
use deno_config::workspace::{
  FolderConfigs, TaskDefinition, TaskOrScript, WorkspaceDirectory, WorkspaceMemberTasksConfig,
  WorkspaceTasksConfig,
};
use deno_core::{
  anyhow::{Context, anyhow, bail},
  error::AnyError,
  futures::{FutureExt, StreamExt, future::LocalBoxFuture, stream::futures_unordered},
  url::Url,
};
use deno_npm_installer::PackageCaching;
use deno_path_util::normalize_path;
use deno_task_shell::{KillSignal, ShellCommand};
use indexmap::{IndexMap, IndexSet};
use regex::Regex;

use crate::{
  args::{CliLockfile, CliOptions, Flags, TaskFlags},
  colors,
  factory::CliFactory,
  node::CliNodeResolver,
  npm::{CliNpmInstaller, CliNpmResolver},
  task_runner,
  task_runner::run_future_forwarding_signals,
  util::{fs::canonicalize_path, progress_bar::ProgressBar},
};
...
pub async fn execute_script(flags: Arc<Flags>, task_flags: TaskFlags) -> Result<i32, AnyError> {
  ...
  let force_use_pkg_json = std::env::var_os(crate::task_runner::USE_PKG_JSON_HIDDEN_ENV_VAR_NAME)
    .map(|v| {
      // always remove so sub processes don't inherit this env var

      #[allow(clippy::undocumented_unsafe_blocks)]
      unsafe {
        std::env::remove_var(crate::task_runner::USE_PKG_JSON_HIDDEN_ENV_VAR_NAME)
      };
      v == "1"
    })
    .unwrap_or(false);

  // TODO(bartlomieju): this whole huge if statement should be a separate function, preferably with
  // unit tests
  let (packages_task_configs, name) = if let Some(filter) = &task_flags.filter {
    ...
    for (folder_url, folder) in workspace.config_folders_sorted_by_dependencies() {
      if !task_flags.recursive && !matches_package(folder, force_use_pkg_json, &package_regex) {
        continue;
      }
...
impl<'a> TaskRunner<'a> {
  ...
  async fn run_tasks_in_parallel(
    ...
    impl<'a> PendingTasksContext<'a> {
      ...
      fn get_next_task<'b>(
        ...
        kill_signal: &KillSignal,
        argv: &'a [String],
      ) -> Option<LocalBoxFuture<'b, Result<(i32, &'a ResolvedTask<'a>), AnyError>>>
      where
        'a: 'b,
      {
  ...
  pub async fn run_deno_task(
    ...
    let cwd = match &self.task_flags.cwd {
      Some(path) => canonicalize_path(Path::new(path)).context("failed canonicalizing --cwd")?,
      None => normalize_path(Cow::Owned(dir_url.to_file_path().unwrap())).into_owned(),
    };

    let custom_commands =
      task_runner::resolve_custom_commands(self.node_resolver, self.npm_resolver)?;
  ...
  async fn run_single(&self, opts: RunSingleOptions<'_>) -> Result<i32, AnyError> {
  ...
  fn output_task(&self, task_name: &str, package_name: Option<&str>, script: &str) {
    log::info!(
      "{} {}{} {}",
      colors::green("Task"),
      colors::cyan(task_name),
      package_name
        .filter(|_| self.task_flags.recursive || self.task_flags.filter.is_some())
...
fn sort_tasks_topo<'a>(
  ...
  fn sort_visit<'a>(
    ...
    if let Some(existing_task) = sorted
      .iter()
      .find(|task| task.name == name && task.task_or_script.folder_url() == folder_url)
    {
      // already exists
...
fn matches_package(config: &FolderConfigs, force_use_pkg_json: bool, regex: &Regex) -> bool {
  if !force_use_pkg_json
...
fn get_available_tasks(
  ...
  for maybe_config in [&tasks_config.member, &tasks_config.root] {
    ...
    if let Some(config) = config.deno_json.as_ref() {
      let is_root =
        !is_cwd_root_dir && config.folder_url == *workspace_dir.workspace.root_dir_url().as_ref();
