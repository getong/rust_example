...
use crate::{
  args::{
    AddFlags, ConfigFlag, Flags, InstallEntrypointsFlags, InstallFlags, InstallFlagsGlobal,
    InstallFlagsLocal, InstallTopLevelFlags, TypeCheckMode, UninstallFlags, UninstallKind,
    resolve_no_prompt,
  },
  factory::CliFactory,
  file_fetcher::{CreateCliFileFetcherOptions, create_cli_file_fetcher},
  graph_container::{CollectSpecifiersOptions, ModuleGraphContainer},
  jsr::JsrFetchResolver,
  npm::{CliNpmResolver, NpmFetchResolver},
  sys::CliSys,
  util::{display, fs::canonicalize_path_maybe_not_exists},
};
...
impl deno_resolver::file_fetcher::GraphLoaderReporter for InstallReporter {
  fn on_load(&self, specifier: &Url, loaded_from: deno_cache_dir::file_fetcher::LoadedFrom) {
...
fn get_installer_bin_dir(cwd: &Path, root_flag: Option<&str>) -> Result<PathBuf, AnyError> {
...
pub async fn uninstall(flags: Arc<Flags>, uninstall_flags: UninstallFlags) -> Result<(), AnyError> {
  ...
  let installation_dir = get_installer_bin_dir(&cwd, uninstall_flags.root.as_deref())?;
...
async fn install_local(
  ...
  match install_flags {
    ...
    InstallFlagsLocal::TopLevel(top_level_flags) => install_top_level(flags, top_level_flags).await,
  }
}
  ...
  for package_json in workspace.package_jsons() {
    ...
    for (_k, v) in deps.dependencies.iter() {
      ...
      match s {
        ...
        deno_package_json::PackageJsonDepValue::Workspace(_package_json_dep_workspace_req) => {
...
pub fn print_install_report(
  ...
  if !rep.stats.intialized_npm.is_empty() || !rep.stats.downloaded_jsr.is_empty() {
    let total_installed = rep.stats.intialized_npm.len() + rep.stats.downloaded_jsr.len();
    log::info!(
      "{} {} {} {} {}",
...
fn check_if_installs_a_single_package_globally(
  ...
  if matches!(url.scheme(), "http" | "https") {
    bail!(
      "Failed to install \"{}\" specifier. If you are trying to install {} globally, run again \
       with `-g` flag:\n  deno install -g {}",
      url.scheme(),
      url.as_str(),
      url.as_str()
...
pub async fn install_command(
  ...
  match install_flags {
    InstallFlags::Global(global_flags) => install_global(flags, global_flags).await,
...
async fn install_global(
  ...
  {
    log::warn!(
      "{} discovered config file will be ignored in the installed command. Use the --config flag \
  ...
  for (i, module_url) in install_flags_global.module_urls.iter().enumerate() {
    ...
    if !cli_options.initial_cwd().join(entry_text).exists() {
      ...
      if i == 1 && install_flags_global.args.is_empty() && Url::parse(entry_text).is_err() {
        bail!(
          concat!(
            "{} is missing a prefix. Deno 3.0 requires `--` before script arguments in `deno \
             install -g`. ",
...
async fn resolve_shim_data(
  ...
  let installation_dir = get_installer_bin_dir(cwd, install_flags_global.root.as_deref())?;
  ...
  if let ConfigFlag::Path(config_path) = &flags.config_flag {
    ...
    if let Some(new_text) = remove_import_map_field_from_text(&config_text) {
      if flags.import_map_path.is_none() {
        log::warn!(
          "{} \"importMap\" field in the specified config file we be ignored. Use the \
           --import-map flag instead.",
          crate::colors::yellow("Warning"),
        );
      }
    ...
    || NpmPackageReqReference::from_specifier(&module_url).is_ok()
  {
    ...
    if let Some(lock_path) = &flags.lock {
      extra_files.push((
        copy_path,
        fs::read_to_string(lock_path).with_context(|| format!("error reading {}", lock_path))?,
      ));
    } else {
...
fn remove_import_map_field_from_text(config_text: &str) -> Option<String> {
  let value = jsonc_parser::cst::CstRootNode::parse(config_text, &Default::default()).ok()?;
...
mod tests {
  ...
  use super::*;
  use crate::{
    args::{ConfigFlag, PermissionFlags, UninstallFlagsGlobal},
    http_util::HttpClientProvider,
    util::fs::canonicalize_path,
  };
  ...
  async fn install_unstable() {
    ...
    if cfg!(windows) {
      assert!(content.contains(r#""run" "--no-config" "http://localhost:4545/echo_server.ts""#));
    } else {
      assert!(content.contains(r#"run --no-config 'http://localhost:4545/echo_server.ts'"#));
    }
  }
  ...
  async fn install_inferred_name_after_redirect_for_no_path_url() {
    ...
    let shim_data = resolve_shim_data(
      &Flags::default(),
      &InstallFlagsGlobal {
        module_urls: vec!["http://localhost:4550/?redirect_to=/subdir/redirects/a.ts".to_string()],
        args: vec![],
        name: None,
        root: Some(env::temp_dir().to_string_lossy().into_owned()),
        force: false,
  ...
  async fn install_with_import_map() {
    ...
    let mut expected_string =
      format!("--import-map '{import_map_url}' --no-config 'http://localhost:4545/cat.ts'");
    if cfg!(windows) {
  ...
  async fn install_file_url() {
    ...
    let file_module_string = Url::from_file_path(module_path).unwrap().to_string();
    ...
    let mut expected_string = format!("run --no-config '{}'", &file_module_string);
    if cfg!(windows) {
      expected_string = format!("\"run\" \"--no-config\" \"{}\"", &file_module_string);
    }
  ...
  async fn uninstall_basic() {
    ...
    {
      let hidden_file = get_hidden_file_with_ext(shim_path.as_path(), "deno.json");
      File::create(hidden_file).unwrap();
    }
    {
      let hidden_file = get_hidden_file_with_ext(shim_path.as_path(), "lock.json");
      File::create(hidden_file).unwrap();
    }
    ...
    assert!(!get_hidden_file_with_ext(shim_path.as_path(), "deno.json").exists());
    assert!(!get_hidden_file_with_ext(shim_path.as_path(), "lock.json").exists());
