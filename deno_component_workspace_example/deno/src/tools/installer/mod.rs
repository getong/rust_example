...
use crate::{
  args::{
    AddFlags, ConfigFlag, Flags, InstallFlags, InstallFlagsGlobal, InstallFlagsLocal,
    TypeCheckMode, UninstallFlags, UninstallKind, resolve_no_prompt,
  },
  factory::CliFactory,
...
async fn create_install_shim(
  ...
  let shim_data = resolve_shim_data(bin_name_resolver, cwd, flags, &install_flags_global).await?;
...
mod tests {
  ...
  async fn install_inferred_name_after_redirect_for_no_path_url() {
    ...
    let shim_data = resolve_shim_data(
      &Flags::default(),
      &InstallFlagsGlobal {
        module_url: "http://localhost:4550/?redirect_to=/subdir/redirects/a.ts".to_string(),
        args: vec![],
        name: None,
        root: Some(env::temp_dir().to_string_lossy().into_owned()),
        force: false,
