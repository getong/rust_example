...
use deno_core::{
  CancelFuture, CancelHandle, anyhow::bail, error::AnyError, futures, parking_lot::Mutex,
  serde_json,
};
use deno_lib::version::DENO_VERSION_INFO;
use jupyter_protocol::messaging;
use jupyter_runtime::{
  ConnectionInfo, ExecutionCount, JupyterMessage, JupyterMessageContent, KernelControlConnection,
  KernelIoPubConnection, KernelShellConnection, ReplyError, ReplyStatus, StreamContent,
};
...
impl JupyterServer {
  pub async fn start(
    ...
    let mut heartbeat =
      jupyter_runtime::create_kernel_heartbeat_connection(&connection_info).await?;
    let shell_connection =
      jupyter_runtime::create_kernel_shell_connection(&connection_info, &session_id).await?;
    let control_connection =
      jupyter_runtime::create_kernel_control_connection(&connection_info, &session_id).await?;
    let mut stdin_connection =
      jupyter_runtime::create_kernel_stdin_connection(&connection_info, &session_id).await?;
    let iopub_connection =
      jupyter_runtime::create_kernel_iopub_connection(&connection_info, &session_id).await?;

    let iopub_connection = Arc::new(Mutex::new(iopub_connection));
    let last_execution_request = Arc::new(Mutex::new(None));

    let (stdin_tx1, mut stdin_rx1) = mpsc::unbounded_channel::<JupyterMessage>();
    ...
    let control_fut = deno_core::unsync::spawn({
      ...
      async move {
        if let Err(err) = Self::handle_control(control_connection, cancel_handle).await {
  ...
  async fn handle_control(
    ...
    loop {
      ...
      match msg.content {
        ...
        _ => {
          log::error!("Unrecognized control message type: {}", msg.message_type());
        }
      }
    }
  }

  async fn handle_shell(&mut self, mut connection: KernelShellConnection) -> Result<(), AnyError> {
    loop {
      let msg = connection.read().await?;
  ...
  async fn handle_shell_message(
    ...
    match msg.content {
      ...
      JupyterMessageContent::CompleteRequest(req) => {
        ...
        if !lsp_completions.is_empty() {
          ...
          let (completions, cursor_start) = if let Some(index) = expr.rfind('.') {
            ...
            let candidates = get_expression_property_names(&mut self.repl_session_proxy, sub_expr)
              .await
              .into_iter()
            ...
            if prop_name.len() > cursor_pos {
              // TODO(bartlomieju): most likely not correct, but better than panicking because of
              // sub with overflow
            ...
            let mut candidates =
              get_expression_property_names(&mut self.repl_session_proxy, "globalThis")
                .await
                .into_iter()
                .chain(get_global_lexical_scope_names(&mut self.repl_session_proxy).await)
      ...
      JupyterMessageContent::CommMsg(_) | JupyterMessageContent::CommClose(_) => {
  ...
  async fn handle_execution_request(
    ...
    *self.last_execution_request.lock() = Some(parent_message.clone());
    ...
    if exception_details.is_none() {
      publish_result(&mut self.repl_session_proxy, &result, self.execution_count).await?;
      ...
      let (name, message, stack) = if let Some(exception) = exception_details.exception {
        let result = self
          .repl_session_proxy
        ...
        match result.result.value {
          Some(serde_json::Value::String(str)) => {
            if let Ok(object) = serde_json::from_str::<HashMap<String, String>>(&str) {
  ...
  async fn send_iopub(&mut self, message: JupyterMessage) -> Result<(), AnyError> {
...
async fn publish_result(
  ...
  let Some(response) = repl_session_proxy.call_function_on(arg0, arg1).await else {
    return Ok(None);
  };
...
// TODO(bartlomieju): dedup with repl::editor
async fn get_global_lexical_scope_names(repl_session_proxy: &mut JupyterReplProxy) -> Vec<String> {
...
async fn get_expression_property_names(
  ...
  if let Some(properties) = get_object_expr_properties(repl_session_proxy, expr).await {
    return properties;
  }
...
async fn get_object_expr_properties(
  ...
  let evaluate_result = evaluate_expression(repl_session_proxy, object_expr).await?;
  let object_id = evaluate_result.result.object_id?;

  let get_properties_response = repl_session_proxy.get_properties(object_id.clone()).await?;
  Some(
