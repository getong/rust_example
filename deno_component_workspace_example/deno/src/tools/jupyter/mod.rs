...
use deno_core::{
  ...
  error::AnyError,
  futures::FutureExt,
  located_script_name, serde_json,
  serde_json::json,
  url::Url,
};
...
use crate::{
  CliFactory,
  args::{Flags, JupyterFlags},
  cdp,
  lsp::ReplCompletionItem,
  ops,
  tools::{
    repl,
    test::{
      TestEventWorkerSender, TestFailureFormatOptions, create_single_test_event_channel,
      reporters::PrettyTestReporter,
    },
  },
};
...
pub async fn kernel(flags: Arc<Flags>, jupyter_flags: JupyterFlags) -> Result<(), AnyError> {
  log::info!(
  ...
  let main_module = resolve_url_or_path("./$deno$jupyter.mts", cli_options.initial_cwd()).unwrap();
  // TODO(bartlomieju): should we run with all permissions?
  let permissions = PermissionsContainer::allow_all(factory.permission_desc_parser()?.clone());
  ...
  let (startup_data_tx, startup_data_rx) = oneshot::channel::<server::StartupData>();
  ...
  let join_handle = std::thread::spawn(move || {
    let fut =
      server::JupyterServer::start(spec, stdio_rx, repl_session_proxy_channels, startup_data_tx)
        .boxed_local();
  ...
  {
    let op_state_rc = repl_session_proxy.repl_session.worker.js_runtime.op_state();
...
impl JupyterReplProxy {
  pub async fn lsp_completions(
    ...
    let Some(JupyterReplResponse::LspCompletions(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }

  pub async fn get_properties(&mut self, object_id: String) -> Option<cdp::GetPropertiesResponse> {
    let _ = self
      .tx
      .send(JupyterReplRequest::JsGetProperties { object_id });
    let Some(JupyterReplResponse::JsGetProperties(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }

  pub async fn evaluate(&mut self, expr: String) -> Option<cdp::EvaluateResponse> {
    let _ = self.tx.send(JupyterReplRequest::JsEvaluate { expr });
    let Some(JupyterReplResponse::JsEvaluate(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }

  pub async fn global_lexical_scope_names(&mut self) -> cdp::GlobalLexicalScopeNamesResponse {
    let _ = self.tx.send(JupyterReplRequest::JsGlobalLexicalScopeNames);
    let Some(JupyterReplResponse::JsGlobalLexicalScopeNames(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }
  ...
  pub async fn evaluate_line_with_object_wrapping(
    ...
    let Some(JupyterReplResponse::JsEvaluateLineWithObjectWrapping(resp)) = self.rx.recv().await
    else {
      unreachable!()
    };
    resp
  ...
  pub async fn call_function_on_args(
    ...
    let Some(JupyterReplResponse::JsCallFunctionOnArgs(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }
  ...
  pub async fn call_function_on(
    ...
    let Some(JupyterReplResponse::JsCallFunctionOn(resp)) = self.rx.recv().await else {
      unreachable!()
    };
    resp
  }
...
impl JupyterReplSession {
  ...
  async fn handle_message(&mut self, msg: JupyterReplRequest) -> Result<(), AnyError> {
    let resp = match msg {
      ...
      JupyterReplRequest::JsGetProperties { object_id } => {
        JupyterReplResponse::JsGetProperties(self.get_properties(object_id).await)
      }
      ...
      JupyterReplRequest::JsGlobalLexicalScopeNames => {
        JupyterReplResponse::JsGlobalLexicalScopeNames(self.global_lexical_scope_names().await)
      }
      ...
      JupyterReplRequest::JsCallFunctionOn { arg0, arg1 } => {
        JupyterReplResponse::JsCallFunctionOn(self.call_function_on(arg0, arg1).await)
      }
    };
