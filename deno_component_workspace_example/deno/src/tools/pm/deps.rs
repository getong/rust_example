...
use deno_config::{
  deno_json::{ConfigFile, ConfigFileRc},
...
use deno_package_json::{PackageJsonDepsMap, PackageJsonRc};
use deno_runtime::deno_permissions::PermissionsContainer;
use deno_semver::{
  StackString, Version, VersionReq,
  jsr::JsrPackageReqReference,
  npm::NpmPackageReqReference,
  package::{PackageNv, PackageReq, PackageReqReference},
};
use import_map::{ImportMap, ImportMapWithDiagnostics, SpecifierMapEntry};
...
use crate::{
  ...
  module_loader::ModuleLoadPreparer,
  npm::{CliNpmInstaller, CliNpmResolver, NpmFetchResolver},
  util::{progress_bar::ProgressBar, sync::AtomicFlag},
};
  ...
  pub fn file_path(&self) -> Cow<'_, std::path::Path> {
    match self {
      DepLocation::DenoJson(arc, _, kind) => match kind {
        ImportMapKind::Inline => Cow::Owned(arc.specifier.to_file_path().unwrap()),
...
fn import_map_entries(
  ...
  import_map
    ...
    .map(|entry| {
      (
        KeyPath::from_parts([KeyPart::Imports, KeyPart::String(entry.raw_key.into())]),
        entry,
      )
    })
    .chain(import_map.scopes().flat_map(|scope| {
      let path = KeyPath::from_parts([KeyPart::Scopes, KeyPart::String(scope.raw_key.into())]);
...
fn to_import_map_value_from_imports(deno_json: &ConfigFile) -> serde_json::Value {
...
fn deno_json_import_map(
  ...
  let (value, kind) = if deno_json.json.imports.is_some() || deno_json.json.scopes.is_some() {
    (
...
fn add_deps_from_deno_json(
  ...
  for (key_path, entry) in import_map_entries(&import_map.import_map) {
    ...
    deps.push(Dep {
      location: DepLocation::DenoJson(deno_json.clone(), key_path, import_map_kind.clone()),
      kind,
      req,
      id,
      alias,
        ...
        deno_package_json::PackageJsonDepValue::Req(req) => {
          ...
          deps.push(Dep {
            ...
            location: DepLocation::PackageJson(
              package_json.clone(),
              KeyPath::from_parts([package_dep_kind.into(), KeyPart::String(k.clone())]),
            ),
            req: req.clone(),
            alias,
          })
...
impl DepManager {
  ...
  async fn run_dependency_resolution(&self) -> Result<(), AnyError> {
    ...
    for dep in &self.deps {
      if dep.location.is_deno_json() {
        match dep.kind {
          DepKind::Npm => {
            roots.push(ModuleSpecifier::parse(&format!("npm:/{}/", dep.req)).unwrap())
          }
          DepKind::Jsr => {
            let resolved_nv = graph.packages.mappings().get(&dep.req);
            let resolved_nv = resolved_nv
              .and_then(|nv| {
                let versions = graph.packages.versions_by_name(&dep.req.name)?;
                let mut best = nv;
                for version in versions {
                  if version.version > best.version && dep.req.version_req.matches(&version.version)
                  {
                    best = version;
                  }
                }
            ...
            info_futures.push(async {
              ...
              if let Some(info) = self.jsr_fetch_resolver.package_version_info(&nv).await {
                let specifier = ModuleSpecifier::parse(&format!("jsr:/{}/", &dep.req)).unwrap();
  ...
  async fn load_latest_versions(&self) -> Result<Vec<PackageLatestVersion>, AnyError> {
      ...
      match dep.kind {
        DepKind::Npm => futs.push_back(
          async {
            ...
            let mut semver_compatible = self.npm_fetch_resolver.req_to_nv(semver_req).await;
            ...
            let latest = info
              .and_then(|info| {
                ...
                let can_use_latest =
                  npm_version_satisfies(latest_tag, &semver_req.version_req, &info);
        ...
        DepKind::Jsr => futs.push_back(
          async {
            ...
            let semver_compatible = self.jsr_fetch_resolver.req_to_nv(semver_req).await;
  ...
  pub fn deps_with_resolved_latest_versions(
    ...
    self
      ...
      .enumerate()
      .map(|(i, (resolved, latest))| (DepId(i), resolved.clone(), latest.clone()))
  }
        ...
        Change::Update(dep_id, version_req) => {
          ...
          match &dep.location {
            DepLocation::DenoJson(arc, key_path, _) => {
              let updater = get_or_create_updater(&mut config_updaters, &dep.location)?;

              let Some(property) = updater.get_property_for_mutation(key_path) else {
                log::warn!(
              ...
              let mut new_value = format!("{}:{}", dep.kind.scheme(), req_reference);
              ...
              property.set_value(jsonc_parser::cst::CstInputValue::String(new_value));
            }
            DepLocation::PackageJson(arc, key_path) => {
              let updater = get_or_create_updater(&mut config_updaters, &dep.location)?;
              let Some(property) = updater.get_property_for_mutation(key_path) else {
...
fn get_or_create_updater<'a>(
  ...
  match config_updaters.entry(location.file_path().into_owned()) {
    std::collections::hash_map::Entry::Occupied(occupied_entry) => Ok(occupied_entry.into_mut()),
    std::collections::hash_map::Entry::Vacant(vacant_entry) => {
      let updater = ConfigUpdater::new(location.config_kind(), location.file_path().into_owned())?;
      Ok(vacant_entry.insert(updater))
    }
  }
}

fn cst_string_literal(property: &jsonc_parser::cst::CstObjectProp) -> Option<String> {
...
fn parse_req_reference(
  input: &str,
  kind: DepKind,
) -> Result<PackageReqReference, deno_semver::package::PackageReqReferenceParseError> {
  Ok(match kind {
