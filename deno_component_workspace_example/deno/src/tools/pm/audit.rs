...
pub async fn audit(flags: Arc<Flags>, audit_flags: AuditFlags) -> Result<i32, AnyError> {
  ...
  let r = npm::call_audits_api(audit_flags, npm_url, workspace, &snapshot, http_client).await?;

  if use_socket {
    socket_dev::call_firewall_api(&snapshot, http_provider.get_or_create().unwrap()).await?;
  }
...
mod npm {
  ...
  fn get_dependency_descriptors_for_deps(
    ...
    let resolution_package = match dev_dependencies_snapshot.package_from_id(package_id) {
      Some(p) => {
        is_dev = true;
        p
      }
    ...
    let mut deps_map = HashMap::with_capacity(resolution_package.dependencies.len());
  ...
  pub fn partition_packages<'a>(pkgs: &'a [&NpmPackageId]) -> Vec<Vec<&'a NpmPackageId>> {
    // 1) Group by name
  ...
  pub async fn call_audits_api(
    ...
    let dev_dependencies_snapshot = npm_resolution_snapshot.subset(&all_dev_deps);
    ...
    for partition in top_level_packages_partitions {
      for package in partition {
        ...
        requires.insert(package.nv.name.to_string(), package.nv.version.to_string());
    ...
    advisories.sort_by_cached_key(|adv| format!("{}@{}", adv.module_name, adv.vulnerable_versions));

    let minimal_severity = AdvisorySeverity::parse(&audit_flags.severity).unwrap();
    print_report(
      &vulns,
      advisories,
      response.actions,
  ...
  fn print_report(
    ...
    for adv in advisories {
      ...
      if actions.len() == 1 {
        _ = writeln!(stdout, "╰ {}    {}", colors::gray("Actions:"), actions[0]);
...
mod socket_dev {
  ...
  pub async fn call_firewall_api(
    ...
    let purls = npm_resolution_snapshot
      ...
      .map(|package| format!("pkg:npm/{}@{}", package.id.nv.name, package.id.nv.version))
      .collect::<Vec<_>>();
  ...
  async fn call_authenticated_api(
    ...
    let socket_dev_url = std::env::var("SOCKET_DEV_URL")
      .ok()
      .unwrap_or_else(|| "https://api.socket.dev/v0/purl?actions=error,warn".to_string());
    ...
    let futures = purls
      ...
      .map(|purl| {
        let url = Url::parse(&format!(
          "{}purl/{}",
          socket_dev_url,
          percent_encoding::utf8_percent_encode(purl, percent_encoding::NON_ALPHANUMERIC)
        ))
        .unwrap();
