...
pub async fn audit(flags: Arc<Flags>, audit_flags: AuditFlags) -> Result<i32, AnyError> {
  ...
  npm::call_audits_api(audit_flags, npm_url, workspace, &snapshot, http_client).await
}
  ...
  impl AuditAdvisory {
    fn find_actions(&self, actions: &[AuditAction]) -> Vec<String> {
      ...
      for action in actions {
        ...
        {
          let target_str = if let Some(target) = &action.target {
            format!("@{}", target)
          } else {
            String::new()
          };
...
mod socket_dev {
  ...
  pub async fn call_firewall_api(
    ...
    let mut purl_responses = purl_results
      ...
      .map(|json_response| {
        let response: FirewallResponse = serde_json::from_str(&json_response).unwrap();
        response
      })
      .collect::<Vec<_>>();
