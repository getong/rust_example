...
fn print_suggestion(compatible: bool) {
  ...
  log::info!(
    "{}",
    color_print::cformat!(
      "<p(245)>Run</> <u>deno update{}</> <p(245)>to update to the latest {} \
...
fn print_outdated(deps: &mut DepManager, compatible: bool) -> Result<(), AnyError> {
  let mut outdated = Vec::new();
  let mut seen = std::collections::BTreeSet::new();
  for (dep_id, resolved, latest_versions) in deps.deps_with_resolved_latest_versions() {
    ...
    if latest > &resolved && seen.insert((dep.kind, dep.req.name.clone(), resolved.version.clone()))
    {
...
pub async fn outdated(flags: Arc<Flags>, update_flags: OutdatedFlags) -> Result<(), AnyError> {
  ...
  let filter_set =
    filter::FilterSet::from_filter_strings(update_flags.filters.iter().map(|s| s.as_str()))?;
  ...
  let mut deps = if update_flags.recursive {
    ...
    super::deps::DepManager::from_workspace_dir(&cli_options.start_dir, filter_fn, args)?
  };
...
fn choose_new_version_req(
  ...
  if let Some(version_req) = explicit_version_req {
    ...
    let candidate_version_req =
      VersionReq::parse_from_specifier(format!("{}{}", operator, preferred.version).as_str())
        .unwrap();
    if preferred.version <= resolved.version && candidate_version_req == dep.req.version_req {
      return ChosenVersionReq::None {
...
async fn update(
  ...
  if !to_update.is_empty() {
    ...
    let mut updated_to_versions = updated_to_versions.into_iter().collect::<Vec<_>>();
    ...
    for (package_name, current_version_req, new_version_req) in updated_to_versions {
    ...
    if !update_to_latest && can_update_to_latest {
      ...
      log::info!(
        "All {maybe_matching}dependencies are at newest compatible versions.\n{note}: newer, \
         incompatible versions are available.\n      Run with `--latest` to update",
      );
    } else {
...
mod filter {
  ...
  impl Filter {
    ...
    pub fn from_str(input: &str) -> Result<Self, AnyError> {
      ...
      let (pattern, version_spec) = if let Some(scope_name) = s.strip_prefix('@') {
        if let Some(idx) = scope_name.find('@') {
          let (pattern, version_spec) = s.split_at(idx + 1);
          (
            pattern,
            Some(
              VersionReq::parse_from_specifier(version_spec.trim_start_matches('@'))
      ...
      Ok(Filter {
        kind,
        regex: pattern_to_regex(pattern).with_context(|| format!("Invalid filter \"{input}\""))?,
        version_spec,
      })
    }
  ...
  #[cfg(test)]
  mod test {
    fn matches_filters<'a, 'b>(filters: impl IntoIterator<Item = &'a str>, name: &str) -> bool {
    ...
    fn multiple_globs() {
      ...
      let filters = super::FilterSet::from_filter_strings(["foo*@1", "bar*@2"]).unwrap();
