...
use console_static_text::{ConsoleSize, TextItem};
use crossterm::{
...
use crate::{
  tools::pm::deps::{DepId, DepKind},
  util::console::{HideCursorGuard, RawMode},
};
...
impl From<PackageInfo> for FormattedPackageInfo {
  fn from(package: PackageInfo) -> Self {
    let new_version_string = package.new_version.version_text().trim_start_matches('^');
    ...
    FormattedPackageInfo {
      ...
      current_version_string: package.current_version.as_ref().map(|v| v.to_string()),
...
impl State {
  fn new(packages: Vec<PackageInfo>) -> anyhow::Result<Self> {
    let mut deduped_packages: HashMap<(String, Option<Version>, VersionReq), FormattedPackageInfo> =
  ...
  fn instructions_line() -> &'static str {
    "Select which packages to update (<space> to select, ↑/↓/j/k to navigate, a to select all, i \
     to invert selection, enter to accept, <Ctrl-c> to cancel)"
  }
  ...
  fn render(&self) -> anyhow::Result<Vec<TextItem<'_>>> {
    ...
    for (i, package) in self.packages.iter().enumerate() {
      ...
      let name_pad = " ".repeat(self.name_width + 2 - package.formatted_name_len);
      write!(
        f,
...
pub fn select_interactive(packages: Vec<PackageInfo>) -> anyhow::Result<Option<HashSet<DepId>>> {
  let mut stderr = io::stderr();

  let raw_mode = RawMode::enable()?;
  let mut static_text = console_static_text::ConsoleStaticText::new(move || {
  ...
  loop {
      ...
      && items.len() + first_line_rows >= rows as usize
    {
      ...
      } else if state.currently_selected + 1 >= scroll_offset + (rows as usize).saturating_sub(adj)
      {
        scroll_offset = (state.currently_selected + 1).saturating_sub(rows as usize) + 1;
      }
      let adj = if scroll_offset == 0 {
        first_line_rows.saturating_sub(1)
      } else {
      ...
      new_items.extend(items.drain(
        scroll_offset .. (scroll_offset + (rows as usize).saturating_sub(adj)).min(items.len()),
      ));
      items = new_items;
    }
    ...
    match event {
      crossterm::event::Event::Key(KeyEvent {
        ...
        (KeyCode::Down | KeyCode::Char('j'), KeyModifiers::NONE) => {
          state.currently_selected = (state.currently_selected + 1) % state.packages.len();
        }
        (KeyCode::Char(' '), _) => {
