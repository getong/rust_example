...
use deno_lib::worker::{LibMainWorker, LibMainWorkerFactory, ResolveNpmBinaryEntrypointError};
use deno_npm_installer::{PackageCaching, graph::NpmCachingStrategy};
use deno_runtime::{
  WorkerExecutionMode, coverage::CoverageCollector, deno_permissions::PermissionsContainer,
  worker::MainWorker,
};
...
use crate::{
  ...
  sys::CliSys,
  tools::run::hmr::{HmrRunner, HmrRunnerState},
  util::{
    file_watcher::{WatcherCommunicator, WatcherRestartMode},
...
impl CliMainWorker {
  ...
  pub async fn run_for_watcher(self) -> Result<(), CoreError> {
    /// The FileWatcherModuleExecutor provides module execution with safe dispatching of life-cycle
    /// events by tracking the state of any pending events and emitting accordingly on drop in
    /// the case of a future cancellation.
    ...
    impl FileWatcherModuleExecutor {
      ...
      pub async fn execute(&mut self) -> Result<(), CoreError> {
        ...
        let result = loop {
          ...
          if !web_continue {
            let node_continue = self.inner.worker.dispatch_process_beforeexit_event()?;
            if !node_continue {
              break Ok(());
            }
          }
  ...
  pub fn maybe_setup_coverage_collector(&mut self) -> Option<CoverageCollector> {
...
pub enum CreateCustomWorkerError {
  ...
  #[class(inherit)]
  #[error(transparent)]
  ResolvePkgFolderFromDenoReq(#[from] deno_resolver::npm::ResolvePkgFolderFromDenoReqError),
  #[class(inherit)]
  #[error(transparent)]
      ...
      Ok(package_ref) => {
        if let Some(npm_installer) = &self.npm_installer {
          ...
          npm_installer
            .add_package_reqs(
              reqs,
              if matches!(self.default_npm_caching_strategy, NpmCachingStrategy::Lazy) {
        ...
        let main_module = self
          .lib_main_worker_factory
          .resolve_npm_binary_entrypoint(&package_folder, package_ref.sub_path())?;
...
mod tests {
  ...
  use deno_core::{FsModuleLoader, resolve_path};
  ...
  fn create_test_worker() -> MainWorker {
    let main_module = resolve_path("./hello.js", &std::env::current_dir().unwrap()).unwrap();
    ...
    MainWorker::bootstrap_from_options::<DenoInNpmPackageChecker, CliNpmResolver, CliSys>(
      &main_module,
  ...
  async fn execute_mod_resolve_error() {
    ...
    let module_specifier =
      resolve_path("./does-not-exist", &std::env::current_dir().unwrap()).unwrap();
    let result = worker.execute_main_module(&module_specifier).await;
