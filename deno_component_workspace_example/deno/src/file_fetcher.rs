...
use deno_cache_dir::{
  ...
  file_fetcher::{BlobData, CacheSetting, File, SendError, SendResponse},
};
use deno_core::{ModuleSpecifier, anyhow::Context, error::AnyError, url::Url};
use deno_resolver::{file_fetcher::PermissionedFileFetcherOptions, loader::MemoryFiles};
use deno_runtime::deno_web::BlobStore;
use http::{HeaderMap, StatusCode};

use crate::{
  colors,
  http_util::{HttpClientProvider, get_response_body_with_progress},
  sys::CliSys,
...
impl TextDecodedFile {
  ...
  pub fn decode(file: File) -> Result<Self, AnyError> {
    ...
    let charset = maybe_charset
      .unwrap_or_else(|| deno_media_type::encoding::detect_charset(&specifier, &file.source));
...
pub type CliFileFetcher =
  deno_resolver::file_fetcher::PermissionedFileFetcher<BlobStoreAdapter, CliSys, HttpClientAdapter>;
pub type CliDenoGraphLoader =
  deno_resolver::file_fetcher::DenoGraphLoader<BlobStoreAdapter, CliSys, HttpClientAdapter>;
  ...
  async fn send_no_follow(&self, url: &Url, headers: HeaderMap) -> Result<SendResponse, SendError> {
    ...
    loop {
      ...
      if response.status().is_server_error() {
        handle_request_or_server_error(&mut retried, url, response.status().to_string())
          .await
        ...
        let body_result =
          get_response_body_with_progress(response, maybe_progress_guard.as_ref()).await;
...
mod tests {
  ...
  use deno_cache_dir::{HttpCache, file_fetcher::HttpClient};
  use deno_core::resolve_url;
  use deno_resolver::{
    file_fetcher::{FetchErrorKind, FetchPermissionsOptionRef},
    loader::MemoryFilesRc,
  };
  use deno_runtime::deno_web::{Blob, InMemoryBlobPart};
  use test_util::TempDir;

  use super::*;
  use crate::{cache::GlobalHttpCache, http_util::HttpClientProvider};

  fn setup(
    cache_setting: CacheSetting,
    maybe_temp_dir: Option<TempDir>,
  ) -> (CliFileFetcher, TempDir) {
    let (file_fetcher, temp_dir, _) = setup_with_blob_store(cache_setting, maybe_temp_dir);
  ...
  // in deno_graph
  async fn test_fetch_remote_encoded(fixture: &str, expected_charset: &str, expected: &str) {
    ...
    let (media_type, maybe_charset) =
      deno_graph::source::resolve_media_type_and_charset_from_headers(&specifier, Some(&headers));
    assert_eq!(
  ...
  async fn test_fetch_data_url() {
    ...
    let specifier = resolve_url(
      "data:application/typescript;base64,\
       ZXhwb3J0IGNvbnN0IGEgPSAiYSI7CgpleHBvcnQgZW51bSBBIHsKICBBLAogIEIsCiAgQywKfQo=",
    )
    .unwrap();
  ...
  async fn test_fetch_blob_url() {
    let (file_fetcher, _, blob_store) = setup_with_blob_store(CacheSetting::Use, None);

    let bytes = "export const a = \"a\";\n\nexport enum A {\n  A,\n  B,\n  C,\n}\n"
      .as_bytes()
      .to_vec();
  ...
  async fn test_fetch_complex() {
    ...
    let (file_fetcher_02, _, _, http_cache_02) =
      setup_with_blob_store_and_cache(CacheSetting::Use, Some(temp_dir.clone()));
    let specifier = ModuleSpecifier::parse("http://localhost:4545/subdir/mod2.ts").unwrap();
  ...
  async fn test_fetch_uses_cache() {
    ...
    let location = temp_dir.path().join("remote").to_path_buf();
    let specifier = resolve_url("http://localhost:4545/subdir/mismatch_ext.ts").unwrap();

    let http_cache = Arc::new(GlobalHttpCache::new(CliSys::default(), location.clone()));
  ...
  async fn test_fetch_redirected() {
    let _http_server_guard = test_util::http_server();
    let (file_fetcher, _, _, http_cache) = setup_with_blob_store_and_cache(CacheSetting::Use, None);
    let specifier = resolve_url("http://localhost:4546/subdir/redirects/redirect1.js").unwrap();
    let redirected_specifier =
      resolve_url("http://localhost:4545/subdir/redirects/redirect1.js").unwrap();
    ...
    assert_eq!(
      get_location_header_from_cache(http_cache.as_ref(), &redirected_specifier),
      None,
    );
  }
  ...
  async fn test_fetch_multiple_redirects() {
    let _http_server_guard = test_util::http_server();
    let (file_fetcher, _, _, http_cache) = setup_with_blob_store_and_cache(CacheSetting::Use, None);
    let specifier = resolve_url("http://localhost:4548/subdir/redirects/redirect1.js").unwrap();
    let redirected_01_specifier =
      resolve_url("http://localhost:4546/subdir/redirects/redirect1.js").unwrap();
    ...
    assert_eq!(
      get_location_header_from_cache(http_cache.as_ref(), &redirected_01_specifier),
      Some("http://localhost:4545/subdir/redirects/redirect1.js".to_string()),
    );
    ...
    assert_eq!(
      get_location_header_from_cache(http_cache.as_ref(), &redirected_02_specifier),
      None,
    );
  }
  ...
  async fn test_fetch_uses_cache_with_redirects() {
    ...
    let temp_dir = TempDir::new();
    let location = temp_dir.path().join("remote").to_path_buf();
    let specifier = resolve_url("http://localhost:4548/subdir/mismatch_ext.ts").unwrap();
    let redirected_specifier = resolve_url("http://localhost:4546/subdir/mismatch_ext.ts").unwrap();
    let http_cache = Arc::new(GlobalHttpCache::new(CliSys::default(), location.clone()));
  ...
  async fn test_fetch_same_host_redirect() {
    ...
    let (file_fetcher, _, _, http_cache) = setup_with_blob_store_and_cache(CacheSetting::Use, None);
    let specifier =
      resolve_url("http://localhost:4550/REDIRECT/subdir/redirects/redirect1.js").unwrap();
    let redirected_specifier =
      resolve_url("http://localhost:4550/subdir/redirects/redirect1.js").unwrap();
  ...
  async fn test_fetch_no_remote() {
    ...
    let specifier = resolve_url("http://localhost:4545/run/002_hello.ts").unwrap();
    ...
    let err = match err.into_kind() {
      deno_resolver::file_fetcher::FetchNoFollowErrorKind::FetchNoFollow(err) => err,
      _ => unreachable!(),
    };
    ...
    match &err {
      deno_cache_dir::file_fetcher::FetchNoFollowErrorKind::NoRemote { .. } => {
        assert_eq!(
          err.to_string(),
          "A remote specifier was requested: \"http://localhost:4545/run/002_hello.ts\", but \
  ...
  async fn test_fetch_cache_only() {
    ...
    let file_fetcher_01 = create_cli_file_fetcher(
      Default::default(),
      Arc::new(GlobalHttpCache::new(CliSys::default(), location.clone())).into(),
      Arc::new(HttpClientProvider::new(None, None)),
      MemoryFilesRc::default(),
      CliSys::default(),
      CreateCliFileFetcherOptions {
    ...
    match &err {
      deno_cache_dir::file_fetcher::FetchNoFollowErrorKind::NotCached { .. } => {
        assert_eq!(
          err.to_string(),
          "Specifier not found in cache: \"http://localhost:4545/run/002_hello.ts\", \
           --cached-only is specified."
        );
      }
      _ => unreachable!(),
    }
  ...
  async fn test_fetch_local_utf_16be() {
    let expected = String::from_utf8(b"console.log(\"Hello World\");\x0A".to_vec()).unwrap();
  ...
  async fn test_fetch_remote_utf16_le() {
    let expected = std::str::from_utf8(b"console.log(\"Hello World\");\x0A").unwrap();
  ...
  async fn test_fetch_remote_window_1255() {
    let expected =
      "console.log(\"\u{5E9}\u{5DC}\u{5D5}\u{5DD} \u{5E2}\u{5D5}\u{5DC}\u{5DD}\");\u{A}";
  ...
  async fn test_fetch_gzip() {
    let _http_server_guard = test_util::http_server();
    let url = Url::parse("http://127.0.0.1:4545/run/import_compression/gziped").unwrap();
    let client = create_http_client_adapter();
    let result = client.send_no_follow(&url, HeaderMap::new()).await;
    if let Ok(SendResponse::Success(headers, body)) = result {
  ...
  async fn test_fetch_brotli() {
    let _http_server_guard = test_util::http_server();
    let url = Url::parse("http://127.0.0.1:4545/run/import_compression/brotli").unwrap();
    let client = create_http_client_adapter();
    let result = client.send_no_follow(&url, HeaderMap::new()).await;
    if let Ok(SendResponse::Success(headers, body)) = result {
      assert!(!body.is_empty());
  ...
  #[track_caller]
  fn get_text_from_cache(http_cache: &dyn HttpCache, url: &ModuleSpecifier) -> String {
