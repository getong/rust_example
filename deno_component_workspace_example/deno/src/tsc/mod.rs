...
use deno_core::{
  FastString, JsRuntime, ModuleSpecifier, OpState, RuntimeOptions,
  anyhow::Context,
  located_script_name, op2,
  serde::{Deserialize, Deserializer, Serialize, Serializer},
  serde_json::json,
  url::Url,
};
use deno_graph::{GraphKind, Module, ModuleGraph, ResolutionResolved};
use deno_lib::{
  util::{checksum, hash::FastInsecureHasher},
  worker::create_isolate_create_params,
};
use deno_path_util::resolve_url_or_path;
use deno_resolver::npm::{
  ResolvePkgFolderFromDenoReqError, managed::ResolvePkgFolderFromDenoModuleError,
};
use deno_semver::npm::NpmPackageReqReference;
use indexmap::IndexMap;
use node_resolver::{
  NodeResolutionKind, ResolutionMode,
  errors::{NodeJsErrorCode, NodeJsErrorCoded, PackageSubpathFromDenoModuleResolveError},
...
use crate::{
  args::{CompilerOptions, TypeCheckMode},
  cache::ModuleInfoCache,
  node::CliNodeResolver,
  npm::CliNpmResolver,
  resolver::CliCjsTracker,
  sys::CliSys,
  util::path::mapped_specifier_for_tsc,
};

mod diagnostics;

pub use self::diagnostics::{Diagnostic, DiagnosticCategory, Diagnostics, Position};
...
macro_rules! maybe_compressed_source {
  ...
  (compressed = $comp: expr, uncompressed = $uncomp: expr) => {{
    ...
    #[cfg(not(feature = "hmr"))]
    {
      ...
      {
        StaticAssetSource::Compressed(CompressedSource::new(include_bytes!(concat!(
          env!("OUT_DIR"),
          "/",
          $comp,
          ".zstd"
...
impl StaticAssetSource {
  pub fn as_str(&'static self) -> &'static str {
    match self {
      StaticAssetSource::Compressed(compressed_source) => compressed_source.get(),
      StaticAssetSource::Uncompressed(src) => src,
      StaticAssetSource::Owned(path, cell) => {
        let str = cell.get_or_init(|| std::fs::read_to_string(path).unwrap().into());
        str.as_ref()
      }
    }
  }
...
pub static LAZILY_LOADED_STATIC_ASSETS: Lazy<IndexMap<&'static str, StaticAsset>> =
  Lazy::new(|| {
    IndexMap::from([
      ...
      maybe_compressed_lib!("lib.deno.webstorage.d.ts", "lib.deno_webstorage.d.ts"),
      ...
      (
        ...
        StaticAsset {
          is_lib: false,
          source: StaticAssetSource::Uncompressed("/// <reference types=\"npm:@types/node\" />\n"),
        },
      ),
    ])
  });
...
impl TypeCheckingCjsTracker {
  pub fn new(cjs_tracker: Arc<CliCjsTracker>, module_info_cache: Arc<ModuleInfoCache>) -> Self {
    Self {
      cjs_tracker,
      module_info_cache,
    }
...
impl State {
  ...
  pub fn maybe_remapped_specifier(&self, specifier: &str) -> Option<&ModuleSpecifier> {
    self
      .remapped_specifiers
      .get(specifier)
...
#[op2(fast)]
fn op_emit(state: &mut OpState, #[string] data: String, #[string] file_name: String) -> bool {
...
pub const MISSING_DEPENDENCY_SPECIFIER: &str = "internal:///missing_dependency.d.ts";
...
fn op_load_inner(
  ...
  fn load_from_node_modules(
    ...
    *is_cjs = npm_state
      .map(|npm_state| npm_state.cjs_tracker.is_cjs(specifier, *media_type, &code))
      .unwrap_or(false);
    Ok(Some(code.into()))
  }
  ...
  // handle the request for that module here.
    ...
    let specifier = if let Some(remapped_specifier) = state.maybe_remapped_specifier(load_specifier)
    {
      remapped_specifier
    } else {
      &specifier
      ...
      match module {
        ...
        Module::External(module) => {
          if module.specifier.scheme() != "file" {
            ...
            let specifier =
              resolve_specifier_into_node_modules(&CliSys::default(), &module.specifier);
    ...
    {
      load_from_node_modules(specifier, Some(npm), &mut media_type, &mut is_cjs)?
    } else {
      None
    };
...
pub fn load_raw_import_source(specifier: &Url) -> Option<&'static str> {
  ...
  let source = match raw_import {
    RawImportKind::Bytes => "const data: Uint8Array<ArrayBuffer>;\nexport default data;\n",
...
#[op2]
#[string]
fn op_remap_specifier(state: &mut OpState, #[string] specifier: &str) -> Option<String> {
...
fn op_resolve_inner(
  ...
  for (is_cjs, specifier) in args.specifiers {
    ...
    let maybe_result = match resolved_dep {
      ...
      _ => {
        match resolve_non_graph_specifier_types(&specifier, &referrer, resolution_mode, state) {
    ...
    let result = match maybe_result {
      Some((specifier, media_type)) => {
        let specifier_str = match specifier.scheme() {
          ...
          _ => {
            if let Some(specifier_str) = mapped_specifier_for_tsc(&specifier, media_type) {
              state
                .remapped_specifiers
                .insert(specifier_str.clone(), specifier);
              specifier_str
...
fn resolve_graph_specifier_types(
  ...
  match maybe_module {
    Some(Module::Js(module)) => Ok(Some((module.specifier.clone(), module.media_type))),
    Some(Module::Json(module)) => Ok(Some((module.specifier.clone(), module.media_type))),
    Some(Module::Wasm(module)) => Ok(Some((module.specifier.clone(), MediaType::Dmts))),
    Some(Module::Npm(module)) => {
      if let Some(npm) = &state.maybe_npm.as_ref() {
        ...
        let res_result = npm.node_resolver.resolve_package_subpath_from_deno_module(
          &package_folder,
          ...
          Err(err) => match err.code() {
            NodeJsErrorCode::ERR_TYPES_NOT_FOUND => {
              let reqs = npm
                ...
                .resolution()
                .package_reqs();
              if let Some((_, types_nv)) = deno_resolver::npm::find_definitely_typed_package(
    ...
    Some(Module::External(module)) => {
      ...
      Ok(state.maybe_npm.as_ref().map(|_| {
        let specifier = resolve_specifier_into_node_modules(&CliSys::default(), &module.specifier);
...
fn resolve_non_graph_specifier_types(
  ...
  resolution_mode: ResolutionMode,
  state: &State,
) -> Result<Option<(ModuleSpecifier, MediaType)>, ResolveNonGraphSpecifierTypesError> {
    ...
    match NpmPackageReqReference::from_str(raw_specifier) {
      Ok(npm_req_ref) => {
        ...
        let maybe_url = match res_result {
          ...
          Err(err) => match err.code() {
            NodeJsErrorCode::ERR_TYPES_NOT_FOUND | NodeJsErrorCode::ERR_MODULE_NOT_FOUND => None,
...
pub(crate) fn decompress_source(contents: &[u8]) -> Arc<str> {
  ...
  let uncompressed = zstd::bulk::decompress(&contents[4 ..], len as usize).unwrap();
...
impl deno_core::ExtCodeCache for TscExtCodeCache {
  fn get_code_cache_info(
    ...
    let data = self
      ...
      .map(Cow::from)
      .inspect(|_| {
        log::debug!("V8 code cache hit for Extension module: {specifier}, [{code_hash:?}]");
      });
    deno_core::SourceCodeCacheInfo {
      hash: code_hash,
      data,
  ...
  fn code_cache_ready(
    ...
    log::debug!("Updating V8 code cache for Extension module: {specifier}, [{source_hash:?}]");
    self.cache.set_sync(
      specifier,
      if esm {
        CodeCacheType::EsModule
...
pub fn exec(
  ...
  let extension_code_cache =
    code_cache.map(|cache| Rc::new(TscExtCodeCache::new(cache)) as Rc<dyn deno_core::ExtCodeCache>);
...
#[cfg(test)]
mod tests {
  use deno_core::{OpState, futures::future, parking_lot::Mutex, serde_json};
  use deno_error::JsErrorBox;
  use deno_graph::{GraphKind, ModuleGraph};
  use deno_runtime::code_cache::CodeCacheType;
  use test_util::PathRef;

  use super::{Diagnostic, DiagnosticCategory, *};
  ...
  impl deno_graph::source::Loader for MockLoader {
    fn load(
      ...
      let response = source_path
        ...
        .map_err(|e| {
          deno_graph::source::LoadError::Other(Arc::new(JsErrorBox::generic(e.to_string())))
        });
  ...
  async fn setup(
    ...
    let specifier =
      maybe_specifier.unwrap_or_else(|| ModuleSpecifier::parse("file:///main.ts").unwrap());
  ...
  async fn test_hash_url() {
    let specifier =
      deno_core::resolve_url("data:application/javascript,console.log(\"Hello%20Deno\");").unwrap();
    assert_eq!(
  ...
  async fn test_load() {
    ...
    .await;
    let actual = op_load_inner(&mut state, "https://deno.land/x/mod.ts").unwrap();
    assert_eq!(
  ...
  async fn test_load_missing_specifier() {
    let mut state = setup(None, None, None).await;
    let actual =
      op_load_inner(&mut state, "https://deno.land/x/mod.ts").expect("should have invoked op");
