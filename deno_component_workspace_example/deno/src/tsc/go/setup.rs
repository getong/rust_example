...
pub async fn ensure_tsgo(
  ...
  std::fs::create_dir_all(&folder_path).map_err(DownloadError::CreateTempDirFailed)?;
  ...
  std::fs::write(&path, &bytes)
    .map_err(|e| DownloadError::WriteZipFailed(path.display().to_string(), e))?;
  ...
  let unpacked_path = crate::util::archive::unpack_into_dir(crate::util::archive::UnpackArgs {
    exe_name: "tsgo",
