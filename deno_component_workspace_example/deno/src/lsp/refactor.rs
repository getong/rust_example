...
pub static EXTRACT_FUNCTION: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static EXTRACT_CONSTANT: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static EXTRACT_TYPE: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static EXTRACT_INTERFACE: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static MOVE_NEWFILE: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static REWRITE_IMPORT: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
  kind: [lsp::CodeActionKind::REFACTOR_REWRITE.as_str(), "import"]
    .join(".")
    .into(),
  matches_callback: Box::new(|tag: &str| {
    tag.starts_with("Convert namespace import") || tag.starts_with("Convert named imports")
  }),
});

pub static REWRITE_EXPORT: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
  kind: [lsp::CodeActionKind::REFACTOR_REWRITE.as_str(), "export"]
    .join(".")
    .into(),
  matches_callback: Box::new(|tag: &str| {
    tag.starts_with("Convert default export") || tag.starts_with("Convert named export")
  }),
});
...
pub static ALL_KNOWN_REFACTOR_ACTION_KINDS: Lazy<Vec<&'static RefactorCodeActionKind>> =
  Lazy::new(|| {
    vec![
      &EXTRACT_FUNCTION,
      &EXTRACT_CONSTANT,
...
pub fn prune_invalid_actions(
  ...
  for action in actions {
    ...
    let action_kind = action.kind.as_ref().map(|a| a.as_str()).unwrap_or_default();
    ...
    // These are the remaining refactors that we can show if we haven't reached the max limit with
    // just common refactors.
  ...
  let top_n_invalid =
    prioritized_actions[0 .. std::cmp::min(number_of_invalid, prioritized_actions.len())].to_vec();
