...
use deno_cache_dir::{HttpCache, npm::NpmCacheDir};
...
use deno_npm_installer::{
  ...
  initializer::{NpmResolutionInitializer, NpmResolverManagedSnapshotOption},
  lifecycle_scripts::NullLifecycleScriptsExecutor,
  package_json::NpmInstallDepsProvider,
  resolution::NpmResolutionInstaller,
};
use deno_path_util::url_to_file_path;
use deno_resolver::{
  DenoResolverOptions, NodeAndNpmResolvers,
  cjs::IsCjsResolutionMode,
  deno_json::{CompilerOptionsResolver, JsxImportSourceConfig},
  graph::FoundPackageJsonDepFlag,
  npm::{
    CreateInNpmPkgCheckerOptions, DenoInNpmPackageChecker, NpmReqResolverOptions,
    managed::{
...
use node_resolver::{
  DenoIsBuiltInNodeModuleChecker, NodeResolutionKind, NodeResolverOptions, PackageJson,
  PackageJsonThreadLocalCache, ResolutionMode,
  cache::{NodeResolutionSys, NodeResolutionThreadLocalCache},
};
use once_cell::sync::Lazy;

use super::{cache::LspCache, documents::DocumentModule, jsr::JsrCacheResolver};
use crate::{
  args::CliLockfile,
  factory::Deferred,
  ...
  sys::CliSys,
  tsc::into_specifier_and_media_type,
  util::progress_bar::{ProgressBar, ProgressBarStyle},
};
...
impl LspScopedResolver {
  ...
  pub fn as_maybe_managed_npm_resolver(&self) -> Option<&Arc<CliManagedNpmResolver>> {
  ...
  pub fn jsr_to_resource_url(&self, req_ref: &JsrPackageReqReference) -> Option<ModuleSpecifier> {
    self.jsr_resolver.as_ref()?.jsr_to_resource_url(req_ref)
  }

  pub fn jsr_lookup_bare_specifier_for_workspace_file(&self, specifier: &Url) -> Option<String> {
    self
      .jsr_resolver
      .as_ref()?
  ...
  pub fn jsr_lookup_export_for_path(&self, nv: &PackageNv, path: &str) -> Option<String> {
  ...
  pub fn npm_to_file_url(
    ...
    Some(into_specifier_and_media_type(Some(
      npm_pkg_req_resolver
        .resolve_req_reference(req_ref, referrer, resolution_mode, resolution_kind)
        .ok()?
        .into_url()
        .ok()?,
    )))
  ...
  pub fn resolve_redirects(&self, specifier: &ModuleSpecifier) -> Option<ModuleSpecifier> {
...
impl LspResolver {
  pub async fn from_config(
    ...
    for (scope, config_data) in config.tree.data_by_scope().as_ref() {
      by_scope.insert(
        scope.clone(),
        Arc::new(
          LspScopedResolver::from_config_data(Some(config_data), cache, http_client_provider).await,
        ),
      );
    }
    let unscoped =
      Arc::new(LspScopedResolver::from_config_data(None, cache, http_client_provider).await);
  ...
  pub fn set_compiler_options_resolver(&self, value: &Arc<CompilerOptionsResolver>) {
    for resolver in std::iter::once(&self.unscoped).chain(self.by_scope.values()) {
      resolver
        .workspace_resolver
  ...
  pub fn get_scoped_resolver(&self, file_referrer: Option<&ModuleSpecifier>) -> &LspScopedResolver {
...
impl ConfiguredDepResolutions {
  fn new(
    ...
    let insert_export_resolutions = |key_prefix: &str,
                                     dep_req_str: &str,
      ...
      for export_key in export_keys {
        ...
        let Some(req_ref) =
          NpmPackageReqReference::from_str(&format!("npm:{dep_req_str}/{export_name}")).ok()
        else {
          continue;
        };
    ...
    if let Some(package_json) = package_json {
      ...
      for name in dependencies.keys() {
        let Some(req_ref) = NpmPackageReqReference::from_str(&format!("npm:{name}")).ok() else {
          continue;
        };
        let mut dep_package_json = None;
        for kind in [NodeResolutionKind::Types, NodeResolutionKind::Execution] {
  ...
  fn dep_key_from_resolution(&self, resolution: &Url, referrer: &Url) -> Option<String> {
    self
      .deps_by_resolution
      .get(resolution)
...
impl<'a> ResolverFactory<'a> {
    ...
    let options = if enable_byonm {
      ...
      let npm_resolution_initializer = Arc::new(NpmResolutionInitializer::new(
        ...
        match self.config_data.and_then(|d| d.lockfile.as_ref()) {
          Some(lockfile) => NpmResolverManagedSnapshotOption::ResolveFromLockfile(lockfile.clone()),
      ...
      let maybe_node_modules_path = self.config_data.and_then(|d| d.node_modules_dir.clone());
  ...
  pub fn cli_resolver(&self) -> &Arc<CliResolver> {
    self.services.cli_resolver.get_or_init(|| {
      ...
      let deno_resolver = Arc::new(deno_resolver::RawDenoResolver::new(DenoResolverOptions {
        in_npm_pkg_checker: self.in_npm_pkg_checker().clone(),
        node_and_req_resolver: match (self.node_resolver(), npm_req_resolver, self.npm_resolver()) {
          (Some(node_resolver), Some(npm_req_resolver), Some(npm_resolver)) => {
  ...
  pub fn workspace_resolver(&self) -> &Arc<WorkspaceResolver<CliSys>> {
    self.services.workspace_resolver.get_or_init(|| {
      let workspace_resolver = self
        .config_data
        .map(|d| {
          let unstable_sloppy_imports = std::env::var("DENO_UNSTABLE_SLOPPY_IMPORTS").is_ok()
  ...
  pub fn in_npm_pkg_checker(&self) -> &DenoInNpmPackageChecker {
    self.services.in_npm_pkg_checker.get_or_init(|| {
      DenoInNpmPackageChecker::new(match &self.services.npm_resolver {
        Some(CliNpmResolver::Byonm(_)) | None => CreateInNpmPkgCheckerOptions::Byonm,
...
type GetHeadersFn = Box<dyn Fn(&Url) -> Option<HashMap<String, String>> + Send + Sync>;
...
impl RedirectResolver {
  fn new(cache: Arc<dyn HttpCache>, lockfile: Option<Arc<CliLockfile>>) -> Self {
...
impl AddNpmReqsThread {
  pub fn create() -> Self {
    let (request_tx, mut request_rx) = tokio::sync::mpsc::unbounded_channel::<AddNpmReqsRequest>();
    let join_handle = std::thread::spawn(move || {
      create_basic_runtime().block_on(async move {
        while let Some((npm_installer, reqs, response_tx)) = request_rx.recv().await {
...
static ADD_NPM_REQS_THREAD: Lazy<AddNpmReqsThread> = Lazy::new(AddNpmReqsThread::create);
...
mod tests {
  ...
  fn test_redirect_resolver() {
    ...
    assert_eq!(
      redirect_resolver.resolve(&Url::parse("https://foo/redirect_1.js").unwrap()),
      Some(Url::parse("https://foo/file.js").unwrap())
    );
    assert_eq!(
      redirect_resolver.resolve(&Url::parse("https://foo/redirect_2.js").unwrap()),
      Some(Url::parse("https://foo/file.js").unwrap())
    );
    assert_eq!(
    ...
    assert_eq!(
      redirect_resolver.chain(&Url::parse("https://foo/redirect_2.js").unwrap()),
      vec![
        (
