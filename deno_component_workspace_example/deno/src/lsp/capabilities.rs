...
use super::{refactor::ALL_KNOWN_REFACTOR_ACTION_KINDS, semantic_tokens::get_legend};
...
fn code_action_capabilities(
  ...
  client_capabilities
    ...
    .map(|_| {
      let mut code_action_kinds = vec![CodeActionKind::QUICKFIX, CodeActionKind::REFACTOR];
...
pub fn semantic_tokens_registration_options() -> SemanticTokensRegistrationOptions {
  ...
  let mut document_filters = Vec::with_capacity(LANGUAGES.len() * SCHEMES.len());
...
pub fn server_capabilities(client_capabilities: &ClientCapabilities) -> ServerCapabilities {
  ...
  ServerCapabilities {
    ...
    completion_provider: Some(CompletionOptions {
      ...
      all_commit_characters: Some(vec![".".to_string(), ";".to_string(), "(".to_string()]),
