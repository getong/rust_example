...
use deno_config::{deno_json::CompilerOptions, workspace::TsTypeLib};
use deno_core::url::Url;
use deno_resolver::deno_json::{
  CompilerOptionsKey, CompilerOptionsResolver, CompilerOptionsType, JsxImportSourceConfig,
  TsConfigFile, get_base_compiler_options_for_emit,
};

use crate::{
  lsp::{config::Config, logging::lsp_warn, resolver::LspResolver},
  sys::CliSys,
  util::fs::canonicalize_path_maybe_not_exists,
};
...
impl LspCompilerOptionsResolver {
      ...
      .map(|(k, d, f)| {
        (
          ...
          LspCompilerOptionsData {
            workspace_dir_or_source_url: d.workspace_dir_or_source_url().cloned(),
            compiler_options: d
  ...
  pub fn for_key(&self, key: &CompilerOptionsKey) -> Option<&LspCompilerOptionsData> {
    self.data.get(key)
  }

  pub fn entries(&self) -> impl Iterator<Item = (&CompilerOptionsKey, &LspCompilerOptionsData)> {
    self.data.iter()
  }
