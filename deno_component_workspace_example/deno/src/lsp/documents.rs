...
use dashmap::DashMap;
use deno_ast::{MediaType, ParsedSource, SourceTextInfo, swc::ecma_visit::VisitWith};
use deno_core::{
  ModuleSpecifier,
  error::AnyError,
  futures::{FutureExt, future, future::Shared},
  parking_lot::RwLock,
  resolve_url,
  url::{Position, Url},
};
...
use super::{
  cache::{LspCache, calculate_fs_version_at_path},
  config::Config,
  logging::lsp_warn,
  resolver::{LspResolver, ScopeDepInfo, SingleReferrerGraphResolver},
  testing::{TestCollector, TestModule},
  text::LineIndex,
  tsc::{ChangeKind, NavigationTree},
  urls::{COMPONENT, normalize_uri, uri_is_file_like, uri_to_file_path, uri_to_url, url_to_uri},
};
use crate::{
  graph_util::CliJsrUrlProvider,
  lsp::compiler_options::{LspCompilerOptionsData, LspCompilerOptionsResolver},
};
...
fn remote_url_to_uri(url: &Url) -> Option<Uri> {
  ...
  for p in url[Position::BeforeHost ..].split('/') {
    string.push('/');
    string.push_str(&percent_encoding::utf8_percent_encode(p, COMPONENT).to_string());
  }
  Uri::from_str(&string)
    .inspect_err(|err| lsp_warn!("Couldn't convert remote URL \"{url}\" to URI: {err}"))
    .ok()
}
...
fn asset_url_to_uri(url: &Url) -> Option<Uri> {
  ...
  Uri::from_str(&format!("deno:/asset{}", url.path()))
    .inspect_err(|err| lsp_warn!("Couldn't convert asset URL \"{url}\" to URI: {err}"))
    .ok()
}
...
fn data_url_to_uri(url: &Url) -> Option<Uri> {
  ...
  Uri::from_str(&format!("deno:/data_url/{hash}{extension}",))
    .inspect_err(|err| lsp_warn!("Couldn't convert data url \"{url}\" to URI: {err}"))
    .ok()
}
...
impl ServerDocument {
  fn load(uri: &Uri) -> Option<Self> {
    ...
    if scheme.eq_lowercase("file") {
      ...
      let text: Arc<str> = bytes_to_content(&url, media_type, bytes, None).ok()?.into();
  ...
  fn remote_url(uri: &Uri, url: Arc<Url>, scope: Option<&Url>, cache: &LspCache) -> Option<Self> {
    let media_type = MediaType::from_specifier(&url);
  ...
  fn data_url(uri: &Uri, url: Arc<Url>) -> Option<Self> {
    let raw_data_url = deno_media_type::data_url::RawDataUrl::parse(&url).ok()?;
  ...
  pub fn text(&self) -> DocumentText {
    match &self.kind {
      ...
      ServerDocumentKind::RemoteUrl { text, .. } => DocumentText::Arc(text.clone()),
      ServerDocumentKind::DataUrl { text, .. } => DocumentText::Arc(text.clone()),
...
pub static ASSET_DOCUMENTS: Lazy<AssetDocuments> = Lazy::new(|| AssetDocuments {
...
impl Documents {
  ...
  pub fn close_notebook(&mut self, uri: &Uri) -> Vec<Arc<OpenDocument>> {
    let uri = normalize_uri(uri);
    let Some(cell_uris) = self.cells_by_notebook_uri.remove(uri.as_ref()) else {
      lsp_warn!(
  ...
  pub fn get(&self, uri: &Uri) -> Option<Document> {
    ...
    let doc = match ServerDocument::load(&uri) {
      ...
      _ => match self.data_urls_by_uri.get(&uri) {
        Some(data_url) => ServerDocument::data_url(&uri, data_url.value().clone())?,
        _ => {
          return None;
        }
      },
  ...
  pub fn get_for_specifier(
    ...
    if scheme == "file" {
      ...
      if let Some(vendored_specifier) = cache.vendored_specifier(specifier, scope) {
        ...
        let doc = Arc::new(ServerDocument::remote_url(&uri, url, scope, cache)?);
  ...
  pub fn filtered_docs(&self, predicate: impl FnMut(&Document) -> bool) -> Vec<Document> {
    self
      .open
      .values()
      .map(|d| Document::Open(d.clone()))
...
impl DocumentModule {
  ...
  pub fn new(
    ...
    let (parsed_source, maybe_module, resolution_mode) = if media_type_is_diagnosable(media_type) {
    ...
    let test_module_fut = get_maybe_test_module_fut(parsed_source.as_ref(), config);
    DocumentModule {
...
impl DocumentModules {
  pub fn update_config(
    ...
    self.documents.server.retain(|_, d| {
      let Some(module) = self.inspect_primary_module(&Document::Server(d.clone())) else {
        return false;
      };
  ...
  pub fn close_document(&mut self, uri: &Uri) -> Result<Arc<OpenDocument>, AnyError> {
    self.dep_info_by_scope = Default::default();
    ...
    if uri.scheme().is_some_and(|s| s.eq_lowercase("file")) && self.config.uri_enabled(uri) {
  ...
  pub fn close_notebook_document(&mut self, uri: &Uri) -> Vec<Arc<OpenDocument>> {
    self.dep_info_by_scope = Default::default();
  ...
  pub fn release(
    ...
    let Some(module) = self.module_for_specifier(specifier, scope, compiler_options_key) else {
      return;
    };
  ...
  fn module_inner(
      ...
      || self.cache.in_global_cache_directory(&specifier)
    {
      ...
      self
        .compiler_options_resolver
        .entry_for_specifier(if scheme != "file" && scope.is_some() {
  ...
  pub fn module(&self, document: &Document, scope: Option<&Url>) -> Option<Arc<DocumentModule>> {
  ...
  pub fn module_for_specifier(
    ...
    let specifier = match JsrPackageReqReference::from_specifier(specifier) {
      Ok(jsr_req_ref) => Cow::Owned(scoped_resolver.jsr_to_resource_url(&jsr_req_ref)?),
  ...
  pub fn inspect_primary_module(&self, document: &Document) -> Option<Arc<DocumentModule>> {
    if let Some(scope) = self.primary_scope(document.uri()) {
      return self
  ...
  fn modules_for_scope(&self, scope: Option<&Url>) -> Option<&Arc<WeakDocumentModuleMap>> {
    match scope {
  ...
  pub fn primary_specifier(&self, document: &Document) -> Option<(Arc<Url>, MediaType)> {
    self
  ...
  pub fn dep_info_by_scope(&self) -> Arc<BTreeMap<Option<Arc<Url>>, Arc<ScopeDepInfo>>> {
    type ScopeEntry<'a> = (Option<&'a Arc<Url>>, &'a Arc<WeakDocumentModuleMap>);
    let dep_info_from_scope_entry = |(scope, modules): ScopeEntry<'_>| {
      ...
      let mut visit_module = |module: &DocumentModule| {
        for dependency in module.dependencies.values() {
          ...
          if dependency.maybe_deno_types_specifier.is_some()
            && let (Some(code_specifier), Some(type_specifier)) = (code_specifier, type_specifier)
      ...
      let config_data = scope.and_then(|s| self.config.tree.data_by_scope().get(s));
      if let Some(config_data) = config_data {
        (|| {
          ...
          let import_source = compiler_options_data
            ...
            .import_source
            .as_ref()?;
          let scoped_resolver = self.resolver.get_scoped_resolver(scope.map(|s| s.as_ref()));
  ...
  pub fn resolve(
    ...
    let referrer_module = self.module_for_specifier(referrer, scope, compiler_options_key);
    ...
    for (is_cjs, raw_specifier) in raw_specifiers {
      ...
      } else if let Some(dep) = dependencies.as_ref().and_then(|d| d.get(raw_specifier)) {
  ...
  pub fn resolve_dependency(
    ...
    let Some(module) = self.module_for_specifier(&specifier, scope, compiler_options_key) else {
      let media_type = media_type.unwrap_or_else(|| MediaType::from_specifier(&specifier));
...
type TestModuleFut = Shared<Pin<Box<dyn Future<Output = Option<Arc<TestModule>>> + Send>>>;
...
fn get_maybe_test_module_fut(
  ...
  if specifier.scheme() != "file" || specifier.as_str().contains("/node_modules/") {
    return None;
  }
...
fn resolve_media_type(
  ...
  if let Some(language_id) = maybe_language_id {
    return MediaType::from_specifier_and_content_type(specifier, language_id.as_content_type());
  }
...
impl OpenDocumentsGraphLoader<'_> {
  fn load_from_docs(&self, specifier: &ModuleSpecifier) -> Option<deno_graph::source::LoadFuture> {
    if specifier.scheme() == "file"
...
fn analyze_module(
  ...
  match parsed_source_result {
    Ok(parsed_source) => {
      ...
      let module_resolution_mode = is_cjs_resolver
        .get_lsp_resolution_mode(&specifier, Some(parsed_source.compute_is_script()));
...
fn bytes_to_content(
  ...
  if media_type == MediaType::Wasm {
    ...
    let charset =
      maybe_charset.unwrap_or_else(|| deno_media_type::encoding::detect_charset(specifier, &bytes));
...
"#
  ...
  async fn test_documents_refresh_dependencies_config_change() {
    ...
    let workspace_settings = serde_json::from_str(r#"{ "enable": true }"#).unwrap();
