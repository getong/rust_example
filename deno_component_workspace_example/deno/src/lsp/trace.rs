...
mod real_tracing {
  ...
  pub use tracing::{Span, span::EnteredSpan};
  ...
  use super::{TracingCollector, TracingConfig, TracingGuard};
  ...
  pub(crate) fn init_tracing_subscriber(
    ...
    let open_telemetry_layer = match config.collector {
      TracingCollector::OpenTelemetry => Some(OpenTelemetryLayer::new(make_tracer(
        config.collector_endpoint.as_deref(),
      )?)),
      _ => None,
    };
