...
use super::{
  definitions::{TestDefinition, TestModule},
  lsp_custom,
  server::TestServerTests,
};
use crate::{
  args::{DenoSubcommand, flags_from_vec, parallelism_count},
  factory::CliFactory,
  lsp::{
    client::{Client, TestingNotification},
    config,
    logging::lsp_log,
    urls::{uri_parse_unencoded, uri_to_url, url_to_uri},
  },
  tools::{
    test,
    test::{FailFastTracker, TestFailure, TestFailureFormatOptions, create_test_event_channel},
  },
};
...
impl LspTestReporter {
  ...
  fn report_uncaught_error(&mut self, origin: &str, js_error: &JsError) {
    ...
    let err_string = format!(
      "Uncaught error from {}: {}\nThis error was not caught from a test and caused the test \
       runner to fail on the referenced module.\nIt most likely originated from a dangling \
       promise, event/timeout handler or top-level code.",
      origin,
...
mod tests {
  ...
  fn test_as_queue_and_filters() {
    ...
    let non_test_specifier = ModuleSpecifier::parse("file:///a/no_tests.ts").unwrap();
    let params = lsp_custom::TestRunRequestParams {
      ...
      exclude: vec![lsp_custom::TestIdentifier {
        ...
        id: Some("69d9fe87f64f5b66cb8b631d4fd2064e8224b8715a049be54276c42189ff8f9f".to_string()),
        step_id: None,
      }],
    };
    ...
    exclude.insert(
      "69d9fe87f64f5b66cb8b631d4fd2064e8224b8715a049be54276c42189ff8f9f".to_string(),
      test_def_b,
    );
    ...
    assert_eq!(
      filter.as_ids(&test_module),
      vec!["0b7c6bf3cd617018d33a1bf982a08fe088c5bb54fcd5eb9e802e7c137ec1af94".to_string()]
    );
  }
}
