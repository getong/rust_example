...
use deno_ast::{SourceRange, SourceRangedForSpanned, SourceTextInfo};
use deno_core::{
  ModuleSpecifier,
  anyhow::anyhow,
  error::AnyError,
...
use deno_semver::{
  SmallStackString, StackString, Version,
  jsr::{JsrPackageNvReference, JsrPackageReqReference},
  npm::NpmPackageReqReference,
  package::{PackageNv, PackageNvReference, PackageReq, PackageReqReference},
};
use import_map::ImportMap;
use lsp_types::Uri;
use node_resolver::{InNpmPackageChecker, NodeResolutionKind, ResolutionMode};
...
use super::{
  diagnostics::{DenoDiagnostic, DiagnosticSource},
  documents::{DocumentModule, DocumentModules},
  language_server,
  resolver::LspResolver,
  tsc,
};
use crate::{
  args::jsr_url, lsp::urls::uri_to_url, tools::lint::CliLinter, util::path::relative_specifier,
};
...
static IMPORT_SPECIFIER_RE: Lazy<Regex> =
  lazy_regex::lazy_regex!(r#"\sfrom\s+["']([^"']*)["']|import\s*\(\s*["']([^"']*)["']\s*\)"#);
...
fn as_lsp_range_from_lint_diagnostic(diagnostic_range: &LintDiagnosticRange) -> Range {
  as_lsp_range(diagnostic_range.range, &diagnostic_range.text_info)
}

fn as_lsp_range(source_range: SourceRange, text_info: &SourceTextInfo) -> Range {
...
impl<'a> TsResponseImportMapper<'a> {
  ...
  pub fn check_specifier(
    ...
    fn concat_npm_specifier(prefix: &str, pkg_req: &PackageReq, sub_path: Option<&str>) -> String {
    ...
    let scoped_resolver = self.resolver.get_scoped_resolver(self.scope.as_deref());

    if let Some(dep_name) = scoped_resolver.resource_url_to_configured_dep_key(specifier, referrer)
    {
    ...
    if let Some(npm_resolver) = scoped_resolver.as_maybe_managed_npm_resolver() {
      let match_specifier = || {
        ...
        let pkg_id = npm_resolver
          ...
          .ok()??;
        let pkg_reqs = maybe_reverse_definitely_typed(&pkg_id, npm_resolver.resolution())
          .unwrap_or_else(|| {
            npm_resolver
              .resolution()
        ...
        let sub_path = npm_resolver
          ...
          .ok()
          .and_then(|pkg_folder| self.resolve_package_path(specifier, &pkg_folder))?;
        ...
        if let Some(pkg_req) = pkg_reqs.first() {
          return Some(concat_npm_specifier("npm:", pkg_req, sub_path.as_deref()));
        }
        None
      };
  ...
  fn resolve_package_path(
    ...
    if specifier_path.extension().and_then(|e| e.to_str()) == Some("js") {
    ...
    } else if let Some(file_name) = specifier_path.file_name().and_then(|f| f.to_str()) {
      ...
      if let Some(file_stem) = file_name.strip_suffix(".d.ts") {
        search_paths.push(specifier_path.with_file_name(format!("{}.js", file_stem)));
      } else if let Some(file_stem) = file_name.strip_suffix(".d.cts") {
        search_paths.push(specifier_path.with_file_name(format!("{}.cjs", file_stem)));
      } else if let Some(file_stem) = file_name.strip_suffix(".d.mts") {
        search_paths.push(specifier_path.with_file_name(format!("{}.mjs", file_stem)));
      }
    }

    for search_path in search_paths {
      if let Some(result) = try_reverse_map_package_json_exports(root_folder, &search_path, exports)
      {
        return Some(result);
      }
    }
  ...
  fn try_reverse_map_package_json_exports_inner(
    ...
    for (key, value) in exports {
      match value {
        ...
        Value::Object(obj) => {
          if let Some(result) =
            try_reverse_map_package_json_exports_inner(root_path, target_path, obj)
          {
            return Some(if let Some(suffix) = key.strip_prefix("./") {
              if result.is_empty() {
                suffix.to_string()
      ...
      let new_lines: Vec<String> = lines
        .map(|line| {
          ...
          if let Some(captures) = IMPORT_SPECIFIER_RE.captures(line) {
            let specifier = captures.iter().skip(1).find_map(|s| s).unwrap().as_str();
...
pub fn fix_ts_import_changes_for_file_rename(
  ...
  for mut change in changes {
    ...
    let import_mapper = language_server.get_ts_response_import_mapper(&target_module);
...
fn fix_ts_import_action<'a>(
  ...
  } else if !import_mapper.is_valid_import(specifier, &module.specifier, module.resolution_mode) {
    None
  } else {
...
pub fn ts_changes_to_edit(
  ...
  for change in changes {
    let Some(edit) = change.to_text_document_edit(module, language_server) else {
      continue;
    };
...
impl CodeActionCollection {
  pub fn add_deno_fix_action(
    ...
    let code_action = DenoDiagnostic::get_code_action(uri, specifier, diagnostic)?;
  ...
  pub fn add_ts_fix_action(
    ...
    let Some(action) = fix_ts_import_action(action, module, language_server) else {
      return Ok(());
    };
