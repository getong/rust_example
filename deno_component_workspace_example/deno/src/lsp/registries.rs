...
use super::{
  completions::IMPORT_COMMIT_CHARS,
  logging::lsp_log,
  path_to_regex::{
    Compiler, Key, MatchResult, Matcher, StringOrNumber, StringOrVec, Token, parse, string_to_regex,
  },
};
use crate::{
  cache::{GlobalHttpCache, HttpCache},
  file_fetcher::{
...
/// URL requires one.
fn parse_url_with_base(url: &str, base: &ModuleSpecifier) -> Result<ModuleSpecifier, AnyError> {
  match Url::parse(url) {
    Ok(url) => Ok(url),
...
fn replace_variable(url: &str, variable: &Key, maybe_value: Option<&str>) -> String {
...
fn validate_config(config: &RegistryConfigurationJson) -> Result<(), AnyError> {
  ...
  for registry in &config.registries {
    ...
    for key_name in &key_names {
      ...
      {
        return Err(anyhow!(
          "Invalid registry configuration. Registry with schema \"{}\" is missing variable \
           declaration for key \"{}\".",
          registry.schema,
          key_name
        ));
    ...
    for variable in &registry.variables {
      ...
      let key_index = key_index.ok_or_else(|| {
        anyhow!(
          "Invalid registry configuration. Registry with schema \"{}\" is missing a path \
           parameter in schema for variable \"{}\".",
          registry.schema,
          variable.key
        )
      })?;
      ...
      for v in replacement_variables {
        if variable.key == v && config.version == 1 {
          return Err(anyhow!(
            "Invalid registry configuration. Url \"{}\" (for variable \"{}\" in registry with \
             schema \"{}\") uses variable \"{}\", which is not allowed because that would be a \
             self reference.",
            variable.url,
            variable.key,
            registry.schema,
        ...
        if key_index.is_none() && variable.key != v {
          return Err(anyhow!(
            "Invalid registry configuration. Url \"{}\" (for variable \"{}\" in registry with \
             schema \"{}\") uses variable \"{}\", which is not allowed because the schema defines \
             \"{}\" to the right of \"{}\".",
            variable.url,
            variable.key,
            registry.schema,
...
impl ModuleRegistry {
  pub fn new(location: PathBuf, http_client_provider: Arc<HttpClientProvider>) -> Self {
    ...
    let fetch_result = self
      ...
      .fetch_with_options(
        ...
        FetchOptions {
          ...
          maybe_auth: None,
          maybe_accept: Some(
            "application/vnd.deno.reg.v2+json, application/vnd.deno.reg.v1+json;q=0.9, \
  ...
  pub async fn get_completions(
    ...
    for registry in registries {
      ...
      loop {
        ...
        if let Some(match_result) = matcher.matches(path) {
          did_match = true;
          let completion_type = get_completion_type(path_char_offset, &tokens, &match_result);
          match completion_type {
            ...
            Some(CompletionType::Key { key, prefix, index }) => {
              ...
              if let Some(url) = maybe_url
                && let Some(items) = self
                  .get_variable_items(&key, url, &resolved, &tokens, &match_result)
                  .await
              {
                let compiler = Compiler::new(&tokens[..= index], None);
                let base = Url::parse(&origin).ok()?;
                let (items, preselect, incomplete) = match items {
                  VariableItems::List(list) => (list.items, list.preselect, list.is_incomplete),
                ...
                for (idx, item) in items.into_iter().enumerate() {
                  ...
                  let full_text =
                    if let Some(suffix) = item_specifier.as_str().strip_prefix(resolved_str) {
                      format!("{text}{suffix}")
                    } else {
                    ...
                    && !specifier_exists(&item_specifier)
                  {
                    Some(lsp::Command {
                      ...
                      arguments: Some(vec![json!([item_specifier]), json!(&resolved)]),
                    })
                  } else {
                    None
                  };
                  ...
                  let data =
                    get_data_with_match(registry, &resolved, &tokens, &match_result, &key, &item);
          ...
          match &tokens[i] {
            ...
            Token::String(s) => {
              if s.starts_with(path) {
                ...
                url.set_path(s);
                let full_text = if let Some(suffix) = url.as_str().strip_prefix(resolved_str) {
                  format!("{text}{suffix}")
                } else {
                  url.into()
                };
              ...
              if let Some(prefix) = &k.prefix {
                  ...
                  && let Some(items) = self.get_items(url).await
                {
                  ...
                  for (idx, item) in items.into_iter().enumerate() {
                    ...
                    let command = if k.name == last_key_name && !specifier_exists(&item_specifier) {
                      Some(lsp::Command {
                        title: "".to_string(),
                        command: "deno.cache".to_string(),
                        arguments: Some(vec![json!([item_specifier]), json!(&resolved)]),
  ...
  pub async fn get_documentation(&self, url: &str) -> Option<lsp::Documentation> {
...
mod tests {
  ...
  async fn test_registry_completions_origin_match() {
    ...
    let mut module_registry =
      ModuleRegistry::new(location, Arc::new(HttpClientProvider::new(None, None)));
    ...
    let completions = module_registry.get_origin_completions("http://localhost", &range);
    assert!(completions.is_some());
    let completions = completions.unwrap().items;
    assert_eq!(completions.len(), 1);
    assert_eq!(completions[0].label, "http://localhost:4545");
  ...
  async fn test_registry_completions() {
    ...
    assert!(completions.items[0].label == *"a" || completions.items[0].label == *"b");
    assert!(completions.items[1].label == *"a" || completions.items[1].label == *"b");
    ...
    let completions = module_registry
      .get_completions("http://localhost:4545/x/a@v1.0.0/", &range, None, |_| false)
      .await;
    assert!(completions.is_some());
    let completions = completions.unwrap().items;
    assert_eq!(completions.len(), 2);
    ...
    let completions = module_registry
      .get_completions("http://localhost:4545/x/a@v1.0.0/b", &range, None, |_| {
        false
      })
      .await;
    assert!(completions.is_some());
  ...
  fn test_parse_replacement_variables() {
    let actual =
      parse_replacement_variables("https://deno.land/_vsc1/modules/${module}/v/${{version}}");
  ...
  async fn test_check_origin_not_supported() {
    ...
    assert!(err.contains("https://example.com/.well-known/deno-import-intellisense.json"));
