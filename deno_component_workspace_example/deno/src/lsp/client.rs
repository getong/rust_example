...
use lsp_types::Uri;
use tower_lsp::{lsp_types as lsp, lsp_types::ConfigurationItem};

use super::{
  config::{SETTINGS_SECTION, WorkspaceSettings},
  lsp_custom,
  testing::lsp_custom as testing_lsp_custom,
};
use crate::lsp::{logging::lsp_warn, repl::get_repl_workspace_settings};
...
impl ClientTrait for TowerClient {
  ...
  async fn send_test_notification(&self, notification: TestingNotification) {
    match notification {
      TestingNotification::Module(params) => {
        self
          .0
          .send_notification::<testing_lsp_custom::TestModuleNotification>(params)
          .await
      }
      TestingNotification::DeleteModule(params) => {
        self
          .0
          .send_notification::<testing_lsp_custom::TestModuleDeleteNotification>(params)
          .await
      }
      TestingNotification::Progress(params) => {
        self
          .0
          .send_notification::<testing_lsp_custom::TestRunProgressNotification>(params)
          .await
      }
    }
  }
  ...
  async fn send_did_refresh_deno_configuration_tree_notification(
    ...
    self
      .0
      .send_notification::<lsp_custom::DidRefreshDenoConfigurationTreeNotification>(params)
      .await
  }
  ...
  async fn send_did_change_deno_configuration_notification(
    ...
    self
      .0
      .send_notification::<lsp_custom::DidChangeDenoConfigurationNotification>(params)
      .await
  }
