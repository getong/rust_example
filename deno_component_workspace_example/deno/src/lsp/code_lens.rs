...
use deno_ast::{
  ParsedSource, SourceRange, SourceRangedForSpanned,
  swc::{
    ast,
...
use super::{
  analysis::source_range_to_lsp_range, config::CodeLensSettings, language_server, text::LineIndex,
  tsc, tsc::NavigationTree,
};
...
impl DenoTestCollector {
  ...
  fn add_code_lenses<N: AsRef<str>>(&mut self, name: N, range: &SourceRange) {
    let range = source_range_to_lsp_range(range, self.parsed_source.text_info_lazy());
        ...
        ast::Expr::Object(obj_lit) => {
          for prop in &obj_lit.props {
            if let ast::PropOrSpread::Prop(prop) = prop
              ...
              && let ast::PropName::Ident(ast::IdentName { sym, .. }) = &key_value_prop.key
              && sym == "name"
            {
...
pub async fn resolve_code_lens(
  ...
  let data: CodeLensData =
    serde_json::from_value(code_lens.data.clone().unwrap()).map_err(|err| {
      LspError::invalid_params(format!("Unable to parse code lens data: {:#}", err))
    })?;
  match data.source {
    CodeLensSource::Implementations => {
      resolve_implementation_code_lens(code_lens, data, language_server, token).await
    }
    CodeLensSource::References => {
      resolve_references_code_lens(code_lens, data, language_server, token).await
    }
  }
}
...
mod tests {
  ...
  fn test_deno_test_collector() {
    ...
    .unwrap();
    let mut collector = DenoTestCollector::new(specifier, parsed_module.clone());
