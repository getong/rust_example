...
use deno_config::{
  deno_json::{DenoJsonCache, FmtConfig, FmtOptionsConfig, NodeModulesDirMode, TestConfig},
  glob::{FilePatterns, PathOrPatternSet},
  workspace::{
    VendorEnablement, Workspace, WorkspaceCache, WorkspaceDirectory,
    WorkspaceDirectoryEmptyOptions, WorkspaceDiscoverOptions,
  },
};
use deno_core::{
  ModuleSpecifier,
...
use deno_lib::{args::has_flag_env_var, util::hash::FastInsecureHasher};
use deno_npm::npm_rc::ResolvedNpmRc;
use deno_npm_cache::NpmCacheSetting;
use deno_npm_installer::{
  LifecycleScriptsConfig, NpmInstallerFactory, NpmInstallerFactoryOptions,
  graph::NpmCachingStrategy, lifecycle_scripts::NullLifecycleScriptsExecutor,
};
use deno_package_json::{PackageJsonCache, PackageJsonCacheResult};
...
use crate::{
  args::{CliLockfile, ConfigFile},
  cache::DenoDir,
  file_fetcher::CliFileFetcher,
  http_util::HttpClientProvider,
  lsp::logging::lsp_warn,
  npm::CliNpmCacheHttpClient,
  sys::CliSys,
  util::{
    fs::canonicalize_path_maybe_not_exists,
...
impl<'de, T: Default + for<'de2> Deserialize<'de2>> Deserialize<'de> for SafeValue<T> {
...
impl WorkspaceSettings {
  pub fn from_raw_settings(deno: Value, javascript: Value, typescript: Value) -> Self {
    fn parse_or_default<T: Default + DeserializeOwned>(value: Value, description: &str) -> T {
      if value.is_null() {
        return T::default();
      }
    ...
    if let Some(inlay_hints) = deno_inlay_hints {
      ...
      if inlay_hints.parameter_names.enabled != Default::default() {
        lsp_warn!(
          "\"deno.inlayHints.parameterNames.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.parameterNames.enabled\" and \
      ...
      {
        lsp_warn!(
          "\"deno.inlayHints.parameterNames.suppressWhenArgumentMatchesName\" is deprecated. \
           Instead use \"javascript.inlayHints.parameterNames.suppressWhenArgumentMatchesName\" \
           and \"typescript.inlayHints.parameterNames.suppressWhenArgumentMatchesName\"."
        );
        settings
          .javascript
          .inlay_hints
      ...
      if inlay_hints.parameter_types.enabled {
        lsp_warn!(
          "\"deno.inlayHints.parameterTypes.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.parameterTypes.enabled\" and \
      ...
      if inlay_hints.variable_types.enabled {
        lsp_warn!(
          "\"deno.inlayHints.variableTypes.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.variableTypes.enabled\" and \
      ...
      if !inlay_hints.variable_types.suppress_when_type_matches_name {
        lsp_warn!(
          "\"deno.inlayHints.variableTypes.suppressWhenTypeMatchesName\" is deprecated. Instead \
           use \"javascript.inlayHints.variableTypes.suppressWhenTypeMatchesName\" and \
      ...
      if inlay_hints.property_declaration_types.enabled {
        lsp_warn!(
          "\"deno.inlayHints.propertyDeclarationTypes.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.propertyDeclarationTypes.enabled\" and \
      ...
      if inlay_hints.function_like_return_types.enabled {
        lsp_warn!(
          "\"deno.inlayHints.functionLikeReturnTypes.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.functionLikeReturnTypes.enabled\" and \
      ...
      if inlay_hints.enum_member_values.enabled {
        lsp_warn!(
          "\"deno.inlayHints.enumMemberValues.enabled\" is deprecated. Instead use \
           \"javascript.inlayHints.enumMemberValues.enabled\" and \
    ...
    if let Some(suggest) = deno_suggest {
      ...
      if suggest.complete_function_calls {
        lsp_warn!(
          "\"deno.suggest.completeFunctionCalls\" is deprecated. Instead use \
      ...
      if !suggest.names {
        lsp_warn!(
          "\"deno.suggest.names\" is deprecated. Instead use \"javascript.suggest.names\" and \
      ...
      if !suggest.paths {
        lsp_warn!(
          "\"deno.suggest.paths\" is deprecated. Instead use \"javascript.suggest.paths\" and \
      ...
      if !suggest.auto_imports {
        lsp_warn!(
          "\"deno.suggest.autoImports\" is deprecated. Instead use \
           \"javascript.suggest.autoImports\" and \"typescript.suggest.autoImports\"."
        );
...
impl Settings {
  ...
  pub fn get_for_path(&self, path: &Path) -> (&WorkspaceSettings, Option<&Arc<Url>>) {
  ...
  pub fn get_for_uri(&self, uri: &Uri) -> (&WorkspaceSettings, Option<&Arc<Url>>) {
...
impl Config {
  ...
  pub fn set_workspace_folders(&mut self, folders: Vec<(Arc<Url>, lsp::WorkspaceFolder)>) {
    self.settings = Arc::new(Settings {
      unscoped: self.settings.unscoped.clone(),
      by_workspace_folder: folders.iter().map(|(s, _)| (s.clone(), None)).collect(),
  ...
  pub fn enabled_inlay_hints_for_specifier(&self, specifier: &ModuleSpecifier) -> bool {
  ...
  pub fn specifier_enabled_for_test(&self, specifier: &ModuleSpecifier) -> bool {
  ...
  pub fn workspace_configuration_capable(&self) -> bool {
    (|| self.client_capabilities.workspace.as_ref()?.configuration)().unwrap_or(false)
  }

  pub fn did_change_watched_files_capable(&self) -> bool {
    (|| {
      let workspace = self.client_capabilities.workspace.as_ref()?;
      let did_change_watched_files = workspace.did_change_watched_files.as_ref()?;
...
impl ConfigData {
  ...
  async fn load(
      ...
      Ok(scope_dir_path) => {
        ...
        WorkspaceDirectory::discover(
          ...
          match specified_config {
            ...
            None => deno_config::workspace::WorkspaceDiscoverStart::Paths(&paths),
          },
    ...
    match discover_result {
      ...
      Err(err) => {
        ...
        let member_dir = WorkspaceDirectory::empty(WorkspaceDirectoryEmptyOptions {
        ...
        for (url, file_type) in files {
          ...
          if file_path.exists() {
            ...
            let canonicalized_specifier = canonicalize_path_maybe_not_exists(&file_path)
              .ok()
  ...
  async fn load_inner(
    ...
    let mut add_watched_file = |specifier: ModuleSpecifier, file_type: ConfigWatchedFileType| {
    ...
    if let Some(deno_json) = member_dir.maybe_deno_json() {
      ...
      add_watched_file(deno_json.specifier.clone(), ConfigWatchedFileType::DenoJson);
    }

    if let Some(pkg_json) = member_dir.maybe_pkg_json() {
      lsp_log!("  Resolved package.json: \"{}\"", pkg_json.specifier());

      add_watched_file(pkg_json.specifier(), ConfigWatchedFileType::PackageJson);
    }

    let node_modules_dir = member_dir.workspace.node_modules_dir().unwrap_or_default();
    ...
    let mut workspace_factory = WorkspaceFactory::new(
      ...
      WorkspaceFactoryOptions {
        ...
        node_modules_dir: Some(resolve_node_modules_dir_mode(&member_dir.workspace, byonm)),
        no_lock: false,
        no_npm: false,
    ...
    let resolver_factory = ResolverFactory::new(
      ...
      ResolverFactoryOptions {
        ...
        allow_json_imports: deno_resolver::loader::AllowJsonImports::WithAttribute,
    ...
    let default_file_pattern_base = scope.to_file_path().unwrap_or_else(|_| PathBuf::from("/"));
    let fmt_config = Arc::new(
      member_dir
        .to_fmt_config(FilePatterns::new_with_base(member_dir.dir_path()))
        .inspect_err(|err| lsp_warn!("  Couldn't read formatter configuration: {}", err))
        .ok()
        .unwrap_or_else(|| FmtConfig::new_with_base(default_file_pattern_base.clone())),
    );

    let test_config = Arc::new(
      member_dir
        .to_test_config(FilePatterns::new_with_base(member_dir.dir_path()))
        .inspect_err(|err| lsp_warn!("  Couldn't read test configuration: {}", err))
        .ok()
        .unwrap_or_else(|| TestConfig::new_with_base(default_file_pattern_base.clone())),
    );
    let exclude_files = Arc::new(
      member_dir
        .workspace
        .resolve_config_excludes()
        .inspect_err(|err| lsp_warn!("  Couldn't read config excludes: {}", err))
        .ok()
        .unwrap_or_default(),
    );
    ...
    let specified_import_map = {
      ...
      if let Some(import_map_url) = &import_map_from_settings {
        add_watched_file(import_map_url.clone(), ConfigWatchedFileType::ImportMap);
        ...
        let value_result = fetch_result
          .map_err(AnyError::from)
          .and_then(|f| serde_json::from_slice::<Value>(&f.source).map_err(|e| e.into()));
        match value_result {
  ...
  pub fn maybe_deno_json(&self) -> Option<&Arc<deno_config::deno_json::ConfigFile>> {
...
impl ConfigTree {
  ...
  pub fn data_for_specifier(&self, specifier: &ModuleSpecifier) -> Option<&Arc<ConfigData>> {
    self
  ...
  pub fn workspace_dir_for_specifier(&self, specifier: &Url) -> Option<&WorkspaceDirectory> {
    self
  ...
  pub fn watched_file_type(&self, specifier: &Url) -> Option<(&Arc<Url>, ConfigWatchedFileType)> {
  ...
  pub fn to_did_refresh_params(
    ...
    let data = self
      ...
      .values()
      .filter_map(|data| {
        let workspace_root_scope_uri = Some(data.member_dir.workspace.root_dir_url())
  ...
  pub async fn refresh(
    ...
    self.global_npm_cache_url = Url::from_directory_path(deno_dir.npm_folder_path())
      .ok()
      .map(Arc::new);
  }
  ...
  pub async fn inject_config_file(&mut self, config_file: ConfigFile) {
    use sys_traits::{FsCreateDirAll, FsWrite};
...
fn resolve_node_modules_dir_mode(workspace: &Workspace, byonm: bool) -> NodeModulesDirMode {
...
#[cfg(test)]
mod tests {
  use deno_core::{resolve_url, serde_json, serde_json::json};
  ...
  fn test_config_specifier_enabled_path() {
    ...
    let workspace_settings = serde_json::from_str(r#"{ "enablePaths": ["worker"] }"#).unwrap();
  ...
  fn test_set_workspace_settings_defaults() {
    ...
    config.set_workspace_settings(serde_json::from_value(json!({})).unwrap(), vec![]);
    assert_eq!(
      ...
      WorkspaceSettings {
        ...
        javascript: LanguageWorkspaceSettings {
          inlay_hints: InlayHintsSettings {
            ...
            property_declaration_types: InlayHintsPropDeclTypesOptions { enabled: false },
            function_like_return_types: InlayHintsFuncLikeReturnTypesOptions { enabled: false },
            enum_member_values: InlayHintsEnumMemberValuesOptions { enabled: false },
          },
          ...
          suggest: CompletionSettings {
            ...
            object_literal_method_snippets: ObjectLiteralMethodSnippets { enabled: true },
          },
  ...
  async fn config_enable_via_config_file_detection() {
    ...
    config
      .tree
      .inject_config_file(ConfigFile::new("{}", root_uri.join("deno.json").unwrap()).unwrap())
      .await;
    assert!(config.specifier_enabled(&root_uri));
  }
  ...
  async fn config_specifier_enabled_for_test() {
    ...
    assert!(config.specifier_enabled_for_test(&root_uri.join("mod1.ts").unwrap()));
    assert!(!config.specifier_enabled_for_test(&root_uri.join("mod2.ts").unwrap()));
    assert!(!config.specifier_enabled_for_test(&root_uri.join("mod3.ts").unwrap()));
