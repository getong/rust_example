...
use deno_ast::{LineAndColumnIndex, ModuleSpecifier, SourceTextInfo};
use deno_core::{anyhow::anyhow, error::AnyError, serde_json};
use lsp_types::Uri;
...
use tower_lsp::{
  ...
  lsp_types::{
    ...
    CompletionTextEdit, CompletionTriggerKind, DidChangeTextDocumentParams,
...
use super::{
  client::Client,
  config::{
    ClassMemberSnippets, CompletionSettings, DenoCompletionSettings, ImportCompletionSettings,
    LanguageWorkspaceSettings, ObjectLiteralMethodSnippets, TestingSettings, WorkspaceSettings,
  },
  urls::{uri_parse_unencoded, url_to_uri},
};
...
impl ReplLanguageServer {
  ...
  pub async fn completions(
    ...
    self.did_change(line_text).await;
    let text_info =
      deno_ast::SourceTextInfo::from_string(format!("{}{}", self.document_text, self.pending_text));
  ...
  fn get_document_uri(&self) -> Uri {
    uri_parse_unencoded(self.cwd_uri.join("$deno$repl.mts").unwrap().as_str()).unwrap()
  }
}

fn lsp_range_to_std_range(text_info: &SourceTextInfo, range: &Range) -> std::ops::Range<usize> {
...
pub fn get_repl_workspace_settings() -> WorkspaceSettings {
  WorkspaceSettings {
    ...
    log_file: false,
    lint: false,
    document_preload_limit: 0, /* don't pre-load any modules as it's expensive and not useful for
                                * the repl */
    ...
    javascript: LanguageWorkspaceSettings {
      suggest: CompletionSettings {
        ...
        names: false,
        object_literal_method_snippets: ObjectLiteralMethodSnippets { enabled: false },
        paths: false,
      },
      ..Default::default()
    },
