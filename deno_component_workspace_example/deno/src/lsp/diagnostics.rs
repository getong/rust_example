...
use deno_resolver::{
  ...
  workspace::sloppy_imports_resolve,
};
use deno_runtime::{deno_node, tokio_util::create_basic_runtime};
use deno_semver::{jsr::JsrPackageReqReference, npm::NpmPackageReqReference, package::PackageReq};
...
use super::{
  analysis,
  analysis::import_map_lookup,
  client::Client,
  documents::{Document, DocumentModule, DocumentModules},
  language_server,
...
use crate::{
  lsp::{
    documents::OpenDocument, language_server::OnceCellMap, lint::LspLinter, logging::lsp_warn,
    urls::uri_to_url,
  },
  sys::CliSys,
  tsc::DiagnosticCategory,
  type_checker::ambient_modules_to_regex_string,
...
fn should_send_diagnostic_batch_notifications() -> bool {
  deno_lib::args::has_flag_env_var("DENO_INTERNAL_DIAGNOSTIC_BATCH_NOTIFICATIONS")
}
...
impl DiagnosticsState {
  fn update(&self, uri: &Uri, version: i32, diagnostics: &[lsp::Diagnostic]) {
    ...
    for diagnostic in diagnostics {
      if diagnostic.source.as_deref() == Some(DiagnosticSource::Ts.as_lsp_source()) {
        ts_diagnostics.push(diagnostic.clone());
      }
      if diagnostic.code == Some(lsp::NumberOrString::String("no-cache".to_string()))
        || diagnostic.code == Some(lsp::NumberOrString::String("not-installed-jsr".to_string()))
        || diagnostic.code == Some(lsp::NumberOrString::String("not-installed-npm".to_string()))
      {
...
impl DiagnosticsServer {
  ...
  pub fn start(&mut self) {
    ...
    let should_send_batch_notifications = should_send_diagnostic_batch_notifications();
      ...
      runtime.block_on(async {
        ...
        while let Some(message) = rx.recv().await {
          ...
          let join_handle = tokio::task::spawn(async move {
            ...
            let open_docs = snapshot.document_modules.documents.open_docs().cloned();
            ...
            .for_each(|result| async {
              ...
              publish_document_diagnostics(&document, diagnostics, &client, &state, &token).await
            })
            .await;
            if should_send_batch_notifications {
  ...
  pub fn update(&self, message: DiagnosticsUpdateMessage) -> Result<(), AnyError> {
...
fn to_lsp_related_information(
  ...
  related_information.as_ref().map(|related| {
    related
      .iter()
      .filter_map(|ri| {
        if let (Some(file_name), Some(start), Some(end)) = (&ri.file_name, &ri.start, &ri.end) {
      ...
      if let (Some(start), Some(end)) = (&d.start, &d.end) {
        Some(lsp::Diagnostic {
          ...
          tags: match d.code {
            ...
            2695 | 6133 | 6138 | 6192 | 6196 | 6198 | 6199 | 6205 | 7027 | 7028 => {
...
impl DenoDiagnostic {
  fn code(&self) -> &str {
    match self {
      ...
      Self::ResolutionError(err) => {
        if deno_resolver::graph::get_resolution_error_bare_node_specifier(err).is_some() {
          "import-node-prefix-missing"
        } else {
          match err {
            ResolutionError::InvalidDowngrade { .. } => "invalid-downgrade",
            ResolutionError::InvalidJsrHttpsTypesImport { .. } => "invalid-jsr-https-types-import",
            ResolutionError::InvalidLocalImport { .. } => "invalid-local-import",
            ResolutionError::InvalidSpecifier { error, .. } => match error {
              SpecifierError::ImportPrefixMissing { .. } => "import-prefix-missing",
    ...
    if let Some(lsp::NumberOrString::String(code)) = &diagnostic.code {
      let code_action = match code.as_str() {
        ...
        "no-cache" | "not-installed-jsr" | "not-installed-npm" => {
          ...
          let title = if matches!(code.as_str(), "not-installed-jsr" | "not-installed-npm") {
          ...
          lsp::CodeAction {
            ...
            edit: Some(lsp::WorkspaceEdit {
              changes: Some(HashMap::from([(
                uri.clone(),
                vec![lsp::TextEdit {
                  new_text: format!("\"{}\"", relative_specifier(&data.to, specifier)),
                  range: diagnostic.range,
                }],
              )])),
              ..Default::default()
  ...
  pub fn to_lsp_diagnostic(&self, range: &lsp::Range) -> lsp::Diagnostic {
    fn no_local_message(specifier: &ModuleSpecifier, suggestion_message: Option<String>) -> String {
    ...
    let (severity, message, data) = match self {
      ...
      Self::ImportMapRemap { from, to } => (
        lsp::DiagnosticSeverity::HINT,
        format!(
          "The import specifier can be remapped to \"{to}\" which will resolve it via the active \
           import map."
        ),
        Some(json!({ "from": from, "to": to })),
      ),
      Self::InvalidAttributeType(assert_type) => (
        lsp::DiagnosticSeverity::ERROR,
        format!(
          "The module is a JSON module and expected an attribute type of \"json\". Instead got \
           \"{assert_type}\"."
        ),
        None,
      ),
      Self::NoAttributeType => (
        lsp::DiagnosticSeverity::ERROR,
        "The module is a JSON module and not being imported with an import attribute. Consider \
         adding `with { type: \"json\" }` to the import statement."
          .to_string(),
        None,
      ),
      Self::NoCache(specifier) => (
        lsp::DiagnosticSeverity::ERROR,
        format!("Uncached or missing remote URL: {specifier}"),
        Some(json!({ "specifier": specifier })),
      ),
      Self::NotInstalledJsr(pkg_req, specifier) => (
        lsp::DiagnosticSeverity::ERROR,
        format!("JSR package \"{pkg_req}\" is not installed or doesn't exist."),
        Some(json!({ "specifier": specifier })),
      ),
      Self::NotInstalledNpm(pkg_req, specifier) => (
        lsp::DiagnosticSeverity::ERROR,
        format!("npm package \"{pkg_req}\" is not installed or doesn't exist."),
        Some(json!({ "specifier": specifier })),
      ),
      Self::NoExportNpm(pkg_ref) => (
        lsp::DiagnosticSeverity::ERROR,
        format!(
          "NPM package \"{}\" does not define an export \"{}\".",
          pkg_ref.req(),
          pkg_ref.sub_path().unwrap_or(".")
        ),
        None,
      ),
      Self::NoLocal(specifier) => {
        let sloppy_resolution = sloppy_imports_resolve(
          specifier,
        ...
        (
          ...
          no_local_message(
            specifier,
            sloppy_resolution.as_ref().map(|(resolved, sloppy_reason)| {
      ...
      Self::UnknownNodeSpecifier(specifier) => (
        lsp::DiagnosticSeverity::ERROR,
        format!("No such built-in module: node:{}", specifier.path()),
        None,
      ),
      Self::BareNodeSpecifier(specifier) => (
        lsp::DiagnosticSeverity::WARNING,
        format!(
          "\"{0}\" is resolved to \"node:{0}\". If you want to use a built-in Node module, add a \
           \"node:\" prefix.",
          specifier
        ),
        Some(json!({ "specifier": specifier })),
...
fn maybe_ambient_import_specifier(diagnostic: &DenoDiagnostic) -> Option<String> {
  match diagnostic {
    DenoDiagnostic::NoCache(url) | DenoDiagnostic::NoLocal(url) => Some(url.to_string()),
    DenoDiagnostic::ResolutionError(err) => maybe_ambient_specifier_resolution_err(err),
    _ => None,
  }
}

fn maybe_ambient_specifier_resolution_err(err: &ResolutionError) -> Option<String> {
  match err {
...
fn diagnose_resolution(
  ...
  match resolution {
    Resolution::Ok(resolved) => {
      ...
      let managed_npm_resolver = scoped_resolver.as_maybe_managed_npm_resolver();
        ...
        Some(module) => {
          ...
          if module.media_type == MediaType::Json {
            match maybe_assert_type {
              ...
              Some(assert_type) => diagnostics.push(DenoDiagnostic::InvalidAttributeType(
        ...
        _ => {
          match JsrPackageReqReference::from_specifier(specifier) {
            Ok(pkg_ref) => {
              ...
              diagnostics.push(DenoDiagnostic::NotInstalledJsr(req, specifier.clone()));
            }
            _ => {
                ...
                _ => {
                  if let Some(module_name) = specifier.as_str().strip_prefix("node:") {
                    if !deno_node::is_builtin_node_module(module_name) {
                      diagnostics.push(DenoDiagnostic::UnknownNodeSpecifier(specifier.clone()));
                    } else if module_name == dependency_key {
                      let mut is_mapped = false;
                      if let Some(import_map) = import_map
                        && let Resolution::Ok(resolved) = &resolution
                        && import_map.resolve(module_name, &resolved.specifier).is_ok()
                      {
                        is_mapped = true;
                      }
    ...
    Resolution::Err(err) => {
      if maybe_ambient_specifier_resolution_err(err).is_none() {
        ...
        deferred_diagnostics.push(DenoDiagnostic::ResolutionError(*err.clone()));
      }
    }
    _ => (),
  }
...
fn diagnose_dependency(
  ...
  if referrer_module.media_type.is_declaration() {
    ...
    if compiler_options_data.is_none() {
      lsp_warn!(
        "Key was not in sync with resolver while checking `skipLibCheck`. This should be \
         impossible."
      );
  ...
  if let Some(import_map) = import_map {
    ...
    if let Some(resolved) = resolved
      && let Some(to) =
        import_map_lookup(import_map, &resolved.specifier, &referrer_module.specifier)
      && dependency_key != to
    {
      diagnostics.push(
  ...
  if is_types_deno_types {
    let range = match &dependency.maybe_type {
      Resolution::Ok(resolved) => language_server::to_lsp_range(&resolved.range),
    ...
    deferred_diagnostics.extend(Box::new(deferred.iter().filter_map(|diag| {
      maybe_ambient_import_specifier(diag).map(|spec| (spec, diag.to_lsp_diagnostic(&range)))
    })));
  }
}
...
async fn generate_document_diagnostics(
  ...
  let module = match snapshot.document_modules.primary_module(&document_) {
    ...
    None => {
      ...
      let Some(module) = snapshot.document_modules.primary_module(&document_) else {
        return Ok(Vec::new());
      };
      module
    }
...
pub async fn generate_module_diagnostics(
  ...
  let mut diagnostics = ts_json_to_diagnostics(ts_diagnostics, module, &snapshot.document_modules);
      ...
      ts_server
        ...
        .and_then(|ambient_modules| {
          ...
          regex::Regex::new(&regex_string)
            .inspect_err(|err| {
              lsp_warn!(
                "Failed to compile ambient modules pattern: {err:#} (pattern is {regex_string:?})"
              );
            })
            .ok()
        })
...
mod tests {
  ...
  async fn setup(
    ...
    let enabled_settings =
      serde_json::from_str::<WorkspaceSettings>(r#"{ "enable": true }"#).unwrap();
  ...
  fn generate_all_deno_diagnostics(snapshot: &StateSnapshot) -> Vec<(Uri, Vec<lsp::Diagnostic>)> {
    snapshot
      .document_modules
      .documents
      .open_docs()
  ...
  fn test_get_code_action_import_map_remap() {
    ...
    let result = DenoDiagnostic::get_code_action(
      &uri,
      &specifier,
      &lsp::Diagnostic {
        range: lsp::Range {
        ...
        message: "The import specifier can be remapped to \"/~/std/assert/mod.ts\" which will \
                  resolve it via the active import map."
          .to_string(),
        data: Some(json!({
          "from": "../std/assert/mod.ts",
          "to": "/~/std/assert/mod.ts"
