...
use deno_cache_dir::{GlobalOrLocalHttpCache, file_fetcher::CacheSetting};
use deno_core::{
  ModuleSpecifier,
  anyhow::anyhow,
  error::AnyError,
...
use deno_graph::{CheckJsOption, GraphKind, Resolution};
use deno_lib::{
...
use deno_runtime::deno_tls::{RootCertStoreProvider, rustls::RootCertStore};
...
use super::{
  analysis::{
    CodeActionCollection, CodeActionData, TsResponseImportMapper, fix_ts_import_changes,
    ts_changes_to_edit,
  },
  cache::LspCache,
  capabilities,
  capabilities::semantic_tokens_registration_options,
  client::Client,
  code_lens, completions,
  config::{Config, SETTINGS_SECTION, UpdateImportsOnFileMoveEnabled, WorkspaceSettings},
  diagnostics,
  diagnostics::{DiagnosticDataSpecifier, DiagnosticsServer, DiagnosticsUpdateMessage},
  documents::{Document, DocumentModule, DocumentModules, LanguageId},
  jsr::CliJsrSearchApi,
  logging::{lsp_log, lsp_warn},
  lsp_custom,
  lsp_custom::TaskDefinition,
  npm::CliNpmSearchApi,
  parent_process_checker,
  performance::Performance,
  refactor,
  registries::ModuleRegistry,
  resolver::LspResolver,
  testing, text, tsc,
  tsc::{ChangeKind, TsServer},
  urls::{uri_to_file_path, uri_to_url, url_to_uri},
};
use crate::{
  args::{Flags, InternalFlags, UnstableFmtOptions},
  factory::CliFactory,
  file_fetcher::{CreateCliFileFetcherOptions, create_cli_file_fetcher},
  graph_util,
  http_util::HttpClientProvider,
  lsp::{
    analysis::fix_ts_import_changes_for_file_rename, compiler_options::LspCompilerOptionsResolver,
  ...
  tools::{
    ...
    upgrade::{check_for_upgrades_for_lsp, upgrade_check_enabled},
  },
  util::{fs::remove_dir_all_if_exists, path::to_percent_decoded_str, sync::AsyncFlag},
};
...
impl LanguageServer {
  ...
  pub async fn cache(
    ...
    async fn create_graph_for_caching(
      ...
      let mut inner_loader = module_graph_builder.create_graph_loader_with_root_permissions();
    ...
    *self.did_change_batch_queue.borrow_mut() = None;
    let prepare_cache_result =
      self
        .inner
        .write()
        .await
        .prepare_cache(specifiers, referrer, force_global_cache);
    ...
    match prepare_cache_result {
      Ok(result) => {
        ...
        let handle =
          spawn(async move { create_graph_for_caching(cli_factory, roots, open_modules).await });
  ...
  pub async fn performance_request(&self, _token: CancellationToken) -> LspResult<Option<Value>> {
    self.init_flag.wait_raised().await;
  ...
  pub async fn virtual_text_document(
    ...
    match params.map(serde_json::from_value) {
      Some(Ok(params)) => Ok(Some(
        serde_json::to_value(self.inner.read().await.virtual_text_document(params)?).map_err(
          |err| {
            error!(
...
impl Inner {
  fn new(client: Client, performance: Arc<Performance>) -> Self {
    ...
    let initial_cwd = std::env::current_dir()
      .unwrap_or_else(|_| panic!("Could not resolve current working directory"));
  ...
  pub fn get_document(
    ...
    let Some(document) = self.document_modules.documents.get(uri) else {
      match exists {
        Exists::Enforce if !uri.scheme().is_some_and(|s| s.eq_lowercase("deno")) => {
  ...
  #[cfg_attr(feature = "lsp-tracing", tracing::instrument(skip_all))]
  pub fn get_primary_module(&self, document: &Document) -> LspResult<Option<Arc<DocumentModule>>> {
  ...
  pub async fn update_global_cache(&mut self) {
    ...
    let global_cache_url = maybe_cache.and_then(|cache_str| {
      if let Ok(url) = Url::from_file_path(cache_str) {
        ...
        root_url
          .join(cache_str)
          .inspect_err(|err| lsp_warn!("Failed to resolve custom cache path: {err}"))
          .ok()
      } else {
        lsp_warn!(
...
impl Inner {
  fn initialize(&mut self, params: InitializeParams) -> LspResult<InitializeResult> {
...

    {
      ...
      if let Some(options) = params.initialization_options {
        ...
        self.force_push_based_diagnostics = settings.force_push_based_diagnostics;
  ...
  fn refresh_workspace_files(&mut self) {
    ...
    if hit_limit {
      let document_preload_limit = self.config.workspace_settings().document_preload_limit;
      if document_preload_limit == 0 {
        ...
        lsp_warn!(
          concat!(
            ...
            "partially enable a workspace or increase the limit via \
      ...
      self.task_queue.queue_task(Box::new(|ls: LanguageServer| {
        spawn(async move {
          let specifier = {
            ...
            let scoped_resolver = inner.resolver.get_scoped_resolver(Some(&referrer));
  ...
  async fn refresh_resolver(&mut self) {
    self.resolver = Arc::new(
      LspResolver::from_config(&self.config, &self.cache, Some(&self.http_client_provider)).await,
    );
  }
  ...
  async fn did_save(&self, params: DidSaveTextDocumentParams) {
    ...
    let Ok(Some(module)) = self.get_primary_module(&document).inspect_err(|err| {
      lsp_warn!("{:#}", err);
    }) else {
      return;
    };
    if module.specifier.scheme() != "file" || module.specifier.as_str().contains("/node_modules/") {
      return;
    }
    self.task_queue.queue_task(Box::new(|ls: LanguageServer| {
      spawn(async move {
  ...
  fn notebook_did_open(&mut self, params: DidOpenNotebookDocumentParams) {
    ...
    let documents = self
      .document_modules
      .open_notebook_document(params.notebook_document.uri, params.cell_text_documents);
  ...
  async fn did_change_configuration(&mut self, params: DidChangeConfigurationParams) {
    if !self.config.workspace_configuration_capable() {
      let config = params.settings.as_object().map(|settings| {
        let deno = serde_json::to_value(settings.get(SETTINGS_SECTION)).unwrap();
        let javascript = serde_json::to_value(settings.get("javascript")).unwrap();
        let typescript = serde_json::to_value(settings.get("typescript")).unwrap();
    ...
    // TODO(nathanwhit): allow updating after startup, needs work to set thread local collector on
    // tsc thread self.update_tracing();
    self.check_semantic_tokens_capabilities();
    self.update_debug_flag();
  ...
  async fn did_change_watched_files(&mut self, params: DidChangeWatchedFilesParams) {
    ...
    if changes.iter().any(|(specifier, _)| {
      ...
      deno_config_changes.extend(changes.iter().filter_map(|(s, e)| {
        self.config.tree.watched_file_type(s).and_then(|t| {
          let configuration_type = match t.1 {
            ...
            ConfigWatchedFileType::PackageJson => lsp_custom::DenoConfigurationType::PackageJson,
            _ => return None,
          };
          Some(lsp_custom::DenoConfigurationChangeEvent {
            scope_uri: url_to_uri(t.0).ok()?,
            file_uri: e.uri.clone(),
            typ: lsp_custom::DenoConfigurationChangeType::from_file_change_type(e.typ),
            configuration_type,
          })
        })
      }));
      ...
      self.send_diagnostics_update();
      if !self.is_using_push_based_diagnostics() && self.config.diagnostic_refresh_capable() {
      ...
      deno_config_changes.extend(changes.iter().filter_map(|(s, e)| {
        self.config.tree.watched_file_type(s).and_then(|t| {
          let configuration_type = match t.1 {
            ConfigWatchedFileType::DenoJson => lsp_custom::DenoConfigurationType::DenoJson,
            ConfigWatchedFileType::PackageJson => lsp_custom::DenoConfigurationType::PackageJson,
            _ => return None,
          };
          Some(lsp_custom::DenoConfigurationChangeEvent {
  ...
  async fn document_symbol(
    ...
    let response = if let Some(child_items) = &navigation_tree.child_items {
      ...
      for item in child_items {
        ...
        item.collect_document_symbols(module.line_index.clone(), &mut document_symbols);
      }
  ...
  async fn formatting(
    ...
    let fmt_config = self.config.tree.fmt_config_for_specifier(&module.specifier);
      ...
      move || {
          ...
          .and_then(|d| d.parsed_source.as_ref())
        {
          ...
          None => {
            let ext = document
              .open()
              .and_then(|d| d.language_id.as_extension().map(|s| s.to_string()));
            format_file(
              &file_path,
  ...
  async fn hover(
    ...
    let hover = if let Some((_, dep, range)) =
      module.dependency_at_position(&params.text_document_position_params.position)
    {
  ...
  #[cfg_attr(feature = "lsp-tracing", tracing::instrument(skip_all))]
  fn resolution_to_hover_text(&self, resolution: &Resolution, scope: Option<&Url>) -> String {
    match resolution {
      Resolution::Ok(resolved) => {
        ...
        match specifier.scheme() {
          ...
          _ => {
            ...
            if let Ok(jsr_req_ref) = JsrPackageReqReference::from_specifier(specifier) {
              let scoped_resolver = self.resolver.get_scoped_resolver(scope);
              if let Some(url) = scoped_resolver.jsr_to_resource_url(&jsr_req_ref) {
  ...
  async fn code_action(
      ...
      .filter(|d| match &d.source {
        Some(source) => match source.as_str() {
          "deno-ts" => match &d.code {
            Some(NumberOrString::String(code)) => ts_fixable_diagnosics.contains(code),
            Some(NumberOrString::Number(code)) => ts_fixable_diagnosics.contains(&code.to_string()),
            _ => false,
          },
    ...
    if !fixable_diagnostics.is_empty() {
      let file_diagnostics = if let Some(diagnostics_server) = &self.diagnostics_server {
          ...
          Some("deno-ts") => {
            ...
            for action in actions {
              ...
              if code_actions.is_fix_all_action(&action, diagnostic, &file_diagnostics) {
                code_actions.add_ts_fix_all_action(&action, &module, diagnostic);
              }
            }
          }
          Some("deno") => {
            if diagnostic.code == Some(NumberOrString::String("no-cache".to_string()))
              || diagnostic.code == Some(NumberOrString::String("not-installed-jsr".to_string()))
              || diagnostic.code == Some(NumberOrString::String("not-installed-npm".to_string()))
            {
              includes_no_cache = true;
            }
            code_actions
              .add_deno_fix_action(document.uri(), &module.specifier, diagnostic)
              .map_err(|err| {
                error!("{:#}", err);
                LspError::internal_error()
              })?
      ...
      if includes_no_cache {
        let no_cache_diagnostics = if let Some(diagnostics_server) = &self.diagnostics_server {
          diagnostics_server
            .state
        ...
        let uncached_deps = no_cache_diagnostics
          ...
          .filter_map(|d| {
            let data =
              serde_json::from_value::<DiagnosticDataSpecifier>(d.data.clone().into()).ok()?;
            Some(data.specifier)
          })
          .collect::<HashSet<_>>();
        if uncached_deps.len() > 1 {
          code_actions
            .add_cache_all_action(&module.specifier, no_cache_diagnostics.as_ref().clone());
        }
      }
    }
    ...
    if !self.config.client_provided_organize_imports_capable()
      && (kinds.is_empty() || kinds.contains(&CodeActionKind::SOURCE_ORGANIZE_IMPORTS))
    {
      ...
      if !organize_imports_edit.is_empty() {
        ...
        all_actions.push(CodeActionOrCommand::CodeAction(CodeAction {
          ...
          edit: file_text_changes_to_workspace_edit(&changes_with_modules, self, token)?,
    ...
    let code_action_disabled_capable = self.config.code_action_disabled_capable();
  ...
  async fn code_action_resolve(
    ...
    let (Some(kind), Some(data)) = (params.kind.clone(), params.data.clone()) else {
      return Ok(params);
    };
    let result = if kind.as_str().starts_with(CodeActionKind::QUICKFIX.as_str()) {
      ...
      let combined_code_actions = self
        .ts_server
        .get_combined_code_fix(self.snapshot(), &module, &code_action_data.fix_id, token)
        .await
        .map_err(|err| {
          if token.is_cancelled() {
            LspError::request_cancelled()
      ...
      let changes = if code_action_data.fix_id == "fixMissingImport" {
        fix_ts_import_changes(&combined_code_actions.changes, &module, self, token).map_err(
          |err| {
            if token.is_cancelled() {
              LspError::request_cancelled()
            } else {
      ...
      code_action.edit = ts_changes_to_edit(&changes, &module, self).map_err(|err| {
      ...
      .strip_prefix(CodeActionKind::REFACTOR.as_str())
    {
      ...
      match refactor_edit_info {
        Ok(mut refactor_edit_info) => {
          if kind_suffix == ".rewrite.function.returnType" || kind_suffix == ".move.newFile" {
            refactor_edit_info.edits =
              fix_ts_import_changes(&refactor_edit_info.edits, &module, self, token).map_err(
                |err| {
                  if token.is_cancelled() {
                    LspError::request_cancelled()
                  } else {
          ...
          code_action.edit = refactor_edit_info.to_workspace_edit(&module, self, token)?;
        }
        Err(err) => {
          if token.is_cancelled() {
            return Err(LspError::request_cancelled());
  ...
  async fn code_lens(
    ...
    {
      code_lenses.extend(
        code_lens::collect_test(&module.specifier, parsed_source, token).map_err(|err| {
          if token.is_cancelled() {
            LspError::request_cancelled()
          } else {
            error!(
  ...
  async fn diagnostic(
      ...
      return Ok(DocumentDiagnosticReportResult::Report(
        DocumentDiagnosticReport::Unchanged(RelatedUnchangedDocumentDiagnosticReport {
          ...
          unchanged_document_diagnostic_report: UnchangedDocumentDiagnosticReport {
            result_id: self.project_version.to_string(),
          },
        }),
      ));
    }
    ...
    let module = match self.get_primary_module(&document)? {
      Some(module) => module,
      None => {
        let url = uri_to_url(document.uri());
        if url.scheme() == "file"
    ...
    Ok(DocumentDiagnosticReportResult::Report(
      DocumentDiagnosticReport::Full(RelatedFullDocumentDiagnosticReport {
        ...
        full_document_diagnostic_report: FullDocumentDiagnosticReport {
          result_id: Some(self.project_version.to_string()),
          items: diagnostics.as_ref().clone(),
        },
      }),
    ))
  }

  async fn get_module_diagnostics(
    &self,
  ...
  async fn incoming_calls(
      ...
      .into_values()
    {
      ...
      let calls = self
        ...
        .unwrap_or_default();
      incoming_calls_with_modules.extend(calls.into_iter().map(|c| (c, module.clone())));
    }
    let root_path = self
      .config
      .root_url()
      ...
      match one_or_many {
        tsc::OneOrMany::One(item) => {
          if let Some(resolved) =
            item.try_resolve_call_hierarchy_item(&module, self, root_path.as_deref())
          {
      ...
      let selection_range = self
        ...
        .map_err(|err| {
          if token.is_cancelled() {
            ...
            lsp_warn!("Unable to get selection ranges from TypeScript: {:#}", err);
            LspError::invalid_request()
          }
        })?;

      selection_ranges.push(selection_range.to_selection_range(module.line_index.clone()));
    }
    self.performance.measure(mark);
  ...
  async fn semantic_tokens_full(
    ...
    let semantic_tokens = module
      ...
      .get_or_try_init(|| async {
        ...
        semantic_classification.to_semantic_tokens(module.line_index.clone(), token)
      })
      .await?
      .clone();
  ...
  async fn semantic_tokens_range(
    ...
    if let Some(tokens) = module.semantic_tokens_full.get() {
      let tokens = super::semantic_tokens::tokens_within_range(tokens, params.range);
  ...
  async fn will_rename_files(
    ...
    for rename in params.files {
        ...
        .into_values()
      {
        ...
        let changes =
          fix_ts_import_changes_for_file_rename(changes, &rename.new_uri, &module, self, token)
            .map_err(|err| {
              if token.is_cancelled() {
                LspError::request_cancelled()
              } else {
        ...
        if !changes.is_empty() {
          changes_with_modules.extend(changes.into_iter().map(|c| (c, module.clone())));
        }
      }
    }
  ...
  async fn symbol(
    ...
    for (compiler_options_key, compiler_options_data) in self.compiler_options_resolver.entries() {
...
impl tower_lsp::LanguageServer for LanguageServer {
          ...
          Ok(version_info) => {
            client.send_did_upgrade_check_notification(
              lsp_custom::DidUpgradeCheckNotificationParams {
                upgrade_available: version_info.map(|info| lsp_custom::UpgradeAvailable {
  ...
  async fn did_change(&self, params: DidChangeTextDocumentParams) {
    ...
    let (batch_queue, token) = {
      ...
      let batch_queue = existing
        ...
        .unwrap_or_else(|| Arc::new(DidChangeBatchQueue::new(params.text_document.uri)));
  ...
  async fn hover(&self, params: HoverParams, token: CancellationToken) -> LspResult<Option<Hover>> {
    self.init_flag.wait_raised().await;
  ...
  async fn diagnostic(
    ...
    self.init_flag.wait_raised().await;
    self.inner.read().await.diagnostic(params, &token).await
  }
...
impl Inner {
    ...
    if self.config.did_change_watched_files_capable() {
      ...
      let options = DidChangeWatchedFilesRegistrationOptions {
        watchers: vec![FileSystemWatcher {
          glob_pattern: GlobPattern::String("**/*.{json,jsonc,lock}".to_string()),
          kind: None,
        }],
      };
  ...
  #[cfg_attr(feature = "lsp-tracing", tracing::instrument(skip_all))]
  fn pre_did_change_workspace_folders(&mut self, params: DidChangeWorkspaceFoldersParams) {
  ...
  async fn test_run_request(&self, params: Option<Value>) -> LspResult<Option<Value>> {
    if let Some(testing_server) = &self.maybe_testing_server {
      match params.map(serde_json::from_value) {
        Some(Ok(params)) => {
          testing_server
  ...
  fn test_run_cancel_request(&self, params: Option<Value>) -> LspResult<Option<Value>> {
    if let Some(testing_server) = &self.maybe_testing_server {
      match params.map(serde_json::from_value) {
  ...
  async fn inlay_hint(
    ...
    let text_span =
      tsc::TextSpan::from_range(&params.range, module.line_index.clone()).map_err(|err| {
...
"#,
        self
          ...
          .by_workspace_folder
          .iter()
          .filter_map(|(folder_url, settings)| Some((
            format!("\"{folder_url}\""),
            settings.as_ref()?
          )))
          .chain(std::iter::once((
      ...
      for (name, count, average_duration) in averages {
        writeln!(contents, "|{}|{}|{}ms|", name, count, average_duration).unwrap();
      }
