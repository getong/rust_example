...
fn main() {
    ...
    let localhost = UdpSocket::bind("0.0.0.0:0").expect(
        "cannot bind to local
socket",
    );
    ...
    let _amt = localhost.send_to(&request_as_bytes, dns_server).expect(
        "socket
misconfigured",
    );
    let (_amt, _remote) = localhost.recv_from(&mut response_as_bytes).expect(
        "timeout
reached",
    );
    let dns_message = Message::from_vec(&response_as_bytes).expect(
        "unable to parse
response",
    );
