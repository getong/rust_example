...
mod csr {
  ...
  // Keeping the `ignoreIllegals` argument out of the default case, and since there is no optional
  // arguments in Rust, this will have to be provided in a separate function (e.g.
  ...
  pub fn highlight(code: String, lang: String) -> Option<String> {
    ...
    highlight_lang(code, options)
      .map(|result| {
        let value =
          get(&result, &"value".into()).expect("HighlightResult failed to contain the value key");
...
mod ssr {
  ...
  // TODO see if there is a Rust-based solution that will enable isomorphic rendering for this
  // feature. the current (disabled) implementation simply calls html_escape.
