  ...
  fn handle_get(&self, get: Get, ctx: &ActorContext) {
    ...
    let reply_with_children = match &get.child_key {
      Some(child_key) => {
        ...
        let child_val =
          unwrap_or_return!(self.open_child(&get.node_id, child_key, &child_val.unwrap(), true));
      ...
      if let Some(_children) = res {
        ...
        for (child_id, child_data) in update_data {
          if let Some(existing) = unwrap_or_return!(children.get(&child_id)) {
            let mut existing: SledNodeData =
              unwrap_or_return!(self.open_child(&node_id, &child_id, &existing, false));
...
mod tests {
  ...
  async fn should_not_reply_with_put_when_checksum_same_as_in_get() {
    ...
    let sled_storage =
      SledStorage::new_with_config(Config::default(), sled::Config::default().path(path), None);
