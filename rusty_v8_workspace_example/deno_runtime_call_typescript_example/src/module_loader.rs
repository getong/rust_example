...
impl TypescriptModuleLoader {
  fn load_module(
    ...
    let (code, should_transpile, media_type, module_type) = if module_specifier.scheme() == "file" {
      ...
      // Determine if this is TypeScript based on URL
      let should_transpile = url.ends_with(".ts") || url.ends_with(".tsx");
      let media_type = if should_transpile {
        MediaType::TypeScript
      } else {
...
impl ModuleLoader for TypescriptModuleLoader {
  ...
  fn load(
    ...
    ModuleLoadResponse::Sync(Self::load_module(source_maps, module_specifier))
  }
