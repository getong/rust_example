...
impl NpmDownloader {
  ...
  pub async fn download_package(&self, specifier: &str) -> Result<CachedPackage> {
    println!("🚀 Starting download for: {}", specifier);

    // Parse the npm: specifier
    let npm_spec = NpmSpecifier::parse(specifier)?;
    println!(
      "📦 Parsed package: {} version: {:?}",
      npm_spec.name, npm_spec.version
    );

    // Fetch package metadata from registry
    let metadata = match self.registry.get_package_metadata(&npm_spec.name).await {
      Ok(m) => m,
      Err(e) => {
        println!("❌ Failed to fetch metadata for {}: {}", npm_spec.name, e);
        return Err(e);
      }
    };
    tracing::info!(
