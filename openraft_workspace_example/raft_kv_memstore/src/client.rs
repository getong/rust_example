...
use openraft::{
  error::{NetworkError, RemoteError},
...
impl ExampleClient {
  ...
  pub async fn linearizable_read(
    ...
    self
      .do_send_rpc_to_leader("linearizable_read", Some(req))
      .await
  }
  ...
  pub async fn init(&self) -> Result<(), RPCError<RaftError<InitializeError>>> {
    self
      .do_send_rpc_to_leader("init", Some(&Vec::<(NodeId, String)>::new()))
      .await
  }
  ...
  pub async fn change_membership(
    ...
    self
      .send_rpc_to_leader("change-membership", Some(req))
      .await
  }
  ...
  /// See [`RaftMetrics`].
  pub async fn metrics(&self) -> Result<RaftMetrics<TypeConfig>, RPCError<RaftError>> {
