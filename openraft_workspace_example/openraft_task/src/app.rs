...
use crate::{
  Raft,
  apalis_raft::{
    DemoTask, DemoTaskSchedulerConfig, RaftApalisStorage, RaftTaskId, run_demo_worker,
  },
  network::Router,
...
pub struct Opt {
  ...
  /// How often the leader task scheduler creates demo tasks.
  #[arg(long, default_value_t = 1)]
  pub scheduler_seconds: u64,

  /// Number of demo tasks the leader scheduler creates on each tick.
  #[arg(long, default_value_t = 1)]
  pub scheduler_batch: usize,

  /// Maximum number of demo tasks generated by the replicated scheduler.
...
pub async fn run(opt: Opt) -> anyhow::Result<()> {
  ...
  let scheduler_interval = Duration::from_secs(opt.scheduler_seconds.max(1));
  ...
  for (node_id, node) in &cluster.nodes {
    ...
    background_handles.push(node_storage.spawn_task_monitor());
    background_handles.push(node_storage.spawn_reclaim_task());
    background_handles.push(
      node_storage.spawn_task_scheduler(DemoTaskSchedulerConfig::new(
        "demo-scheduler",
        scheduler_interval,
        opt.scheduler_batch,
        opt.scheduled_tasks,
