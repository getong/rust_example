...
# enqueue an apalis email task backed by the apalis raft group
...
This project includes a Raft-backed apalis storage in the =apalis= raft group.
Tasks are stored as keys under =apalis:apalis:task:<task-id>= and state changes
(queued, running, done, failed) are written through OpenRaft. The built-in email
worker runs with the node. The current Raft leader schedules queued tasks to
voter nodes other than itself, and a worker executes a task only when its local
OpenRaft state is =Follower= and the task is assigned to that node.
