use std::{convert::Infallible, fmt::Display, io::Cursor, sync::Arc};
  ...
  C: RaftTypeConfig<Node = NodeInfo, SnapshotData = Cursor<Vec<u8>>>,
{
  match path {
    ...
    "/snapshot" => {
      ...
      let res: Result<SnapshotResponse<C>, RaftError<C>> = raft
    ...
    "/transfer-leader" => {
      let req = serde_json::from_slice(&body).map_err(bad_request)?;
      let res: Result<TransferLeaderResponse<C>, RaftError<C>> = raft
