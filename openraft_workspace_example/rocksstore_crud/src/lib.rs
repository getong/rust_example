...
use openraft::{
  ...
  AnyError, EntryPayload, LogId, OptionalSend, RaftSnapshotBuilder, RaftTypeConfig, SnapshotMeta,
...
impl RaftStateMachine<TypeConfig> for RocksStateMachine {
    ...
    Strm: Stream<Item = Result<EntryResponder<TypeConfig>, io::Error>> + Unpin + OptionalSend,
  {
      ...
      let response = match entry.payload {
        ...
        EntryPayload::Normal(ref req) => match req {
          RocksRequest::Set { key, value } => {
            ...
            updates.push((key.clone(), Some(value.clone())));
            RocksResponse {
    ...
    if !updates.is_empty() {
      let mut kvs = self.data.write().await;
  ...
  async fn install_snapshot(
...

    {
      let mut kvs = self.data.write().await;
      kvs.clear();
      for (key, value) in snapshot_data_clone.iter() {
        let key = String::from_utf8_lossy(key).to_string();
        let value = String::from_utf8_lossy(value).to_string();
        kvs.insert(key, value);
      }
