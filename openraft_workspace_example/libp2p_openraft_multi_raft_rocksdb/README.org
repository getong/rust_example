...
Kameo remote actor demo runs alongside the raft node. Start at least 2 nodes
(for example, =./run-2nodes.sh=), then call:
...
curl -X POST http://127.0.0.1:3002/kameo/inc \
  -H 'content-type: application/json' \
  -d '{"amount":5}'
#+end_src

Note: =/kameo/inc= forwards to a remote incrementor, so you need more than one node.
