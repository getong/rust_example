...
impl RaftSnapshotBuilder<TypeConfig> for Arc<Store> {
  ...
  async fn build_snapshot(&mut self) -> Result<Snapshot<TypeConfig>, StorageError<NodeId>> {
...

    {
      ...
      data =
        serde_json::to_vec(&state_machine).map_err(|e| StorageIOError::read_state_machine(&e))?;
...
impl Store {
  pub(crate) async fn new<P: AsRef<Path>>(db_path: P) -> Arc<Store> {
    ...
    let db =
      DB::open_cf_descriptors(&db_opts, db_path, vec![store, state_machine, data, logs]).unwrap();
