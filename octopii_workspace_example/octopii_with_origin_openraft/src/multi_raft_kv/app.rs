...
use super::{
  GroupId, NodeId, Raft, StateMachineStore, api, encode,
  router::{NodeMessage, NodeRx, NodeTx, Router},
  typ,
};
...
impl Node {
  ...
  pub fn add_group(&mut self, group_id: GroupId, raft: Raft, state_machine: StateMachineStore) {
    let app = GroupApp {
  ...
  pub fn get_raft(&self, group_id: &GroupId) -> Option<&Raft> {
