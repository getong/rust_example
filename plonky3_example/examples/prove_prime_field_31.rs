...
use p3_baby_bear::{
  ...
  BabyBear, GenericPoseidon2LinearLayersBabyBear, Poseidon2BabyBear,
};
...
use p3_examples::{
  airs::ProofObjective,
  dfts::DftChoice,
  parsers::{DftOptions, FieldOptions, MerkleHashOptions, ProofOptions},
  proofs::{
...
use p3_mersenne_31::{
  ...
  MERSENNE31_POSEIDON2_PARTIAL_ROUNDS_16, MERSENNE31_S_BOX_DEGREE, Mersenne31, Poseidon2Mersenne31,
};
  ...
  match args.field {
    FieldOptions::KoalaBear => {
      ...
      let dft = match args.discrete_fourier_transform {
        ...
        DftOptions::None => panic!(
          "Please specify what dft to use. Options are recursive-dft, radix-2-dit-parallel and \
           small-batch-dft"
        ),
      };
    ...
    FieldOptions::Mersenne31 => {
      ...
      match args.discrete_fourier_transform {
        DftOptions::None => {}
        _ => panic!(
          "Currently there are no available DFT options when using Mersenne31. Please remove the \
           --discrete_fourier_transform flag."
        ),
      };
      ...
      match args.merkle_hash {
        ...
        MerkleHashOptions::Poseidon2 => {
          ...
          let result =
            prove_m31_poseidon2::<_, EF, _, _, _>(&proof_goal, num_hashes, perm16, perm24);
          report_result(result);
        }
      };
    }
