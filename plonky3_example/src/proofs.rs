...
use p3_field::{
  ExtensionField, Field, PrimeField32, PrimeField64, TwoAdicField,
  extension::{BinomialExtensionField, ComplexExtendable},
};
  ...
  StandardUniform: Distribution<F>,
{
  ...
  let conjectured_soundness_bits = fri_params.conjectured_soundness_bits();
  ...
  if result.is_ok() {
    report_conjectured_soundness(conjectured_soundness_bits);
  }
  result
}
...
/// Report the FRI conjectured soundness bits available in the crates.io release.
#[inline]
pub fn report_conjectured_soundness(conjectured_soundness_bits: usize) {
  println!("Conjectured FRI soundness: {conjectured_soundness_bits} bits");
}
