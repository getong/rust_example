...
impl<F: Default> Default for DftChoice<F> {
  // We have to fix a default for the `TwoAdicSubgroupDft` trait. We choose `Radix2DitParallel` as
  // one of the features of `RecursiveDft` is that it works better when initialized with knowledge
