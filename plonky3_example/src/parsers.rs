...
//!
//! For each enum, we allow the user to specify the enum either using the whole string or any
//! substring which fully determines the choice. We additionally add a few extra aliases if other
//! natural ones exist.
//!
...
/// allowed strings:
///
/// `koala-bear, k, ko, koa, koal, koala, koala-, koala-b, koala-be, koala-bea, koalab, koalabe,
/// koalabea, koalabear, kb`
...
fn get_aliases(
  ...
  match alias {
    None => {
      PossibleValue::new(base).aliases((min_unique_base_prefix .. base.len()).map(|i| &base[.. i]))
    }
    Some(vec) => PossibleValue::new(base).aliases(
      (min_unique_base_prefix .. base.len())
        ...
        .chain(
          vec
            .into_iter()
            .flat_map(|(alias, min_unique)| (min_unique .. alias.len() + 1).map(|i| &alias[.. i])),
        ),
    ),
  }
}
...
impl ValueEnum for FieldOptions {
  ...
  fn to_possible_value(&self) -> Option<PossibleValue> {
    Some(match self {
      ...
      Self::KoalaBear => get_aliases("koala-bear", 1, Some(vec![("koalabear", 6), ("kb", 2)])),
      Self::Mersenne31 => get_aliases("mersenne-31", 1, Some(vec![("mersenne31", 9), ("m31", 2)])),
    })
  }
}
...
impl ValueEnum for MerkleHashOptions {
  ...
  fn to_possible_value(&self) -> Option<PossibleValue> {
    Some(match self {
      ...
      Self::Poseidon2 => get_aliases("poseidon-2", 1, Some(vec![("poseidon2", 9), ("p2", 2)])),
    })
  }
}
