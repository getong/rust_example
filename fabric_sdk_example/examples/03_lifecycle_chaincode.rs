...
/// Demonstrates using the Lifecycle client to manage chaincode
/// lifecycle on the Fabric peer (install, approve, commit, query).
///
/// Prerequisites:
...
///   cargo run --example 03_lifecycle_chaincode
#[tokio::main]
async fn main() -> Result<()> {
    ...
    println!("Lifecycle client obtained");

    // Lifecycle operations (requires admin privileges and
    // appropriate network setup):
    //
    // lifecycle.install_chaincode(...)
    // lifecycle.approve_chaincode(...)
    // lifecycle.commit_chaincode(...)
    // lifecycle.query_installed(...)
    // lifecycle.query_approved(...)
    // lifecycle.query_committed(...)
