...
DEFAULT_SQL_FILE="${ROOT_DIR}/scripts/bootstrap.sql"
...
DATABASE_URL_VALUE="$(resolve_config_value "" DATABASE_URL)"
...
if [[ -z "${PGPASSWORD_VALUE}" ]]; then
  echo "Set DATABASE_URL or PGPASSWORD/POSTGRES_PASSWORD before running this script" >&2
  exit 1
fi

DATABASE_URL_FROM_COMPONENTS="$(build_database_url "${PGUSER}")"
...
if run_psql "${DATABASE_URL_FROM_COMPONENTS}" 2>"${TMP_ERROR_FILE}"; then
  exit 0
fi
...
if grep -q "Tenant or user not found" "${TMP_ERROR_FILE}" && [[ -n "${POOLER_TENANT_ID_VALUE}" ]]; then
  ...
  run_psql "$(build_database_url "${TENANT_QUALIFIED_USER}")"
  exit 0
fi
