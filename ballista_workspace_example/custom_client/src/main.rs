...
use ballista::{
  datafusion::{assert_batches_eq, error::Result, prelude::SessionContext},
...
/// # Extending Ballista
///
/// This example demonstrates how to extend ballista scheduler and executor registering new object
/// store registry. It uses local [minio](https://min.io) to act as S3 object store.
///
/// Ballista will be extended providing custom session configuration, runtime environment and
/// session state.
///
...
async fn main() -> Result<()> {
  ...
  ctx
    .sql(&format!("SET s3.access_key_id = '{S3_ACCESS_KEY_ID}'"))
    .await?
    .show()
    .await?;
  ctx
    .sql(&format!("SET s3.secret_access_key = '{S3_SECRET_KEY}'"))
    .await?
    .show()
    .await?;
  ctx
    .sql("SET s3.endpoint = 'http://localhost:9000'")
    .await?
    .show()
    .await?;
  ctx.sql("SET s3.allow_http = true").await?.show().await?;
