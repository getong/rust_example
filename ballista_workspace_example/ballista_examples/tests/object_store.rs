...
mod standalone {
  ...
  async fn should_execute_sql_write() -> datafusion::error::Result<()> {
    ...
    node
      .exec(crate::common::create_bucket_command())
      .await
      .unwrap();
    ...
    let write_dir_path = &format!("s3://{}/write_test.parquet", crate::common::BUCKET);
    ...
    ctx
      .register_parquet("written_table", write_dir_path, Default::default())
      .await?;
...
mod remote {
  ...
  async fn should_execute_sql_write() -> datafusion::error::Result<()> {
    ...
    let (host, port) = crate::common::setup_test_cluster_with_state(state.clone()).await;
...
mod custom_s3_config {
  ...
  use ballista::{extension::SessionContextExt, prelude::SessionConfigExt};
  use ballista_core::{
    RuntimeProducer,
    object_store::{CustomObjectStoreRegistry, S3Options},
  };
  use ballista_examples::test_util::examples_test_data;
  use datafusion::{
    assert_batches_eq,
    error::DataFusionError,
    execution::{SessionState, SessionStateBuilder, runtime_env::RuntimeEnvBuilder},
  ...
  #[tokio::test]
  async fn should_configure_s3_execute_sql_write_remote() -> datafusion::error::Result<()> {
    let test_data = examples_test_data();
    ...
    let config_producer = Arc::new(ballista_core::object_store::session_config_with_s3_support);
    ...
    let session_builder = Arc::new(ballista_core::object_store::session_state_with_s3_support);
    ...
    ctx
      .sql(&format!("SET s3.access_key_id = '{}'", ACCESS_KEY_ID))
      .await?
      .show()
      .await?;
    ctx
      .sql(&format!("SET s3.secret_access_key = '{}'", SECRET_KEY))
      .await?
      .show()
      .await?;
    ctx
    ...
    ctx
      .sql("select name, value from information_schema.df_settings where name like 's3.%'")
      .await?
      .show()
      .await?;
  ...
  #[tokio::test]
  async fn should_configure_s3_execute_sql_write_standalone() -> datafusion::error::Result<()> {
    let test_data = examples_test_data();
  ...
  fn produce_state(session_config: SessionConfig) -> datafusion::common::Result<SessionState> {
