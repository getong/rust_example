...
async fn main() -> anyhow::Result<()> {
  let _ = tracing_subscriber::fmt()
    .with_env_filter("libp2p_browser_webrtc_example=debug,libp2p_webrtc=info,libp2p_ping=debug")
    .try_init();
...
/// To allow the client to connect, we replace the `__LIBP2P_ENDPOINT__` placeholder with the actual
/// address.
async fn get_index(
