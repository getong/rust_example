  ...
  loop {
    match swarm.select_next_some().await {
      ...
      SwarmEvent::Behaviour(BehaviourEvent::Autonat(autonat::v2::client::Event {
        ...
        println!(
          "Tested {tested_addr} with {server}. Sent {bytes_sent} bytes for verification. \
           Everything Ok and verified."
        );
      }
      SwarmEvent::Behaviour(BehaviourEvent::Autonat(autonat::v2::client::Event {
        server,
        ...
        println!(
          "Tested {tested_addr} with {server}. Sent {bytes_sent} bytes for verification. Failed \
           with {e:?}."
        );
      }
