...
use libp2p::{
  Multiaddr, PeerId, StreamProtocol, Swarm, SwarmBuilder, core,
  core::{
    ConnectedPoint,
  ...
  identity::Keypair,
  kad::{
    Config as KademliaConfig, Event as KademliaEvent, GetClosestPeersOk, ProgressStep, QueryResult,
    record::store::MemoryStore, store::MemoryStore,
  },
...
enum Opt {
  ...
  Dht {
    ...
    peer_id: PeerId,
    /// Network of the peer.
    #[arg(long, possible_values = &Network::variants(), case_insensitive = true)]
    network: Network,
  },
}
...
async fn main() {
  ...
  let timed_lookup =
    future::timeout(std::time::Duration::from_secs(20), lookup).map_err(|_| LookupError::Timeout);
...
impl LookupClient {
  fn new(network: Option<Network>) -> Self {
    ...
    let behaviour = {
      ...
      let identify = identify::Behaviour::new(
        identify::Config::new(proto_version, local_key.public()).with_agent_version(user_agent),
      );
  ...
  async fn wait_for_identify(&mut self, peer: PeerId) -> Result<Peer, LookupError> {
    loop {
      match self.swarm.next().await.expect("Infinite Stream.") {
        SwarmEvent::Behaviour(LookupBehaviourEvent::Identify(identify::Event::Received {
          peer_id,
          info:
            identify::Info {
              protocol_version,
...
// copy from https://github.com/mxinden/libp2p-lookup
