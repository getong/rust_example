...
use std::{fmt::Debug, fs, io, io::Cursor, path::PathBuf, sync::Arc};
...
use crate::{kv_types, raft_store::TypeConfig};
    ...
    Strm: Stream<Item = Result<EntryResponder<TypeConfig>, io::Error>> + Unpin + OptionalSend,
  {
    ...
    while let Some((entry, responder)) = entries.try_next().await? {
      ...
      let response = match entry.payload {
        EntryPayload::Blank => kv_types::Response::none(),
        EntryPayload::Normal(ref req) => match req {
          kv_types::Request::Set { key, value } => {
            let cf_data = self.cf_sm_data();

            batch.put_cf(cf_data, key.as_bytes(), value.as_bytes());
            kv_types::Response::new(value.clone())
          }
        },
