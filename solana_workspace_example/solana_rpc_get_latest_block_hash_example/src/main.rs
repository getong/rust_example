...
async fn get_latest_block_data(client: &RpcClient) -> Result<(String, u64, Option<i64>), Box<dyn std::error::Error>> {
    ...
    Ok((latest_block_hash.to_string(), latest_block_height, block_info.block_time))
}

async fn get_latest_confirmed_block(client: &RpcClient, max_retries: usize) -> Result<EncodedConfirmedBlock, Box<dyn std::error::Error>> {
    let mut retries = 0;
    ...
    while retries < max_retries {
        match client.get_block(slot) {
            ...
            Err(err) => {
                eprintln!("Block not available for slot {}: {}. Retrying...", slot, err);
