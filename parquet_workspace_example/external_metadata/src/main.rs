...
use parquet::{
  ...
  file::{
    metadata::{PageIndexPolicy, ParquetMetaData, ParquetMetaDataReader, ParquetMetaDataWriter},
...
///
/// 4. Reads the metadata from the separate file and uses that to read the Parquet file, thus
///    avoiding a second IO to read metadata or reparsing the footer.
