...
fn main() {
    ...
    let channel_id_value = u128::from_be_bytes(channel_id.try_into().expect("ChannelId slice size mismatch"));
    ...
    let expired_at_value = u128::from_be_bytes(expired_at.try_into().expect("Expired slice size mismatch"));
