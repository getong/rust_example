...
impl Executor {
  fn new() -> Self {
    ...
    poll
      .registry()
      .register(&mut client1, Token(1), Interest::READABLE)
      .unwrap();

    poll
      .registry()
      .register(&mut client2, Token(2), Interest::READABLE)
      .unwrap();

    poll
      .registry()
      .register(&mut client3, Token(3), Interest::READABLE)
      .unwrap();
    Executor {
...
fn main() {
  ...
  loop {
    if executor.client_total[0] == 20
      && executor.client_total[1] == 20
      && executor.client_total[2] == 20
    {
      break;
    }
    executor.poll();
