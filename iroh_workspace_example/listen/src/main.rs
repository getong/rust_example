...
//!
//! This example uses the default relay servers to attempt to holepunch, and will use that relay
//! server to relay packets if the two devices cannot establish a direct UDP connection.
...
async fn main() -> Result<()> {
  ...
  // Build a `Endpoint`, which uses PublicKeys as endpoint identifiers, uses QUIC for directly
  // connecting to other endpoints, and uses the relay protocol and relay servers to holepunch
  // direct connections between endpoints when there are NATs or firewalls preventing direct
  // connections. If no direct connection can be made, packets are relayed over the relay servers.
  let endpoint = Endpoint::builder(presets::N0)
    // The secret key is used to authenticate with other endpoints. The PublicKey portion of this
    // secret key is how we identify endpoints, often referred to as the `endpoint_id` in our
    // codebase.
    ...
    // If you want to experiment with relaying using your own relay server, you must pass in the
    // same custom relay url to both the `listen` code AND the `connect` code
    .relay_mode(RelayMode::Default)
    // you can choose a port to bind to, but passing in `0` will bind the socket to a random
    // available port
    .bind()
    .await?;
  ...
  println!(
    "\tcargo run --example connect -- --endpoint-id {me} --addrs \"{local_addrs}\" --relay-url \
     {relay_url}\n"
  );
