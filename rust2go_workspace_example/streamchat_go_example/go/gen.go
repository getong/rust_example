...
} StreamChatTokenRequestRef;

typedef struct StreamChatTokenResponseRef {
  struct StringRef token;
  struct StringRef error;
} StreamChatTokenResponseRef;
*/
import "C"
import (
	"reflect"
...
var StreamChatCallImpl StreamChatCall
...
type StreamChatTokenRequest struct {
	api_key    string
	api_secret string
	user_id    string
}

func newStreamChatTokenRequest(p C.StreamChatTokenRequestRef) StreamChatTokenRequest {
	return StreamChatTokenRequest{
		api_key:    newString(p.api_key),
		api_secret: newString(p.api_secret),
		user_id:    newString(p.user_id),
	}
}
func ownStreamChatTokenRequest(p C.StreamChatTokenRequestRef) StreamChatTokenRequest {
	return StreamChatTokenRequest{
...
func cntStreamChatTokenRequest(s *StreamChatTokenRequest, cnt *uint) [0]C.StreamChatTokenRequestRef {
	_ = s
	_ = cnt
	return [0]C.StreamChatTokenRequestRef{}
}
func refStreamChatTokenRequest(p *StreamChatTokenRequest, buffer *[]byte) C.StreamChatTokenRequestRef {
	return C.StreamChatTokenRequestRef{
...
type StreamChatTokenResponse struct {
	token string
	error string
}

func newStreamChatTokenResponse(p C.StreamChatTokenResponseRef) StreamChatTokenResponse {
	return StreamChatTokenResponse{
		token: newString(p.token),
		error: newString(p.error),
	}
}
func ownStreamChatTokenResponse(p C.StreamChatTokenResponseRef) StreamChatTokenResponse {
	return StreamChatTokenResponse{
		token: ownString(p.token),
		error: ownString(p.error),
	}
}
func cntStreamChatTokenResponse(s *StreamChatTokenResponse, cnt *uint) [0]C.StreamChatTokenResponseRef {
	_ = s
	_ = cnt
	return [0]C.StreamChatTokenResponseRef{}
}
func refStreamChatTokenResponse(p *StreamChatTokenResponse, buffer *[]byte) C.StreamChatTokenResponseRef {
	return C.StreamChatTokenResponseRef{
		token: refString(&p.token, buffer),
