...
typedef struct HttpFetchRequestRef {
...
typedef struct HttpFetchResponseRef {
  struct StringRef status;
  struct ListRef lines;
  struct StringRef error;
} HttpFetchResponseRef;
*/
import "C"
import (
	"reflect"
...
var HttpFetchCallImpl HttpFetchCall

type HttpFetchCall interface {
	fetch(req *HttpFetchRequest) HttpFetchResponse
}

//export CHttpFetchCall_fetch
func CHttpFetchCall_fetch(req C.HttpFetchRequestRef, slot *C.void, cb *C.void) {
	_new_req := newHttpFetchRequest(req)
	resp := HttpFetchCallImpl.fetch(&_new_req)
	resp_ref, buffer := cvt_ref(cntHttpFetchResponse, refHttpFetchResponse)(&resp)
...
func newHttpFetchRequest(p C.HttpFetchRequestRef) HttpFetchRequest {
	return HttpFetchRequest{
		url:       newString(p.url),
		max_lines: newC_uint32_t(p.max_lines),
	}
}
func ownHttpFetchRequest(p C.HttpFetchRequestRef) HttpFetchRequest {
	return HttpFetchRequest{
		url:       ownString(p.url),
		max_lines: newC_uint32_t(p.max_lines),
	}
}
func cntHttpFetchRequest(s *HttpFetchRequest, cnt *uint) [0]C.HttpFetchRequestRef {
	_ = s
	_ = cnt
	return [0]C.HttpFetchRequestRef{}
}
func refHttpFetchRequest(p *HttpFetchRequest, buffer *[]byte) C.HttpFetchRequestRef {
	return C.HttpFetchRequestRef{
		url:       refString(&p.url, buffer),
		max_lines: refC_uint32_t(&p.max_lines, buffer),
	}
}
...
func newHttpFetchResponse(p C.HttpFetchResponseRef) HttpFetchResponse {
	return HttpFetchResponse{
		status: newString(p.status),
		lines:  new_list_mapper(newString)(p.lines),
		error:  newString(p.error),
	}
}
func ownHttpFetchResponse(p C.HttpFetchResponseRef) HttpFetchResponse {
	return HttpFetchResponse{
		status: ownString(p.status),
		lines:  new_list_mapper(ownString)(p.lines),
		error:  ownString(p.error),
	}
}
func cntHttpFetchResponse(s *HttpFetchResponse, cnt *uint) [0]C.HttpFetchResponseRef {
	cnt_list_mapper(cntString)(&s.lines, cnt)
	return [0]C.HttpFetchResponseRef{}
}
func refHttpFetchResponse(p *HttpFetchResponse, buffer *[]byte) C.HttpFetchResponseRef {
	return C.HttpFetchResponseRef{
		status: refString(&p.status, buffer),
		lines:  ref_list_mapper(refString)(&p.lines, buffer),
